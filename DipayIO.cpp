...
const char* SQL_SELECTACCDIV_BEGIN =
    "SELECT ac.id, trans_no, divint_no, tran_type, ac.sec_no, ac.wi, ac.secid, ac.sec_xtend, "
    "acct_type, elig_date, sec_symbol, div_type, div_factor, ac.units, orig_face, "
    "pcpl_amt, income_amt, trd_date, stl_date, eff_date, entry_date, ac.curr_id, "
    ...
    "inc_sys_xrate, orig_yld, eff_mat_date, eff_mat_price, acct_mthd, trans_srce, dr_cr, "
    "dtc_inclusion, dtc_resolve, income_flag, letter_flag, ledger_flag, ac.created_by, "
    ...
    "WHERE ac.eff_date <= ? AND ac.suspend_flag <> 'Y' AND ac.delete_flag <> 'Y'";

const char* SQL_SELECTACCDIV_MODE_A = " AND ac.id = ?";
const char* SQL_SELECTACCDIV_MODE_S = " AND ac.id = ? AND ac.sec_no = ? AND ac.wi = ? AND ac.sec_xtend = ? AND acct_type = ? AND trans_no = ?";
const char* SQL_SELECTACCDIV_TYPE_S = " AND (tran_type in ('SD', 'SP', 'SB', 'SX'))";
const char* SQL_SELECTACCDIV_TYPE_I = " AND (tran_type not in ('SD', 'SP', 'SB', 'SX'))";
const char* SQL_SELECTACCDIV_END = " ORDER BY ac.id, ac.sec_no, ac.wi, ac.sec_xtend, acct_type, divint_no, tran_type, trans_no, dr_cr, trd_date";
...
const char* SQL_SELECT_PORTTAX =
    ...
    "FROM porttax a "
    "WHERE id = ? and TaxDate = (select max(taxdate) from porttax where id = a.id and taxdate <= ?)";
...
static void FillAccdivStruct(nanodbc::result& result, ACCDIV* pzAccdiv, int* iSecType, 
                             double* fTradingUnit, double* fCurExrate, double* fCurIncExrate)
{
    ...
    *fTradingUnit = result.get<double>("trad_unit", 0.0);
    ...
    // If security itself has an exchange rate for the effective date, use that, else if there is
    // an exchange rate for the income currency, use that, if even that does not exists, use
    // the current exchange rates from asset record.
    if (fSecExrate > 0)
    {
    ...
    if (strcmp(pzAccdiv->sCurrId, pzAccdiv->sIncCurrId) == 0 && *fCurExrate != *fCurIncExrate)
    {
        if (*fCurExrate > 0)
...
DLLAPI void STDCALL SelectAllAccdiv(ACCDIV *pzAccdiv, int *iSecType, double *fTradingUnit, 
                                    double *fCurExrate, double *fCurIncExrate, char *sMode, 
                                    char *sType, int iID, char *sSecNo, char *sWi, 
                                    char *sSecXtend, char *sAcctType, long lTransNo, 
                                    long lEffDate, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"", 0, -1, 0, (char*)"SelectAllAccdiv", FALSE);
        return;
    }
    ...
    try
    {
        ...
        bool needNewQuery = !(
            ...
            (sMode[0] != 'S' || (
                strcmp(g_accdivState.sSecNo, sSecNo) == 0 &&
        ...
        {
            g_accdivState.cRows++;
            FillAccdivStruct(*g_accdivState.result, pzAccdiv, iSecType, fTradingUnit, fCurExrate, fCurIncExrate);
        }
        else
        {
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), 0, 0, (char*)"", 0, -1, 0, (char*)"SelectAllAccdiv", FALSE);
        g_accdivState.result.reset();
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in SelectAllAccdiv", 0, 0, (char*)"", 0, -1, 0, (char*)"SelectAllAccdiv", FALSE);
        g_accdivState.result.reset();
    }
}
...
DLLAPI void STDCALL UpdateAccdivDeleteFlag(char *sDeleteFlag, int iID, char *sSecNo, char *sWi, 
                                           char *sSecXtend, char *sAcctType, long lTransNo, 
                                           long lDivintNo, ERRSTRUCT *pzErr)
{
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"", 0, -1, 0, (char*)"UpdateAccdivDeleteFlag", FALSE);
        return;
    }
    ...
    try
    {
        ...
        int paramIndex = 0;
        stmt.bind(paramIndex++, sDeleteFlag);
        stmt.bind(paramIndex++, &iID);
        stmt.bind(paramIndex++, sSecNo);
        stmt.bind(paramIndex++, sWi);
        stmt.bind(paramIndex++, sSecXtend);
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), 0, 0, (char*)"", 0, -1, 0, (char*)"UpdateAccdivDeleteFlag", FALSE);
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in UpdateAccdivDeleteFlag", 0, 0, (char*)"", 0, -1, 0, (char*)"UpdateAccdivDeleteFlag", FALSE);
    }
}
...
DLLAPI void STDCALL SelectPorttax(int iID, long lTaxDate, PORTTAX *pzPTax, ERRSTRUCT *pzErr)
{
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"", 0, -1, 0, (char*)"SelectPorttax", FALSE);
        return;
    }
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), 0, 0, (char*)"", 0, -1, 0, (char*)"SelectPorttax", FALSE);
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in SelectPorttax", 0, 0, (char*)"", 0, -1, 0, (char*)"SelectPorttax", FALSE);
    }
}
...
DLLAPI void STDCALL SelectWithrclm(WITHHOLDRCLM *pzWH, ERRSTRUCT *pzErr)
{
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"", 0, -1, 0, (char*)"SelectWithrclm", FALSE);
        return;
    }
    ...
    try
    {
        ...
        if (g_withrclmState.result && g_withrclmState.result->next())
        {
            ...
            read_string(*g_withrclmState.result, "curr_id", pzWH->sCurrId, sizeof(pzWH->sCurrId));
            pzWH->fFixedincWithhold = g_withrclmState.result->get<double>("fixedinc_withhold", 0.0);
            pzWH->fFixedincRclm = g_withrclmState.result->get<double>("fixedinc_rclm", 0.0);
            pzWH->fEquityWithhold = g_withrclmState.result->get<double>("equity_withhold", 0.0);
            pzWH->fEquityRclm = g_withrclmState.result->get<double>("equity_rclm", 0.0);
        }
        else
        {
            pzErr->iSqlError = SQLNOTFOUND;
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), 0, 0, (char*)"", 0, -1, 0, (char*)"SelectWithrclm", FALSE);
        g_withrclmState.result.reset();
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in SelectWithrclm", 0, 0, (char*)"", 0, -1, 0, (char*)"SelectWithrclm", FALSE);
        g_withrclmState.result.reset();
    }
}
...
#ifdef DEBUG
    PrintError("InitializeDivIntPayIO called", 0, 0, "", 0, 0, 0, "InitializeDivIntPayIO", FALSE);
#endif
...
#ifdef DEBUG
    PrintError("CloseDivIntPayIO called", 0, 0, "", 0, 0, 0, "CloseDivIntPayIO", FALSE);
#endif
}
...
#ifdef DEBUG
    PrintError("FreeDivIntPayIO called (RAII cleanup)", 0, 0, "", 0, 0, 0, "FreeDivIntPayIO", FALSE);
#endif
}

/**
  ...
  2015-04-02 VI#57005 Fix for when qtrly composite's members data ends before
qtr end - yb. 2014-07-01 VI#54971 More updates for handling IPVs and daily - yb.
  2014-06-23 VI#54971 Updates for handling IPVs and daily - yb.
  2013-06-11 VI#52878 sum notional flows - yb.
  2013-04-16 VI#52266 inserts can be for non-period end date. delete all
dsumdata for period - yb. 03/10/2011 VI#45707 Prevented negative UVs from making
it into unitvalue -mk 03-27/2006 only update MAPCOmpMemTransEx for partnership
composites, portfolio type = 7 - ssi 10-26-2005 Added code to write to
MapCompMemTransEx, and to merge a composite partnership account - ssi
**/
...
void STDCALL WINAPI CreateComposite(int iOwnerID, long lDate,
                                    int iPortfolioType, long lHoldingsFlag,
                                    ERRSTRUCT *zErr) {
  ...
  char sHoldingsName[18 + NT], sHoldcashName[18 + NT], sPortmainName[18 + NT],
      sPortbalName[18 + NT], sPayrecName[18 + NT], sHXrefName[18 + NT],
      sHoldtotName[18 + NT];
  BOOL bUseTrans;
  long lInitDate;
  ...
  lInitDate = lDate;
  lpprSelectHoldmap(lInitDate, sHoldingsName, sHoldcashName, sPortmainName,
                    sPortbalName, sPayrecName, sHXrefName, sHoldtotName, zErr);
  ...
  /*If iPortfolioType	= 7 = Partnership Composite, use tables indicated in
   * lHoldingsFlag*/
  ...
  while (TRUE) {
    ...
    if (zErr->iSqlError == SQLNOTFOUND) {
      ...
      lpfnPrintError("Error Reading COMPPORT Table cursor", iPortfolioID, 0, "",
                     0, zErr->iBusinessError, zErr->iIsamCode,
                     "COMPCREATE SELECTALLMEMBERSOFACOMPOSITE", FALSE);
      return;
    }
    ...
    GetHoldingsHoldcashInTables(iPortfolioID, &zHoldingsTable, &zHoldcashTable,
                                zErr);
  } /*while*/
  ...
  /*Creates the composite for all the holdings and hold cash for members of
   * composite*/
  ...
  __try {
    CreateCompositeRecord_Internal(iOwnerID, &zHoldingsTable, &zHoldcashTable,
                                   sHoldingsName, sHoldcashName, sPortmainName,
                                   lDate, bUseTrans, iPortfolioType, zErr);
...
/**
        This routine fills the holdcash,holdings table in the memory for the
given portfolio.
**/
...
void GetHoldingsHoldcashInTables(int iPortfolioID, HOLDTABLE *pHoldingsTable,
  ...
  while (!zErr->iSqlError) {
    ...
    if (zErr->iSqlError == SQLNOTFOUND)
      ...
      lpfnPrintError("Error Reading Holdings Table cursor", iPortfolioID, 0, "",
                     0, zErr->iBusinessError, zErr->iIsamCode,
                     "COMPCREATE SELECTALLHOLDINGSFORANACCOUNT", FALSE);
      return;
    }
  ...
  while (!zErr->iSqlError) {
    ...
    if (zErr->iSqlError == SQLNOTFOUND) {
      ...
      lpfnPrintError("Error Reading HOLDCASHTABLE Table cursor", iPortfolioID,
                     0, "", 0, zErr->iBusinessError, zErr->iIsamCode,
                     "COMPCREATE SELECTALLHOLDCASHFORANACCOUNT", FALSE);
      return;
    }

    ++iNumHoldcash;

    /*if its not first hold cash then check if the
       key(sec_no,wi,secXtend,acct_type) already exist in the table of collected
       hold cash in the memory.if the key does not exist add the selected hold
       cash to the table(memory).If the key exist then add the
       units,tot_cost,mkt_value to the existing holdcash*/
    //		if (iNumHoldcash > 1)
    //		{
    ...
    /*If the key is not found then add the cash hold to the cash hold
     * table(memory)*/
    ...
    /* Key is found add the units,tot_cost,mkt_value to matched cash hold*/
      ...
      pHoldcashTable->pzHoldCash[HoldCashKeyIndex].fTotCost +=
          zHoldcash.fTotCost;
...
void AddTempHoldingToTable(HOLDTABLE *pHoldingsTable, HOLDINGS zHoldingsTemp,
  ...
  if (pHoldingsTable->iNumHoldCreated == pHoldingsTable->iNumHoldings) {
    ...
    pHoldingsTable->pzHoldings =
        (HOLDINGS *)realloc(pHoldingsTable->pzHoldings,
                            pHoldingsTable->iNumHoldCreated * sizeof(HOLDINGS));
    if (pHoldingsTable->pzHoldings == NULL) {
      lpfnPrintError("Insufficient Memory For HoldTable", 0, 0, "", 997, 0, 0,
                     "COMPCREATE ADDTEMPHOLD", FALSE);
      return;
    }
  }
...
void AddTempHoldcashToTable(HOLDCASHTABLE *pHoldcashTable,
  ...
  if (pHoldcashTable->iNumHoldCashCreated == pHoldcashTable->iNumHoldCash) {
    ...
    pHoldcashTable->pzHoldCash = (HOLDCASH *)realloc(
        pHoldcashTable->pzHoldCash,
        pHoldcashTable->iNumHoldCashCreated * sizeof(HOLDCASH));
    if (pHoldcashTable->pzHoldCash == NULL) {
      lpfnPrintError("Insufficient Memory For HoldcashTable", 0, 0, "", 997, 0,
                     0, "COMPCREATE ADDTEMPHOLD", FALSE);
      return;
    }
  }
...
void CreateCompositeRecord_Internal(int iCompositeID, HOLDTABLE *pHoldingsTable,
  ...
  lpprAccountDeleteHoldings(iCompositeID, sSecNo, sWi, sHoldingsTable, FALSE,
                            zErr);
  ...
  /*Go through the collected holdings and insert them into holdings with new
   * id(composite id)*/
  iInsertCount = 0;
  for (iNumHoldings = 0; iNumHoldings < pHoldingsTable->iNumHoldings;
       ++iNumHoldings) {
    lpprInitializeErrStruct(zErr);
    memcpy(&zHoldingsTemp, &pHoldingsTable->pzHoldings[iNumHoldings],
           sizeof(HOLDINGS));
  ...
  */
  ...
  lpprAccountDeleteHoldcash(iCompositeID, sSecNo, sWi, sHoldCashTable, FALSE,
                            zErr);

  /*Insert all the hold cash for the compositte*/
  for (iNumHoldcash = 0; iNumHoldcash < pHoldcashTable->iNumHoldCash;
       ++iNumHoldcash) {
    lpprInitializeErrStruct(zErr);
    memcpy(&zHoldcashTemp, &pHoldcashTable->pzHoldCash[iNumHoldcash],
           sizeof(HOLDCASH));
  ...
  if (lValDate > 0) {
    ...
    if (zErr->iSqlError != 0 || zErr->iBusinessError != 0) {
      /*if the composite portfolio does not exist in portmain_yyyymmdd or
       * adhoc*/
      if (zErr->iSqlError == SQLNOTFOUND) {
        lpprAccountInsertPortmain(iCompositeID, sTargetPortmain,
                                  (char *)"PORTMAIN", zErr);
        /*if it fails to insert the porfolio into portmain_yyyymmdd or adhoc*/
        if (zErr->iSqlError != 0 || zErr->iBusinessError != 0) {
          lpfnPrintError("Error Inserting into Portmain Table ", iCompositeID,
                         0, "", 0, zErr->iBusinessError, zErr->iIsamCode,
                         "COMPCREATE AccountInsertPortmain", FALSE);
          return;
        }
        /*Successfully Inserted the portfolio into portmain;Now Update the val
           date*/
        else {
          lpprUpdatePortmainValDate(iCompositeID, lValDate, zErr);
          if (zErr->iSqlError != 0 || zErr->iBusinessError != 0)
      ...
      /*Some Other Error occured*/
        ...
        lpfnPrintError("Error Updating Portmain Table cursor", iCompositeID, 0,
                       "", 0, zErr->iBusinessError, zErr->iIsamCode,
                       "COMPCREATE UpdatePortmainValDate", FALSE);
        return;
      }
    }
  }
...
**/
void STDCALL InitializeDllRoutines(long lAsofDate, char *sDBAlias, char *sMode,
                                   char *sType, BOOL bPrepareQueries,
                                   char *sErrFile, ERRSTRUCT *zErr) {
  ...
  if (!bCompCreateInit) {
    ...
    lpprOLEDBIOInit =
        (LPPR3PCHAR1LONG1BOOL)GetProcAddress(hStarsIODll, "InitializeOLEDBIO");
    if (!lpprOLEDBIOInit) {
      lpfnPrintError("Unable To Load InitializeOLEDBIO function", 0, 0, "",
                     GetLastError(), 0, 0, "CompCreate INIT1", FALSE);
    ...
    if (!lpprInitializeErrStruct) {
      lpfnPrintError("Unable To Initialize Error Structure function", 0, 0, "",
                     GetLastError(), 0, 0, "CompCreate INIT2", FALSE);
      zErr->iBusinessError = -1;
      return;
    }
    ...
    if (!lpfnStartTransaction) {
      lpfnPrintError("Unable To Load StartDBTransaction function", 0, 0, "",
                     GetLastError(), 0, 0, "CompCreate INIT2a", FALSE);
      zErr->iBusinessError = -1;
      return;
    }
    ...
    if (!lpfnCommitTransaction) {
      lpfnPrintError("Unable To Load CommitDBTransaction function", 0, 0, "",
                     GetLastError(), 0, 0, "CompCreate INIT2b", FALSE);
      zErr->iBusinessError = -1;
      return;
    }
    ...
    if (!lpfnRollbackTransaction) {
      lpfnPrintError("Unable To Load RollbackDBTransaction function", 0, 0, "",
                     GetLastError(), 0, 0, "CompCreate INIT2c", FALSE);
      zErr->iBusinessError = -1;
      return;
    }
    ...
    if (!lpfnAbortTransaction) {
      lpfnPrintError("Unable To Load AbortDBTransaction function", 0, 0, "",
                     GetLastError(), 0, 0, "CompCreate INIT2d", FALSE);
      zErr->iBusinessError = -1;
      return;
    }
    ...
    if (!lpfnGetTransCount) {
      lpfnPrintError("Unable To Load GetTransCount function", 0, 0, "",
                     GetLastError(), 0, 0, "CompCreate INIT2e", FALSE);
      zErr->iBusinessError = -1;
      return;
    }

    lpprSelectAllMembersOfAComposite =
        (LPPRSELECTALLMEMBERSOFACOMPOSITE)GetProcAddress(
            hStarsIODll, "SelectAllMembersOfAComposite");
    if (!lpprSelectAllMembersOfAComposite) {

      lpfnPrintError("Unable To Load SelectAllMembersOfAComposite function", 0,
                     0, "", GetLastError(), 0, 0, "CompCreate INIT5", FALSE);
      zErr->iBusinessError = -1;
      return;
    }

    // load insert for mapcompmemtransex
    lpprInsertMapCompMemTransEx = (LPPRINSERTMAPCOMPMEMTRANSEX)GetProcAddress(
        hStarsIODll, "InsertMapCompMemTransEx");
    if (!lpprInsertMapCompMemTransEx) {
      lpfnPrintError("Unable To Load InsertMapCompMemTransEx function", 0, 0,
                     "", GetLastError(), 0, 0, "CompCreate INIT-new1", FALSE);
      zErr->iBusinessError = -1;
      return;
    }

    // load Delete for mapcompmemtransex
    lpprDeleteMapCompMemTransEx = (LPPRDELETEMAPCOMPMEMTRANSEX)GetProcAddress(
        hStarsIODll, "DeleteMapCompMemTransEx");
    if (!lpprDeleteMapCompMemTransEx) {
      lpfnPrintError("Unable To Load DeleteMapCompMemTransEx function", 0, 0,
                     "", GetLastError(), 0, 0, "CompCreate INIT-new2", FALSE);
      zErr->iBusinessError = -1;
      return;
    }
    ...
    lpprSelectAllHoldingsForAnAccount =
        (LPPRALLHOLDINGSFORANACCOUNT)GetProcAddress(
            hStarsIODll, "SelectAllHoldingsForAnAccount");
    if (!lpprSelectAllHoldingsForAnAccount) {
      lpfnPrintError("Unable To Load SelectAllHoldingsForAnAccount function", 0,
                     0, "", GetLastError(), 0, 0, "CompCreate INIT6", FALSE);
      zErr->iBusinessError = -1;
      return;
    }
    ...
    lpprSelectAllHoldcashForAnAccount =
        (LPPRALLHOLDCASHFORANACCOUNT)GetProcAddress(
            hStarsIODll, "SelectAllHoldcashForAnAccount");
    if (!lpprSelectAllHoldcashForAnAccount) {
      lpfnPrintError("Unable To Load SelectAllHoldcashForAnAccount function", 0,
                     0, "", GetLastError(), 0, 0, "CompCreate INIT7", FALSE);
      zErr->iBusinessError = -1;
      return;
    }

    lpprInsertHoldings =
        (LPPRHOLDINGS)GetProcAddress(hStarsIODll, "InsertHoldings");
    if (!lpprInsertHoldings) {
      lpfnPrintError("Unable To Load InsertHoldings function", 0, 0, "",
                     GetLastError(), 0, 0, "CompCreate INIT8", FALSE);
      zErr->iBusinessError = -1;
      return;
    }
    /*
    lpprInsertHoldingsBatch =
    (LPPRHOLDINGSBATCH)GetProcAddress(hStarsIODll,"InsertHoldingsBatch"); if
    (!lpprInsertHoldingsBatch)
    {
            lpfnPrintError("Unable To Load InsertHoldingsBatch function", 0, 0,
    "", GetLastError(), 0, 0, "CompCreate INIT8a", FALSE); zErr->iBusinessError
    = -1; return;
    }
    */
    lpprAccountDeleteHoldings = (LPPR1LONG3PCHAR1BOOL)GetProcAddress(
        hStarsIODll, "AccountDeleteHoldings");
    if (!lpprAccountDeleteHoldings) {
      lpfnPrintError("Unable To Load lpprAccountDeleteHoldings function", 0, 0,
                     "", GetLastError(), 0, 0, "CompCreate INIT9", FALSE);
      zErr->iBusinessError = -1;
      return;
    }

    lpprInsertHoldcash =
        (LPPRHOLDCASH)GetProcAddress(hStarsIODll, "InsertHoldcash");
    if (!lpprInsertHoldcash) {
      lpfnPrintError("Unable To Load InsertHoldcash function", 0, 0, "",
                     GetLastError(), 0, 0, "CompCreate INIT10", FALSE);
      zErr->iBusinessError = -1;
      return;
    }

    lpprAccountDeleteHoldcash = (LPPR1LONG3PCHAR1BOOL)GetProcAddress(
        hStarsIODll, "AccountDeleteHoldcash");
    ...
    lpprUpdatePortmainValDate =
        (LPPR1INT1LONG)GetProcAddress(hStarsIODll, "UpdatePortmainValDate");
    ...
    lpprSelectAdhocPortmain = (LPPRSELECTPORTMAINTABLE)GetProcAddress(
        hStarsIODll, "SelectAdhocPortmain");
    ...
    /*		lpprSelectHoldtotFor =
    (LPPRSELECTHOLDTOTFOR)GetProcAddress(hStarsIODll,"SelectHoldtotFor"); if
    (!lpprSelectHoldtotFor){ lpfnPrintError("Error Loading SelectHoldtotFor
    Function", 0, 0, "", GetLastError(), 0, 0, "CompCreate INIT14", FALSE);
            zErr->iBusinessError = -1;
            return;
    }*/

    lpprSelectHoldmap =
        (LPPR1LONG7PCHAR)GetProcAddress(hStarsIODll, "SelectHoldmap");
    ...
    lpprSelectPortmain =
        (LPPRPORTMAIN)GetProcAddress(hStarsIODll, "SelectPortmain");
    ...
    lpprAccountInsertPortmain = (LPPRACCOUNTINSERTPORTMAIN)GetProcAddress(
        hStarsIODll, "AccountInsertPortmain");
    ...
    lpprGetSummarizedDataForCompositeEx =
        (LPPRGETSUMMARIZEDDATAFORCOMPOSITE)GetProcAddress(
            hStarsIODll, "GetSummarizedDataForCompositeEx");
    if (!lpprGetSummarizedDataForCompositeEx) {
      lpfnPrintError("Error Loading GetSummarizedDataForCompositeEx Function",
                     0, 0, "", GetLastError(), 0, 0, "CompCreate INIT17a",
                     FALSE);
      zErr->iBusinessError = -1;
      return;
    }

    lpprBuildMergeCompSegMap = (LPPRBUILDMERGECOMPSEGMAP)GetProcAddress(
        hStarsIODll, "BuildMergeCompSegMap");
    ...
    lpprBuildMergeCompport = (LPPRBUILDMERGECOMPPORT)GetProcAddress(
        hStarsIODll, "BuildMergeCompport");
    ...
    lpprBuildMergeSData =
        (LPPRBUILDMERGESDATA)GetProcAddress(hStarsIODll, "BuildMergeSData");
    ...
    lpprBuildMergeUV =
        (LPPRBUILDMERGESDATA)GetProcAddress(hStarsIODll, "BuildMergeUV");
    if (!lpprBuildMergeUV) {
    ...
    lpprDeleteMergeSessionData =
        (LPPR1CHAR1LONG)GetProcAddress(hStarsIODll, "DeleteMergeSessionData");
    ...
    lpprUpdateUnitvalueMonthlyIPV =
        (LPPRUPDATEUNITVALUEMONTHLYIPV)GetProcAddress(
            hStarsIODll, "UpdateUnitvalueMonthlyIPV");
    ...
    lpprDeleteMergeUVGracePeriod =
        (LPPR1CHAR1LONG)GetProcAddress(hStarsIODll, "DeleteMergeUVGracePeriod");
    ...
    lpprDeleteSummdata =
        (LPPR3LONG)GetProcAddress(hStarsIODll, "DeleteSummdataForPortfolio");
    ...
    lpprDeleteSummdataForSegment =
        (LPPR3LONG)GetProcAddress(hStarsIODll, "DeleteSummdata");
    ...
    lpprDeleteMonthSum =
        (LPPR3LONG)GetProcAddress(hStarsIODll, "DeleteMonthSumForPortfolio");
    ...
    lpprDeleteMonthSumForSegment =
        (LPPR3LONG)GetProcAddress(hStarsIODll, "DeleteMonthSum");
    ...
    lpprDeleteDSumdata =
        (LPPR3LONG)GetProcAddress(hStarsIODll, "DeleteDSumdataForPortfolio");
    ...
    lpprDeleteDSumdataForSegment =
        (LPPR3LONG)GetProcAddress(hStarsIODll, "DeleteDSumdata");
    ...
    lpprDeleteUnitvalueSince2 = (LPPR4LONG)GetProcAddress(
        hStarsIODll, "DeleteUnitValueForPortfolioSince2");
    if (!lpprDeleteUnitvalueSince2) {
      lpfnPrintError(
          "Unable To Load DeleteUnitValueForPortfolioSince2 function", 0, 0, "",
          GetLastError(), 0, 0, "CompCreate INIT21", FALSE);
      zErr->iBusinessError = -1;
      return;
    }

    lpprDeleteUnitvalueSinceForSegment =
        (LPPR4LONG)GetProcAddress(hStarsIODll, "DeleteUnitValueSince");
    ...
    lpprInsertPeriodSummdata =
        (LPPRSUMMDATA)GetProcAddress(hStarsIODll, "InsertPeriodSummdata");
    ...
    lpprInsertUnitValue =
        (LPPRINSERTUNITVALUE)GetProcAddress(hStarsIODll, "InsertUnitValue");
    ...
    lpprInsertMonthlySummdata =
        (LPPRSUMMDATA)GetProcAddress(hStarsIODll, "InsertMonthlySummdata");
    ...
    lpprInsertDailySummdata =
        (LPPRSUMMDATA)GetProcAddress(hStarsIODll, "InsertDailySummdata");
    ...
    lpprCopySummaryData =
        (LPPRCOPYSUMMARYDATA)GetProcAddress(hStarsIODll, "CopySummaryData");
    ...
    lpprDeleteTaxperfForSegment =
        (LPPR3LONG)GetProcAddress(hStarsIODll, "DeleteTaxperfForSegment");
    ...
    lpprSummarizeTaxperf =
        (LPPRSUMMARIZETAXPERF)GetProcAddress(hStarsIODll, "SummarizeTaxPerf");
    ...
    lpprSummarizeMonthsum =
        (LPPRSUMMARIZEMONTHSUM)GetProcAddress(hStarsIODll, "SummarizeMonthsum");
    ...
    lpprSummarizeInceptionSummdata =
        (LPPRSUMMARIZEINCEPTIONSUMMDATA)GetProcAddress(
            hStarsIODll, "SummarizeInceptionSummdata");
    ...
    lpprSubtractInceptionSummdata =
        (LPPRSUBTRACTINCEPTIONSUMMDATA)GetProcAddress(
            hStarsIODll, "SubtractInceptionSummdata");
    ...
    if (!lpprSelectSysvalues) {
      lpfnPrintError("Unable To Load SelectSysvalues", 0, 0, "", GetLastError(),
                     0, 0, "CompCreate INIT32", FALSE);
      zErr->iBusinessError = -1;
      return;
    }

    lpprInitPerformance = (LPFN1LONG3PCHAR1BOOL1PCHAR)GetProcAddress(
        hPerformanceDll, "InitPerformance");
    ...
    lpfnAddNewSegment =
        (LPFN1INT2PCHAR1INT)GetProcAddress(hPerformanceDll, "AddNewSegment");
    ...
    *zErr =
        lpprInitPerformance(lAsofDate, sDBAlias, sMode, sType, FALSE, sErrFile);
    if (zErr->iSqlError != 0 || zErr->iBusinessError != 0)
      return;
...
void InitializeHoldcashTable(HOLDCASHTABLE *pHoldcashTable) {
  if (pHoldcashTable->iNumHoldCashCreated > 0 &&
...
/**
        This function checks if the input key already exist in the holdcash
table(memory) if it does exist then return the index of the matched holdcash
record.
**/
int FindHoldCashKey(HOLDCASHTABLE *pHoldCashTable, HOLDCASHKEY zHoldCashKey) {
  ...
  for (i = 0; i < pHoldCashTable->iNumHoldCash; i++) {
    ...
    if (strcmp(pHoldCashTable->pzHoldCash[i].sSecNo, zHoldCashKey.sSecNo) ==
            0 &&
        strcmp(pHoldCashTable->pzHoldCash[i].sWi, zHoldCashKey.sWi) == 0 &&
        strcmp(pHoldCashTable->pzHoldCash[i].sSecXtend,
               zHoldCashKey.sSecXtend) == 0 &&
        strcmp(pHoldCashTable->pzHoldCash[i].sAcctType,
               zHoldCashKey.sAcctType) == 0) {
      return i;
    }
  } /* for loop*/
  return -1;
...
ERRSTRUCT STDCALL WINAPI MergeCompositePortfolio(int iID, long lFromDate,
  ...
  __try {
    ...
    __try {
      ...
      if (!bMergeOnAFly) {
        lpprBuildMergeCompSegMap(iID, lFromDate, lToDate, (char *)&sSessionID,
                                 &zErr);
      } else {
        strcpy_s(sSessionID, sInSessionID);
        // if called from reports (SessionID == NULL) then do not work with
        // daily values
        bDaily = FALSE;
      ...
      // fix month end unitvalues with incorrect ror source (like 3 if EOM is
      // also IPV)
      ...
      bSinceInception =
          (lFromDate <= lEarliestDate && !lpfnIsItAMonthEnd(lEarliestDate));
      while (zErr.iSqlError == 0 && zErr.iBusinessError == 0) {
        // build and/or pickup summarized data from "prebuilt" values in
        // Merge_SData
        lpprGetSummarizedDataForCompositeEx(&zSegmain, &zMonthsum, &zUV, &fROR,
                                            iID, lFromDate, lToDate, bDaily,
                                            sSessionID, &zErr);

        if (zErr.iSqlError == 0 && zErr.iBusinessError == 0) {
          if (zMonthsum.lPerformDate != 0 &&
              zUV.lUVDate != 0) // if have both summary record and return
          {

            if (zSummdata.iID != 0 && zSummdata.iID != zMonthsum.iID &&
                (!bDaily ||
                 bIsItAMonthEnd)) { // save Period record for previous segment
            ...
            // if this is "Merge on a fly" then delete has to happen for each
            // segment (but once per ror_type) while for regular merge it only
            // happens once for entire composite and therefore, bDelete will
            // always be set to FALSE after 1st delete occurs
            bDelete =
                bDelete || (bMergeOnAFly && (zUV.iRorType >= iLastRorType));
            iLastRorType = zUV.iRorType;

            if (bDelete) { // started processing -- delete old summary data for
                           // entire composite or segment
              ...
              if (!bMergeOnAFly) {
                ...
                lpprDeleteSummdataForSegment(zSegmain.iID, lFromDate + 1,
                                             lToDate, &zErr);
                if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
                  break;

                if (bSinceInception)
                  lpprDeleteSummdataForSegment(zSegmain.iID, lEarliestDate,
                                               lToDate, &zErr);
              }
              ...
              if (!bMergeOnAFly)
                ...
                lpprDeleteMonthSumForSegment(zSegmain.iID, lFromDate + 1,
                                             lToDate, &zErr);
              ...
              if (!bMergeOnAFly)
                ...
                lpprDeleteTaxperfForSegment(zSegmain.iID, lToDate, lToDate,
                                            &zErr);
              ...
              if (!bMergeOnAFly) {
                ...
                if (!bDaily)
                  lpprDeleteUnitvalueSince2(iID, lFromDate + 1, GTWRor,
                                            NTWAfterTaxRor, &zErr);
                else
                  lpprDeleteUnitvalueSince2(iID, lToDate, GTWRor,
                                            NTWAfterTaxRor, &zErr);
              } else {
                ...
                for (i = iLastRorType; i > ROR_SUMMARY; i--) {
                  ...
                  if (!bDaily)
                    lpprDeleteUnitvalueSinceForSegment(iID, zSegmain.iID,
                                                       lFromDate + 1, i, &zErr);
                  else
                    lpprDeleteUnitvalueSinceForSegment(iID, zSegmain.iID,
                                                       lToDate, i, &zErr);
              ...
              if (bDaily) {
                if (!bMergeOnAFly)
                  ...
                  lpprDeleteDSumdataForSegment(zSegmain.iID, 0, lFromDate,
                                               &zErr);
                ...
                //										lpprDeleteDSumdata
                //(iID, lFromDate, -1, &zErr);
                else
                  lpprDeleteDSumdataForSegment(zSegmain.iID, lToDate, -1,
                                               &zErr);
                //										lpprDeleteDSumdataForSegment
                //(zSegmain.iID, lFromDate, -1, &zErr);
                ...
                if (!bMergeOnAFly)
                  lpprCopySummaryData(iID, lToDate, (char *)"SUMMDATA",
                                      (char *)"DSUMDATA", &zErr);
            ...
            if (iLastRorType > NTWAfterTaxRor) {
              if (!bDaily)
                lpprDeleteUnitvalueSinceForSegment(
                    iID, zSegmain.iID, lFromDate + 1, iLastRorType, &zErr);
              else
                lpprDeleteUnitvalueSinceForSegment(iID, zSegmain.iID, lToDate,
                                                   iLastRorType, &zErr);
            }
            ...
            if (zSegmain.iID == 0) // if new segment for a composite - create it
            {
              if (zSegmain.iSegmentTypeID != iCurrSegTypeID) {
                zSegmain.iID = lpfnAddNewSegment(iID, (char *)"", (char *)"",
            ...
            if ((zMonthsum.lPerformDate == lToDate) &&
              ...
              // if UV has become 0 or negative for any reason, then stream
              // can't continue we must ensure new stream is starting (i.e. data
              // hole appears) (also, if UV is extremely high and won't fit
              // table field spec) and recalc UV using assumed BUV = 100
      ...
      // save last remaining Period record for previous segment we were working
      // with
      if (zSummdata.iID != 0) {
        if (!bDaily || bIsItAMonthEnd) {
          lpprInsertPeriodSummdata(zSummdata, &zErr);
      ...
      if (bNeedMonthlyFlows) {
        ...
        while (lFromDate <= lTempDate1) {
          ...
          lpprSummarizeMonthsum(iID, lTempDate1, lTempDate2, lToDate,
                                sSessionID, &zErr);
          if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
            return zErr;
      ...
      /*
                      ...
                      {
                              lpprSummarizeInceptionSummdata (iID,
         lEarliestDate, sSessionID, &zErr); if (zErr.iBusinessError != 0 ||
         zErr.iSqlError != 0) return zErr;

                              lpprSubtractInceptionSummdata (iID, lEarliestDate,
         lToDate, &zErr); if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
                                      return zErr;
                      }
      */
  ...
  __finally {
    ...
    if (!bMergeOnAFly) {
      ...
      // if all operations above were successful, then use latest error code,
      // zErr2, (from cleanup call) as final result otherwise, report and return
      // original error code, zErr
      if (zErr.iBusinessError == 0 && zErr.iSqlError == 0)
        zErr = zErr2;
    }
  }
...
/*
DLLAPI void STDCALL WINAPI CreateCompositeHoldTot(int iCompositeId,long
lDate,char *sHoldTotTable,ERRSTRUCT *zErr)
{
        int iPortfolioID;
        HOLDTOT zHoldTot;
...
int IsSegmentExist(HOLDTOTTABLE* zCompositeHoldTot,int iSegmentTypeId)
{
    ...
    {
            if (iSegmentTypeId ==
zCompositeHoldTot->pzHoldTot[i].iSegmentTypeID) return i;
    }
    return -1;
}
...
void 	AddHoldtoToComposite(HOLDTOTTABLE* zCompositeHoldTot,HOLDTOT zHoldTot)
{
    ...
    if (zCompositeHoldTot->iCreated == zCompositeHoldTot->iTotRecords){
            ...
            zCompositeHoldTot->pzHoldTot =
(HOLDTOT*)realloc(zCompositeHoldTot->pzHoldTot,zCompositeHoldTot->iCreated*sizeof(HOLDTOT));
if (zCompositeHoldTot->pzHoldTot == NULL)
            {
                    lpfnPrintError("Insufficient Memory For HoldTable", 0, 0,
"", 997, 0, 0, "COMPCREATE AddHoldtoToComposite", FALSE); return;
            }
            index = IsSegmentExist(zCompositeHoldTot,zHoldTot.iSegmentTypeID);
            if (index = -1)
                    zCompositeHoldTot->pzHoldTot[zCompositeHoldTot->iTotRecords++]
= zHoldTot; else SumUpHoldTot(&zCompositeHoldTot->pzHoldTot[index],zHoldTot);
    }
...
{
        if (zCompositeHoldTot->iCreated > 0 && zCompositeHoldTot->pzHoldTot !=
NULL) free(zCompositeHoldTot->pzHoldTot); zCompositeHoldTot->pzHoldTot = NULL;

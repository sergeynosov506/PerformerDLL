...
{
#ifdef DEBUG
	PrintError("Entering", 0, 0, sPortmainName, 0, 0, 0, "SelectLastTransNo", FALSE);
#endif
	InitializeErrStruct(pzErr);
	try
	{
		std::string sql = "SELECT lasttransno FROM %PORTMAIN_TABLE_NAME% WHERE id = ?";
	...
	{
		*pzErr = PrintError("SelectLastTransNo", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}
...
{
#ifdef DEBUG
	PrintError("Entering", 0, 0, sPortmainName, 0, 0, 0, "SelectLastRollDate", FALSE);
#endif
	InitializeErrStruct(pzErr);
	try
	{
	...
	{
		*pzErr = PrintError("SelectLastRollDate", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}
...
{
#ifdef DEBUG
	PrintError("Entering", 0, 0, sPortmainName, 0, 0, 0, "UpdateRollDateAndLastTransNo", FALSE);
#endif
	InitializeErrStruct(pzErr);
	try
	{
		std::string sql = "UPDATE %Portmain_TABLE_NAME% SET RollDate = ?, LastTransNo = ?, AcctRolledOn = GetDate() WHERE id = ?";
		sql = ReplaceTableName(sql, "%Portmain_TABLE_NAME%", sPortmainName);
	...
	{
		*pzErr = PrintError("UpdateRollDateAndLastTransNo", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}
...
DLLAPI void STDCALL UpdateRollDate(long iID, long lRollDate, char *sPortmainName, ERRSTRUCT *pzErr)
{
	...
	try
	{
		std::string sql = "UPDATE %Portmain_TABLE_NAME% SET RollDate = ?, AcctRolledOn = GetDate() WHERE id = ?";
		sql = ReplaceTableName(sql, "%Portmain_TABLE_NAME%", sPortmainName);
	...
	{
		*pzErr = PrintError("UpdateRollDate", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

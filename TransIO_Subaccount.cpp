...
void AddSubAcct(SUBACCT zSAcct)
{
  if (zSATable.iNumSAcct == NUMSUBACCT) 
    PrintError((char*)"SubAcct Table is Too Small", 0, 0, (char*)"", 999, 0, 0, (char*)"OLEDBIO ADDSUBACCT", FALSE);
  else 
  {
...
DLLAPI void STDCALL SelectSellAcctType(char *sSellAcctType, char *sAcctType, ERRSTRUCT *pzErr)
{
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"", 0, -1, 0, (char*)"SelectSellAcctType", FALSE);
        return;
    }
    ...
    try
    {
        ...
        nanodbc::prepare(stmt, NANODBC_TEXT("SELECT sell_acct_type FROM subacct WHERE acct_type = ? "));

        stmt.bind(0, sAcctType);
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), 0, 0, (char*)"", 0, -1, 0, (char*)"SelectSellAcctType", FALSE);
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in SelectSellAcctType", 0, 0, (char*)"", 0, -1, 0, (char*)"SelectSellAcctType", FALSE);
    }
}

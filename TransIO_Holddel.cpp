...
void FillHolddelStruct(nanodbc::result& result, HOLDDEL* pzHD)
{
    ...
    pzHD->lCreateDate = timestamp_to_long(result.get<nanodbc::timestamp>("create_date"));
    ...
    pzHD->lAsofDate = timestamp_to_long(result.get<nanodbc::timestamp>("asof_date"));
    ...
    pzHD->lTrdDate = timestamp_to_long(result.get<nanodbc::timestamp>("trd_date"));
    pzHD->lEffDate = timestamp_to_long(result.get<nanodbc::timestamp>("eff_date"));
    pzHD->lEligDate = timestamp_to_long(result.get<nanodbc::timestamp>("elig_date"));
    pzHD->lStlDate = timestamp_to_long(result.get<nanodbc::timestamp>("stl_date"));
    pzHD->fOrigYield = result.get<double>("orig_yield", 0.0);
    pzHD->lEffMatDate = timestamp_to_long(result.get<nanodbc::timestamp>("eff_mat_date"));
    pzHD->fEffMatPrice = result.get<double>("eff_mat_price", 0.0);
    pzHD->fCostEffMatYld = result.get<double>("cost_eff_mat_yld", 0.0);
    pzHD->lAmortStartDate = timestamp_to_long(result.get<nanodbc::timestamp>("amort_start_date"));
    read_string(result, "orig_trans_type", pzHD->sOrigTransType, sizeof(pzHD->sOrigTransType));
    read_string(result, "orig_trans_srce", pzHD->sOrigTransSrce, sizeof(pzHD->sOrigTransSrce));
    read_string(result, "last_trans_type", pzHD->sLastTransType, sizeof(pzHD->sLastTransType));
    pzHD->lLastTransNo = result.get<long>("last_trans_no", 0);
    read_string(result, "last_trans_srce", pzHD->sLastTransSrce, sizeof(pzHD->sLastTransSrce));
    pzHD->lLastPmtDate = timestamp_to_long(result.get<nanodbc::timestamp>("last_pmt_date"));
    read_string(result, "last_pmt_type", pzHD->sLastPmtType, sizeof(pzHD->sLastPmtType));
    pzHD->lLastPmtTrNo = result.get<long>("last_pmt_tr_no", 0);
    pzHD->lNextPmtDate = timestamp_to_long(result.get<nanodbc::timestamp>("next_pmt_date"));
    pzHD->fNextPmtAmt = result.get<double>("next_pmt_amt", 0.0);
    pzHD->lLastPdnDate = timestamp_to_long(result.get<nanodbc::timestamp>("last_pdn_date"));
    ...
    read_string(result, "benchmark_sec_no", pzHD->sBenchmarkSecNo, sizeof(pzHD->sBenchmarkSecNo));
    read_string(result, "perm_lt_flag", pzHD->sPermLtFlag, sizeof(pzHD->sPermLtFlag));
    read_string(result, "valuation_srce", pzHD->sValuationSrce, sizeof(pzHD->sValuationSrce));
    read_string(result, "primary_type", pzHD->sPrimaryType, sizeof(pzHD->sPrimaryType));
...
DLLAPI void STDCALL InsertHolddel(HOLDDEL zHD, ERRSTRUCT *pzErr)
{
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"T", 0, -1, 0, (char*)"InsertHolddel", FALSE);
        return;
    }
    ...
    try
    {
        ...
        nanodbc::timestamp tsCreateDate, tsAsofDate, tsTrdDate, tsEffDate, tsEligDate, tsStlDate;
        ...
        stmt.bind(idx++, zHD.sSecNo);
        stmt.bind(idx++, zHD.sWi);
        stmt.bind(idx++, zHD.sSecXtend);
        stmt.bind(idx++, zHD.sAcctType);
        stmt.bind(idx++, &zHD.lTransNo);
        stmt.bind(idx++, &zHD.iSecID);
        stmt.bind(idx++, &tsAsofDate);
        stmt.bind(idx++, zHD.sSecSymbol);
        ...
        stmt.bind(idx++, zHD.sOrigTransType);
        stmt.bind(idx++, zHD.sOrigTransSrce);
        stmt.bind(idx++, zHD.sLastTransType);
        stmt.bind(idx++, &zHD.lLastTransNo);
        stmt.bind(idx++, zHD.sLastTransSrce);
        stmt.bind(idx++, &tsLastPmtDate);
        stmt.bind(idx++, zHD.sLastPmtType);
        stmt.bind(idx++, &zHD.lLastPmtTrNo);
        stmt.bind(idx++, &tsNextPmtDate);
        stmt.bind(idx++, &zHD.fNextPmtAmt);
        stmt.bind(idx++, &tsLastPdnDate);
        stmt.bind(idx++, zHD.sLtStInd);
        ...
        stmt.bind(idx++, zHD.sSafekInd);
        stmt.bind(idx++, &zHD.fCollateralUnits);
        stmt.bind(idx++, &zHD.fHedgeValue);
        stmt.bind(idx++, zHD.sBenchmarkSecNo);
        stmt.bind(idx++, zHD.sPermLtFlag);
        stmt.bind(idx++, zHD.sValuationSrce);
        stmt.bind(idx++, zHD.sPrimaryType);
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), zHD.iID, zHD.lTransNo, (char*)"T", 0, -1, 0, (char*)"InsertHolddel", FALSE);
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in InsertHolddel", zHD.iID, zHD.lTransNo, (char*)"T", 0, -1, 0, (char*)"InsertHolddel", FALSE);
    }
}
                             ...
                             char *sSecXtend, char *sAcctType, long lTransNo, ERRSTRUCT *pzErr)
{
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"T", 0, -1, 0, (char*)"SelectHolddel", FALSE);
        return;
    }
    ...
    try
    {
        ...
        stmt.bind(0, &lCreateTransNo);
        stmt.bind(1, &tsCreateDate);
        stmt.bind(2, &iID);
        stmt.bind(3, sSecNo);
        stmt.bind(4, sWi);
        stmt.bind(5, sSecXtend);
        stmt.bind(6, sAcctType);
        stmt.bind(7, &lTransNo);
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), iID, lTransNo, (char*)"T", 0, -1, 0, (char*)"SelectHolddel", FALSE);
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in SelectHolddel", iID, lTransNo, (char*)"T", 0, -1, 0, (char*)"SelectHolddel", FALSE);
    }
}
...
DLLAPI void STDCALL HolddelUpdate(int iID, long lRevTransNo, long lCreateTransNo, long lCreateDate, ERRSTRUCT *pzErr)
{
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"T", 0, -1, 0, (char*)"HolddelUpdate", FALSE);
        return;
    }
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), iID, 0, (char*)"T", 0, -1, 0, (char*)"HolddelUpdate", FALSE);
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in HolddelUpdate", iID, 0, (char*)"T", 0, -1, 0, (char*)"HolddelUpdate", FALSE);
    }
}

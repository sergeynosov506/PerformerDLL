...
DLLAPI void STDCALL SelectCallPut(char *sCallPut, char *sSecNo,
                                  char *sWhenIssue, ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);

  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"D", 0,
                        -1, 0, (char *)"SelectCallPut", FALSE);
    return;
  }
  ...
  try {
    ...
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"D", 0, -1, 0,
                        (char *)"SelectCallPut", FALSE);
  } catch (...) {
    *pzErr = PrintError((char *)"Unexpected error in SelectCallPut", 0, 0,
                        (char *)"D", 0, -1, 0, (char *)"SelectCallPut", FALSE);
  }
}

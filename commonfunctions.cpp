...
* 
* DESCRIPTION:	This is a copy of Commonfunctionc.c file with C++ extension (cpp)
*				No other changes
* 
* PUBLIC FUNCTION(S): 
* 
...
long GetStlDate(HOLDINGS zHold)
{
	...
	if (strcmp(zHold.sOrigTransType, "FR") == 0 && zHold.fTotCost < zHold.fOrigCost)
...
double TruncateDouble(double fNumber, int iPrecision)
{
	...
	// e.g. if the number is 100.80 and the required precision is 2, fTemp1 will be 0.79999999998836
	// and fTemp2 will become 79.999999998836 which will be wrong. So after adding 0.0001, the number
...
HINSTANCE LoadLibrarySafe(LPCTSTR LibFileName)
{
	...
	GetPrivateProfileString("GENERAL", "CommonLibPath", "", libPath, MAX_PATH, ChangeFileExt(appFullName, ".ini").c_str());
	if (libPath[0] == '\0')

 ...
 *
 * $Header:   J:/Performer/Data/archives/Master/C/UPDHOLD2.C-arc   1.29   21 Mar
 * 2005 12:28:52   GriffinJ  $
 *
 **/
...
**/
void STDCALL InitializeErrStruct(ERRSTRUCT *pzErr) {
  pzErr->iID = 0;
  pzErr->lRecNo = 0;
...
**/
void STDCALL InitializeHoldingsStruct(HOLDINGS *pzHoldings) {
  pzHoldings->iID = 0;
...
**/
void STDCALL InitializeHoldcashStruct(HOLDCASH *pzHoldcash) {
  pzHoldcash->iID = 0;
  ...
  pzHoldcash->fCurrencyGl = pzHoldcash->fCollateralUnits =
      pzHoldcash->fHedgeValue = 0;
} /* initholdcash */

/**
** Function to initialize Trans structure defined in trans.h file
**/
void STDCALL InitializeTransStruct(TRANS *pzTrans) {
  pzTrans->iID = 0;
  pzTrans->lTransNo = 0;
...
void InitializeHedgeXrefStruct(HEDGEXREF *pzHedgeXref) {
  ...
  pzHedgeXref->fHedgeUnits = pzHedgeXref->fHedgeUnits2 =
      pzHedgeXref->fHedgeValBase = 0;
...
void CopyFieldsFromHoldingsToTrans(TRANS *pzTR, HOLDINGS zHR,
  ...
  if (strcmp(zHR.sOrigTransType, "FR") == 0 ||
      strcmp(zHR.sOrigTransType, "TS") == 0) {
...
ERRSTRUCT GetSecurityCharacteristics(char *sSecNo, char *sWi, char *sPType,
  ...
  */
  lpprSecCharacteristics(sPType, sSType, sPostnInd, sLotInd, sCostInd,
                         sLotExistInd, sAvgInd, pfTrdUnit, sCurrId, sSecNo, sWi,
                         &zErr);
...
void CalculateNewCostXRate(double fHoldCost, double fTransCost, double fHoldCXR,
  ...
  /* SB 10/3/2012 - Idea of using absolute values was wrong. If you use absolute
   * value, you'll get wrong *
   *	exchange rate, values should be used as they are. Commented out
   * following code
   */
  /*if (fHoldCost < 0)
...
// Linkage fix: Remove DLLAPI (extern "C") to match DEF file's expected C++
// decorated name
int STDCALL SetErrorFileName(char *sErrFile) {
...
ERRSTRUCT STDCALL PrintError(const char *sErrMsg, int iID, long lRecNo,
  ...
  if (fp != NULL) {
    fprintf(fp,
            "%s  %s  %s|%s|SqlErr - %ld|IsamErr - %ld|%s|ErrMsg - "
            "%s|MsgLocation - %s\n",
            sDate, sTime, sErrWarn, sTemp, iSqlErr, iIsamCode, sRecDesc,
            sErrMsg, sMsgIdentifier);
    fclose(fp);
  }

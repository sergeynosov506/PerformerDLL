 ...
 *
 * DESCRIPTION: To create Holdtot records from holdings/holdcash of a portfolio
 * for the given date
 *
 * PUBLIC FUNCTION(S):
 *                      1. DLLAPI ERRSTRUCT STDCALL WINAPI InitHoldtot(long
 * lAsofDate, char *sDBAlias, char *sErrFile);
 *                      2. DLLAPI ERRSTRUCT STDCALL WINAPI CreateHoldtot(int
 * iPortfolioID, long lDate);
 *
 * NOTES: Calling program MUST call InitHoldtot once before calling
 * CreateHoldtot to initialize the Dll
 *
 * USAGE:
 *
 ...
 **/
// History
// 2010-09-16 VI#44799 Added capability to calculate average final bond maturity
// - mk. 2010-09-14 VI#44798 Added flag to exclude zero-RAT/MATs' market values
// in average calculation in holdtot - mk. 2010-06-10 VI#44127 Added flag to
// include zero-YTMs' market values in average calculation in holdtot - mk.
// 2007-11-08 Added Vendor id    - yb
// 07/22/2004 - Fixed IsAssetInternational (was looking for pointer <> 0 rather
// than contents) for Level2 - mk 05/25/2004 - Added calcualtions of equity
// statistics (had to rename to *.cpp) - vay
...
DLLAPI ERRSTRUCT STDCALL WINAPI CreateHoldtot(int iPortfolioID, long lDate) {
  ...
  CleanUpPortfolioTables(&zHoldingsAssetsTable, &zSegmainTable,
                         &zPortfolioRules, &zATable);
  ...
  if (zErr.iSqlError != 0 || zErr.iBusinessError != 0) {
    if (zErr.iSqlError == SQLNOTFOUND)
      lpfnPrintError("Portfolio Not Found", iPortfolioID, 0, "", 1,
                     zErr.iSqlError, 0, "CREATEHOLDTOT1", FALSE);
  ...
  __try {
    ...
    zErr = GetHoldingsAndAssetTables(zPmain.iID, zPmain.iVendorID, lDate,
                                     &zHoldingsAssetsTable, &zATable);
    if (zErr.iBusinessError != 0 || zErr.iSqlError != 0) {
      CleanUpPortfolioTables(&zHoldingsAssetsTable, &zSegmainTable,
                             &zPortfolioRules, &zATable);
      if (bUseTrans)
    ...
    // For Stock & Bond accounts, don't want to create any segments here and
    // those accounts have no transactions, so check if there are any
    // transaction for this portfolio
    ...
    if (zErr.iSqlError || zErr.iBusinessError) {
      ...
      if (bUseTrans)
        lpfnRollbackTransaction();
      return (lpfnPrintError("Error Reading Regular Count From Trans",
                             zPmain.iID, 0, "", 0, zErr.iSqlError,
                             zErr.iIsamCode, "CREATEHOLDTOT2", FALSE));
    }

    /*
    ** There are some portfolios where performance is not calculated at all or
    * is calculated only
    ** for total portfolio or level1 but some of the reports (account summary,
    * analytical summary)
    ** require holdtoy information upto level2. If portfolio is calculating
    * performance on all the
    ** rules used by system then create segments for all its perfules, else
    * create the segments
    ** for all the system rules.
    ** Don't do this for stock and bond portfolios (even composites will be
    * missed because they
    ** don't have any transaction but that's OK, composite merge of performance
    * will create any
    ** segments, if required.
    */
    if (lTransCount > 0 || zPmain.bIsMarketIndex) {
      if (zPortfolioRules.iCount >= zSystemRules.iCount)
        zErr = CreateSegmainsIfRequired(&zSegmainTable, zPmain, lDate, zATable,
                                        zPortfolioRules);
      else
        zErr = CreateSegmainsIfRequired(&zSegmainTable, zPmain, lDate, zATable,
                                        zSystemRules);
    ...
    zErr = CreateHoldtotRecord(zPmain, lDate, &zHoldingsAssetsTable,
                               &zSegmainTable);
...
HRESULT CoCreateStatSrv(IStatisticsPtr &obj) {
  struct __declspec(uuid(
      "729052B6-696B-4E48-A295-53E79D1DB159")) CLASS_Statistics;
  struct __declspec(uuid(
      "00000001-0000-0000-C000-000000000046")) IID_CLASS_FACTORY;
  ...
  BOOL(WINAPI * DllGetClassObject)(REFCLSID, REFIID, LPVOID) = (BOOL(WINAPI *)(
      REFCLSID, REFIID, LPVOID))GetProcAddress(hDll, "DllGetClassObject");

  if (!DllGetClassObject)
    return HRESULT_FROM_WIN32(GetLastError());

  CComPtr<IClassFactory> pClassFactory;
  HRESULT hr = DllGetClassObject(__uuidof(CLASS_Statistics),
                                 __uuidof(IID_CLASS_FACTORY), &pClassFactory);
  if (FAILED(hr))
    return hr;

  hr =
      pClassFactory->CreateInstance(NULL, __uuidof(IStatistics), (void **)&obj);
  if (FAILED(hr))
    return hr;
...
/*
** For each script it checks whether the Perfkey exists. If it exists then for
* each Asset
...
*/
ERRSTRUCT CreateHoldtotRecord(PARTPMAIN zPartPortMain, long lDate,
                              HOLDINGSASSETSTABLE *zHoldingsAssetsTable,
                              SEGMAINTABLE *pzSegmainTable) {
  ERRSTRUCT zErr;
  int i, j, Level2, iInternationalLvlId;
  int iSegmentId = 0, IntIndex = 0, iLevel = 0, iUnsupervisedSegmentId,
      iPlIndex = 0;
  int iPledgedSegmentId = 0, iPledgeSegment = 0, iSegTypeId, iSegLvlId;
  double TotalMktValue = 0, TotalCost = 0, fTemp = 0;
  long lScrhdrNo = 0;
  char sCheckFor, sSecurity[1 + NT], sSecNo[12 + NT], sWi[1 + NT],
      sSecXtend[2 + NT];
  BOOL bResult, bAddToHoldtot, bInternational, bLevel2 = FALSE;
  MKTVALUES zTotalMV, zSegmentMV, zInterSegmentTotals, zOtherSegmentTotals,
      zInternationalHoldTotTotal[10], zPledgedHoldTotTotal[10];
  PARTASSET2 zPartAsset;
  HOLDTOT zHoldtot, zInterHoldtot, zOtherHoldtot, zInternationalHoldTot[10],
      zUnsupervisedHoldTot, zPledgedHoldTot[10];
  LEVELINFO zLevels;
  ...
  /*
  ** Add up the market value totals to use for calculating the weighted values.
  * Unsupervised
  ** taxlots are not included in the totals. If the corresponding value to be
  * weighted is 0,
  ** the mkt val for the security is not added i.e. the weightage will be
  * calculated for non
  ...
  */
  for (i = 0; i < zHoldingsAssetsTable->iNumHoldingsAssets; i++) {
    ...
    if (!IsValueZero(zHoldingsAssetsTable->pzHoldingsAssets[i].fMvSysXrate,
                     12)) // mkt val in sys curr
      zTotalMV.fSysMktVal +=
          zHoldingsAssetsTable->pzHoldingsAssets[i].fMktVal /
          zHoldingsAssetsTable->pzHoldingsAssets[i].fMvSysXrate;
    ...
    fTemp = zHoldingsAssetsTable->pzHoldingsAssets[i].fMktVal /
            zHoldingsAssetsTable->pzHoldingsAssets[i].fMvBaseXrate;
    ...
    // yield to maturity
    if (!IsValueZero(zHoldingsAssetsTable->pzHoldingsAssets[i].fMktEffMatYld,
                     5))
    ...
    // ratings
    // if (GetNumericalRating(zHoldingsAssetsTable->pzHoldingsAssets[i].sRating,
    // zHoldingsAssetsTable->pzHoldingsAssets[i].sPrimaryType[0]) != 0)
    ...
    if (sCheckFor == 'M') {
      if (GetNumericalRating(
              zHoldingsAssetsTable->pzHoldingsAssets[i].sMoodyRating,
              zHoldingsAssetsTable->pzHoldingsAssets[i].sPrimaryType[0]) != 0)
        zTotalMV.fMktValForRatings += fTemp;
    }
    ...
    if (sCheckFor == 'S' || sCheckFor == 'B') {
      if (GetNumericalRating(
              zHoldingsAssetsTable->pzHoldingsAssets[i].sSpRating,
              zHoldingsAssetsTable->pzHoldingsAssets[i].sPrimaryType[0]) != 0)
        zTotalMV.fMktValForRatings += fTemp;
    }
    ...
    // maturity real
    if (!IsValueZero(zHoldingsAssetsTable->pzHoldingsAssets[i].fMaturityReal,
                     3))
    ...
    if (zHoldingsAssetsTable->pzHoldingsAssets[i].sPrimaryType[0] == 'E') {
      ...
      if (!IsValueZero(zHoldingsAssetsTable->pzHoldingsAssets[i].fBookValue,
                       2) &&
          !IsValueZero(zHoldingsAssetsTable->pzHoldingsAssets[i].fClosePrice,
                       6))
        zTotalMV.fMktValForPriceBook += fTemp;

      if (!IsValueZero(zHoldingsAssetsTable->pzHoldingsAssets[i].fSales, 0) &&
          !IsValueZero(
              zHoldingsAssetsTable->pzHoldingsAssets[i].fSharesOutstand, 0) &&
          !IsValueZero(zHoldingsAssetsTable->pzHoldingsAssets[i].fClosePrice,
                       6))
        zTotalMV.fMktValForPriceSales += fTemp;

      if (!IsValueZero(zHoldingsAssetsTable->pzHoldingsAssets[i].fTrail12mPE,
                       3))
      ...
      if (!IsValueZero(zHoldingsAssetsTable->pzHoldingsAssets[i].fAnnDivCpn,
                       7) &&
          !IsValueZero(zHoldingsAssetsTable->pzHoldingsAssets[i].fClosePrice,
                       6))
  ...
  /*
  ** Go through each script in the system and for each one of them, find the
  * appropriate
  ** segment type, if the portfolio does not have that segment type, skip the
  * record.
  ** For each segment type, go through all the holdings record and test each one
  * of them
  ** to see if they pass current script or not, if they do it should be added to
  * current
  ** holdtot record.
  */
  for (i = 0; i < g_zSHdrDetTable.iNumSHdrDet; i++) {
    ...
    iSegLvlId =
        GetSegmentLevelId(g_zSHdrDetTable.pzSHdrDet[i].zHeader.iSegmentTypeID);
    ...
    for (j = 0; j < zHoldingsAssetsTable->iNumHoldingsAssets; j++) {
      GetPAssetsFromHoldingsAssets(&zPartAsset, &zLevels,
                                   zHoldingsAssetsTable->pzHoldingsAssets[j],
                                   lDate);

      // test asset against perf script detail and process if passes
      zErr = lpprTestAsset(zPartAsset, g_zSHdrDetTable.pzSHdrDet[i].pzDetail,
                           g_zSHdrDetTable.pzSHdrDet[i].iNumDetail, TRUE,
                           zPartAsset.pzDAInfo[0].lDate, zPartPortMain,
                           &bResult, lDate, lDate);
      ...
      bAddToHoldtot = TRUE;
      AddHoldingsToHoldtot(zHoldingsAssetsTable->pzHoldingsAssets[j], &zHoldtot,
                           lDate, &zSegmentMV);

      iInternationalLvlId = GetSegmentLevelId(
          g_zSHdrDetTable.pzSHdrDet[i].zHeader.iSegmentTypeID);
      strcpy_s(sSecurity, "O");

      if (strcmp(zHoldingsAssetsTable->pzHoldingsAssets[j].sSecXtend, "PL") ==
              0 &&
          bLevel2) {
        iPledgedSegmentId = GetPledgedSegmentId(
            zHoldingsAssetsTable->pzHoldingsAssets[j].iIndustLevel1, -1);
        if (iPledgedSegmentId != 0) {
          if (iPledgeSegment == iPledgedSegmentId) {
            AddHoldingsToHoldtot(zHoldingsAssetsTable->pzHoldingsAssets[j],
                                 &zPledgedHoldTot[iPlIndex], lDate,
                                 &zPledgedHoldTotTotal[iPlIndex]);
          } else {
            iPlIndex++;
            AddHoldingsToHoldtot(zHoldingsAssetsTable->pzHoldingsAssets[j],
                                 &zPledgedHoldTot[iPlIndex], lDate,
          ...
          if (iPledgedSegmentId != 0) {
            if (strcmp(sSecNo,
                       zHoldingsAssetsTable->pzHoldingsAssets[j].sSecNo) != 0 ||
                strcmp(sWi, zHoldingsAssetsTable->pzHoldingsAssets[j].sWi) !=
                    0 ||
                strcmp(sSecXtend,
                       zHoldingsAssetsTable->pzHoldingsAssets[j].sSecXtend) !=
                    0)
      ...
      if (iInternationalLvlId == INTERNATIONALLEVELID &&
          strcmp(sSecurity, "P") != 0 && bLevel2) {
        bInternational = IsAssetInternational(
            zPartPortMain, zHoldingsAssetsTable->pzHoldingsAssets[j].sCurrId,
            zHoldingsAssetsTable->pzHoldingsAssets[j].iIndustLevel1, &Level2);
        if (bInternational) {
          if (strcmp(sSecNo,
                     zHoldingsAssetsTable->pzHoldingsAssets[j].sSecNo) != 0 ||
          ...
          if (iSegmentId != 0) {
            ...
            if (iLevel == iSegmentId)
              AddHoldingsToHoldtot(
                  zHoldingsAssetsTable->pzHoldingsAssets[j],
                  &zInternationalHoldTot[IntIndex], lDate,
                  &zInternationalHoldTotTotal[IntIndex]); //&zInterHoldtot
            else {
              IntIndex++;
              AddHoldingsToHoldtot(
                  zHoldingsAssetsTable->pzHoldingsAssets[j],
      ...
      if (strcmp(sSecurity, "O") == 0) {
        AddHoldingsToHoldtot(zHoldingsAssetsTable->pzHoldingsAssets[j],
                             &zOtherHoldtot, lDate, &zOtherSegmentTotals);
        // Note that even though current secno and wi is bieng compared to
        // previous values, current secxtend is not. Reason is USD, IN & RP
        // should be treated as same, but PL and RP/IN should be treated
        // differently, that's why if previous secxtend is PL, then it does not
        // matter what current sec_xtend is (it can only be RP/IN), it should be
        // considered a different segment.
        if (strcmp(sSecNo, zHoldingsAssetsTable->pzHoldingsAssets[j].sSecNo) !=
                0 ||
            strcmp(sWi, zHoldingsAssetsTable->pzHoldingsAssets[j].sWi) != 0 ||
    ...
    if (bAddToHoldtot) {
      if (zInterHoldtot.iNumberofsecurities > 0) {
        FillAdditionalHoldTotData(&zInterHoldtot, zInterSegmentTotals,
                                  zTotalMV);
        ...
        zInterHoldtot.iScrhdrNo =
            g_zSHdrDetTable.pzSHdrDet[i].zHeader.lScrhdrNo;
        zInternationalHoldTot[IntIndex].iScrhdrNo =
            g_zSHdrDetTable.pzSHdrDet[i].zHeader.lScrhdrNo;
      }

      if (zPledgedHoldTot[iPlIndex].iNumberofsecurities > 0) {
        zPledgedHoldTot[iPlIndex].iId = zPartPortMain.iID;
        zPledgedHoldTot[iPlIndex].iScrhdrNo =
            g_zSHdrDetTable.pzSHdrDet[i].zHeader.lScrhdrNo;
      }
      ...
      if (zOtherHoldtot.iNumberofsecurities > 0) {
        ...
        FillAdditionalHoldTotData(&zOtherHoldtot, zOtherSegmentTotals,
                                  zTotalMV);

        zOtherHoldtot.iId = zPartPortMain.iID;
        zOtherHoldtot.iSegmentTypeID =
            g_zSHdrDetTable.pzSHdrDet[i].zHeader.iSegmentTypeID;
        zOtherHoldtot.iScrhdrNo =
            g_zSHdrDetTable.pzSHdrDet[i].zHeader.lScrhdrNo;
  ...
  for (i = 1; i <= IntIndex; i++) {
    FillAdditionalHoldTotData(&zInternationalHoldTot[i],
                              zInternationalHoldTotTotal[i], zTotalMV);
  ...
  lpprSelectUnsupervised(zPartPortMain.iID, UNSUPERVISED_SECXTEND,
                         &TotalMktValue, &TotalCost, &zErr);
  if (zErr.iSqlError == SQLNOTFOUND)
    zErr.iSqlError = 0;
  ...
  for (i = 1; i <= iPlIndex; i++) {
    if (zPledgedHoldTot[i].iNumberofsecurities > 0) {
      FillAdditionalHoldTotData(&zPledgedHoldTot[i], zPledgedHoldTotTotal[i],
                                zTotalMV);
      if (zPledgedHoldTot[i].iNumberofsecurities > 0) {
...
/*==============================================================================================================
** This routine reads all the Holdings and Holdcash record for an account. It
searches
...
ERRSTRUCT GetHoldingsAndAssetTables(int iPortfolioID, int iVendorID, long lDate,
                                    HOLDINGSASSETSTABLE *pHoldingsAssetsTable,
                                    ASSETTABLE2 *pzATable) {
  HOLDINGSASSETS zHoldingsAssets;
  ERRSTRUCT zErr;
  PARTASSET2 zTempAsset;
  LEVELINFO zLevels;
  char sLastSecNo[12 + NT], sLastWi[1 + NT], sLastSecXtend[2 + NT],
      sLastAcctType[1 + NT];
  ...
  while (!zErr.iSqlError) {
    lpprInitializeErrStruct(&zErr);
    memset(&zHoldingsAssets, 0, sizeof(HOLDINGSASSETS));
    lpprSelectAllForHoldTot(iPortfolioID, lDate, &zHoldingsAssets, iVendorID,
                            &zErr);
    if (zErr.iSqlError == SQLNOTFOUND) {
      zErr.iSqlError = 0;
      break;
      ...
      lpprInitializePartAsset(&zTempAsset);
      return (lpfnPrintError("Error Reading Holdings and assets Table ",
                             iPortfolioID, 0, "", zErr.iBusinessError,
                             zErr.iSqlError, zErr.iIsamCode,
                             "HOLDTOT GETHOLDINGSASSETS1", FALSE));
    }
    ...
    iAIdx =
        lpfnFindAssetInTable(*pzATable, zHoldingsAssets.sSecNo,
                             zHoldingsAssets.sWi, TRUE, zTempAsset.iLongShort);
    if (iAIdx < 0 || iAIdx >= pzATable->iNumAsset) {
      zErr = lpfnAddAssetToTable(pzATable, zTempAsset, zLevels, zPSTypeTable,
                                 &iAIdx, lDate, lDate);
      if (zErr.iSqlError != 0 || zErr.iBusinessError != 0) {
        lpprInitializePartAsset(&zTempAsset);
        return zErr;
    ...
    /*If there is a duplicate fetch next records and discard the duplicate
     * record*/
    if (iLastPortfolioId == zHoldingsAssets.iID &&
        strcmp(sLastSecNo, zHoldingsAssets.sSecNo) == 0 &&
        strcmp(sLastWi, zHoldingsAssets.sWi) == 0 &&
        strcmp(sLastSecXtend, zHoldingsAssets.sSecXtend) == 0 &&
        lLastTransNo == zHoldingsAssets.lTransNo &&
        strcmp(sLastAcctType, zHoldingsAssets.sAcctType) == 0)
      continue;
    ...
    if (strcmp(zHoldingsAssets.sTableName, "C") == 0) {
      zHoldingsAssets.fMktCurYld =
          zHoldingsAssets.fYieldToMaturity; // zHoldingsAssets.fAnnDivCpn /
                                            // zHoldingsAssets.fClosePrice;
      zHoldingsAssets.fMktEffMatYld =
          zHoldingsAssets.fYieldToMaturity; // zHoldingsAssets.fAnnDivCpn /
                                            // zHoldingsAssets.fClosePrice;
      zHoldingsAssets.fAnnualIncome = zHoldingsAssets.fUnits *
                                      zHoldingsAssets.fAnnDivCpn *
                                      zHoldingsAssets.fTradUnit / 100;
    } /**/

    // Unsupervised, tech-short and tech-long don't get included in the
    // calculations
    if (strcmp(zHoldingsAssets.sSecXtend, "UP") == 0)
      continue;
    ...
    // zHoldingsAssets.iSTypeIndex is initialized to -1 in StarsIO, will be
    // loaded here later

    if (zHoldingsAssets.fMvBaseXrate == 0.0)
      return (lpfnPrintError("Invalid Mv Base Xrate", iPortfolioID, 0, "T", 67,
                             0, 0, "HOLDTOT GETHOLDINGSASSET2", FALSE));

    if (zHoldingsAssets.fAiBaseXrate == 0.0)
      return (lpfnPrintError("Invalid Ai Base Xrate", iPortfolioID, 0, "T", 123,
                             0, 0, "HOLDTOT GETHOLDINSASSET3", FALSE));

    if (zHoldingsAssets.fBaseCostXrate == 0.0)
      return (lpfnPrintError("Invalid Base Cost Xrate", iPortfolioID, 0, "T",
                             123, 0, 0, "HOLDTOT GETHOLDINSASSET4", FALSE));
...
void AddHoldingsToHoldtot(HOLDINGSASSETS zHoldingsAssets, HOLDTOT *pzHoldtot,
                          long lDate, MKTVALUES *pzSegmentMV) {
  double iRatingVal = 0;
  char sCheckFor;

  // If it is a equity, make the yield and units 0 (shobhit)
  if (zHoldingsAssets.sPrimaryType[0] == 'E') {
    //		zHoldingsAssets.fMktEffMatYld = 0; 6/14/00 Changed back because
    // this causes a descripency between CAP and Analytical Summary
  ...
  pzHoldtot->fNativeCostYield += zHoldingsAssets.fCostEffMatYld *
                                 zHoldingsAssets.fMktVal /
                                 zHoldingsAssets.fMvBaseXrate;
  pzHoldtot->fNativeCurrYield += zHoldingsAssets.fMktCurYld *
                                 zHoldingsAssets.fMktVal /
                                 zHoldingsAssets.fMvBaseXrate;
  ...
  pzHoldtot->fNativeGainorloss += zHoldingsAssets.fSecurityGl +
                                  zHoldingsAssets.fCurrencyGl +
                                  zHoldingsAssets.fSecurityGl;
  ...
  if (!IsValueZero(zHoldingsAssets.fMvBaseXrate, 12)) {
    pzSegmentMV->fBaseMktVal +=
        zHoldingsAssets.fMktVal / zHoldingsAssets.fMvBaseXrate;
    pzHoldtot->fBaseMarketValue +=
        zHoldingsAssets.fMktVal / zHoldingsAssets.fMvBaseXrate;
    pzHoldtot->fBaseCurrGorl +=
        zHoldingsAssets.fCurrencyGl / zHoldingsAssets.fMvBaseXrate;
    pzHoldtot->fBaseSecGorl +=
        zHoldingsAssets.fSecurityGl / zHoldingsAssets.fMvBaseXrate;
    pzHoldtot->fBaseCostYield += zHoldingsAssets.fCostEffMatYld *
                                 zHoldingsAssets.fMktVal /
                                 zHoldingsAssets.fMvBaseXrate;
    pzHoldtot->fBaseParvalue += zHoldingsAssets.fUnits;

    // If WeightedStatisticsFlag determines whether the market value for this
    // security is added to total(for figuring out average of the statistics) or
    // not if the statistics is zero
    if (zSysSet.zSyssetng.sWeightedStatisticsFlag[0] == STATISTICSFLAG_2) {
      // Yield To Maturity
      if (!IsValueZero(zHoldingsAssets.fMktEffMatYld, 5) ||
          zSysSet.iWeightedStatisticsExcludeYTM == 2) {
        pzHoldtot->fCurrentYieldtomaturity += zHoldingsAssets.fMktEffMatYld *
                                              zHoldingsAssets.fMktVal /
                                              zHoldingsAssets.fMvBaseXrate;
        pzSegmentMV->fMktValForYTM +=
            zHoldingsAssets.fMktVal / zHoldingsAssets.fMvBaseXrate;
      }

      // Current Yield
      if (!IsValueZero(zHoldingsAssets.fMktCurYld, 5)) {
        pzHoldtot->fBaseCurrYield += zHoldingsAssets.fMktCurYld *
                                     zHoldingsAssets.fMktVal /
                                     zHoldingsAssets.fMvBaseXrate;
        pzSegmentMV->fMktValForCurYield +=
            zHoldingsAssets.fMktVal / zHoldingsAssets.fMvBaseXrate;
      }

      // Coupon Rate
      if (!IsValueZero(zHoldingsAssets.fCouponRate, 5)) {
        pzHoldtot->fCouponRate += zHoldingsAssets.fCouponRate *
                                  zHoldingsAssets.fMktVal /
                                  zHoldingsAssets.fMvBaseXrate;
        pzSegmentMV->fMktValForCoupon +=
            zHoldingsAssets.fMktVal / zHoldingsAssets.fMvBaseXrate;
      }

      // Duration - Right now this is special - even if the duration is zero,
      // its mkt val is being used to calculate average unlike any other
      // statistics.
      if (zHoldingsAssets.sPrimaryType[0] == 'B' ||
          zHoldingsAssets.sPrimaryType[0] == 'C' ||
          zHoldingsAssets.sPrimaryType[0] == 'M') {
        pzHoldtot->fDuration += zHoldingsAssets.fDuration *
                                zHoldingsAssets.fMktVal /
                                zHoldingsAssets.fMvBaseXrate;
        pzSegmentMV->fMktValForDuration +=
            zHoldingsAssets.fMktVal / zHoldingsAssets.fMvBaseXrate;
      }
      // Rating
      // iRatingVal = GetNumericalRating(zHoldingsAssets.sRating,
      // zHoldingsAssets.sPrimaryType[0]); if (iRatingVal != 0)
      //{
      //	pzHoldtot->fRating	+=  (iRatingVal *
      // zHoldingsAssets.fMktVal) / zHoldingsAssets.fMvBaseXrate;
      //	pzSegmentMV->fMktValForRatings += zHoldingsAssets.fMktVal /
      // zHoldingsAssets.fMvBaseXrate;
      //}
      // Rating
      // GK Changed 2000-11-14
      ...
      if (sCheckFor == 'M') {
        iRatingVal = GetNumericalRating(zHoldingsAssets.sMoodyRating,
                                        zHoldingsAssets.sPrimaryType[0]);
      }
      if (sCheckFor == 'S' || sCheckFor == 'B') {
        iRatingVal = GetNumericalRating(zHoldingsAssets.sSpRating,
                                        zHoldingsAssets.sPrimaryType[0]);
      }
      if (iRatingVal != 0) {
        pzHoldtot->fRating += (iRatingVal * (zHoldingsAssets.fMktVal /
                                             zHoldingsAssets.fMvBaseXrate));
        pzSegmentMV->fMktValForRatings +=
            zHoldingsAssets.fMktVal / zHoldingsAssets.fMvBaseXrate;
      }

      // Maturity
      if (!IsValueZero(zHoldingsAssets.fMaturity, 3)) {
        pzHoldtot->fMaturity += zHoldingsAssets.fMaturity *
                                zHoldingsAssets.fMktVal /
                                zHoldingsAssets.fMvBaseXrate;
        pzSegmentMV->fMktValForMaturity +=
            zHoldingsAssets.fMktVal / zHoldingsAssets.fMvBaseXrate;
      }

      // Maturity Real
      if (!IsValueZero(zHoldingsAssets.fMaturityReal, 3)) {
        pzHoldtot->fMaturityReal += zHoldingsAssets.fMaturityReal *
                                    zHoldingsAssets.fMktVal /
                                    zHoldingsAssets.fMvBaseXrate;
        pzSegmentMV->fMktValForMaturityReal +=
            zHoldingsAssets.fMktVal / zHoldingsAssets.fMvBaseXrate;
      }
      ...
      if (zHoldingsAssets.sPrimaryType[0] == 'E') {
        ...
        if (pzHoldtot->iNumberofsecurities <=
            10) // limit to top 10 equity holdings
        ...
        if (!IsValueZero(zHoldingsAssets.fBookValue, 2) &&
          ...
          pzHoldtot->fPriceBook +=
              zHoldingsAssets.fClosePrice / zHoldingsAssets.fBookValue * fTemp;
          pzSegmentMV->fMktValForPriceBook += fTemp;
        }

        if (!IsValueZero(zHoldingsAssets.fPctEPS1Yr, 3) &&
            !IsValueZero(zHoldingsAssets.fEquityPerShare, 2)) {
          pzHoldtot->fROE += zHoldingsAssets.fPctEPS1Yr /
                             zHoldingsAssets.fEquityPerShare * fTemp;
        ...
        if (!IsValueZero(zHoldingsAssets.fSales, 0) &&
            ...
            !IsValueZero(zHoldingsAssets.fClosePrice, 6)) {
          pzHoldtot->fPriceSales += zHoldingsAssets.fClosePrice *
                                    zHoldingsAssets.fSharesOutstand * fTemp /
        ...
        if (!IsValueZero(zHoldingsAssets.fSharesOutstand, 0) &&
            !IsValueZero(zHoldingsAssets.fClosePrice, 6)) {
          pzHoldtot->fAvgWtdCap += zHoldingsAssets.fClosePrice *
                                   zHoldingsAssets.fSharesOutstand * fTemp;
        ...
        if (!IsValueZero(zHoldingsAssets.fAnnDivCpn, 7) &&
            !IsValueZero(zHoldingsAssets.fClosePrice, 6)) {
          pzHoldtot->fDivYield += zHoldingsAssets.fAnnDivCpn /
                                  zHoldingsAssets.fClosePrice * 100 * fTemp;
        ...
        if (!IsValueZero(zHoldingsAssets.fSharesOutstand, 0) &&
            !IsValueZero(zHoldingsAssets.fClosePrice, 6)) {
          pzHoldtot->fMedianCap =
              zHoldingsAssets.fClosePrice * zHoldingsAssets.fSharesOutstand;
    ...
    else /*if (zSysSettings.sWeightedStatisticsFlag[0] == STATISTICSFLAG_1)*/
    {
      ...
      // Ratings
      // iRatingVal = GetNumericalRating(zHoldingsAssets.sRating,
      // zHoldingsAssets.sPrimaryType[0]); pzHoldtot->fRating	+=  (iRatingVal
      // * zHoldingsAssets.fMktVal) / zHoldingsAssets.fMvBaseXrate;
      // pzSegmentMV->fMktValForRatings += zHoldingsAssets.fMktVal /
      // zHoldingsAssets.fMvBaseXrate;
      // Ratings
      ...
      if ((iRatingVal != 0 || zSysSet.iWeightedStatisticsExcludeRAT == 2) ||
          (zHoldingsAssets.sPrimaryType[0] != 'M' &&
           zHoldingsAssets.sPrimaryType[0] != 'C')) {
        pzHoldtot->fRating += (iRatingVal * (zHoldingsAssets.fMktVal /
                                             zHoldingsAssets.fMvBaseXrate));
      ...
      // Maturity
      if ((zHoldingsAssets.fMaturity != 0 ||
      ...
      // Maturity Real
      if ((zHoldingsAssets.fMaturityReal != 0 ||
           zSysSet.iWeightedStatisticsExcludeRAT == 2) ||
          (zHoldingsAssets.sPrimaryType[0] != 'M' &&
           zHoldingsAssets.sPrimaryType[0] != 'C')) {
        pzHoldtot->fMaturityReal += zHoldingsAssets.fMaturityReal *
  ...
  if (!IsValueZero(zHoldingsAssets.fBaseCostXrate, 12)) {
    pzHoldtot->fBaseOrigCost +=
        zHoldingsAssets.fOrigCost / zHoldingsAssets.fBaseCostXrate;
    pzHoldtot->fBaseTotCost +=
        zHoldingsAssets.fTotCost / zHoldingsAssets.fBaseCostXrate;
  }
  if (!IsValueZero(zHoldingsAssets.fAiBaseXrate, 12) &&
      !IsValueZero(zHoldingsAssets.fMvBaseXrate, 12))
    pzHoldtot->fBaseGainorloss +=
        zHoldingsAssets.fAccrualGl / zHoldingsAssets.fAiBaseXrate +
        zHoldingsAssets.fCurrencyGl / zHoldingsAssets.fMvBaseXrate +
        zHoldingsAssets.fSecurityGl / zHoldingsAssets.fMvBaseXrate;

  if (!IsValueZero(zHoldingsAssets.fAiBaseXrate, 12)) {
    pzHoldtot->fBaseAccrual +=
        zHoldingsAssets.fAccrInt / zHoldingsAssets.fAiBaseXrate;
    pzHoldtot->fBaseAccrGorl +=
        zHoldingsAssets.fAccrualGl / zHoldingsAssets.fAiBaseXrate;
    pzHoldtot->fBaseIncome +=
        zHoldingsAssets.fAnnualIncome / zHoldingsAssets.fAiBaseXrate;
  }
  ...
  if (!IsValueZero(zHoldingsAssets.fSysCostXrate, 12)) {
    pzHoldtot->fSystemOrigCost +=
        zHoldingsAssets.fOrigCost / zHoldingsAssets.fSysCostXrate;
    pzHoldtot->fSystemTotCost +=
        zHoldingsAssets.fTotCost / zHoldingsAssets.fSysCostXrate;
  }

  if (!IsValueZero(zHoldingsAssets.fAiSysXrate, 12) &&
      !IsValueZero(zHoldingsAssets.fMvSysXrate, 12)) {
    pzHoldtot->fSystemGainorloss +=
        zHoldingsAssets.fAccrualGl / zHoldingsAssets.fAiSysXrate +
        zHoldingsAssets.fCurrencyGl / zHoldingsAssets.fMvSysXrate +
        zHoldingsAssets.fSecurityGl / zHoldingsAssets.fMvSysXrate;
  }

  if (!IsValueZero(zHoldingsAssets.fAiSysXrate, 12)) {
    pzHoldtot->fSystemAccrGorl +=
        zHoldingsAssets.fAccrualGl / zHoldingsAssets.fAiSysXrate;
    pzHoldtot->fSystemIncome +=
        zHoldingsAssets.fAnnualIncome / zHoldingsAssets.fAiSysXrate;
    pzHoldtot->fSystemAccrual +=
        zHoldingsAssets.fAccrInt / zHoldingsAssets.fAiSysXrate;
  }

  if (!IsValueZero(zHoldingsAssets.fMvSysXrate, 12)) {
    pzSegmentMV->fSysMktVal +=
        zHoldingsAssets.fMktVal / zHoldingsAssets.fMvSysXrate;
    pzHoldtot->fSystemCurrGorl +=
        zHoldingsAssets.fCurrencyGl / zHoldingsAssets.fMvSysXrate;
    pzHoldtot->fSystemSecGorl +=
        zHoldingsAssets.fSecurityGl / zHoldingsAssets.fMvSysXrate;
    pzHoldtot->fSystemMarketValue +=
        zHoldingsAssets.fMktVal / zHoldingsAssets.fMvSysXrate;
    pzHoldtot->fSystemCostYield += zHoldingsAssets.fCostEffMatYld *
                                   zHoldingsAssets.fMktVal /
                                   zHoldingsAssets.fMvSysXrate;
    pzHoldtot->fSystemCurrYield += zHoldingsAssets.fMktCurYld *
                                   zHoldingsAssets.fMktVal /
                                   zHoldingsAssets.fMvSysXrate;
  }
...
** to PartAsset struct
**/
void GetPAssetsFromHoldingsAssets(PARTASSET2 *pzPartAsset, LEVELINFO *pzLevels,
                                  HOLDINGSASSETS zHoldingsAssets, long lDate) {
...
double GetNumericalRating(char *sRating, char sPrimaryType) {
  ...
  for (i = 0; i < zRatingTable.iNumRating; i++) {
    if (_stricmp(zRatingTable.pzRating[i].sRatingChar, sRating) == 0 &&
        (zRatingTable.pzRating[i].sRatingType[0] == sCheckFor))
...
void InitializeHoldingsAssetsTable(HOLDINGSASSETSTABLE *pzHSTable) {
  if (pzHSTable->iHoldingsAssetsCreated > 0 &&
...
**/
/*ERRSTRUCT AddScriptHeaderToTable(PSCRHDR zPSHeader, PSCRHDRDETTABLE
*pzSHdrDetTable, int *piArrayIndex)
{
  ERRSTRUCT zErr;
...
ERRSTRUCT AddDetailToScrHdrDet(PSCRDET zSDetail, PSCRHDRDET *pzScrHdrDet)
{
  ...
  * /
  ...
  if (zSDetail.lSeqNo > pzScrHdrDet->iDetailCreated)
    return(lpfnPrintError("Invalid SeqNo in PerfScrDet Table", 0, 0, "", 504, 0,
0, "HOLDTOT ADDSCRDET2", FALSE));
...
/**
** This function adds a holding record in the holding table. This function does
* not
** check whether the passed holding record already exist in the table or not.
**/
ERRSTRUCT AddHoldingsAssetsToTable(HOLDINGSASSETSTABLE *pHoldingsAssetsTable,
                                   HOLDINGSASSETS zHoldingsAssets) {
  ERRSTRUCT zErr;
  ...
  /* If table is full to its limit, allocate more space */
  if (pHoldingsAssetsTable->iHoldingsAssetsCreated ==
      pHoldingsAssetsTable->iNumHoldingsAssets) {
    pHoldingsAssetsTable->iHoldingsAssetsCreated += EXTRAHOLD;
    pHoldingsAssetsTable->pzHoldingsAssets = (HOLDINGSASSETS *)realloc(
        pHoldingsAssetsTable->pzHoldingsAssets,
        pHoldingsAssetsTable->iHoldingsAssetsCreated * sizeof(HOLDINGSASSETS));
    if (pHoldingsAssetsTable->pzHoldingsAssets == NULL)
      return (lpfnPrintError("Insufficient Memory For HoldingsAssetsTable", 0,
                             0, "", 997, 0, 0, "HOLDTOT ADDHOLDINGSASSETS",
                             FALSE));
  }

  pHoldingsAssetsTable
      ->pzHoldingsAssets[pHoldingsAssetsTable->iNumHoldingsAssets] =
      zHoldingsAssets;
  pHoldingsAssetsTable->iNumHoldingsAssets++;
...
ERRSTRUCT LoadRatingTable(RATINGTABLE *pzRatingTable) {
  ...
  pzRatingTable->pzRating = (RATING *)realloc(
      pzRatingTable->pzRating, pzRatingTable->iRatingCreated * sizeof(RATING));
  if (pzRatingTable->pzRating == NULL)
    return (lpfnPrintError("Insufficient Memory For pzRatingTable", 0, 0, "",
                           997, 0, 0, "HOLDTOT LoadRatingTable", FALSE));
...
void FillAdditionalHoldTotData(HOLDTOT *pzHoldtot, MKTVALUES zMV,
                               MKTVALUES zTotalMV) {
  ...
  if (!IsValueZero(zMV.fMktValForEquity, 2))
    pzHoldtot->fPcTopTenHold =
        (pzHoldtot->fPcTopTenHold / zMV.fMktValForEquity) * 100;
...
ERRSTRUCT AddSegmentToTable(SEGMENTSTABLE *pzSegmentsTable,
                            SEGMENTS zSegments) {
  ERRSTRUCT zErr;
  ...
  if (pzSegmentsTable->iCapacity == pzSegmentsTable->iCount) {
    ...
    pzSegmentsTable->pzSegments =
        (SEGMENTS *)realloc(pzSegmentsTable->pzSegments,
                            sizeof(SEGMENTS) * pzSegmentsTable->iCapacity);

    if (pzSegmentsTable->pzSegments == NULL)
      return (lpfnPrintError("Insufficient Memory", 0, 0, "", 999, 0, 0,
                             "HOLDTOT AddSegmentToTable", FALSE));
  }
...
ERRSTRUCT AddSegmapToTable(SEGMAPTABLE *pzSegmapTable, SEGMAP zSegmap) {
  ...
  if (pzSegmapTable->iCapacity == pzSegmapTable->iCount) {
    ...
    pzSegmapTable->pzSegmap = (SEGMAP *)realloc(
        pzSegmapTable->pzSegmap, sizeof(SEGMAP) * pzSegmapTable->iCapacity);

    if (pzSegmapTable->pzSegmap == NULL)
      return (lpfnPrintError("Insufficient Memory", 0, 0, "", 999, 0, 0,
                             "HOLDTOT AddSegmapToTable", FALSE));
  }
...
int GetSegmapid(const int iIndustLevel1, const int iIndustLevel2,
                const int iIndustLevel3) {
  int j;
...
BOOL IsAssetInternational(PARTPMAIN zPmain, char sCurrId[],
                          const int IndustLevel1, int *Level2) {
...
int GetUnsupervisedSegmentId(void) {
  ...
  for (i = 0; i < zSegmentsTable.iCount; i++) {
    if (zSegmentsTable.pzSegments[i].iLevelID == UNSUPERVISEDLEVELID &&
        zSegmentsTable.pzSegments[i].iSequenceNbr == UNSUPERVISEDSQNNBR) {
...
/*This routine gets all the segments for the portfolio and put them in memory
 * table*/
ERRSTRUCT GetAllSegmentsFromSegmainForPortId(int iPortfolioID,
                                             SEGMAINTABLE *pzSegmainTable) {
  SEGMAIN zSegmain;
  ERRSTRUCT zErr;
...
ERRSTRUCT AddSegmainToTable(SEGMAINTABLE *pzSegmainTable, SEGMAIN zSegmain) {
  ...
  if (pzSegmainTable->iCount == pzSegmainTable->iCapacity) {
    ...
    pzSegmainTable->pzSegmain = (SEGMAIN *)realloc(
        pzSegmainTable->pzSegmain, pzSegmainTable->iCapacity * sizeof(SEGMAIN));
    if (pzSegmainTable->pzSegmain == NULL)
      return (lpfnPrintError("Insufficient Memory For Segmain Table", 0, 0, "",
                             997, 0, 0, "HOLDTOT ADDSEGMAIN", FALSE));
  }
...
/**
** This function is used to create segmain records, if required for a portfolio.
* This function
** is used by createholdtot dll to create segments without running performance.
* It is written
** in performance dll so that all the functions it needs don't need to be
* exported. When createholdtot
** dll calls this function, perfruletable has all the standard rules used by the
* system rather than
** the rules used by the portfolio, this makes sure that even if all levels of
* performance is
** not being run for this portfolio, it still creates required segments for the
* portfolio on which
** we need summary values in holdtot table for some reports (e.g. analytical
* summary and account summary)
**/
ERRSTRUCT CreateSegmainsIfRequired(SEGMAINTABLE *pzSTable, PARTPMAIN zPmain,
                                   long lCurrentDate, ASSETTABLE2 zATable,
  ...
  /*
  ** Examine all rules in the table against all the asset and for each
  * rule-asset combination
  ** check if a segmain record can be generated. If a segmain record can be
  * generated, check if
  ** it already exist, if it does not then create it(both in memory and in the
  * database).
  */
  ...
  for (i = 0; i < zRuleTable.iCount; i++) {
    ...
    */
    ...
    /*
    ** If the rule is for Single Security Segment, segment on
    * individual/composite accounts
    ...
    */
    ...
    for (j = 0; j < zATable.iNumAsset; j++) {
      ...
      ** currency, both of these can generate diferent segmains(if the detail
      * has
      ...
      */
      for (k = 0; k < 2; k++) {
        if (k == 0)
          ...
          if (strcmp(zATable.pzAsset[j].sCurrId,
                     zATable.pzAsset[j].sIncCurrId) == 0)
            break;
        } /* second pass */

        /* create a virtual perf script using current template and asset */
        zErr = lpfnCreateVirtualPerfScript(zTHdrDetTable.pzTHdrDet[m],
                                           zATable.pzAsset[j], 0, bSecCurr,
                                           &zVirtualSHD);
        if (zErr.iBusinessError != 0) {
        ...
        if (zATable.pzAsset[j].iDailyCount > 0) {
           ...
           ** If it is a special rule(equity+equity cash or fixed+fixed cash),
           * create a key
           ** only if portfolio has Equity or Fixed, i.e. if the portfolio has
           * fixed and cash
           ** but no equity, only fixed + fixed cash key will be generated for
           * this rule.
           */
          if (strcmp(zRuleTable.pzPRule[i].sWeightedRecordIndicator, "E") ==
                  0 ||
              strcmp(zRuleTable.pzPRule[i].sWeightedRecordIndicator, "F") == 0)
            zErr = lpprTestAsset(zATable.pzAsset[j], zVirtualSHD.pzDetail, 1,
                                 bSecCurr, zATable.pzAsset[j].pzDAInfo[0].lDate,
                                 zPmain, &bResult, lCurrentDate, lCurrentDate);
          else
            zErr = lpprTestAsset(zATable.pzAsset[j], zVirtualSHD.pzDetail,
                                 zVirtualSHD.iNumDetail, bSecCurr,
                                 zATable.pzAsset[j].pzDAInfo[0].lDate, zPmain,
                                 &bResult, lCurrentDate, lCurrentDate);
        }
        if (zErr.iBusinessError != 0 || zErr.iSqlError != 0) {
          lpfnInitializePScrHdrDet(&zVirtualSHD);
          return zErr;
        ...
        */
        ...
        /* Asset passed the test, find or create script header for the virtual
         * script */
        zErr = lpfnFindCreateScriptHeader(
            zPmain.iID, lCurrentDate, zATable, &g_zSHdrDetTable, &zVirtualSHD,
            FALSE, zTHdrDetTable.pzTHdrDet[m].zHeader.lTmphdrNo, j, &iIndex);
        if (zErr.iBusinessError != 0 || zErr.iSqlError != 0) {
          lpfnInitializePScrHdrDet(&zVirtualSHD);
          return zErr;
        }
         ...
         */
        if (FindSegmentByType(
                *pzSTable,
                g_zSHdrDetTable.pzSHdrDet[iIndex].zHeader.iSegmentTypeID) < 0) {
          zSegmain.iID = lpfnAddNewSegment(
              zPmain.iID,
              g_zSHdrDetTable.pzSHdrDet[iIndex].zHeader.sDescription,
              g_zSHdrDetTable.pzSHdrDet[iIndex].zHeader.sDescription,
              g_zSHdrDetTable.pzSHdrDet[iIndex].zHeader.iSegmentTypeID);
          zSegmain.iSegmentTypeID =
              g_zSHdrDetTable.pzSHdrDet[iIndex].zHeader.iSegmentTypeID;
...
ERRSTRUCT GetSystemTables() {
  ...
  while (TRUE) {
    ...
    if (zErr.iSqlError == SQLNOTFOUND) {
      if (iNumRecord == 0)
        return (lpfnPrintError("No Script Header and Detail Defined Found", 0,
                               0, "", 0, zErr.iSqlError, zErr.iIsamCode,
                               "HOLDTOT INITLIB", FALSE));
      zErr.iSqlError = 0;
      break;
    } else if (zErr.iSqlError)
      return (lpfnPrintError("Error Fetching Script Header and Detail", 0, 0,
      ...
      */
      if (zSDetail.lScrhdrNo == zSHeader.lScrhdrNo) {
        ...
        zErr = lpfnAddDetailToScrHdrDet(zSDetail,
                                        &g_zSHdrDetTable.pzSHdrDet[iIndex]);
        if (zErr.iBusinessError != 0)
          return zErr;
...
ERRSTRUCT GetAllPerfrulesForAPortfolio(PERFRULETABLE *pzRuleTable, int iID,
                                       long lDate) {
  ERRSTRUCT zErr;
  PERFRULE zPerfRule;
...
// function to clean up (release memory) of all portfolio specific dynamic
// tables
void CleanUpPortfolioTables(HOLDINGSASSETSTABLE *pzHATable,
                            SEGMAINTABLE *pzSTable, PERFRULETABLE *pzPTable,
                            ASSETTABLE2 *pzATable) {
...
**/
extern "C" {
DLLAPI ERRSTRUCT STDCALL WINAPI InitHoldtot(long lAsofDate, char *sDBAlias,
                                            char *sErrFile) {
  ...
  ERRSTRUCT zErr;
  int iError;
  char sHoldingsName[HOLDMAPNAMESIZE], sHoldcashName[HOLDMAPNAMESIZE],
      sPortmainName[HOLDMAPNAMESIZE], sPortbalName[HOLDMAPNAMESIZE];
  char sPayrecName[HOLDMAPNAMESIZE], sHXrefName[HOLDMAPNAMESIZE],
      sHoldtotName[HOLDMAPNAMESIZE];
  ...
  if (!bInit) {
    ...
    if (!lpprInitializeErrStruct) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Initialize Error Structure function", 0,
                             0, "", iError, 0, 0, "CREATEHOLDTOT INIT1",
                             FALSE));
    }
    lpprInitializeErrStruct(&zErr);
    ...
    if (!lpfnPrintError) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load PrintError function", 0, 0, "",
                             iError, 0, 0, "CREATEHOLDTOT INIT2", FALSE));
    }
    ...
    if (!lpfnStartTransaction) {
      ...
      return (lpfnPrintError("Unable To Load StartDBTransaction function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT2a", FALSE));
    }
    ...
    if (!lpfnCommitTransaction) {
      ...
      return (lpfnPrintError("Unable To Load CommitDBTransaction function", 0,
                             0, "", iError, 0, 0, "CREATEHOLDTOT INIT2b",
                             FALSE));
    }
    ...
    if (!lpfnRollbackTransaction) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load RollbackDBTransaction function", 0,
                             0, "", iError, 0, 0, "CREATEHOLDTOT INIT2c",
                             FALSE));
    }
    ...
    if (!lpfnAbortTransaction) {
      ...
      return (lpfnPrintError("Unable To Load AbortDBTransaction function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT2d", FALSE));
    }
    ...
    if (!lpfnGetTransCount) {
      ...
      return (lpfnPrintError("Unable To Load GetTransCount function", 0, 0, "",
                             iError, 0, 0, "CREATEHOLDTOT INIT2e", FALSE));
    }

    lpprDeleteHoldtotForAnAccount =
        (LPPR1LONG)GetProcAddress(OledbIODll, "DeleteHoldtotForAnAccount");
    if (!lpprDeleteHoldtotForAnAccount) {
      iError = GetLastError();
      return (
          lpfnPrintError("Unable To Load DeleteHoldtotForAnAccount function", 0,
                         0, "", iError, 0, 0, "CREATEHOLDTOT INIT3", FALSE));
    }

    lpprSelectHoldmap =
        (LPPR1LONG7PCHAR)GetProcAddress(OledbIODll, "SelectHoldmap");
    if (!lpprSelectHoldmap) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load SelectHoldmap function", 0, 0, "",
                             iError, 0, 0, "CREATEHOLDTOT INIT4", FALSE));
    }

    lpprInsertHoldtot =
        (LPPRHOLDTOT)GetProcAddress(OledbIODll, "InsertHoldtot");
    if (!lpprInsertHoldtot) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load InsertHoldtot function", 0, 0, "",
                             iError, 0, 0, "CREATEHOLDTOT INIT5", FALSE));
    }

    lpprSelectAllForHoldTot = (LPPRSELECTALLFORHOLDTOT)GetProcAddress(
        OledbIODll, "SelectAllForHoldTot");
    if (!lpprSelectAllForHoldTot) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load SelectAllForHoldTot function", 0,
                             0, "", iError, 0, 0, "CREATEHOLDTOT INIT6",
                             FALSE));
    }
    ...
    if (!lpprSelectAllPerfrule) {
      ...
      return (lpfnPrintError("Unable To Load SelectAllPerfrule function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT7", FALSE));
    }
    ...
    if (!lpprSelectOnePartPortmain) {
      ...
      return (lpfnPrintError("Unable To Load SelectOnePartPortmain function", 0,
                             0, "", iError, 0, 0, "CREATEHOLDTOT INIT8",
                             FALSE));
    }

    lpprSelectAllRatings =
        (LPPRSELECTALLRATING)GetProcAddress(OledbIODll, "SelectAllRatings");
    if (!lpprSelectAllRatings) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load SelectAllRatings function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT9", FALSE));
    }
    ...
    if (!lpprSelectAllScriptHeaderAndDetails) {
      ...
      return (lpfnPrintError(
          ...
          iError, 0, 0, "CREATEHOLDTOT INIT10", FALSE));
    }

    lpprSelectSegmainForPortfolio =
        (LPPRSELECTSEGMAINFORPORTFOLIO)GetProcAddress(
            OledbIODll, "SelectSegmainForPortfolio");
    if (!lpprSelectSegmainForPortfolio) {
      iError = GetLastError();
      return (
          lpfnPrintError("Unable To Load SelectSegmainForPortfolio Function", 0,
                         0, "", iError, 0, 0, "CREATEHOLDTOT INIT11", FALSE));
    }
    ...
    if (!lpprSelectStarsDate) {
      ...
      return (lpfnPrintError("Unable To Load SelectStarsDate function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT12", FALSE));
    }

    lpprSelectUnsupervised = (LPPRSELECTUNSUPERVISED)GetProcAddress(
        OledbIODll, "SelectUnsupervised");
    if (!lpprSelectUnsupervised) {
      iError = GetLastError();
      return (lpfnPrintError("Unable to load SelectUnSupervised function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT13", FALSE));
    }

    lpprRegTransCount =
        (LPPR1LONG1PLONG)GetProcAddress(OledbIODll, "CheckIfTransExists");
    if (!lpprRegTransCount) {
      iError = GetLastError();
      return (lpfnPrintError("Error Loading CheckIfTransExists function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT14", FALSE));
    }

    lpprSelectAllSegments =
        (LPPRSELECTALLSEGMENTS)GetProcAddress(OledbIODll, "SelectAllSegments");
    if (!lpprSelectAllSegments) {
      iError = GetLastError();
      return (lpfnPrintError("Unable to load SelectAllSegments function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT15", FALSE));
    }

    lpprSelectAllSegmap =
        (LPPRSELECTALLSEGMAP)GetProcAddress(OledbIODll, "SelectAllSegmap");
    if (!lpprSelectAllSegmap) {
      iError = GetLastError();
      return (lpfnPrintError("Unable to load SelectAllSegmap function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT16", FALSE));
    }
    ...
    if (!lpprSelectSysSettings) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load SelectSysSettings Function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT17", FALSE));
    }
    ...
    if (!lpprSelectAllTemplateHeaderAndDetails) {
      ...
      return (lpfnPrintError(
          ...
          iError, 0, 0, "CREATEHOLDTOT INIT19", FALSE));
    }

    // Load functions from Performance dll
    lpfnAddAssetToTable =
        (LPFNADDASSET)GetProcAddress(PerfCalcDll, "AddAssetToTable");
    if (!lpfnAddAssetToTable) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load AddAssetToTable Function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT20", FALSE));
    }

    lpfnFindAssetInTable =
        (LPFNFINDASSET)GetProcAddress(PerfCalcDll, "FindAssetInTable");
    if (!lpfnFindAssetInTable) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load FindAssetInTable Function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT21", FALSE));
    }

    lpfnCreateNewScript =
        (LPFN2PSCRHDRDET)GetProcAddress(PerfCalcDll, "CreateNewScript");
    if (!lpfnCreateNewScript) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load CreateNewScript Function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT22", FALSE));
    }

    lpprInitPerformance = (LPFN1LONG3PCHAR1BOOL1PCHAR)GetProcAddress(
        PerfCalcDll, "InitPerformance");
    if (!lpprInitPerformance) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load InitPerformance function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT23", FALSE));
    }

    lpprInitializeAssetTable =
        (LPPRASSETTABLE)GetProcAddress(PerfCalcDll, "InitializeAssetTable");
    if (!lpprInitializeAssetTable) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load InitializeAssetTable Function", 0,
                             0, "", iError, 0, 0, "CREATEHOLDTOT INIT24",
                             FALSE));
    }
    ...
    if (!lpprTestAsset) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load TestAsset function", 0, 0, "",
                             iError, 0, 0, "CREATEHOLDTOT INIT25", FALSE));
    }

    lpprInitPerfrule =
        (LPPRPPERFRULE)GetProcAddress(PerfCalcDll, "InitializePerfrule");
    if (!lpprInitPerfrule) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load InitializePerfrule Function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT26", FALSE));
    }

    lpprInitPerfruleTable = (LPPRPPERFRULETABLE)GetProcAddress(
        PerfCalcDll, "InitializePerfruleTable");
    if (!lpprInitPerfruleTable) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load InitializePerfruleTable Function",
                             0, 0, "", iError, 0, 0, "CREATEHOLDTOT INIT27",
                             FALSE));
    }

    lpfnAddPerfruleToTable =
        (LPFNADDPERFRULE)GetProcAddress(PerfCalcDll, "AddPerfruleToTable");
    if (!lpfnAddPerfruleToTable) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load AddPerfruleToTable Function", 0, 0,
                             "", iError, 0, 0, "CREATEHOLDTOT INIT28", FALSE));
    }

    lpfnFindCreateScriptHeader = (LPFNFINDCREATESCRIPTHEADER)GetProcAddress(
        PerfCalcDll, "FindCreateScriptHeader");
    if (!lpfnFindCreateScriptHeader) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load FindCreateScriptHeader Function",
                             0, 0, "", iError, 0, 0, "CREATEHOLDTOT INIT29",
                             FALSE));
    }

    lpfnCreateVirtualPerfScript = (LPFNCREATEPERFSCRIPT)GetProcAddress(
        PerfCalcDll, "CreateVirtualPerfScript");
    if (!lpfnCreateVirtualPerfScript) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load CreateVirtualPerfScript Function",
                             0, 0, "", iError, 0, 0, "CREATEHOLDTOT INIT30",
                             FALSE));
    }

    lpfnAddScriptHeaderToTable =
        (LPFNPSCRHDR)GetProcAddress(PerfCalcDll, "AddScriptHeaderToTable");
    if (!lpfnAddScriptHeaderToTable) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load AddScriptHeaderToTable Function",
                             0, 0, "", iError, 0, 0, "CREATEHOLDTOT INIT31",
                             FALSE));
    }

    lpfnAddDetailToScrHdrDet =
        (LPFNADDSCRDETAIL)GetProcAddress(PerfCalcDll, "AddDetailToScrHdrDet");
    if (!lpfnAddDetailToScrHdrDet) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load AddDetailToScrHdrDet Function", 0,
                             0, "", iError, 0, 0, "CREATEHOLDTOT INIT32",
                             FALSE));
    }

    lpfnInitializePScrHdrDet =
        (LPPRPSCRHDRDET)GetProcAddress(PerfCalcDll, "InitializePScrHdrDet");
    if (!lpfnInitializePScrHdrDet) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load InitializePScrHdrDet Function", 0,
                             0, "", iError, 0, 0, "CREATEHOLDTOT INIT33",
                             FALSE));
    }

    lpfnFillPartSectypeTable = (LPFNPPARTSECTYPETABLE)GetProcAddress(
        PerfCalcDll, "FillPartSectypeTable");
    if (!lpfnFillPartSectypeTable) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load FillPartSectypeTable Function", 0,
                             0, "", iError, 0, 0, "CREATEHOLDTOT INIT34",
                             FALSE));
    }

    lpfnAddNewSegment =
        (LPFN1INT2PCHAR1INT)GetProcAddress(PerfCalcDll, "AddNewSegment");
    if (!lpfnAddNewSegment) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load AddNewSegment Function", 0, 0, "",
                             iError, 0, 0, "CREATEHOLDTOT INIT35", FALSE));
    }

    lpprInitializePerfTmpHdr =
        (LPPRPPTMPHDR)GetProcAddress(PerfCalcDll, "InitializePerfTmpHdr");
    if (!lpprInitializePerfTmpHdr) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load InitializePerfTmpHdr Function", 0,
                             0, "", iError, 0, 0, "CREATEHOLDTOT INIT36",
                             FALSE));
    }

    lpprInitializePerfTmpDet =
        (LPPRPPTMPDET)GetProcAddress(PerfCalcDll, "InitializePerfTmpDet");
    if (!lpprInitializePerfTmpDet) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load InitializePerfTmpDet Function", 0,
                             0, "", iError, 0, 0, "CREATEHOLDTOT INIT37",
                             FALSE));
    }

    lpfnAddTemplateHeaderToTable = (LPFNADDTEMPLATEHEADER)GetProcAddress(
        PerfCalcDll, "AddTemplateHeaderToTable");
    if (!lpfnAddTemplateHeaderToTable) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load AddTemplateHeaderToTable Function",
                             0, 0, "", iError, 0, 0, "CREATEHOLDTOT INIT38",
                             FALSE));
    }

    lpfnAddTemplateDetailToTable = (LPFNADDTEMPLATEDETAIL)GetProcAddress(
        PerfCalcDll, "AddTemplateDetailToTable");
    if (!lpfnAddTemplateDetailToTable) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load AddTemplateDetailToTable Function",
                             0, 0, "", iError, 0, 0, "CREATEHOLDTOT INIT39",
                             FALSE));
    }

    lpprInitializePartAsset =
        (LPPRPARTASSET2)GetProcAddress(PerfCalcDll, "InitializePartialAsset");
    if (!lpprInitializePartAsset) {
      iError = GetLastError();
      return (lpfnPrintError("Unable To Load InitializePartialAsset Function",
                             0, 0, "", iError, 0, 0, "CREATEHOLDTOT INIT40",
                             FALSE));
    }

    lpfnAllocateMemoryToDailyAssetInfo = (LPFNPARTASSET2)GetProcAddress(
        PerfCalcDll, "AllocateMemoryToDailyAssetInfo");
    if (!lpfnAllocateMemoryToDailyAssetInfo)
      return (lpfnPrintError(
          "Unable To Load AllocateMemoryToDailyAssetInfo Function", 0, 0, "",
          GetLastError(), 0, 0, "CREATEHOLDTOT INIT41", FALSE));
  ...
  if (lLastAsofDate != lAsofDate) {
    ...
    lpprSelectHoldmap(lAsofDate, sHoldingsName, sHoldcashName, sPortmainName,
                      sPortbalName, sPayrecName, sHXrefName, sHoldtotName,
                      &zErr);
    if (zErr.iBusinessError != 0)
      return zErr;
    else if (zErr.iSqlError ==
             SQLNOTFOUND) { // if the AsOfDate is Trade date or Pricing Date, do
                            // the current holdtot

...
void FillHoldingsStruct(nanodbc::result &result, HOLDINGS *pzHR) {
  ...
  pzHR->lAmortStartDate =
      timestamp_to_long(result.get<nanodbc::timestamp>("amort_start_date"));
  read_string(result, "orig_trans_type", pzHR->sOrigTransType,
              sizeof(pzHR->sOrigTransType));
  read_string(result, "orig_trans_srce", pzHR->sOrigTransSrce,
              sizeof(pzHR->sOrigTransSrce));
  read_string(result, "last_trans_type", pzHR->sLastTransType,
              sizeof(pzHR->sLastTransType));
  pzHR->lLastTransNo = result.get<long>("last_trans_no", 0);
  read_string(result, "last_trans_srce", pzHR->sLastTransSrce,
              sizeof(pzHR->sLastTransSrce));
  pzHR->lLastPmtDate =
      timestamp_to_long(result.get<nanodbc::timestamp>("last_pmt_date"));
  read_string(result, "last_pmt_type", pzHR->sLastPmtType,
              sizeof(pzHR->sLastPmtType));
  ...
  read_string(result, "benchmark_sec_no", pzHR->sBenchmarkSecNo,
              sizeof(pzHR->sBenchmarkSecNo));
  read_string(result, "perm_lt_flag", pzHR->sPermLtFlag,
              sizeof(pzHR->sPermLtFlag));
  read_string(result, "valuation_srce", pzHR->sValuationSrce,
              sizeof(pzHR->sValuationSrce));
  read_string(result, "primary_type", pzHR->sPrimaryType,
              sizeof(pzHR->sPrimaryType));
  pzHR->iRestrictionCode = result.get<int>("restriction_code", 0);
}

DLLAPI void STDCALL DeleteHoldings(int iID, char *sSecNo, char *sWi,
  ...
  try {
    ...
    nanodbc::prepare(
        stmt,
        NANODBC_TEXT("Delete from holdings "
    ...
    *pzErr = PrintError((char *)msg.c_str(), iID, lTransNo, (char *)"T", 0, -1,
                        0, (char *)"DeleteHoldings", FALSE);
  } catch (...) {
    *pzErr =
        PrintError((char *)"Unexpected error in DeleteHoldings", iID, lTransNo,
                   (char *)"T", 0, -1, 0, (char *)"DeleteHoldings", FALSE);
  }
}
...
DLLAPI void STDCALL InsertHoldings(HOLDINGS zHR, ERRSTRUCT *pzErr) {
  ...
  try {
    ...
    safe_bind_string(stmt, idx, zHR.sSecNo);
    safe_bind_string(stmt, idx, zHR.sWi);
    safe_bind_string(stmt, idx, zHR.sSecXtend);
    safe_bind_string(stmt, idx, zHR.sAcctType);
    stmt.bind(idx++, &zHR.lTransNo);
    stmt.bind(idx++, &zHR.iSecID);
    stmt.bind(idx++, &tsAsofDate);
    safe_bind_string(stmt, idx, zHR.sSecSymbol);
    ...
    safe_bind_string(stmt, idx, zHR.sOrigTransType);
    safe_bind_string(stmt, idx, zHR.sOrigTransSrce);
    safe_bind_string(stmt, idx, zHR.sLastTransType);
    stmt.bind(idx++, &zHR.lLastTransNo);
    safe_bind_string(stmt, idx, zHR.sLastTransSrce);
    stmt.bind(idx++, &tsLastPmtDate);
    safe_bind_string(stmt, idx, zHR.sLastPmtType);
    stmt.bind(idx++, &zHR.lLastPmtTrNo);
    stmt.bind(idx++, &tsNextPmtDate);
    stmt.bind(idx++, &zHR.fNextPmtAmt);
    stmt.bind(idx++, &tsLastPdnDate);
    safe_bind_string(stmt, idx, zHR.sLtStInd);
    ...
    safe_bind_string(stmt, idx, zHR.sSafekInd);
    stmt.bind(idx++, &zHR.fCollateralUnits);
    stmt.bind(idx++, &zHR.fHedgeValue);
    safe_bind_string(stmt, idx, zHR.sBenchmarkSecNo);
    safe_bind_string(stmt, idx, zHR.sPermLtFlag);
    safe_bind_string(stmt, idx, zHR.sValuationSrce);
    safe_bind_string(stmt, idx, zHR.sPrimaryType);
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), zHR.iID, zHR.lTransNo, (char *)"T",
                        0, -1, 0, (char *)"InsertHoldings", FALSE);
  } catch (...) {
    *pzErr = PrintError((char *)"Unexpected error in InsertHoldings", zHR.iID,
                        zHR.lTransNo, (char *)"T", 0, -1, 0,
                        (char *)"InsertHoldings", FALSE);
  }
}

static std::vector<HOLDINGS> g_HoldingsBatch;

DLLAPI void STDCALL InsertHoldingsBatch(HOLDINGS *pzHR, long lBatchSize,
                                        ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);

  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"InsertHoldingsBatch", FALSE);
    return;
  }
  ...
  try {
    ...
    if (g_HoldingsBatch.size() >= (size_t)lBatchSize &&
        g_HoldingsBatch.size() > 0) {
      ...
      for (const auto &zHR : g_HoldingsBatch) {
        ...
        if (pzErr->iSqlError != 0) {
          ...
          // The original code says "any failure will cause entire trans to
          // rollback" But InsertHoldings catches exceptions. If InsertHoldings
          // returns error, we should probably throw to trigger rollback? But
          // InsertHoldings already handles error reporting. Let's just return.
          return;
        }
      }
    ...
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"T", 0, -1, 0,
                        (char *)"InsertHoldingsBatch", FALSE);
  } catch (...) {
    *pzErr =
        PrintError((char *)"Unexpected error in InsertHoldingsBatch", 0, 0,
                   (char *)"T", 0, -1, 0, (char *)"InsertHoldingsBatch", FALSE);
  }
}

DLLAPI void STDCALL SelectHoldings(HOLDINGS *pzHR, int iID, char *sSecNo,
  ...
  try {
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), iID, lTransNo, (char *)"T", 0, -1,
                        0, (char *)"SelectHoldings", FALSE);
  } catch (...) {
    *pzErr =
        PrintError((char *)"Unexpected error in SelectHoldings", iID, lTransNo,
                   (char *)"T", 0, -1, 0, (char *)"SelectHoldings", FALSE);
  }
}
  ...
  try {
    ...
    nanodbc::prepare(
        ...
        NANODBC_TEXT(
            ...
            "mkt_cur_yld=?,  safek_ind=?,  collateral_units=?,  hedge_value=?, "
            " "
            "benchmark_sec_no=?,  perm_lt_flag=?,  valuation_srce=?,  "
            "primary_type=?,  "
            "restriction_code=? "
            "where ID=? and sec_no=? and wi=? and sec_xtend=? and "
            "acct_type=? and trans_no=? "));
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), zHR.iID, zHR.lTransNo, (char *)"T",
                        0, -1, 0, (char *)"UpdateHoldings", FALSE);
  } catch (...) {
    *pzErr = PrintError((char *)"Unexpected error in UpdateHoldings", zHR.iID,
                        zHR.lTransNo, (char *)"T", 0, -1, 0,
                        (char *)"UpdateHoldings", FALSE);
  }
}

DLLAPI void STDCALL HoldingsForFifoAndAvgAccts(HOLDINGS *pzHoldings, int iID,
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"HoldingsForFifoAndAvgAccts", FALSE);
    return;
  }
  ...
  try {
    ...
    nanodbc::prepare(
        ...
        NANODBC_TEXT(
            ...
            "orig_cost, base_cost_xrate, sys_cost_xrate, trd_date, eff_date, "
            "stl_date, "
            "orig_yield, eff_mat_date, eff_mat_price, cost_eff_mat_yld, "
            "safek_ind, "
            "collateral_units, perm_lt_flag,cur_liability,open_liability, "
            "restriction_code  "
            "FROM Holdings "
    ...
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"HoldingsForFifoAndAvgAccts", FALSE);
  } catch (...) {
    *pzErr = PrintError(
        (char *)"Unexpected error in HoldingsForFifoAndAvgAccts", iID, 0,
        (char *)"", 0, -1, 0, (char *)"HoldingsForFifoAndAvgAccts", FALSE);
  }
}

DLLAPI void STDCALL HoldingsForLifoAccts(HOLDINGS *pzHoldings, int iID,
                                         char *sSecNo, char *sWi,
                                         char *sSecXtend, char *sAcctType,
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"HoldingsForLifoAccts", FALSE);
    return;
  }
  ...
  try {
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"HoldingsForLifoAccts", FALSE);
  } catch (...) {
    *pzErr =
        PrintError((char *)"Unexpected error in HoldingsForLifoAccts", iID, 0,
                   (char *)"", 0, -1, 0, (char *)"HoldingsForLifoAccts", FALSE);
  }
}

DLLAPI void STDCALL HoldingsForHighAccts(HOLDINGS *pzHoldings, int iID,
                                         char *sSecNo, char *sWi,
                                         char *sSecXtend, char *sAcctType,
                                         long lTrdDate, ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"HoldingsForHighAccts", FALSE);
    return;
  }
  ...
  try {
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"HoldingsForHighAccts", FALSE);
  } catch (...) {
    *pzErr =
        PrintError((char *)"Unexpected error in HoldingsForHighAccts", iID, 0,
                   (char *)"", 0, -1, 0, (char *)"HoldingsForHighAccts", FALSE);
  }
}

DLLAPI void STDCALL HoldingsForLowAccts(HOLDINGS *pzHoldings, int iID,
                                        char *sSecNo, char *sWi,
                                        char *sSecXtend, char *sAcctType,
                                        long lTrdDate, ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"HoldingsForLowAccts", FALSE);
    return;
  }
  ...
  try {
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"HoldingsForLowAccts", FALSE);
  } catch (...) {
    *pzErr =
        PrintError((char *)"Unexpected error in HoldingsForLowAccts", iID, 0,
                   (char *)"", 0, -1, 0, (char *)"HoldingsForLowAccts", FALSE);
  }
}

DLLAPI void STDCALL HoldingsForMinimumGainAccts(
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"HoldingsForMinimumGainAccts", FALSE);
    return;
  }
  ...
  try {
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"HoldingsForMinimumGainAccts", FALSE);
  } catch (...) {
    *pzErr = PrintError(
        (char *)"Unexpected error in HoldingsForMinimumGainAccts", iID, 0,
        (char *)"", 0, -1, 0, (char *)"HoldingsForMinimumGainAccts", FALSE);
  }
}

DLLAPI void STDCALL HoldingsForMaximumGainAccts(
    HOLDINGS *pzHoldings, int iID, char *sSecNo, char *sWi, char *sSecXtend,
    char *sAcctType, long lTrdDate, long lMinTrdDate, long lMaxTrdDate,
    ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"HoldingsForMaximumGainAccts", FALSE);
    return;
  }
  ...
  try {
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"HoldingsForMaximumGainAccts", FALSE);
  } catch (...) {
    *pzErr = PrintError(
        (char *)"Unexpected error in HoldingsForMaximumGainAccts", iID, 0,
        (char *)"", 0, -1, 0, (char *)"HoldingsForMaximumGainAccts", FALSE);
  }
}

DLLAPI void STDCALL HoldingsSumSelect(
    double *pfTotUnits, double *pfTotTotCost, double *pfTotOrigCost,
    double *pfTotBaseTCost, double *pfTotSysTCost, double *pfTotOpenLiability,
    double *pfTotBaseOLiability, double *pfTotSysOLiability, int iID,
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"HoldingsSumSelect", FALSE);
    return;
  }
  ...
  try {
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"HoldingsSumSelect", FALSE);
  } catch (...) {
    *pzErr =
        PrintError((char *)"Unexpected error in HoldingsSumSelect", iID, 0,
                   (char *)"", 0, -1, 0, (char *)"HoldingsSumSelect", FALSE);
  }
}

DLLAPI void STDCALL HoldingsSumUpdate(
    double fTotUnitCost, double fTradingUnits, double fTotOrigUnit,
    double fUnitLiability, double fNewBaseXrate, double fNewSysXrate,
    char *sTranType, long lTaxlotNo, char *sTransSrce, int iID, char *sSecNo,
    char *sWi, char *sSecXtend, char *sAcctType, long lInvalidDate,
    ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);

  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"HoldingsSumUpdate", FALSE);
    return;
  }
  ...
  try {
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"HoldingsSumUpdate", FALSE);
  } catch (...) {
    *pzErr =
        PrintError((char *)"Unexpected error in HoldingsSumUpdate", iID, 0,
                   (char *)"", 0, -1, 0, (char *)"HoldingsSumUpdate", FALSE);
  }
}

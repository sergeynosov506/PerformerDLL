...
const char *SQL_SELECT_PORTCLOS =
    "SELECT TOP 1 portfolioid FROM Portclos WHERE portfolioid = ? AND "
    "dateclosedfor >= ?";

const char *SQL_INSERT_DTRNDESC = "INSERT INTO DtrnDesc (ID, Dtrans_No, Seqno, "
                                  "Close_Type, Taxlot_No, Units, Desc_Info) "
                                  "VALUES (?, ?, ?, ?, ?, ?, ?)";
...
const char *SQL_SELECT_TRANSNO_BY_DTRANSNO =
    "SELECT MAX(Trans_No) FROM Trans WHERE ID = ? AND DTrans_No = ? AND "
    "created_by = ? "
...
const char *SQL_SELECT_TRANSNO_BY_UNITS_DATES =
    "SELECT trans_no FROM Trans WHERE ID = ? AND Sec_No = ? AND Wi = ? AND "
    "Sec_Xtend = ? "
    "AND Tran_Type = ? AND Units = ? AND Trd_Date = ? AND Stl_Date = ? AND "
    "Rev_Trans_No = 0";

const char *SQL_SELECT_MAPTRANSNOEX =
    "SELECT TransNo, DtransNo FROM MapTransNoEx "
    "WHERE VendorID = ? AND RuleNo = ? AND PortId = ? "
    "AND Field1 LIKE ? AND Field2 LIKE ? AND Field3 LIKE ? AND Field4 LIKE ? "
    "AND SequenceNo = ?";
...
const char *SQL_INSERT_MAPTRANSNOEX =
    ...
    "Field1, Field2, Field3, Field4, SequenceNo, RevField1, RevField2, "
    "RevField3, RevField4, RevSequenceNo) "
    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

const char *SQL_UPDATE_MAPTRANSNOEX =
    "UPDATE MapTransNoEx SET TransNo = ?, DtransNo = ?, RevField1 = ?, "
    "RevField2 = ?, "
    "RevField3 = ?, RevField4 = ?, RevSequenceNo = ? "
    "WHERE VendorID = ? AND RuleNo = ? AND PortId = ? "
    "AND Field1 LIKE ? AND Field2 LIKE ? AND Field3 LIKE ? AND Field4 LIKE ? "
...
const char *SQL_INSERT_TAXLOTRECON =
    "INSERT INTO TaxlotRecon (VendorID, AccountID, FileDate, SecurityID, "
    "PositionID, "
    "Units, Base_mv, MV_xrate, Base_inc, Inc_xrate, Base_OriginalCost, "
    "Base_TotalCost, "
    "BaseCost_xrate, Original_face, Price, TradeDate, SettleDate, "
    "Restriction_Code, Ignore_Fields, batchID) "
    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

const char *SQL_UPDATE_TRADEEXCHANGE_BY_PK =
    "UPDATE TradeExchange SET OutVendorId = ?, OutFileNo = ?, EffronTransNo = "
    "?, Sent = GetDate() "
    "WHERE AccountId = ? AND TransNo = ? AND VendorID = ?";

const char *SQL_UPDATE_BNKSETEX_REVNO =
    "UPDATE bnksetex SET RevTransNo = ? WHERE Bank_id = ? AND AcctName = ? AND "
    "TransNo = ?";
...
// SelectPortClos
// ============================================================================
DLLAPI void STDCALL SelectPortClos(long iID, long lCloseDate,
                                   ERRSTRUCT *pzErr) {
#ifdef DEBUG
...
DLLAPI void STDCALL InsertDTrans(DTRANS *pzDTrans, ERRSTRUCT *pzErr) {
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"InsertDTrans", FALSE);
    return;
  }
  try {
    // Use INSERT with OUTPUT to get identity value
    std::string sql =
        "INSERT INTO Dtrans (ID, Tran_Type, Sec_No, Wi, Sec_Xtend, Acct_Type, "
        "SecID, Sec_Symbol, "
        "Units, Orig_Face, Unit_Cost, Tot_Cost, Orig_Cost, Pcpl_Amt, Opt_Prem, "
        "Amort_Val, Basis_Adj, "
        "Comm_Gcr, Net_Comm, Comm_Code, Sec_Fees, Misc_Fee1, Fee_Code1, "
        "Misc_Fee2, Fee_Code2, "
        "Accr_Int, Income_Amt, Net_Flow, Broker_Code, Broker_Code2, Trd_Date, "
        "Stl_Date, Eff_Date, "
        "Entry_Date, Taxlot_No, Xref_Trans_No, Pend_Div_No, Rev_Trans_No, "
        "Rev_Type, New_Trans_No, "
        "Orig_Trans_No, Block_Trans_No, X_ID, X_Trans_No, X_Sec_No, X_Wi, "
        "X_Sec_Xtend, X_Acct_Type, "
        "X_SecID, Curr_Id, Curr_Acct_Type, Inc_Curr_Id, Inc_Acct_Type, "
        "X_Curr_Id, X_Curr_Acct_Type, "
        "Sec_Curr_Id, Accr_Curr_Id, Base_Xrate, Inc_Base_Xrate, "
        "Sec_Base_Xrate, Accr_Base_Xrate, "
        "Sys_Xrate, Inc_Sys_Xrate, Base_Open_Xrate, Sys_Open_Xrate, "
        "Open_Trd_Date, Open_Stl_Date, "
        "Open_Unit_Cost, Orig_Yld, Eff_Mat_Date, Eff_Mat_Price, Acct_Mthd, "
        "Trans_Srce, Adp_Tag, "
        "Div_Type, Div_Factor, Divint_No, Roll_Date, Perf_Date, Misc_Desc_Ind, "
        "Dr_Cr, Bal_To_Adjust, "
        "Cap_Trans, Safek_Ind, Dtc_Inclusion, Dtc_Resolve, Recon_Flag, "
        "Recon_Srce, Income_Flag, "
        "Letter_Flag, Ledger_Flag, Gl_Flag, Created_By, Create_Date, "
        "Create_Time, Post_Date, "
        "Bkof_Frmt, Bkof_Seq_No, Price, Restriction_Code, Status_Flag, "
        "Err_Status_Code, Process_Tag) "
    ...
    // Convert dates
    nanodbc::timestamp tsTrdDate, tsStlDate, tsEffDate, tsEntryDate,
        tsOpenTrdDate, tsOpenStlDate;
    nanodbc::timestamp tsEffMatDate, tsRollDate, tsPerfDate, tsCreateDate,
        tsPostDate;
    ...
    safe_bind_string(stmt, p, pzDTrans->zT.sTranType);
    safe_bind_string(stmt, p, pzDTrans->zT.sSecNo);
    safe_bind_string(stmt, p, pzDTrans->zT.sWi);
    safe_bind_string(stmt, p, pzDTrans->zT.sSecXtend);
    safe_bind_string(stmt, p, pzDTrans->zT.sAcctType);
    stmt.bind(p++, &pzDTrans->zT.iSecID);
    safe_bind_string(stmt, p, pzDTrans->zT.sSecSymbol);
    stmt.bind(p++, &fUnits);
    ...
    stmt.bind(p++, &fNetComm);
    safe_bind_string(stmt, p, pzDTrans->zT.sCommCode);
    stmt.bind(p++, &fSecFees);
    stmt.bind(p++, &fMiscFee1);
    safe_bind_string(stmt, p, pzDTrans->zT.sFeeCode1);
    stmt.bind(p++, &fMiscFee2);
    safe_bind_string(stmt, p, pzDTrans->zT.sFeeCode2);
    stmt.bind(p++, &fAccrInt);
    stmt.bind(p++, &fIncomeAmt);
    stmt.bind(p++, &fNetFlow);
    safe_bind_string(stmt, p, pzDTrans->zT.sBrokerCode);
    safe_bind_string(stmt, p, pzDTrans->zT.sBrokerCode2);
    ...
    safe_bind_string(stmt, p, pzDTrans->zT.sRevType);
    ...
    safe_bind_string(stmt, p, pzDTrans->zT.sXSecXtend);
    safe_bind_string(stmt, p, pzDTrans->zT.sXAcctType);
    stmt.bind(p++, &pzDTrans->zT.iXSecID);
    safe_bind_string(stmt, p, pzDTrans->zT.sCurrId);
    safe_bind_string(stmt, p, pzDTrans->zT.sCurrAcctType);
    safe_bind_string(stmt, p, pzDTrans->zT.sIncCurrId);
    safe_bind_string(stmt, p, pzDTrans->zT.sIncAcctType);
    safe_bind_string(stmt, p, pzDTrans->zT.sXCurrId);
    safe_bind_string(stmt, p, pzDTrans->zT.sXCurrAcctType);
    safe_bind_string(stmt, p, pzDTrans->zT.sSecCurrId);
    safe_bind_string(stmt, p, pzDTrans->zT.sAccrCurrId);
    ...
    stmt.bind(p++, &fEffMatPrice);
    safe_bind_string(stmt, p, pzDTrans->zT.sAcctMthd);
    safe_bind_string(stmt, p, pzDTrans->zT.sTransSrce);
    safe_bind_string(stmt, p, pzDTrans->zT.sAdpTag);
    safe_bind_string(stmt, p, pzDTrans->zT.sDivType);
    stmt.bind(p++, &fDivFactor);
    stmt.bind(p++, &pzDTrans->zT.lDivintNo);
    stmt.bind(p++, &tsRollDate);
    stmt.bind(p++, &tsPerfDate);
    stmt.bind(p++, &tsPerfDate);
    safe_bind_string(stmt, p, pzDTrans->zT.sMiscDescInd);
    safe_bind_string(stmt, p, pzDTrans->zT.sDrCr);
    safe_bind_string(stmt, p, pzDTrans->zT.sBalToAdjust);
    safe_bind_string(stmt, p, pzDTrans->zT.sCapTrans);
    safe_bind_string(stmt, p, pzDTrans->zT.sSafekInd);
    safe_bind_string(stmt, p, pzDTrans->zT.sDtcInclusion);
    safe_bind_string(stmt, p, pzDTrans->zT.sDtcResolve);
    safe_bind_string(stmt, p, pzDTrans->zT.sReconFlag);
    safe_bind_string(stmt, p, pzDTrans->zT.sReconSrce);
    safe_bind_string(stmt, p, pzDTrans->zT.sIncomeFlag);
    safe_bind_string(stmt, p, pzDTrans->zT.sLetterFlag);
    safe_bind_string(stmt, p, pzDTrans->zT.sLedgerFlag);
    safe_bind_string(stmt, p, pzDTrans->zT.sGlFlag);
    safe_bind_string(stmt, p, pzDTrans->zT.sCreatedBy);
    stmt.bind(p++, &tsCreateDate);
    stmt.bind(p++, &tsCreateDate);
    safe_bind_string(stmt, p, pzDTrans->zT.sCreateTime);
    stmt.bind(p++, &tsPostDate);
    safe_bind_string(stmt, p, pzDTrans->zT.sBkofFrmt);
    stmt.bind(p++, &pzDTrans->zT.lBkofSeqNo);
    stmt.bind(p++, &fPrice);
    stmt.bind(p++, &pzDTrans->zT.iRestrictionCode);
    safe_bind_string(stmt, p, pzDTrans->sStatusFlag);
    ...
    if (result.next()) {
      ...
      nanodbc::just_execute(gConn, "UPDATE Dtrans SET Process_Tag = " +
                                       std::to_string(pzDTrans->lProcessTag) +
                                       " WHERE Dtrans_no = " +
    ...
    msg += e.what();
    *pzErr =
        PrintError((char *)msg.c_str(), pzDTrans->zT.iID, pzDTrans->lProcessTag,
                   (char *)"D", 0, -1, 0, (char *)"InsertDTrans", FALSE);
  }
}
...
DLLAPI void STDCALL InsertDTrnDesc(TRNDESC *pzDTrnDesc, ERRSTRUCT *pzErr) {
  ...
  try {
    ...
    int p = 0; // Explicitly init p for reuse/consistency
    stmt.bind(p++, &pzDTrnDesc->iID);
    stmt.bind(p++, &pzDTrnDesc->lDtransNo);
    stmt.bind(p++, &pzDTrnDesc->iSeqno);
    safe_bind_string(stmt, p, pzDTrnDesc->sCloseType);
    stmt.bind(p++, &pzDTrnDesc->lTaxlotNo);
    stmt.bind(p++, &fUnits);
    safe_bind_string(stmt, p, pzDTrnDesc->sDescInfo);
    nanodbc::execute(stmt);
  } catch (const nanodbc::database_error &e) {
    std::string msg = "Database error: ";
    msg += e.what();
    *pzErr =
        PrintError((char *)msg.c_str(), pzDTrnDesc->iID, pzDTrnDesc->lDtransNo,
                   (char *)"D", 0, -1, 0, (char *)"InsertDTrnDesc", FALSE);
  }
}
...
DLLAPI void STDCALL InsertDPayTran(PAYTRAN *pzDPayTran, ERRSTRUCT *pzErr) {
  ...
  try {
    ...
    int p = 0;
    stmt.bind(p++, &pzDPayTran->iID);
    stmt.bind(p++, &pzDPayTran->lTransNo);
    stmt.bind(p++, &pzDPayTran->lPayeeID);
    safe_bind_string(stmt, p, pzDPayTran->sDsc);
    nanodbc::execute(stmt);
  } catch (const nanodbc::database_error &e) {
    std::string msg = "Database error: ";
    msg += e.what();
    *pzErr =
        PrintError((char *)msg.c_str(), pzDPayTran->iID, pzDPayTran->lTransNo,
                   (char *)"D", 0, -1, 0, (char *)"InsertDPayTran", FALSE);
  }
}
...
DLLAPI void STDCALL SelectTransNoByDtransNo(TRANS *pzTrans, ERRSTRUCT *pzErr) {
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"SelectTransNoByDtransNo", FALSE);
    return;
  }
  try {
    nanodbc::statement stmt(gConn);
    ...
    int p = 0;
    stmt.bind(p++, &pzTrans->iID);
    stmt.bind(p++, &pzTrans->lDtransNo);
    safe_bind_string(stmt, p, pzTrans->sCreatedBy);
    auto result = nanodbc::execute(stmt);
    if (result.next()) {
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectTransNoByDtransNo", FALSE);
  }
}
...
// ============================================================================
DLLAPI void STDCALL SelectTransNoByUnitsAndDates(TRANS *pzTrans,
                                                 ERRSTRUCT *pzErr) {
#ifdef DEBUG
  PrintError("Entering", 0, 0, "", 0, 0, 0, "SelectTransNoByUnitsAndDates",
             FALSE);
#endif
  InitializeErrStruct(pzErr);
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"SelectTransNoByUnitsAndDates", FALSE);
    return;
  }
  try {
    nanodbc::statement stmt(gConn);
    ...
    int p = 0;
    stmt.bind(p++, &pzTrans->iID);
    safe_bind_string(stmt, p, pzTrans->sSecNo);
    safe_bind_string(stmt, p, pzTrans->sWi);
    safe_bind_string(stmt, p, pzTrans->sSecXtend);
    safe_bind_string(stmt, p, pzTrans->sTranType);
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectTransNoByUnitsAndDates", FALSE);
  }
}
...
DLLAPI void STDCALL SelectMapTransNoEx(MAPTRANSNOEX *pzMapTransNoEx,
                                       ERRSTRUCT *pzErr) {
#ifdef DEBUG
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"SelectMapTransNoEx", FALSE);
    return;
  }
  try {
    nanodbc::statement stmt(gConn);
    ...
    safe_bind_string(stmt, p, pzMapTransNoEx->sField1);
    safe_bind_string(stmt, p, pzMapTransNoEx->sField2);
    safe_bind_string(stmt, p, pzMapTransNoEx->sField3);
    safe_bind_string(stmt, p, pzMapTransNoEx->sField4);
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectMapTransNoEx", FALSE);
  }
}

DLLAPI void STDCALL SelectMapTransNoExByTransNo(MAPTRANSNOEX *pzMapTransNoEx,
                                                ERRSTRUCT *pzErr) {
#ifdef DEBUG
  PrintError("Entering", 0, 0, "", 0, 0, 0, "SelectMapTransNoExByTransNo",
             FALSE);
#endif
  InitializeErrStruct(pzErr);
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"SelectMapTransNoExByTransNo", FALSE);
    return;
  }
  try {
    nanodbc::statement stmt(gConn);
    ...
    auto result = nanodbc::execute(stmt);
    if (result.next()) {
      read_string(result, 0, pzMapTransNoEx->sField1,
                  sizeof(pzMapTransNoEx->sField1));
      read_string(result, 1, pzMapTransNoEx->sField2,
                  sizeof(pzMapTransNoEx->sField2));
      read_string(result, 2, pzMapTransNoEx->sField3,
                  sizeof(pzMapTransNoEx->sField3));
      read_string(result, 3, pzMapTransNoEx->sField4,
                  sizeof(pzMapTransNoEx->sField4));
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectMapTransNoExByTransNo", FALSE);
  }
}

DLLAPI void STDCALL InsertMapTransNoEx(MAPTRANSNOEX *pzMapTransNoEx,
                                       ERRSTRUCT *pzErr) {
#ifdef DEBUG
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"InsertMapTransNoEx", FALSE);
    return;
  }
  try {
    nanodbc::statement stmt(gConn);
    ...
    safe_bind_string(stmt, p, pzMapTransNoEx->sRevField1);
    safe_bind_string(stmt, p, pzMapTransNoEx->sRevField2);
    safe_bind_string(stmt, p, pzMapTransNoEx->sRevField3);
    safe_bind_string(stmt, p, pzMapTransNoEx->sRevField4);
    stmt.bind(p++, &pzMapTransNoEx->lRevSequenceNo);
    nanodbc::execute(stmt);
  } catch (const nanodbc::database_error &e) {
    // Ignore PK violation (DB_E_INTEGRITYVIOLATION)
    std::string what = e.what();
    if (what.find("2627") != std::string::npos ||
        what.find("2601") != std::string::npos) {
      pzErr->iSqlError = -2147217873; // DB_E_INTEGRITYVIOLATION
    } else {
      std::string msg = "Database error: ";
      msg += e.what();
      *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                          (char *)"InsertMapTransNoEx", FALSE);
    }
  }
}

DLLAPI void STDCALL UpdateMapTransNoEx(MAPTRANSNOEX *pzMapTransNoEx,
                                       ERRSTRUCT *pzErr) {
#ifdef DEBUG
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"UpdateMapTransNoEx", FALSE);
    return;
  }
  try {
    nanodbc::statement stmt(gConn);
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"UpdateMapTransNoEx", FALSE);
  }
}

DLLAPI void STDCALL DeleteMapTransNoEx(MAPTRANSNOEX *pzMapTransNoEx,
                                       ERRSTRUCT *pzErr) {
#ifdef DEBUG
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"DeleteMapTransNoEx", FALSE);
    return;
  }
  try {
    nanodbc::statement stmt(gConn);
...
DLLAPI void STDCALL InsertTaxlotRecon(TAXLOTRECON *pzTaxlotRecon,
                                      ERRSTRUCT *pzErr) {
#ifdef DEBUG
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"InsertTaxlotRecon", FALSE);
    return;
  }
  try {
    nanodbc::statement stmt(gConn);
    ...
    safe_bind_string(stmt, p, pzTaxlotRecon->sAccountID);
    stmt.bind(p++, &tsFileDate);
    safe_bind_string(stmt, p, pzTaxlotRecon->sSecurityID);
    safe_bind_string(stmt, p, pzTaxlotRecon->sPositionID);
    ...
    stmt.bind(p++, &tsTrdDate);
    stmt.bind(p++, &tsStlDate);
    safe_bind_string(stmt, p, pzTaxlotRecon->sRestrictionCode);
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"InsertTaxlotRecon", FALSE);
  }
}

DLLAPI void STDCALL InsertOneTaxlotRecon(TAXLOTRECON *pzTaxlotRecon,
                                         long lBatchSize, ERRSTRUCT *pzErr) {
#ifdef DEBUG
...
#endif
  InitializeErrStruct(pzErr);
  if (g_taxlotReconBatch.records.empty())
    return;
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"InsertAllTaxlotRecon", FALSE);
    return;
  }
  try {
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"InsertAllTaxlotRecon", FALSE);
  }
}
...
DLLAPI void STDCALL InsertTradeExchange(TRADEEXCHANGE *pzTradeExchange,
                                        ERRSTRUCT *pzErr) {
#ifdef DEBUG
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"InsertTradeExchange", FALSE);
    return;
  }
  try {
    // First get the next SequenceNo
    std::string seqSql =
        "SELECT ISNULL(MAX(SequenceNo),-1)+1 FROM TradeExchange "
        "WHERE TrdDate = ? AND AccountID = ? AND AssetID = ? AND TransCode = ? "
        "AND VendorID = ?";
    ...
    int idx = 1;
    safe_bind_string(seqStmt, idx, pzTradeExchange->sAccountID);
    safe_bind_string(seqStmt, idx, pzTradeExchange->sAssetID);
    safe_bind_string(seqStmt, idx, pzTradeExchange->sTransCode);
    seqStmt.bind(idx++, &pzTradeExchange->iVendorID);
    auto seqResult = nanodbc::execute(seqStmt);
    long lSequenceNo = 0;
    if (seqResult.next())
      lSequenceNo = seqResult.get<long>(0, 0);

    // Now insert
    std::string sql =
        "INSERT INTO TradeExchange (AccountId, TransNo, VendorID, FileDate, "
        "FileNo, Received, "
        "AssetId, TransCode, TrdDate, StlDate, RecordType, OrigTransNo, "
        "MasterTicketNo, Units, UnitCode, "
        "PcplAmt, IncPcplCode, Price, NetComm, CommCode, SecFees, AccrInt, "
        "OrigFace, MortFactor, ExRate, "
        "StlLocCode, WriteDownMtd, TaxlotNo, TradeType, Broker, Desc1, Desc2, "
        "Desc3, Desc4, SequenceNo) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
        "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    nanodbc::statement stmt(gConn);
    nanodbc::prepare(stmt, NANODBC_TEXT(sql.c_str()));
    ...
    int p = 0;
    safe_bind_string(stmt, p, pzTradeExchange->sAccountID);
    ...
    safe_bind_string(stmt, p, pzTradeExchange->sAssetID);
    safe_bind_string(stmt, p, pzTradeExchange->sTransCode);
    stmt.bind(p++, &tsTrdDate);
    stmt.bind(p++, &tsStlDate);
    ...
    stmt.bind(p++, &fUnits);
    safe_bind_string(stmt, p, pzTradeExchange->sUnitCode);
    stmt.bind(p++, &fPcplAmt);
    safe_bind_string(stmt, p, pzTradeExchange->sIncPcplCode);
    stmt.bind(p++, &pzTradeExchange->fPrice);
    stmt.bind(p++, &fNetComm);
    safe_bind_string(stmt, p, pzTradeExchange->sCommCode);
    stmt.bind(p++, &fSecFees);
    stmt.bind(p++, &fAccrInt);
    stmt.bind(p++, &fOrigFace);
    stmt.bind(p++, &fMortFactor);
    stmt.bind(p++, &fExRate);
    safe_bind_string(stmt, p, pzTradeExchange->sStlLocCode);
    safe_bind_string(stmt, p, pzTradeExchange->sWriteDownMtd);
    safe_bind_string(stmt, p, pzTradeExchange->sTaxlotNo);
    safe_bind_string(stmt, p, pzTradeExchange->sTradeType);
    safe_bind_string(stmt, p, pzTradeExchange->sBroker);
    safe_bind_string(stmt, p, pzTradeExchange->sDesc1);
    safe_bind_string(stmt, p, pzTradeExchange->sDesc2);
    safe_bind_string(stmt, p, pzTradeExchange->sDesc3);
    safe_bind_string(stmt, p, pzTradeExchange->sDesc4);
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"InsertTradeExchange", FALSE);
  }
}
...
DLLAPI void STDCALL InsertBnkSet(BNKSET *pzBnkSet, ERRSTRUCT *pzErr) {
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"InsertBnkSet", FALSE);
    return;
  }
  try {
    // First get duplicate_counter
    std::string cntSql =
        "SELECT ISNULL(MAX(duplicate_counter),-1)+1 FROM BnkSet "
        "WHERE id = ? AND stl_date = ? AND tran_type = ? AND dr_cr = ? AND "
        "sec_no = ? AND bank_id = ?";
    ...
    if (cntResult.next())
      iDuplicateCounter = cntResult.get<int>(0, 0);

    std::string sql = "INSERT INTO bnkset (id, stl_date, tran_type, dr_cr, "
                      "sec_no, bank_id, duplicate_counter, "
                      "cusip, trd_date, description, units, commission, "
                      "sec_fees, short_gain, long_gain, broker, cost, "
                      "base_amount, native_amount, exrate, currency, "
                      "principal_income_flag, entry_date, reversal_code, "
    ...
    safe_bind_string(stmt, p, pzBnkSet->sSecNo);
    safe_bind_string(stmt, p, pzBnkSet->sBankID);
    stmt.bind(p++, &iDuplicateCounter);
    safe_bind_string(stmt, p, pzBnkSet->sCusip);
    stmt.bind(p++, &tsTrdDate);
    safe_bind_string(stmt, p, pzBnkSet->sDescription);
    stmt.bind(p++, &fUnits);
    stmt.bind(p++, &fCommission);
    stmt.bind(p++, &fSecFees);
    stmt.bind(p++, &fShortGain);
    stmt.bind(p++, &fLongGain);
    safe_bind_string(stmt, p, pzBnkSet->sBroker);
    stmt.bind(p++, &fCost);
    stmt.bind(p++, &fBaseAmount);
    stmt.bind(p++, &fNativeAmount);
    stmt.bind(p++, &fExrate);
    safe_bind_string(stmt, p, pzBnkSet->sCurrency);
    safe_bind_string(stmt, p, pzBnkSet->sPrincipalIncomeFlag);
    stmt.bind(p++, &tsEntryDate);
    safe_bind_string(stmt, p, pzBnkSet->sReversalCode);
...
DLLAPI void STDCALL InsertBnkSetEx(BNKSETEX *pzBnkSetEx, ERRSTRUCT *pzErr) {
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"InsertBnkSetEx", FALSE);
    return;
  }
  try {
    std::string sql =
        "INSERT INTO bnksetex (ID, Stl_date, Tran_type, Dr_Cr, Sec_no, Wi, "
        "Duplicate_counter, "
        "Cusip, Bank_id, AcctName, Units, Base_amount, Native_amount, "
        "OrigFace, OrigCost, Cost, IncomeAmt, "
        "Commission, Sec_fees, Short_gain, Long_gain, Exrate, Currency, "
        "Broker, Trd_date, RestrictionCode, "
        "TransNo, RevTranType, RevTransNo, BalToAdjust, Entry_date, "
        "Principal_income_flag, "
    ...
    stmt.bind(p++, &tsStlDate);
    safe_bind_string(stmt, p, pzBnkSetEx->sTranType);
    safe_bind_string(stmt, p, pzBnkSetEx->sDrCr);
    safe_bind_string(stmt, p, pzBnkSetEx->sSecNo);
    safe_bind_string(stmt, p, pzBnkSetEx->sWi);
    stmt.bind(p++, &pzBnkSetEx->iDuplicateCounter);
    safe_bind_string(stmt, p, pzBnkSetEx->sCusip);
    safe_bind_string(stmt, p, pzBnkSetEx->sBankId);
    safe_bind_string(stmt, p, pzBnkSetEx->sAcctName);
    stmt.bind(p++, &fUnits);
    stmt.bind(p++, &fBaseAmount);
    stmt.bind(p++, &fNativeAmount);
    stmt.bind(p++, &fOrigFace);
    ...
    stmt.bind(p++, &fLongGain);
    stmt.bind(p++, &fExrate);
    safe_bind_string(stmt, p, pzBnkSetEx->sCurrency);
    safe_bind_string(stmt, p, pzBnkSetEx->sBroker);
    stmt.bind(p++, &tsTrdDate);
    safe_bind_string(stmt, p, pzBnkSetEx->sRestrictionCode);
    safe_bind_string(stmt, p, pzBnkSetEx->sTransNo);
    safe_bind_string(stmt, p, pzBnkSetEx->sRevTranType);
    safe_bind_string(stmt, p, pzBnkSetEx->sRevTransNo);
    safe_bind_string(stmt, p, pzBnkSetEx->sBalToAdjust);
    stmt.bind(p++, &tsEntryDate);
    safe_bind_string(stmt, p, pzBnkSetEx->sPrincipalIncomeFlag);
    stmt.bind(p++, &tsReconDate);
    stmt.bind(p++, &pzBnkSetEx->lEffronTransNo);
    safe_bind_string(stmt, p, pzBnkSetEx->sDescription);
    nanodbc::execute(stmt);
  } catch (const nanodbc::database_error &e) {
    ...
    if (what.find("2627") != std::string::npos ||
      ...
      msg += e.what();
      *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                          (char *)"InsertBnkSetEx", FALSE);
    }
  }
}

DLLAPI void STDCALL UpdateTradeExchangeByPK(TRADEEXCHANGE *pzTradeExchange,
                                            ERRSTRUCT *pzErr) {
#ifdef DEBUG
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"UpdateTradeExchangeByPK", FALSE);
    return;
  }
  try {
    nanodbc::statement stmt(gConn);
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"UpdateTradeExchangeByPK", FALSE);
  }
}
...
DLLAPI void STDCALL UpdateBnkSetExRevNo(BNKSETEX *pzBnkSetEx,
                                        ERRSTRUCT *pzErr) {
#ifdef DEBUG
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"UpdateBnkSetExRevNo", FALSE);
    return;
  }
  try {
    nanodbc::statement stmt(gConn);
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"UpdateBnkSetExRevNo", FALSE);
  }
}

DLLAPI void STDCALL SelectBnksetexNoByUnitsAndDates(BNKSETEX *pzBnksetex,
                                                    ERRSTRUCT *pzErr) {
#ifdef DEBUG
  PrintError("Entering", 0, 0, "", 0, 0, 0, "SelectBnksetexNoByUnitsAndDates",
             FALSE);
#endif
  InitializeErrStruct(pzErr);
  if (!gConn.connected()) {
    *pzErr =
        PrintError((char *)"Database not connected", 0, 0, (char *)"", 0, -1, 0,
                   (char *)"SelectBnksetexNoByUnitsAndDates", FALSE);
    return;
  }
  try {
    nanodbc::statement stmt(gConn);
    ...
    auto result = nanodbc::execute(stmt);
    if (result.next()) {
      read_string(result, 0, pzBnksetex->sTransNo,
                  sizeof(pzBnksetex->sTransNo));
    } else {
      pzErr->iSqlError = SQLNOTFOUND;
    }
  } catch (const nanodbc::database_error &e) {
...
DLLAPI void STDCALL GrouppedHoldingsFor(HOLDINGS *pzHoldings, int iID,
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"GrouppedHoldingsFor", FALSE);
    return;
  }
  try {
    bool needNewQuery =
    ...
    if (g_grouppedHoldingsState.result &&
        g_grouppedHoldingsState.result->next()) {
      g_grouppedHoldingsState.cRows++;
      pzHoldings->iID = g_grouppedHoldingsState.result->get<int>(0, 0);
      read_string(*g_grouppedHoldingsState.result, 1, pzHoldings->sSecNo,
                  sizeof(pzHoldings->sSecNo));
      read_string(*g_grouppedHoldingsState.result, 2, pzHoldings->sWi,
                  sizeof(pzHoldings->sWi));
      read_string(*g_grouppedHoldingsState.result, 3, pzHoldings->sSecXtend,
                  sizeof(pzHoldings->sSecXtend));
      read_string(*g_grouppedHoldingsState.result, 4, pzHoldings->sAcctType,
    ...
    std::string msg = "Database error: ";
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"GrouppedHoldingsFor", FALSE);
    g_grouppedHoldingsState.result.reset();
  }
}
...
void FreeTransImportIO(void) {
  CloseTransImportIO();
#ifdef DEBUG
  PrintError("FreeTransImportIO called (RAII cleanup)", 0, 0, "", 0, 0, 0,
             "FreeTransImportIO", FALSE);
#endif
}

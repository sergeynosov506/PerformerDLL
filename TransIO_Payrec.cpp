					        ...
					        long lDivintNo, ERRSTRUCT *pzErr)
{
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"T", 0, -1, 0, (char*)"DeletePayrec", FALSE);
        return;
    }
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), iID, lTransNo, (char*)"T", 0, -1, 0, (char*)"DeletePayrec", FALSE);
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in DeletePayrec", iID, lTransNo, (char*)"T", 0, -1, 0, (char*)"DeletePayrec", FALSE);
    }
}
...
DLLAPI void STDCALL InsertPayrec(PAYREC zPYR, ERRSTRUCT *pzErr)
{
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"T", 0, -1, 0, (char*)"InsertPayrec", FALSE);
        return;
    }
    ...
    try
    {
        ...
        stmt.bind(idx++, zPYR.sSecNo);
        stmt.bind(idx++, zPYR.sWi);
        stmt.bind(idx++, zPYR.sSecXtend);
        stmt.bind(idx++, zPYR.sAcctType);
        stmt.bind(idx++, &zPYR.lTransNo);
        stmt.bind(idx++, &zPYR.iSecID);
        stmt.bind(idx++, &tsAsofDate);
        stmt.bind(idx++, zPYR.sTranType);
        ...
        stmt.bind(idx++, zPYR.sValuationSrce);
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), zPYR.iID, zPYR.lTransNo, (char*)"T", 0, -1, 0, (char*)"InsertPayrec", FALSE);
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in InsertPayrec", zPYR.iID, zPYR.lTransNo, (char*)"T", 0, -1, 0, (char*)"InsertPayrec", FALSE);
    }
}
							...
							long lTransNo, long lDivintNo, ERRSTRUCT *pzErr)
{
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"T", 0, -1, 0, (char*)"SelectPayrec", FALSE);
        return;
    }
    ...
    try
    {
        ...
        if (result.next())
        {
            ...
            read_string(result, "sec_xtend", pzPYR->sSecXtend, sizeof(pzPYR->sSecXtend));
            read_string(result, "acct_type", pzPYR->sAcctType, sizeof(pzPYR->sAcctType));
            pzPYR->lTransNo = result.get<long>("trans_no", 0);
            pzPYR->iSecID = result.get<int>("secid", 0);
            pzPYR->lAsofDate = timestamp_to_long(result.get<nanodbc::timestamp>("asof_date"));
            read_string(result, "tran_type", pzPYR->sTranType, sizeof(pzPYR->sTranType));
            ...
            pzPYR->lEffDate = timestamp_to_long(result.get<nanodbc::timestamp>("eff_date"));
            pzPYR->fMvBaseXrate = result.get<double>("mv_base_xrate", 0.0);
            pzPYR->fMvSysXrate = result.get<double>("mv_sys_xrate", 0.0);
            read_string(result, "valuation_srce", pzPYR->sValuationSrce, sizeof(pzPYR->sValuationSrce));
        }
        else
        {
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), iID, lTransNo, (char*)"T", 0, -1, 0, (char*)"SelectPayrec", FALSE);
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in SelectPayrec", iID, lTransNo, (char*)"T", 0, -1, 0, (char*)"SelectPayrec", FALSE);
    }
}

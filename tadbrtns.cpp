...
*
* $Header:   J:/Performer/Data/archives/Master/C/TADBRTNS.C-arc   1.21   22 Mar 2004 14:04:04   vay  $
*
**/
...
**/
DLLAPI void STDCALL InitializePortmainStruct(PORTMAIN *pzPortmain)
{
   pzPortmain->iID = 0;
   sprintf_s(pzPortmain->sUniqueName, "%-*s", sizeof(pzPortmain->sUniqueName)-1, "");
   sprintf_s(pzPortmain->sAbbrev, "%-*s", sizeof(pzPortmain->sAbbrev)-1, "");
   sprintf_s(pzPortmain->sDescription, "%-*s", sizeof(pzPortmain->sDescription)-1, "");
	 ...
	 pzPortmain->iInvestmentStyle = pzPortmain->iScope = pzPortmain->iDecisionMaking = 0;
   ...
   sprintf_s(pzPortmain->sPeriodType, "%-*s", sizeof(pzPortmain->sPeriodType)-1, "");
	 pzPortmain->lInceptionDate = 0;
	 pzPortmain->bUserInceptionDate = FALSE;
   sprintf_s(pzPortmain->sPortfolioType, "%-*s", sizeof(pzPortmain->sPortfolioType)-1, "");
   sprintf_s(pzPortmain->sAdministrator, "%-*s", sizeof(pzPortmain->sAdministrator)-1, "");
   sprintf_s(pzPortmain->sManager, "%-*s", sizeof(pzPortmain->sManager)-1, "");
   sprintf_s(pzPortmain->sAddress1, "%-*s", sizeof(pzPortmain->sAddress1)-1, "");
   sprintf_s(pzPortmain->sAddress2, "%-*s", sizeof(pzPortmain->sAddress2)-1, "");
   sprintf_s(pzPortmain->sAddress3, "%-*s", sizeof(pzPortmain->sAddress3)-1, "");
   sprintf_s(pzPortmain->sAcctMethod, "%-*s", sizeof(pzPortmain->sAcctMethod)-1, "");
   sprintf_s(pzPortmain->sTax, "%-*s", sizeof(pzPortmain->sTax)-1, "");
   sprintf_s(pzPortmain->sBaseCurrId, "%-*s", sizeof(pzPortmain->sBaseCurrId)-1, "");
   pzPortmain->bIncome = pzPortmain->bActions = FALSE;
	 pzPortmain->bMature = pzPortmain->bCAvail = pzPortmain->bFAvail = FALSE;
   sprintf_s(pzPortmain->sAlloc, "%-*s", sizeof(pzPortmain->sAlloc)-1, "");
   ...
   sprintf_s(pzPortmain->sCurrHandler, "%-*s", sizeof(pzPortmain->sCurrHandler)-1, "");
	 pzPortmain->bAmortMuni = pzPortmain->bAmortOther = pzPortmain->bAccreteDisc = FALSE;
   ...
   pzPortmain->lAmortStartDate = 0;
	 ...
	 pzPortmain->iIncomeMoneyMarketFund = pzPortmain->iPrincipalMoneyMarketFund = FALSE;
   sprintf_s(pzPortmain->sIncomeProcessing,"%-*s",sizeof(pzPortmain->sIncomeProcessing)-1,"");
	 pzPortmain->lPricingEffectiveDate = pzPortmain->lLastTransNo = pzPortmain->lPurgeDate = 0;
...
//	lpprTimer(30);
  if (zErr.iSqlError)
  {
    ...
    return(PrintError(sMsg, 0, 0, "", 0, zErr.iSqlError,
                      zErr.iIsamCode, "TALOC SELECT TRANTYPE", FALSE));
  }
...
//	lpprTimer(32);
  if (zErr.iSqlError != 0)
  {
    ...
    return(PrintError(sMsg, 0, 0, "", 0, zErr.iSqlError,
                      zErr.iIsamCode, "TALOC SELECT ASSET", FALSE));
...
//	lpprTimer(34);
  if (zErr.iSqlError)
  {
    ...
    return(PrintError(sMsg, 0, 0, "", 0, zErr.iSqlError,
                      zErr.iIsamCode, "TALOC SELECT SECTYPE", FALSE));
...
{
  pzTI->fUnits = pzTI->fOrigFace = pzTI->fPcplAmt = pzTI->fNetComm = pzTI->fSecFees = 0;
  pzTI->fMiscFee1 = pzTI->fMiscFee2 = pzTI->fAccrInt = pzTI->fIncomeAmt = pzTI->fTotCost = 0;
...
{
  pzPV->fShares = pzPV->fOrigFace = pzPV->fUnitCost = pzPV->fTotCost = pzPV->fOrigCost = 0;
  pzPV->fOpenLiability = pzPV->fBaseCostXrate = pzPV->fSysCostXrate = pzPV->fOrgYield = 0;
  pzPV->fMatPrice = pzPV->fCollateralUnits = pzPV->fSharesRemoved = pzPV->fSharesToRemove = 0;
  ...
  if (iSize > 0) // if requesting table expansion
	{
	  // If the table is full, create more space 
		if (pzPITable->iCount == pzPITable->iSize)
		{
			...
			pzPITable->pzPInfo = (POSINFO *) realloc(pzPITable->pzPInfo, sizeof(POSINFO) * pzPITable->iSize);
			if (pzPITable->pzPInfo == NULL)
			{ // error allocating more memory - exit reporting problem
				pzPITable->iSize = pzPITable->iCount = 0;
	      zErr = PrintError("Insufficient Memory To Create Table", 0, 0, "", 997, 0, 0, 
					                "TALLOC InitializePosInfoTable", FALSE);
			} 
			else
			{ // memory reallocated, initialize array
...
** If the table is full, it uses realloc to allocate more space before adding trans
*/
DLLAPI ERRSTRUCT STDCALL WINAPI AddTransToTransTable2(TRANSTABLE2 *pzTTable2, TRANS zTrans)
{
  ...
  if (pzTTable2->iCount == pzTTable2->iSize)
  {
    ...
    pzTTable2->pzTrans = (TRANS *) realloc(pzTTable2->pzTrans, sizeof(TRANS) * pzTTable2->iSize);
    if (pzTTable2->pzTrans == NULL)
      return(PrintError("Insufficient Memory To Create Table", 0, 0, "", 997, 0, 0, "TALLOC ADDTRANSTOTRANSTABLE2", FALSE));
  }

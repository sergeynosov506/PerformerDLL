...
void FillPortmainStruct(nanodbc::result &result, PORTMAIN *pzPR) {
  ...
  read_string(result, "unique_name", pzPR->sUniqueName,
              sizeof(pzPR->sUniqueName));
  read_string(result, "abbrev", pzPR->sAbbrev, sizeof(pzPR->sAbbrev));
  read_string(result, "description", pzPR->sDescription,
              sizeof(pzPR->sDescription));
  pzPR->lDateHired =
      timestamp_to_long(result.get<nanodbc::timestamp>("datehired"));
  pzPR->fIndividualMinAnnualFee =
      result.get<double>("individual_min_annual_fee", 0.0);
  pzPR->fIndividualMinAcctSize =
      result.get<double>("individual_min_acct_size", 0.0);
  ...
  pzPR->lAsofDate =
      timestamp_to_long(result.get<nanodbc::timestamp>("asofdate"));
  pzPR->iFiscalYearEndMonth = result.get<int>("fiscalyearendmonth", 0);
  pzPR->iFiscalYearEndDay = result.get<int>("fiscalyearendday", 0);
  read_string(result, "periodtype", pzPR->sPeriodType,
              sizeof(pzPR->sPeriodType));
  pzPR->lInceptionDate =
      timestamp_to_long(result.get<nanodbc::timestamp>("inceptiondate"));

  char sUserInceptionDate[2] = {0};
  read_string(result, "userinceptiondate", sUserInceptionDate,
              sizeof(sUserInceptionDate));
  pzPR->bUserInceptionDate = Char2BOOL(sUserInceptionDate);

  read_string(result, "portfoliotype", pzPR->sPortfolioType,
              sizeof(pzPR->sPortfolioType));
  read_string(result, "administrator", pzPR->sAdministrator,
              sizeof(pzPR->sAdministrator));
  ...
  read_string(result, "acctmethod", pzPR->sAcctMethod,
              sizeof(pzPR->sAcctMethod));
  read_string(result, "tax", pzPR->sTax, sizeof(pzPR->sTax));
  read_string(result, "basecurrid", pzPR->sBaseCurrId,
              sizeof(pzPR->sBaseCurrId));
  ...
  read_string(result, "currhandler", pzPR->sCurrHandler,
              sizeof(pzPR->sCurrHandler));
  ...
  read_string(result, "discretionaryauthority", sDiscretionaryAuthority,
              sizeof(sDiscretionaryAuthority));
  pzPR->bDiscretionaryAuthority = Char2BOOL(sDiscretionaryAuthority);

  char sVotingAuthority[2] = {0};
  read_string(result, "votingauthority", sVotingAuthority,
              sizeof(sVotingAuthority));
  pzPR->bVotingAuthority = Char2BOOL(sVotingAuthority);

  char sSpecialArrangements[2] = {0};
  read_string(result, "specialarrangements", sSpecialArrangements,
              sizeof(sSpecialArrangements));
  ...
  pzPR->iPrincipalMoneyMarketFund =
      result.get<int>("principalmoneymarketfund", 0);
  read_string(result, "incomeprocessing", pzPR->sIncomeProcessing,
              sizeof(pzPR->sIncomeProcessing));
  pzPR->lPricingEffectiveDate =
      timestamp_to_long(result.get<nanodbc::timestamp>("pricingeffectivedate"));
  pzPR->lLastTransNo = result.get<long>("lasttransno", 0);
  pzPR->lPurgeDate =
      timestamp_to_long(result.get<nanodbc::timestamp>("purgedate"));
  pzPR->lLastActivity =
      timestamp_to_long(result.get<nanodbc::timestamp>("lastactivity"));
  pzPR->lRollDate =
...
DLLAPI void STDCALL UpdatePortmainLastTransNo(long lLastTransNo, int iID,
                                              ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);

  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"UpdatePortmainLastTransNo", FALSE);
    return;
  }
  ...
  try {
    ...
    nanodbc::prepare(
        stmt, NANODBC_TEXT("UPDATE Portmain SET lasttransno = ? WHERE ID = ?"));
    ...
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"UpdatePortmainLastTransNo", FALSE);
  } catch (...) {
    *pzErr = PrintError((char *)"Unexpected error in UpdatePortmainLastTransNo",
                        iID, 0, (char *)"", 0, -1, 0,
                        (char *)"UpdatePortmainLastTransNo", FALSE);
  }
}
  ...
  try {
    ...
    nanodbc::prepare(
        ...
        NANODBC_TEXT(
            ...
            "purgedate, lastactivity, rolldate, pinfo.DefaultVendorID, "
            "pinfo.accretmuni, pinfo.accretother "
            "FROM portmain as p "
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectPortmain", FALSE);
  } catch (...) {
    *pzErr = PrintError((char *)"Unexpected error in SelectPortmain", iID, 0,
                        (char *)"", 0, -1, 0, (char *)"SelectPortmain", FALSE);
  }
}

DLLAPI void STDCALL SelectPortmainByUniqueName(PORTMAIN *pzPR, char *sName,
                                               ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);

  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"SelectPortmainByUniqueName", FALSE);
    return;
  }
  ...
  try {
    ...
    nanodbc::prepare(
        ...
        NANODBC_TEXT(
            ...
            "pinfo.accretmuni, pinfo.accretother "
            "FROM portmain as p "
            "left join portinfo as pinfo on pinfo.id = p.id WHERE "
            "p.unique_name=?"));

    int idx = 0;
    safe_bind_string(stmt, idx, sName);
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), pzPR->iID, 0, sName, 0, -1, 0,
                        (char *)"SelectPortmainByUniqueName", FALSE);
  } catch (...) {
    *pzErr = PrintError(
        (char *)"Unexpected error in SelectPortmainByUniqueName", pzPR->iID, 0,
        sName, 0, -1, 0, (char *)"SelectPortmainByUniqueName", FALSE);
  }
}

DLLAPI void STDCALL SelectAcctMethod(char *sAcctMethod, int iID,
                                     ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"SelectAcctMethod", FALSE);
    return;
  }
  ...
  try {
    nanodbc::statement stmt(gConn);
    nanodbc::prepare(
        stmt, NANODBC_TEXT("SELECT acctmethod FROM Portmain WHERE ID = ?"));
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectAcctMethod", FALSE);
  } catch (...) {
    *pzErr =
        PrintError((char *)"Unexpected error in SelectAcctMethod", iID, 0,
                   (char *)"", 0, -1, 0, (char *)"SelectAcctMethod", FALSE);
  }
}

...
static void FillTransStruct(nanodbc::result& result, TRANS* pzTR)
{
	...
	// 30: broker_code
	strcpy_s(pzTR->sBrokerCode, STR12LEN, result.get<std::string>(29, "").c_str());

	// 31: broker_code2
	strcpy_s(pzTR->sBrokerCode2, STR12LEN, result.get<std::string>(30, "").c_str());
	// 32: trd_date
	...
	// 44: x_id
	strcpy_s(pzTR->sXID, STR1LEN, result.get<std::string>(43, "").c_str());
	// 45: x_trans_no
	...
	// 52: curr_acct_type
	strcpy_s(pzTR->sCurrAcctType, STR3LEN, result.get<std::string>(51, "").c_str());
	// 53: inc_curr_id
	strcpy_s(pzTR->sIncCurrId, STR3LEN, result.get<std::string>(52, "").c_str());
	// 54: inc_acct_type
	strcpy_s(pzTR->sIncAcctType, STR3LEN, result.get<std::string>(53, "").c_str());

	// 55: x_curr_id
	strcpy_s(pzTR->sXCurrId, STR3LEN, result.get<std::string>(54, "").c_str());
	// 56: x_curr_acct_type
	strcpy_s(pzTR->sXCurrAcctType, STR3LEN, result.get<std::string>(55, "").c_str());
	// 57: sec_curr_id
	...
	// 81: misc_desc_ind
	strcpy_s(pzTR->sMiscDescInd, STR1LEN, result.get<std::string>(80, "").c_str());
	// 82: dr_cr
	strcpy_s(pzTR->sDrCr, STR1LEN, result.get<std::string>(81, "").c_str());
	// 83: bal_to_adjust
	strcpy_s(pzTR->sBalToAdjust, STR1LEN, result.get<std::string>(82, "").c_str());
	// 84: cap_trans
	...
	// 86: dtc_inclusion
	strcpy_s(pzTR->sDtcInclusion, STR1LEN, result.get<std::string>(85, "").c_str());
	// 87: dtc_resolve
...
DLLAPI void STDCALL SelectForwardTrans(long iID, long lEffDate1, long lEffDate2, char *sSecNo, char *sWi, 
								  BOOL bSpecificSecNo, TRANS *pzTR, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
	...
	try
	{
		...
		nanodbc::prepare(stmt, NANODBC_TEXT("SELECT id, trans_no, tran_type, sec_no, wi, sec_xtend, acct_type, secid, sec_symbol, units, orig_face, unit_cost, tot_cost, orig_cost, pcpl_amt, opt_prem, amort_val, basis_adj, comm_gcr, net_comm, comm_code, sec_fees, misc_fee1, fee_code1, misc_fee2, fee_code2, accr_int, income_amt, net_flow, broker_code, broker_code2, trd_date, stl_date, eff_date, entry_date, taxlot_no, xref_trans_no, pend_div_no, rev_trans_no, rev_type, new_trans_no, orig_trans_no, block_trans_no, x_id, x_trans_no, x_sec_no, x_wi, x_sec_xtend, x_acct_type, x_secid, curr_id, curr_acct_type, inc_curr_id, inc_acct_type, x_curr_id, x_curr_acct_type, sec_curr_id, accr_curr_id, base_xrate, inc_base_xrate, sec_base_xrate, accr_base_xrate, sys_xrate, inc_sys_xrate, base_open_xrate, sys_open_xrate, open_trd_date, open_stl_date, open_unit_cost, orig_yld, eff_mat_date, eff_mat_price, acct_mthd, trans_srce, adp_tag, div_type, div_factor, divint_no, roll_date, perf_date, misc_desc_ind, dr_cr, bal_to_adjust, cap_trans, safek_ind, dtc_inclusion, dtc_resolve, recon_flag, recon_srce, income_flag, letter_flag, ledger_flag, gl_flag, created_by, create_date, create_time, post_date, bkof_frmt, bkof_seq_no, dtrans_no, price, restriction_code FROM trans WHERE id = ? AND rev_trans_no = 0 AND eff_date > ? AND eff_date <= ? ORDER BY trans_no"));
	...
	{
		*pzErr = PrintError("SelectForwardTrans", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

DLLAPI void STDCALL SelectBackwardTrans(long iID, long lEffDate1, long lEffDate2, char *sSecNo, char *sWi,
								   BOOL bSpecificSecNo, TRANS *pzTR, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
	...
	{
		nanodbc::statement stmt(gConn);
		nanodbc::prepare(stmt, NANODBC_TEXT("SELECT id, trans_no, tran_type, sec_no, wi, sec_xtend, acct_type, secid, sec_symbol, units, orig_face, unit_cost, tot_cost, orig_cost, pcpl_amt, opt_prem, amort_val, basis_adj, comm_gcr, net_comm, comm_code, sec_fees, misc_fee1, fee_code1, misc_fee2, fee_code2, accr_int, income_amt, net_flow, broker_code, broker_code2, trd_date, stl_date, eff_date, entry_date, taxlot_no, xref_trans_no, pend_div_no, rev_trans_no, rev_type, new_trans_no, orig_trans_no, block_trans_no, x_id, x_trans_no, x_sec_no, x_wi, x_sec_xtend, x_acct_type, x_secid, curr_id, curr_acct_type, inc_curr_id, inc_acct_type, x_curr_id, x_curr_acct_type, sec_curr_id, accr_curr_id, base_xrate, inc_base_xrate, sec_base_xrate, accr_base_xrate, sys_xrate, inc_sys_xrate, base_open_xrate, sys_open_xrate, open_trd_date, open_stl_date, open_unit_cost, orig_yld, eff_mat_date, eff_mat_price, acct_mthd, trans_srce, adp_tag, div_type, div_factor, divint_no, roll_date, perf_date, misc_desc_ind, dr_cr, bal_to_adjust, cap_trans, safek_ind, dtc_inclusion, dtc_resolve, recon_flag, recon_srce, income_flag, letter_flag, ledger_flag, gl_flag, created_by, create_date, create_time, post_date, bkof_frmt, bkof_seq_no, dtrans_no, price, restriction_code FROM trans WHERE id = ? AND rev_trans_no = 0 AND eff_date > ? AND eff_date <= ? ORDER BY trans_no DESC"));
	...
	{
		*pzErr = PrintError("SelectBackwardTrans", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

DLLAPI void STDCALL SelectAsofTrans(long iID, long lEffDate, long lTransNo, char *sSecNo, char *sWi, 
								BOOL bSpecificSecNo, TRANS *pzTR, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
	...
	{
		nanodbc::statement stmt(gConn);
		nanodbc::prepare(stmt, NANODBC_TEXT("SELECT id, trans_no, tran_type, sec_no, wi, sec_xtend, acct_type, secid, sec_symbol, units, orig_face, unit_cost, tot_cost, orig_cost, pcpl_amt, opt_prem, amort_val, basis_adj, comm_gcr, net_comm, comm_code, sec_fees, misc_fee1, fee_code1, misc_fee2, fee_code2, accr_int, income_amt, net_flow, broker_code, broker_code2, trd_date, stl_date, eff_date, entry_date, taxlot_no, xref_trans_no, pend_div_no, rev_trans_no, rev_type, new_trans_no, orig_trans_no, block_trans_no, x_id, x_trans_no, x_sec_no, x_wi, x_sec_xtend, x_acct_type, x_secid, curr_id, curr_acct_type, inc_curr_id, inc_acct_type, x_curr_id, x_curr_acct_type, sec_curr_id, accr_curr_id, base_xrate, inc_base_xrate, sec_base_xrate, accr_base_xrate, sys_xrate, inc_sys_xrate, base_open_xrate, sys_open_xrate, open_trd_date, open_stl_date, open_unit_cost, orig_yld, eff_mat_date, eff_mat_price, acct_mthd, trans_srce, adp_tag, div_type, div_factor, divint_no, roll_date, perf_date, misc_desc_ind, dr_cr, bal_to_adjust, cap_trans, safek_ind, dtc_inclusion, dtc_resolve, recon_flag, recon_srce, income_flag, letter_flag, ledger_flag, gl_flag, created_by, create_date, create_time, post_date, bkof_frmt, bkof_seq_no, dtrans_no, price, restriction_code FROM trans WHERE id = ? AND eff_date <= ? AND trans_no > ? order by trans_no"));
	...
	{
		*pzErr = PrintError("SelectAsofTrans", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

DLLAPI void STDCALL SelectTransByTransNo(long iID, long lTransNo1, long lTransNo2, char *sSecNo, char *sWi,
									BOOL bSpecificSecNo, TRANS *pzTR, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
	...
	{
		nanodbc::statement stmt(gConn);
		nanodbc::prepare(stmt, NANODBC_TEXT("SELECT id, trans_no, tran_type, sec_no, wi, sec_xtend, acct_type, secid, sec_symbol, units, orig_face, unit_cost, tot_cost, orig_cost, pcpl_amt, opt_prem, amort_val, basis_adj, comm_gcr, net_comm, comm_code, sec_fees, misc_fee1, fee_code1, misc_fee2, fee_code2, accr_int, income_amt, net_flow, broker_code, broker_code2, trd_date, stl_date, eff_date, entry_date, taxlot_no, xref_trans_no, pend_div_no, rev_trans_no, rev_type, new_trans_no, orig_trans_no, block_trans_no, x_id, x_trans_no, x_sec_no, x_wi, x_sec_xtend, x_acct_type, x_secid, curr_id, curr_acct_type, inc_curr_id, inc_acct_type, x_curr_id, x_curr_acct_type, sec_curr_id, accr_curr_id, base_xrate, inc_base_xrate, sec_base_xrate, accr_base_xrate, sys_xrate, inc_sys_xrate, base_open_xrate, sys_open_xrate, open_trd_date, open_stl_date, open_unit_cost, orig_yld, eff_mat_date, eff_mat_price, acct_mthd, trans_srce, adp_tag, div_type, div_factor, divint_no, roll_date, perf_date, misc_desc_ind, dr_cr, bal_to_adjust, cap_trans, safek_ind, dtc_inclusion, dtc_resolve, recon_flag, recon_srce, income_flag, letter_flag, ledger_flag, gl_flag, created_by, create_date, create_time, post_date, bkof_frmt, bkof_seq_no, dtrans_no, price, restriction_code FROM trans WHERE id = ? AND trans_no >= ? AND trans_no <= ?"));
	...
	{
		*pzErr = PrintError("SelectTransByTransNo", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

DLLAPI void STDCALL SelectTransByTradeDate(long iID, long lTrdDate1, long lTrdDate2, char *sSecNo, char *sWi,
									  BOOL bSpecificSecNo, TRANS *pzTR, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
	...
	{
		nanodbc::statement stmt(gConn);
		nanodbc::prepare(stmt, NANODBC_TEXT("SELECT id, trans_no, tran_type, sec_no, wi, sec_xtend, acct_type, secid, sec_symbol, units, orig_face, unit_cost, tot_cost, orig_cost, pcpl_amt, opt_prem, amort_val, basis_adj, comm_gcr, net_comm, comm_code, sec_fees, misc_fee1, fee_code1, misc_fee2, fee_code2, accr_int, income_amt, net_flow, broker_code, broker_code2, trd_date, stl_date, eff_date, entry_date, taxlot_no, xref_trans_no, pend_div_no, rev_trans_no, rev_type, new_trans_no, orig_trans_no, block_trans_no, x_id, x_trans_no, x_sec_no, x_wi, x_sec_xtend, x_acct_type, x_secid, curr_id, curr_acct_type, inc_curr_id, inc_acct_type, x_curr_id, x_curr_acct_type, sec_curr_id, accr_curr_id, base_xrate, inc_base_xrate, sec_base_xrate, accr_base_xrate, sys_xrate, inc_sys_xrate, base_open_xrate, sys_open_xrate, open_trd_date, open_stl_date, open_unit_cost, orig_yld, eff_mat_date, eff_mat_price, acct_mthd, trans_srce, adp_tag, div_type, div_factor, divint_no, roll_date, perf_date, misc_desc_ind, dr_cr, bal_to_adjust, cap_trans, safek_ind, dtc_inclusion, dtc_resolve, recon_flag, recon_srce, income_flag, letter_flag, ledger_flag, gl_flag, created_by, create_date, create_time, post_date, bkof_frmt, bkof_seq_no, dtrans_no, price, restriction_code FROM trans WHERE id = ? AND trd_date >= ? AND trd_date <= ?"));
	...
	{
		*pzErr = PrintError("SelectTransByTradeDate", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

DLLAPI void STDCALL SelectTransByEffectiveDate(long iID, long lEffDate1, long lEffDate2, char *sSecNo, char *sWi,
										  BOOL bSpecificSecNo, TRANS *pzTR, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
	PrintError("Entering", 0, 0, "", 0, 0, 0, "SelectTransByEffectiveDate", FALSE);
#endif
	InitializeErrStruct(pzErr);
	try
	{
		nanodbc::statement stmt(gConn);
		nanodbc::prepare(stmt, NANODBC_TEXT("SELECT id, trans_no, tran_type, sec_no, wi, sec_xtend, acct_type, secid, sec_symbol, units, orig_face, unit_cost, tot_cost, orig_cost, pcpl_amt, opt_prem, amort_val, basis_adj, comm_gcr, net_comm, comm_code, sec_fees, misc_fee1, fee_code1, misc_fee2, fee_code2, accr_int, income_amt, net_flow, broker_code, broker_code2, trd_date, stl_date, eff_date, entry_date, taxlot_no, xref_trans_no, pend_div_no, rev_trans_no, rev_type, new_trans_no, orig_trans_no, block_trans_no, x_id, x_trans_no, x_sec_no, x_wi, x_sec_xtend, x_acct_type, x_secid, curr_id, curr_acct_type, inc_curr_id, inc_acct_type, x_curr_id, x_curr_acct_type, sec_curr_id, accr_curr_id, base_xrate, inc_base_xrate, sec_base_xrate, accr_base_xrate, sys_xrate, inc_sys_xrate, base_open_xrate, sys_open_xrate, open_trd_date, open_stl_date, open_unit_cost, orig_yld, eff_mat_date, eff_mat_price, acct_mthd, trans_srce, adp_tag, div_type, div_factor, divint_no, roll_date, perf_date, misc_desc_ind, dr_cr, bal_to_adjust, cap_trans, safek_ind, dtc_inclusion, dtc_resolve, recon_flag, recon_srce, income_flag, letter_flag, ledger_flag, gl_flag, created_by, create_date, create_time, post_date, bkof_frmt, bkof_seq_no, dtrans_no, price, restriction_code FROM trans WHERE id = ? AND eff_date >= ? AND eff_date <= ?"));
	...
	{
		*pzErr = PrintError("SelectTransByEffectiveDate", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

DLLAPI void STDCALL SelectTransByEntryDate(long iID, long lEntryDate1, long lEntryDate2, char *sSecNo, char *sWi, 
									  BOOL bSpecificSecNo, TRANS *pzTR, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
	...
	{
		nanodbc::statement stmt(gConn);
		nanodbc::prepare(stmt, NANODBC_TEXT("SELECT id, trans_no, tran_type, sec_no, wi, sec_xtend, acct_type, secid, sec_symbol, units, orig_face, unit_cost, tot_cost, orig_cost, pcpl_amt, opt_prem, amort_val, basis_adj, comm_gcr, net_comm, comm_code, sec_fees, misc_fee1, fee_code1, misc_fee2, fee_code2, accr_int, income_amt, net_flow, broker_code, broker_code2, trd_date, stl_date, eff_date, entry_date, taxlot_no, xref_trans_no, pend_div_no, rev_trans_no, rev_type, new_trans_no, orig_trans_no, block_trans_no, x_id, x_trans_no, x_sec_no, x_wi, x_sec_xtend, x_acct_type, x_secid, curr_id, curr_acct_type, inc_curr_id, inc_acct_type, x_curr_id, x_curr_acct_type, sec_curr_id, accr_curr_id, base_xrate, inc_base_xrate, sec_base_xrate, accr_base_xrate, sys_xrate, inc_sys_xrate, base_open_xrate, sys_open_xrate, open_trd_date, open_stl_date, open_unit_cost, orig_yld, eff_mat_date, eff_mat_price, acct_mthd, trans_srce, adp_tag, div_type, div_factor, divint_no, roll_date, perf_date, misc_desc_ind, dr_cr, bal_to_adjust, cap_trans, safek_ind, dtc_inclusion, dtc_resolve, recon_flag, recon_srce, income_flag, letter_flag, ledger_flag, gl_flag, created_by, create_date, create_time, post_date, bkof_frmt, bkof_seq_no, dtrans_no, price, restriction_code FROM trans WHERE id = ? AND entry_date >= ? AND entry_date <= ?"));
	...
	{
		*pzErr = PrintError("SelectTransByEntryDate", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

DLLAPI void STDCALL SelectAsofTransCount(long iID, long lEffDate, long lTransNo, char *sSecNo, char *sWi, 
									BOOL bSpecificSecNo, long *piCount, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
	...
	{
		nanodbc::statement stmt(gConn);
		nanodbc::prepare(stmt, NANODBC_TEXT("SELECT count(*) FROM trans WHERE id = ? AND eff_date <= ? AND trans_no > ?"));
	...
	{
		*pzErr = PrintError("SelectAsofTransCount", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

DLLAPI void STDCALL SelectRegularTransCount(long iID, long lEffDate1, long lEffDate2, char *sSecNo, char *sWi, 
									   BOOL bSpecificSecNo, long *piCount, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
	...
	{
		nanodbc::statement stmt(gConn);
		nanodbc::prepare(stmt, NANODBC_TEXT("SELECT count(*) FROM trans WHERE id = ? AND rev_trans_no = 0 AND eff_date > ? AND eff_date <= ?"));
	...
	{
		*pzErr = PrintError("SelectRegularTransCount", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

DLLAPI void STDCALL CheckIfTransExists(long iID, long *piCount, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
	...
	{
		nanodbc::statement stmt(gConn);
		nanodbc::prepare(stmt, NANODBC_TEXT("SELECT count(*) FROM trans WHERE id = ?"));
	...
	{
		*pzErr = PrintError("CheckIfTransExists", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

DLLAPI void STDCALL UpdatePerfDate(long iId, long lStartDate, long lEndDate, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
	...
	{
		nanodbc::statement stmt(gConn);
		nanodbc::prepare(stmt, NANODBC_TEXT("Update trans set perf_date = eff_date where id = ? and eff_date > ? and eff_date <= ? and eff_date <> perf_date"));
	...
	{
		*pzErr = PrintError("UpdatePerfDate", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
	}
}

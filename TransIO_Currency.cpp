...
DLLAPI void STDCALL SelectCurrencySecno(char *sSecNo, char *sWi, int *piSecid,
                                        char *sCurrId, ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);

  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"T", 0,
                        -1, 0, (char *)"SelectCurrencySecno", FALSE);
    return;
  }

  try {
    nanodbc::statement stmt(gConn);
    nanodbc::prepare(stmt, NANODBC_TEXT("SELECT sec_no, wi, secid "
                                        "FROM currency WHERE curr_id = ? "));

    int idx = 0;
    safe_bind_string(stmt, idx, sCurrId);
    ...
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"T", 0, -1, 0,
                        (char *)"SelectCurrencySecno", FALSE);
  } catch (...) {
    *pzErr =
        PrintError((char *)"Unexpected error in SelectCurrencySecno", 0, 0,
                   (char *)"T", 0, -1, 0, (char *)"SelectCurrencySecno", FALSE);
  }
}

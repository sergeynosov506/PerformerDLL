...
*
// 2018-08-30 J#PER-8997 Restored logic w/future types, but kept exchange rate -mk.
...
*F*/
DLLAPI int STDCALL WINAPI CalcGainLoss (TRANS zTR, char *sTranCode, long lSecImpact, char *sPrimaryType, 
																	 char *sSecondaryType, char *sBaseCurrency, double *pzCGL, double *pzSGL, 
																	 double *pzSTGL, double *pzMTGL, double *pzLTGL, double *pzTotGL)
{
  ...
  *pzMTGL = *pzTotGL = 0;
  ...
  /* 
  ** if not a closing transaction and it is not a "LD" transaction, or an "FD"/"FX" transaction,
	** there is no gain/loss. 
	** SB   3/27/2001 - According to Rella, Now Gain/Loss will be calculated even for UP. 
	** vay 11/15/2001 - by SB's request GL will be calculated on PM/ST/LT trans also
  */
  if (((strcmp(sTranCode, "C") != 0 && strcmp(zTR.sTranType, "LD") == 0) ||
		  (strcmp(zTR.sTranType, "FD") == 0) || (strcmp(zTR.sTranType, "FX") == 0)) // || strcmp(zTR.sSecXtend, "UP") == 0))
	  && !(strcmp(zTR.sTranType, "PM") == 0 || 
           strcmp(zTR.sTranType, "LT") == 0 || 
  ...
  if (zTR.fBaseOpenXrate == 0)
	...
	else if ((*sPrimaryType == PTYPE_FUTURE)	&& (*sSecondaryType == STYPE_INDEX || *sSecondaryType == STYPE_FOREIGN))
	{
		*pzCGL = 0;
		//*pzTotGL = *pzSGL = zTR.fPcplAmt * lSecImpact * -1;
		//if (strcmp(zTR.sDrCr, "DR") == 0)
			*pzTotGL = *pzSGL = -1 * lSecImpact * (zTR.fPcplAmt / zTR.fBaseXrate);// * zTR.fSecBaseXrate/ zTR.fBaseOpenXrate;
		//else
		//	*pzTotGL = *pzSGL = (zTR.fPcplAmt / zTR.fBaseXrate) * zTR.fSecBaseXrate/ zTR.fBaseOpenXrate;
	} // Forward contract
  else if (lSecImpact == 1)
  {
		// Total Gain Loss = totcost/basexrate - (pcplamt/basexrate)*secbasexrate - optprem/secbasexrate
    *pzTotGL = (zTR.fTotCost / zTR.fSecBaseXrate) -
			         (((zTR.fPcplAmt / zTR.fBaseXrate) * zTR.fSecBaseXrate) / zTR.fBaseOpenXrate) - 
							 (zTR.fOptPrem / zTR.fSecBaseXrate);

		// Currency GainLoss = pcplamt/basexrate - ((pcplamt/basexrate)*secbasexrate)/baseopenxrate
		*pzCGL = (zTR.fPcplAmt / zTR.fBaseXrate) - 
						 (((zTR.fPcplAmt / zTR.fBaseXrate) * zTR.fSecBaseXrate) / zTR.fBaseOpenXrate);
	} // short
	else
	{
		// Total Gain Loss = pcplamt /basexrate - totcost/baseopenxrate - optprem/secbasexrate
		*pzTotGL = (zTR.fPcplAmt / zTR.fBaseXrate) -
			         (zTR.fTotCost / zTR.fBaseOpenXrate) - 
							 (zTR.fOptPrem / zTR.fSecBaseXrate);
		/* 
		** Currency Gain Loss = totcost / basexrate - totcost / baseopenxrate. For a loan, it's 
		** the other way  
		*/
		if (*sPrimaryType == PTYPE_LOAN)  /* closing of a loan */
			*pzCGL = (zTR.fTotCost/zTR.fBaseOpenXrate) - (zTR.fTotCost/zTR.fSecBaseXrate);
		else 
			*pzCGL = (zTR.fTotCost/zTR.fSecBaseXrate) - (zTR.fTotCost/zTR.fBaseOpenXrate);
  ...
  {
		if (*sPrimaryType == PTYPE_FUTURE && *sSecondaryType != STYPE_INDEX && *sSecondaryType != STYPE_FOREIGN) /* future */
    {
			*pzCGL = 0;
			*pzSGL = zTR.fPcplAmt;
  ...
  if (*sPrimaryType == PTYPE_DISCOUNT &&
      zTR.lEffDate > lDec311990 && zTR.lEffDate < lJan11994)
  {
		 *pzSGL = 0;
     *pzCGL = (zTR.fTotCost/zTR.fSecBaseXrate) - (zTR.fTotCost/zTR.fBaseOpenXrate);
     *pzTotGL = *pzCGL;
  ...
  STLTGainLoss(zTR, pzSGL, pzSTGL, pzLTGL, lSecImpact, sPrimaryType, sSecondaryType);
...
*F*/
DLLAPI int STDCALL WINAPI CalcMarketDiscount(TRANS zTR, long lIssueDate, long lMaturityDate, double fRedemptionPrice, double fTradUnit,
																				double fGainLoss, char *sPrimaryType, char *sSecondaryType, double *pfMarketDiscount)
{
  ...
  mdy[2] = 1993;
	...
	// No market discount for bonds issued before 7/19/1984 and purchased before 5/1/1993
	if (lIssueDate < lTempDate1 && zTR.lOpenTrdDate < lTempDate2)
		return 0;

	// No market discount for Municipal Bond purchased before 5/1/1999
	if (*sPrimaryType == PTYPE_BOND && *sSecondaryType == STYPE_MUNICIPALS && zTR.lOpenTrdDate < lTempDate2)
		return 0;
	...
	if (zTR.fUnits == 0 || fTradUnit == 0)
		...
		fUnitDiscount = fRedemptionPrice - (zTR.fTotCost / (zTR.fUnits * fTradUnit));
	...
	// Market discount applies only if unit discount is greater than 1/4 of 1% of the
...
DLLAPI int STDCALL WINAPI CalcMarketDiscountConstantYield(TRANS zTR, double fYieldAtCost, long lIssueDate, long lMaturityDate, double fRedemptionPrice,
																										 double fTradUnit, double fGainLoss, char *sPrimaryType, char *sSecondaryType, double *pfMarketDiscount)
{
  ...
  fPrice = lpfnCalculatePrice(fYieldAtCost, zTR.sSecNo, zTR.sWi, "C", zTR.lEffDate, 2);
	if (fPrice <= 0) // error calculating correct cost
		return 0;
	...
	//{
		*pfMarketDiscount = fGainLoss - (fDiscount * zTR.fUnits * fTradUnit);
		// Market Discount can not be more that the total gain

...
*
* $Header:   J:/Performer/Data/archives/Master/C/tranalloc.c-arc   1.67   27 Jan 2005 16:16:02   YegorovV  $
*
**/
// HISTORY
// 2018-07-31 VI# 61746 Added ability to calculate cost in transengine and G\L in calcgainloss for Futures, with secondary type N -mk
// 2018-03-06 VI# 60490: added code to adjust logic for total cost and G\L on futures -mk.  
// 2013-12-11 VI# 54307: initialize zTIPSAdjustments.iSize=0 - sergeyn  
// 2011-05-10 VI# 46071: increase precision for unit cost - sergeyn
...
// 3/18/2004 - Don't put descriptions on PI/BA - vay
...
DLLAPI ERRSTRUCT STDCALL WINAPI TranAlloc(TRANS *pzTrans, TRANTYPE zTranType, SECTYPE zSecType, ASSETS zAssets, 
																		 DTRANSDESC zDTransDesc[], int iNumDTItems, PAYTRAN *pzPayTran, 
																		 char *sCurrPrior, BOOL bDoTransaction)
{
  ERRSTRUCT zErr, zDeleteErr;
	...
	if (bDoTransaction) // if TranAlloc is responsible for DB transactions - start new one here
	{
	...
	// Lock the portfolio, by putting an entry in portlock, if a duplicate key error occurs,
	// wait for 5 seconds and try agai, if even after 6 tries(total of 30 seconds), portfolio
	// is still locked, return with an error.
	for (i = 0; i < 6; i++)
	{
		if (lpfnInsertPortLock(pzTrans->iID, pzTrans->sCreatedBy, &zErr) == 9729) //Duplicated Key which means the portfolio is locked
		{
	...
	{
		PrintError("Portfolio locked by another user", pzTrans->iID, pzTrans->lTransNo, 
							 "T", 135, 0, 0, "TALOC - PORTLOCK", FALSE);
		if (bDoTransaction) 
			lpfnRollbackTransaction();
	...
	__try
	{
		zErr = DoTranAlloc(pzTrans, zTranType, zSecType, zAssets, 
											 zDTransDesc, iNumDTItems, pzPayTran,
											 sCurrPrior, FALSE);
	...
	if (bDoTransaction) // if TranAlloc is responsible for DB transactions - commit/rollback it here
	{
...
ERRSTRUCT DoTranAlloc(TRANS *pzTrans, TRANTYPE zTranType, SECTYPE zSecType, ASSETS zAssets, 
											DTRANSDESC zDTransDesc[], int iNumDTItems, PAYTRAN *pzPayTran,
											char *sCurrPrior, BOOL bDoTransaction)
{
  ERRSTRUCT zErr;
  ...
  */ 
  ...
  {
     case 'I': return(ProcessIncomeInTranAlloc(pzTrans, zDTransDesc, iNumDTItems, 
																							 sCurrPrior, bDoTransaction));
   
     case 'M': return(ProcessMoneyInTranAlloc(pzTrans, zDTransDesc, iNumDTItems, 
																						  pzPayTran, sCurrPrior, bDoTransaction));

     case 'O': zErr = ProcessOpenInTranAlloc(pzTrans, zSecType, zAssets, zTranType, 
																						 zDTransDesc,iNumDTItems, pzPayTran,
               ...
               */   
               if (bExerciseFlag)
                 zErr = ExerciseOptionInTranAlloc(pzTrans, fExerciseUnits, zTranType, zSecType, zAssets,
                                                  zDTransDesc, iNumDTItems, sCurrPrior, bDoTransaction);
               return zErr;
   
     case 'C': zErr = ProcessCloseInTranAlloc(pzTrans, zTranType, zSecType, zAssets, 
																							zDTransDesc, iNumDTItems, pzPayTran, 
                                              &bExerciseFlag, sCurrPrior, &fExerciseUnits, bDoTransaction);
               if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
                  return zErr;
     ...
     case 'A': return(ProcessAdjustInTranAlloc(pzTrans, zDTransDesc, iNumDTItems, 
																							 sCurrPrior, bDoTransaction)); 

     case 'S': return(ProcessSplitInTranAlloc(pzTrans, zDTransDesc, iNumDTItems, 
																							sCurrPrior, bDoTransaction));
   
     case 'X': return(ProcessTransferInTranAlloc(pzTrans, zTranType, zSecType, zAssets, 
																								 zDTransDesc, iNumDTItems, 
																								 sCurrPrior, bDoTransaction));
   
     case 'R': return(ProcessCancelInTranAlloc(pzTrans, zDTransDesc, iNumDTItems, 
																				       sCurrPrior, bDoTransaction));
   
     default : return(PrintError("Invalid TranCode", pzTrans->iID, lRecNo, sRecType, 103, 0, 0, "TALOC - TRANCODE SWITCH", FALSE));
  } /*End Switch */
}/* TranAloc */
...
**/
ERRSTRUCT ProcessIncomeInTranAlloc(TRANS *pzTrans, 
																	 ...
																	 char *sCurrPrior, BOOL bDoTransaction) 
{
  ...
  */
  if (strcmp(pzTrans->sSecXtend, "TL") == 0 || strcmp(pzTrans->sSecXtend, "TS") == 0) 
    return(PrintError("Invalid Security Extention", pzTrans->iID, lRecNo, sRecType, 107, 0, 0, "TALOC - INCOME2", FALSE));
  ...
  */
  ...
  zErr = CurrencySweepProcessor(*pzTrans, zPortmain, sCurrPrior, bDoTransaction, ADDANDWRITEVALUE, TRUE);
...
**/
ERRSTRUCT ProcessMoneyInTranAlloc(TRANS *pzTrans, DTRANSDESC zDTransDesc[], int iNumDTItems, 
																	PAYTRAN *pzPayTran, 
																	char *sCurrPrior, BOOL bDoTransaction)
{
  ERRSTRUCT zErr;
  ...
  */
  if (IsValueZero(pzTrans->fPcplAmt, 2)  && IsValueZero(pzTrans->fIncomeAmt, 2))
    return(PrintError("Invalid Principal Amount", pzTrans->iID, lRecNo, sRecType, 26, 0, 0, "TALOC - MONEY1", FALSE));
...
**/
ERRSTRUCT ProcessOpenInTranAlloc(TRANS *pzTrans, SECTYPE zSecType, ASSETS zAssets, TRANTYPE zTranType, 
																 DTRANSDESC zDTransDesc[], int iNumDTItems, 
																 PAYTRAN *pzPayTran,
																 BOOL *bExerciseFlag, char *sCurrPrior, double *pfExerciseUnits, BOOL bDoTransaction)
{
  ...
  */
  if (IsValueZero(pzTrans->fUnits, 5))
    return(PrintError("Invalid Units", pzTrans->iID, lRecNo, sRecType, 28, 0, 0, "TALOC - OPEN1", FALSE));
  ...
  */
  if (zTranType.lCashImpact == -1)
  	...
  	fGrossProceeds = pzTrans->fPcplAmt + pzTrans->fCommGcr + pzTrans->fSecFees +
										 pzTrans->fMiscFee1 + pzTrans->fMiscFee2;
	else if (zTranType.lCashImpact == 0) // Added on 7/22/99 (to calculate UnitCost for FR transactions) - SB
...
// SB 1/27/1999 : As Per Leno this check is being removed 
  /*if (zTranType.lCashImpact != 0 && zTranType.lCashImpact != 2 &&
    ...
    return(PrintError("Invalid Principal Amount", pzTrans->iID, lRecNo,
					                 sRecType, 26, 0, 0, "TALOC - OPEN2", FALSE));*/
...
//		lpprTimer(22);
  	if (zErr.iSqlError)
      return(PrintError("Error Selecting Original Face Drom FIXEDINC", pzTrans->iID, lRecNo, sRecType, 0, 
												zErr.iSqlError, zErr.iIsamCode, "TALOC OPEN3", FALSE));

		if (zPFinc.sOriginalFace[0] == 'Y' && IsValueZero(pzTrans->fOrigFace, 3) && IsValueZero(pzTrans->fUnits, 5))
		  return(PrintError("Invalid Original Face", pzTrans->iID, lRecNo, sRecType, 29, 0, 0, "TALOC OPEN4", FALSE));

  } /* if fixed income security */

  /* 
  ** If there is no unit cost, calculate a new unit cost by using the following formula:
  ...
  */
// SB 7/22/99 Even for CashImpact 0, calculate unitcost  if (zTranType.lCashImpact != 0 && pzTrans->fUnitCost == 0)
  if (IsValueZero(pzTrans->fUnitCost, 6))
  {
	   if (IsValueZero(zAssets.fTradUnit, 3))
       return(PrintError("Invalid Trading Unit", pzTrans->iID, lRecNo, sRecType, 21, 0, 0, "TALOC OPEN5", FALSE));
     ...
     */
     if (strcmp(pzTrans->sCurrId, pzTrans->sSecCurrId) != 0)
       pzTrans->fUnitCost = ((fGrossProceeds / pzTrans->fBaseXrate) * pzTrans->fSecBaseXrate) / 
														(pzTrans->fUnits * zAssets.fTradUnit);
		 else
	     pzTrans->fUnitCost = fGrossProceeds / (pzTrans->fUnits * zAssets.fTradUnit);
  ...
  */
  if (IsValueZero(pzTrans->fTotCost, 3) && zTranType.lCashImpact != 0)
  {
     ...
     */
			 pzTrans->fTotCost = ((pzTrans->fPcplAmt / pzTrans->fBaseXrate) * pzTrans->fSecBaseXrate) + pzTrans->fOptPrem;
  } /* if purchase and total cost is zero */
  else  if (IsValueZero(pzTrans->fTotCost, 3) && zSecType.sPrimaryType[0] == PTYPE_FUTURE && 
			(zSecType.sSecondaryType[0] == STYPE_INDEX || zSecType.sSecondaryType[0] == STYPE_FOREIGN))
  {
  ...
  /* 
  ** If the transaction is purchase (cash impact not = zero) and if the orig_cost
...
//	lpprTimer(24);
  ...
  */
  if (zSecType.sPositionInd[0] == 'C')
    ...
    pzTrans->lTaxlotNo = DEFAULTMMLOT;/* assign the number of existing taxlot */
  else
...
**/
ERRSTRUCT ProcessCloseInTranAlloc(TRANS *pzTrans, TRANTYPE zTranType, SECTYPE zSecType, ASSETS zAssets,
                                  DTRANSDESC zDTransDesc[], int iNumDTItems, 
																	PAYTRAN *pzPayTran,
																	BOOL *bExerciseFlag, char *sCurrPrior, double *pfExerciseUnits, BOOL bDoTransaction)
{
  ...
  double       fGrossProceeds, fTotalCostRemoved, fOptPrem, fSellUnits, fUnitCost, fRoundCost;
  ...
  if (IsValueZero(pzTrans->fUnits, 5))
    return(PrintError("Invalid Units", pzTrans->iID, lRecNo, sRecType, 28, 0, 0, "TALOC CLOSE1", FALSE));

  /* 
  ** Calculate the gross proceeds of the transaction 
  **   Gross Proceeds   = Prncpl Amt + Commsn + SecFees + MiscFee1 + MiscFee2		For CshImpact 1
  **									OR	Prncpl Amt - Commsn - SecFees - MiscFee1 - MiscFee2		For CshImpact -1
  ...
  ** is first converted into security's local value and then divided by units
  */
// SB 7/22/99 - Calculate it even for Cash Impact 0 (FD)  if (pzTrans->fUnitCost == 0 && zTranType.lCashImpact != 0)
  if (IsValueZero(pzTrans->fUnitCost, 6))
  {
     if (IsValueZero(zAssets.fTradUnit, 3))
       return(PrintError("Invalid Trading Unit", pzTrans->iID, lRecNo, sRecType, 21, 0, 0, "TALOC - CLOSE3", FALSE));
...
//		lpprTimer(25);
		lpprSelectHoldings(&zHoldings, pzTrans->iID, pzTrans->sSecNo, pzTrans->sWi,
										   pzTrans->sSecXtend, pzTrans->sAcctType, pzTrans->lTaxlotNo, &zErr);
//		lpprTimer(26);
    ...
    // Increase the last trans no in portdir file and assign that number to TransNo in Trans record
    zErr = IncrementPortmainLastTransNo(&zPortmain, pzTrans->iID);
    if (zErr.iSqlError != 0)
       return zErr;
  ...
  */
  if (strcmp(pzTrans->sTranType, "PD") != 0 && zSecType.sPrimaryType[0] == 'B')
  {
		lpprSelectPartFixedinc(pzTrans->sSecNo, pzTrans->sWi, &zPFinc, &zErr);	  
		if (zErr.iSqlError || zErr.iBusinessError)
      return(PrintError("Error Selecting Amort & Original Face From FIXEDINC", pzTrans->iID, lRecNo, 
												sRecType, 0, zErr.iSqlError, zErr.iIsamCode, "TALOC - CLOSE4", FALSE));
  } /* if fixed income security */
  else 
  {
  ...
  */
  *bExerciseFlag = FALSE;
  if (strcmp(pzTrans->sTranType, "EX") != 0 && strcmp(pzTrans->sTranType, "ES") != 0)
  {
    for (i = 0; i < iNumDTItems; i++) /* # of DtransDesc items */
    {
      if (strcmp(zDTransDesc[i].sCloseType, "AE") == 0)
  ...
  if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
	...
	*/
	...
	zErr = GetExistingPosition(*pzTrans, &zPosInfo, &fTotalUnits, 
                             zTranType, zSecType, zPFinc.sOriginalFace, zAssets.fTradUnit);
	if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
	{
		/* Free the position information table */
		InitializePosInfoTable(&zPosInfo, 0);
	...
	*/ 
	if (strcmp(pzTrans->sTranType, "EX") != 0 && strcmp(pzTrans->sTranType, "ES") != 0)
	{
		// SB 7/10/2002 Added following two lines to get rid of warning, these have no logical effect
		fRoundCost = 0;
		lOpenDate = 0;
		for (i = 0; i < iNumDTItems; i++) /* # of DtransDesc Items */
		{
			...
			*/
			if (zDTransDesc[i].lTaxlotNo == 0 && (strcmp(pzTrans->sTranType, "WE") == 0 || strcmp(pzTrans->sTranType, "WI") == 0))
			{
				bMissingLot = TRUE;
				break;
			}
			...
			{      
				...
				zErr = DecipherDescInfo(&lOpenDate, &fSellUnits, &fUnitCost, zDTransDesc[i].sDescInfo, pzTrans->iID, lRecNo, sRecType);
				if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
				{
					/* Free the position information table */
					InitializePosInfoTable(&zPosInfo, 0);
			...
			{
				// If the taxlot id is not assigned, search by unitcost and trade date, else search by taxlot id 
				if (zDTransDesc[i].lTaxlotNo == 0) 
				{
					if ((zPosInfo.pzPInfo[j].fUnitCostRound == fRoundCost) && (zPosInfo.pzPInfo[j].lTradeDate == lOpenDate))
					{
						bMissingLot = FALSE;
				...
				else
	      {
				...
				*/
				if (zPosInfo.pzPInfo[j].fSharesToRemove >= zDTransDesc[i].fUnits)
				{
					...
					strcpy_s(zPosInfo.pzPInfo[j].sAcctMthd, "V");/*mark the current lot as versus*/
				...
				** an error message                                   
					...
					return(PrintError("Insufficient Shares for Versus", pzTrans->iID,
						               lRecNo, sRecType, 105, 0, 0, "TALOC CLOSE5", FALSE));
	...
	if (bMissingLot)
	{
		/* Free the position information table */
		InitializePosInfoTable(&zPosInfo, 0);
		return(PrintError("Unable to Identify TaxLot for Versus", pzTrans->iID, lRecNo, sRecType, 
											133, 0, 0, "TALOC - CLOSE6", FALSE));
	}

	/* If Total Shares Removed not = Total Shares to sell */
	if (!IsValueZero(zTIOriginal.fUnits - fTotalSharesRemoved, 5))
		RemoveByAcctMethod(zPosInfo, &fRemainingSharesToRemove, &fTotalSharesRemoved, &fTotalCostRemoved);

	/* 
	** If there are still move shares to sell, but no positions to sell them
	** against, bounce the trade (NO MORE tech-long/tech-short closing transactions)
	*/
	if (!IsValueZero(zTIOriginal.fUnits - fTotalSharesRemoved, 5))
	{
		/* Free the position information table */
		InitializePosInfoTable(&zPosInfo, 0);
		return(PrintError("Insufficient Shares To Sell", pzTrans->iID, lRecNo, sRecType, 134, 0, 0, "TALOC - CLOSE7", FALSE));
	}
	...
	*/ 
	zErr = TransactionProcess(zPosInfo, pzTrans, &zTIOriginal, &zTIRemaining, 
														zDTransDesc, iNumDTItems, pzPayTran,
														sCurrPrior, &lFirstTransNo, zPFinc.sOriginalFace,
														zSecType.sPrimaryType, zSecType.sSecondaryType, zPFinc.sPayType, bDoTransaction);
	if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
	{
		/* Free the position information table */
		InitializePosInfoTable(&zPosInfo, 0);
	...
	*/
	/*if (fTotalSharesRemoved != zTIOriginal.fUnits)	
		zErr = ProcessTechnicals(pzTrans, zTranType, &zTIOriginal, &zTIRemaining, zDTransDesc, 
														 iNumDTItems, sCurrPrior, &lFirstTransNo, bDoTransaction);*/
...
** and trade date 
**/
ERRSTRUCT DecipherDescInfo(long *plOpenDate, double *pfSellUnits, double *pfUnitCost, char *sDescInfo,
                           int iID, long lRecNo, char *sRecType) 
{
  ...
  */
  if (*sAtSign != '@')
    return(PrintError("Invalid @ - At Sign on Versus ", iID, lRecNo, sRecType, 203, 0, 0, "TALOC - DECIPHER DTRANS", FALSE));
  ...
  *pfUnitCost = atof(sPrice);
  if (IsValueZero(*pfUnitCost, 6))
    return(PrintError("Invalid Unit Cost on Versus", iID, lRecNo, sRecType, 201, 0, 0, "TALOC - DECIPHER DTRANS1", FALSE));
...
//cw  rstrdate(sTradeDate, lOpenDate);
  if (*plOpenDate == 0)
    return(PrintError("Invalid Trade Date on Versus", iID, lRecNo, sRecType, 202, 0, 0, "TALOC - DECIPHER DTRANS2", FALSE));

  /* Calculate the units */
	*pfSellUnits = atof(sSellUnits);
  if (IsValueZero(*pfSellUnits, 5))
    return(PrintError("Invalid Units on Versus ", iID, lRecNo, sRecType, 200, 0, 0, "TALOC - DECIPHER DTRANS3", FALSE));
...
**/
ERRSTRUCT ExerciseOptionInTranAlloc(TRANS *pzTrans, double fExerciseUnits, TRANTYPE zTranType, 
																		...
																		char *sCurrPrior, BOOL bDoTransaction)
{
  ...
  */
  zNTrans.iID = pzTrans->sXID;
  zNTrans.sXID = pzTrans->iID;
...
//  strcpy_s(zNTrans.sCreateTime, pzTrans->sCreateTime);
  ...
  */
    ...
    return(PrintError("Error Reading Portmain Acct Method", zNTrans.iID, 0, "",
													0, zErr.iSqlError, zErr.iIsamCode, "TALOC EXERCISE1", FALSE));
  ...
  ** dr_cr to 'CR'
  */
  if ((strcmp(sCallPut, "P") == 0 && 
       (strcmp(pzTrans->sTranType, "PS") == 0 || strcmp(pzTrans->sTranType, "CS") == 0)) ||
      (strcmp(sCallPut, "C") == 0 && 
       (strcmp(pzTrans->sTranType, "SL") == 0 || strcmp(pzTrans->sTranType, "SS") == 0))) 
  {
  ...
  */
	...
	if (zErr.iSqlError != 0 || zErr.iSqlError != 0)
		zErr = PrintError("Error Updating TRANS", iID, lOrgTransNo, "T", 0,
...
**/
ERRSTRUCT CalcOptionPremium(TRANS zTrans, double *pfExerciseUnits, double *pfOptPrem)
{
  ASSETS				zAssets;
  SECTYPE				zSecType;
  TRANTYPE			zTranType;
  ...
  long					lRecNo;
  double				fTotalCostRemoved, fRemainingSharesToRemove, fTotalUnits, fTotalSharesRemoved;
  int						i;
 
  /* Initialize all temporary variables */
  fTotalCostRemoved = fTotalUnits = fTotalSharesRemoved = fRemainingSharesToRemove = 0;
  i = 0;
  ...
  */
  if ((strcmp(sCallPut, "P") == 0 && 
		   (strcmp(zTrans.sTranType, "PS") == 0 || strcmp(zTrans.sTranType, "CS") == 0)) ||
      (strcmp(sCallPut, "C") == 0 && 
			 (strcmp(zTrans.sTranType, "SL") == 0 || strcmp(zTrans.sTranType, "SS") == 0))) 
  {
  ...
  */
  if (strcmp(zTrans.sSecCurrId, zAssets.sCurrId) != 0)
    return(PrintError("Currency Id Mismatch between Option & Security", zTrans.iID, lRecNo, 
											sRecType, 999, 0, 0, "TALOC CALCOPT1", FALSE));
  ...
  *pfExerciseUnits = zTrans.fUnits / zAssets.fTradUnit;
  ...
  if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
	...
	if (fTotalUnits < fRemainingSharesToRemove)
	{
		/* Free the position information table */
		InitializePosInfoTable(&zPosInfo, 0);
		return(PrintError("Insufficient Option Units", zTrans.iID, lRecNo, sRecType, 105, 0, 0, "TALOC CALCOPT2", FALSE));
	}
...
**/
ERRSTRUCT ProcessAdjustInTranAlloc(TRANS *pzTrans, DTRANSDESC zDTransDesc[], int iNumDTItems, 
																	 char *sCurrPrior, BOOL bDoTransaction)
{
  ERRSTRUCT zErr;
  PORTMAIN  zPortmain;
  ...
  ** pair off, sakekeeping, update cost, set or unset permanent gain/loss flag,
	** return of principal, set or unset restriction, update date, purcahse or sell of
	** basis transaction, the lot number must be provided, else if the transaction is a 
	** 'SE' or AG and balance to adjust is null or blank(just check 1st character), return an error 
  */
  if ( (strcmp(pzTrans->sTranType, "LD") == 0 || strcmp(pzTrans->sTranType, "AM") == 0 ||
        strcmp(pzTrans->sTranType, "PO") == 0 || strcmp(pzTrans->sTranType, "SK") == 0 ||
        strcmp(pzTrans->sTranType, "SG") == 0 || strcmp(pzTrans->sTranType, "UK") == 0 ||
        strcmp(pzTrans->sTranType, "UG") == 0 || strcmp(pzTrans->sTranType, "UC") == 0 || 
        strcmp(pzTrans->sTranType, "SR") == 0 || strcmp(pzTrans->sTranType, "UR") == 0 || 
			  strcmp(pzTrans->sTranType, "RP") == 0 || strcmp(pzTrans->sTranType, "UD") == 0 ||
				strcmp(pzTrans->sTranType, "PZ") == 0 || strcmp(pzTrans->sTranType, "SZ") == 0)
					&&	pzTrans->lTaxlotNo == 0)
    return(PrintError("Invalid Taxlot Number", pzTrans->iID, lRecNo, sRecType, 51, 0, 0, "TALOC ADJUST1", FALSE));
  else if (strcmp(pzTrans->sTranType, "SE") == 0 || strcmp(pzTrans->sTranType, "AG") == 0)  
  {     
    if (pzTrans->sBalToAdjust[0] == ' ' || pzTrans->sBalToAdjust[0] == '\0') 
       return(PrintError("Invalid Balance To Adjust", pzTrans->iID, lRecNo, sRecType, 89, 0, 0, "TALOC ADJUST2", FALSE));
    else if (strcmp(pzTrans->sBalToAdjust, "TAX") == 0 ||
             strcmp(pzTrans->sBalToAdjust, "NTAX") == 0)
       return(PrintError("Invalid Balance To Adjust", pzTrans->iID, lRecNo, sRecType, 114, 0, 0, "TALOC ADJUST3", FALSE));
  } /* if SE or SG */
	else if (strcmp(pzTrans->sTranType, "UB") == 0) // update broker - needs the original transaction
	{
		lpprSelectOneTrans(pzTrans->iID, pzTrans->lOrigTransNo, &zTR, &zErr);
		if (zErr.iSqlError == SQLNOTFOUND)
       return(PrintError("Invalid Transaction Number", pzTrans->iID, pzTrans->lOrigTransNo, "T", 50, 0, 0, "TALOC ADJUST4", FALSE));
		else if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
			return zErr;

		if (zTR.lRevTransNo != 0)
      return(PrintError("Invalid Transaction Number. Transaction Has Been Reversed", pzTrans->iID, pzTrans->lOrigTransNo, "T", 50, 0, 0, "TALOC ADJUST5", FALSE));
	}
 
	if (strcmp(pzTrans->sTranType,"UD") == 0 && (strcmp(pzTrans->sBalToAdjust, "TRD") != 0 &&
		  strcmp(pzTrans->sBalToAdjust, "STL") != 0 && strcmp(pzTrans->sBalToAdjust, "ELIG") != 0))
    return(PrintError("Invalid Date To Update", pzTrans->iID, lRecNo, sRecType, 136, 0, 0, "TALOC ADJUST6", FALSE));

  /* 
  ** Check that the principal amount/income amt column on the transaction contains
  ** a value, if the value is zero, generate an error message and fail the trade
  */
  if (IsValueZero(pzTrans->fPcplAmt, 2) && IsValueZero(pzTrans->fIncomeAmt, 2) && 
		  strcmp(pzTrans->sTranType, "SG") != 0 && strcmp(pzTrans->sTranType, "SK") != 0 && 
			strcmp(pzTrans->sTranType, "UG") != 0 && strcmp(pzTrans->sTranType, "UK") != 0 && 
			strcmp(pzTrans->sTranType, "SE") != 0 && strcmp(pzTrans->sTranType, "UD") != 0 &&
			strcmp(pzTrans->sTranType, "SR") != 0 && strcmp(pzTrans->sTranType, "UR") != 0 &&
			strcmp(pzTrans->sTranType, "UB") != 0)
    return(PrintError("Invalid Principal Amount", pzTrans->iID, lRecNo, sRecType, 26, 0, 0, "TALOC ADJUST7", FALSE));
  ...
  */
  if (strcmp(pzTrans->sTranType, "LD") == 0 || strcmp(pzTrans->sTranType, "RP") == 0)
  {
    /* Get the holding */
		lpprSelectHoldings(&zHoldings, pzTrans->iID, pzTrans->sSecNo, pzTrans->sWi,
										   pzTrans->sSecXtend, pzTrans->sAcctType, pzTrans->lTaxlotNo, &zErr);
    ...
    if (bRecFound == TRUE)
    {
      ...
      /*
			** If the lot was created by a FR/TS then the trade and settlement dates on the lot might
			** not be the dates on which trade became effective in the system. In this case the original
			** trade and settlement dates may be lost(if the lot was sold completely and there is no
			** holddel information to recreate it). In all the other cases, original trade and settlement
			** dates will be preserved.
			*/
			if (strcmp(zHoldings.sOrigTransType, "FR") == 0 || strcmp(zHoldings.sOrigTransType, "TS") == 0)
			{
...
**/
ERRSTRUCT ProcessSplitInTranAlloc(TRANS *pzTrans, DTRANSDESC zDTransDesc[], int iNumDTItems, 
																	char *sCurrPrior, BOOL bDoTransaction)
{
  ERRSTRUCT zErr;
  PORTMAIN	zPortmain;
  ...
  if (IsValueZero(pzTrans->fUnits, 5))
    return(PrintError("Invalid Units", pzTrans->iID, lRecNo, sRecType, 28, 0, 0, "TALOC SPLIT1", FALSE));
  ...
  */
  if (strcmp(pzTrans->sSecXtend, "TL") == 0 || strcmp(pzTrans->sSecXtend, "TS") == 0) 
    return(PrintError("Invalid Security Extention", pzTrans->iID, lRecNo, sRecType, 107, 0, 0, "TALOC - SPLIT2", FALSE));

  /* Verify that a tax lot number has been assigned */
  if (pzTrans->lTaxlotNo == 0)
    return(PrintError("Invalid Taxlot Number", pzTrans->iID, lRecNo, sRecType, 51, 0, 0, "TALOC - SPLIT3", FALSE));
...
**/
ERRSTRUCT ProcessTransferInTranAlloc(TRANS *pzTrans, TRANTYPE zTranType, SECTYPE zSecType, ASSETS zAssets, 
																		 DTRANSDESC zDTransDesc[], int iNumDTItems, 
																		 char *sCurrPrior, BOOL bDoTransaction)
{
  PORTMAIN		zPortmain;
  ...
  if (pzTrans->iID != pzTrans->sXID)
    return(PrintError("Invalid Transfer", pzTrans->iID, lRecNo, sRecType, 7, 0, 0, "TALOC - TRANSFER1", FALSE));
  ...
  */
  ...
  /*Pass the transaction to the process close function for further processing */
    zErr = ProcessCloseInTranAlloc(pzTrans, zTranType, zSecType, zAssets, 
																	 zDTransDesc, iNumDTItems, NULL, 
																	 &bExerciseFlag, sCurrPrior, &fExerciseUnits, bDoTransaction); 
...
**/
ERRSTRUCT ProcessCancelInTranAlloc(TRANS *pzTrans, DTRANSDESC zDTransDesc[], int iNumDTItems, 
                                   char *sCurrPrior, BOOL bDoTransaction)
{
  ERRSTRUCT	zErr;
  PORTMAIN  zPortmain;
  ...
  if (pzTrans->lRevTransNo == 0)
    return(PrintError("Invalid Reversal Transaction Number", pzTrans->iID,
								      pzTrans->lRevTransNo, "T", 53, 0, 0, "TALOC - CANCEL1", FALSE));

	lpprSelectTrancode(sTranCode, pzTrans->sRevType, pzTrans->sDrCr, &zErr);	  
	if (zErr.iSqlError)
    return(PrintError("Error Reading TRANTYPE", pzTrans->iID, pzTrans->lRevTransNo, "T", 0, 
											zErr.iSqlError, zErr.iIsamCode, "TALOC - CANCEL2", FALSE));
  ...
  */ 
  if (pzTrans->lXrefTransNo != 0 &&
      (sTranCode[0] == 'C' || sTranCode[0] == 'X' || sTranCode[0] == 'I' || sTranCode[0] == 'M' ||
       sTranCode[0] == 'S' || (sTranCode[0] == 'A' && (strcmp(pzTrans->sRevType, "LD") == 0 || strcmp(pzTrans->sRevType, "BA") == 0))))
  {
     while (zErr.iSqlError == 0 && zErr.iBusinessError == 0)
     {
        /* retrieve the effected trade */
				InitializeTransStruct(&zTR);
				lpprSelectTransForMatchingXref(pzTrans->iID, pzTrans->lXrefTransNo,&zTR, &zErr);
        if (zErr.iSqlError == SQLNOTFOUND)
        {
        ...
        */
        ...
        zTR.lEntryDate = pzTrans->lEntryDate;
				...
				zErr = UpdateHold(zTR, zPortmain, zDTransDesc, iNumDTItems, 
					                NULL, sCurrPrior, bDoTransaction);
				if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
					return zErr;
				
				zErr = CurrencySweepProcessor(zTR, zPortmain, sCurrPrior, bDoTransaction, ADDVALUE, TRUE);
				if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
           return zErr;
        ...
        /* Update the rev_trans_no on the original transaction */ 
				lpprUpdateRevTransNo(pzTrans->lTransNo, pzTrans->iID, zTR.lRevTransNo, &zErr);	  
				if (zErr.iSqlError)
					 zErr = PrintError("Error Updating TRANS", zTR.iID,zTR.lRevTransNo, "T", 0, 
															zErr.iSqlError, zErr.iIsamCode, "TALOC - CANCEL4", FALSE);
     }/* while SqlError = 0 and businesserror = 0 */
		 
		 if(bUpDateHoldCalled) 
				zErr = CurrencySweepProcessor(*pzTrans, zPortmain, sCurrPrior, bDoTransaction, JUSTWRITEVALUE, FALSE);
     if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
       return zErr;
  ...
  else
  {
     ...
     */
     ...
     /* Update the rev_trans_no on the original transaction */ 
		 lpprUpdateRevTransNo(pzTrans->lTransNo, pzTrans->iID, pzTrans->lRevTransNo, &zErr);	  
		 if (zErr.iSqlError)
		   zErr = PrintError("Error Updating TRANS", pzTrans->iID, pzTrans->lTransNo, "T", 0, 
													zErr.iSqlError, zErr.iIsamCode, "TALOC - CANCEL4", FALSE);	
  } 
...
**/
ERRSTRUCT GetExistingPosition(TRANS zTrans, POSINFOTABLE *pzPv, double *pfTotalUnits, 
															TRANTYPE zTranType, SECTYPE zSecType, char *sOriginalFace, double fTradingUnit)
{
  ERRSTRUCT zErr;
  int       i, j;
  ...
  */
  if (strcmp(zTrans.sTranType, "EX") == 0 || strcmp(zTrans.sTranType, "ES") == 0)
    strcpy_s(sAcctType, "%");
  else
    strcpy_s(sAcctType, zTrans.sAcctType);

  /* 
  ** portfolio's accounting method determines the order in which records are retrived from the 
	** holdings/holdcash table. Records are sorted in ascending order of the effdate for accounting 
	** method "F"(FIFO) and "A"(average-cost), in descending order of effdate for "L"(LIFO), in 
	** descending order of totcost for "H"(High), in ascending order of totcost for "S"(low), in 
	** descending order of totcost and long term and then in descending order of totcost and 
	** short term for M(Minimum Gain), in ascending order of totcost and short term and then in 
	** ascending order of totcost and long term for X(Maximum Gain), in descending order of the
	** unit cost where unit cost is less or equal to sale price and then descending order of the
	** unit cost where unit cost is geater than sale price for G(Minimum Gain and Maximum Loss) and
	** in ascending order of the unit cost where unit cost is greater or equal to sale price and 
	** then descending order of the unit cost where unit cost is less than sale price for O(Minimum Loss and Minimum Gain)
  */
  if (zTrans.sAcctMthd[0] != 'F' && zTrans.sAcctMthd[0] != 'A' && zTrans.sAcctMthd[0] != 'L' 
		  && zTrans.sAcctMthd[0] != 'H' && zTrans.sAcctMthd[0] != 'S' && zTrans.sAcctMthd[0] != 'M'
			&& zTrans.sAcctMthd[0] != 'X' && zTrans.sAcctMthd[0] != 'G' && zTrans.sAcctMthd[0] != 'O')
		return(PrintError("Invalid Accounting Method", zTrans.iID, lRecNo, sRecType, 77, 0, 0, "TALOC - GETPOSVER1", FALSE));
	if (zErr.iBusinessError != 0)
		return zErr;

	if (IsValueZero(zTrans.fUnits * fTradingUnit, 7))
		return(PrintError("Invalid Unit/Trading Unit", zTrans.iID, lRecNo, sRecType, 21, 0, 0, "TALOC - GETPOSVER1A", FALSE));
	else
	...
	{
		/*
		** For minimize gain and maximize gain the criteria on cost remains same but the criteria on
		** date changes (minimize gain first selects long term taxlots and then short term taxlots,
		** while maximize gain first selects short term and then long term taxlots), for other types
		** of accounting method, there is no such criteria on dates. So if accounting method is
		** minimize gain or maximize gain, run the same query twice and figure out correct dates 
		** and for other accounting method, run the query only once.
		*/
		if (strcmp(zTrans.sAcctMthd, "M") == 0) // minimize gain
		{
			if (j == 0) // first time, select long term taxlots in order of descending unit cost
			{
				lMinTrdDate = 0;
		...
		{
			if (j == 0) // first time, select short term taxlots in order of descending unit cost
			{
				// subtract 12 months from the transaction's trade date
		...
		else if (strcmp(zTrans.sAcctMthd, "G") != 0 && strcmp(zTrans.sAcctMthd, "O") != 0 && j > 0)
			continue;
	    ...
	    /* 
			...
			*/
	    if (strcmp(zTrans.sAcctMthd, "F") == 0 || strcmp(zTrans.sAcctMthd, "A") == 0)
		  {
			   if (strcmp(zSecType.sPositionInd, "C") == 0)
					 lpprHoldcashForFifoAndAvgAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, 
					 															 zTrans.sSecXtend, zTrans.sAcctType, 0, &zErr);
				 else
				 	 lpprHoldingsForFifoAndAvgAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, 
																				 zTrans.sSecXtend, zTrans.sAcctType, 0, &zErr);
			} // Fifo or Average
		  else if (strcmp(zTrans.sAcctMthd, "L") == 0)
			{
				if (strcmp(zSecType.sPositionInd, "C") == 0)
					lpprHoldcashForLifoAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, 
																  zTrans.sSecXtend, zTrans.sAcctType, 0, &zErr);
				else
					lpprHoldingsForLifoAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, 
																  zTrans.sSecXtend, zTrans.sAcctType, 0, &zErr);
			} // Lifo 
			else if (strcmp(zTrans.sAcctMthd, "H") == 0)
			{
				if (strcmp(zSecType.sPositionInd, "C") == 0)
			 		lpprHoldcashForHighAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, 
																  zTrans.sSecXtend, zTrans.sAcctType, 0, &zErr);
				else
					lpprHoldingsForHighAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, 
																	zTrans.sSecXtend, zTrans.sAcctType, 0, &zErr);
			} // High 
			else if (strcmp(zTrans.sAcctMthd, "S") == 0)
			{
				if (strcmp(zSecType.sPositionInd, "C") == 0)
					lpprHoldcashForLowAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, 
																 zTrans.sSecXtend, zTrans.sAcctType, 0, &zErr);
				else
			 		lpprHoldingsForLowAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, 
																 zTrans.sSecXtend, zTrans.sAcctType, 0, &zErr);
			} // Low 
			else if (strcmp(zTrans.sAcctMthd, "M") == 0)
			{
				if (strcmp(zSecType.sPositionInd, "C") == 0)
					lpprHoldcashForMinimumGainAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, zTrans.sSecXtend, 
																				 zTrans.sAcctType, 0, lMinTrdDate, lMaxTrdDate, &zErr);
				else
			 		lpprHoldingsForMinimumGainAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, zTrans.sSecXtend, 
																				 zTrans.sAcctType, 0, lMinTrdDate, lMaxTrdDate, &zErr);
			} // minimum gain
			else if (strcmp(zTrans.sAcctMthd, "X") == 0)
			{
				if (strcmp(zSecType.sPositionInd, "C") == 0)
					lpprHoldcashForMaximumGainAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, zTrans.sSecXtend, 
																				 zTrans.sAcctType, 0, lMinTrdDate, lMaxTrdDate, &zErr);
				else
			 		lpprHoldingsForMaximumGainAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, zTrans.sSecXtend, 
																				 zTrans.sAcctType, 0, lMinTrdDate, lMaxTrdDate, &zErr);
			} // maximum gain
			else if (strcmp(zTrans.sAcctMthd, "G") == 0)
			{
				if (j == 0) 
				{
					/* SB 11/5/02 - For cash position type, since holdcash has no lots, it does not matter how the 
					** records are selected from the table, no need to have a seperate query for acctmethed "G",
					...
					*/
					if (strcmp(zSecType.sPositionInd, "C") == 0)
						lpprHoldcashForFifoAndAvgAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, 
			 			...
			 			lpprMinimumGainHoldings(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, zTrans.sSecXtend, 
																	  zTrans.sAcctType, 0, fSalePrice, &zErr);
				} //j = 0
				else // j = 1; maximum loss
				{
					if (strcmp(zSecType.sPositionInd, "C") == 0)
						break; // for cash, the record has already been retreived (when j = 0) 
					else
			 			lpprMaximumLossHoldings(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, zTrans.sSecXtend, 
																	  zTrans.sAcctType, 0, fSalePrice, &zErr);
				} // j = 1
			...
			else // "O"
			{
				...
				{
					/* SB 11/5/02 - For cash position type, since holdcash has no lots, it does not matter how the 
					** records are selected from the table, no need to have a seperate query for acctmethed "O",
					** use the same query which is used for "F" (fifo)
					*/
					if (strcmp(zSecType.sPositionInd, "C") == 0)
						lpprHoldcashForFifoAndAvgAcct(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, 
			 			...
			 			lpprMinimumLossHoldings(&zHoldings, zTrans.iID, zTrans.sSecNo, zTrans.sWi, zTrans.sSecXtend, 
																	  zTrans.sAcctType, 0, fSalePrice, &zErr);
				} //j = 0
			...
			if (zErr.iSqlError == SQLNOTFOUND)
			{
				...
				return(PrintError("Error Reading Posver Cursor", zTrans.iID, lRecNo, sRecType, 0, 
													zErr.iSqlError, zErr.iIsamCode, "TALOC - GETPOSVER3", FALSE));		

			// 5/23/2003 - Make sure not to close a lot whose settlement date is after the settlement date of the sell
			if (zHoldings.lStlDate > zTrans.lStlDate)
				continue;

			// if it is a future, put curr_libility to tot_cost and open_liability to orig_cost
			...
			/* 
			** If the position has collateral units, check to see if the whole lot is collaterialized, 
			** if this is true, skip this lot for processing, otherwise set the reduce flag to true.
			*/
			...
			/* 
			** If the units are negative, (this should only happen where the security impact is +1, or 2 
			** and dr_cr column is "dr" or an 'ex' transaction, convert the units and cost columns from 
			** the holdings to a positive value. The reason is that values on the transaction record 
			** are always stated in positive values and failure to convert will result in inaccurate 
			...
			*/
			...
			/* 
			** If the reduce flag is true, reduce the total cost and original cost using a reduction ratio,
			** this is necessary to maintain the correct cost basis for selling. The units and original face 
			** must also be reduced before inserting them into the array. There is no check for zero divide 
			** as the holdings units can never be zero at this point. The reduction ratio can never be zero 
			** (the whole lot is collateralized) as these lots have already been excluded
			*/
			if (bReduceFlag)
			{
				...
				/*
				** SB 12/29/1999 - If a original face is specified on the trade, that's what should be
				** used instead of figuring it out based on original face on holdings. In this case put
				** a 
				*/
	    ...
	    if (bShortFlag)
			...
			*pfTotalUnits += pzPv->pzPInfo[i].fSharesToRemove;
			...
			if (i == pzPv->iSize) 
			{
			  ...
			  if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
					return(PrintError("Posver Table Too Small", zTrans.iID, lRecNo, sRecType, zErr.iBusinessError, 
													zErr.iSqlError, zErr.iIsamCode, "TALOC - GETPOSVER4", FALSE));						
			}
		} /* End of While Loop  */
	} // for j < 2
...
void RemoveByAcctMethod(POSINFOTABLE zPv, double *pfSharesToRemove, 
												double *pfTotalSharesRemoved, double *pfTotalCostRemoved)
{
  int   i;
  ...
  for (i = 0; i < zPv.iCount; i++)
  {
    ...
    if (!IsValueZero(zPv.pzPInfo[i].fShares, 5))
	    fReductionRatio = 1 - (zPv.pzPInfo[i].fSharesRemoved / zPv.pzPInfo[i].fShares);
		else
      fReductionRatio = 1;
    ...
    {
      fTotCost = zPv.pzPInfo[i].fTotCost - (zPv.pzPInfo[i].fTotCost * fReductionRatio);
...
**/
ERRSTRUCT TransactionProcess(POSINFOTABLE zPv, TRANS *pzTrans, TRADEINFO *pzOriginal, TRADEINFO *pzRemaining, 
														 DTRANSDESC zDTransDesc[], int iNumDTItems, PAYTRAN *pzPayTran,
														 char *sCurrPrior, long *plFirstTransNo, char *sOriginalFace,
														 char *sPrimaryType, char *sSecondaryType, char *sPayType, BOOL bDoTransaction)
{
	ERRSTRUCT		zErr;
	PORTMAIN		zPortmain;
  ...
  for (i = 0; i < zPv.iCount; i++) 
  {
       ...
       continue;
		...
		/*
		** If Not a Treasury Inflation protected security (TIPS) transaction then get the
		** next transaction number from portmain and assign it to this transaction, else
		** (if a TIPS closing transaction) don't do anything with transaction number, this
		** (and other, if any) transaction will be saved in a table and posted later (because
		...
		*/
			  ...
			  sPayType[0] != PAYTYPE_INFPROTCTD)
		{
			...
			*/
			...
			/* 
			** If this is the first trade to be generated for the closing/transfer transaction, store the 
			** transaction number in the FirstTransNo column this value will be assigned to the xref_trans_no 
			** column on this transaction and all subsequent transactions that are generated for this trade.
			*/
    ...
    */
    ...
    /* 
    ** Calculate the total and original cost for the sale record. If the reduction ratio is zero 
		** (signifies that the whole lot is to be sold) there is no need of a calculation, assign the 
		** whole portion. Otherwise, multiply cost by the ratio, and subtract the result from total 
    ...
    */
    ...
    /*
		** Calculate the original face to be associated with the number of units being removed - do this 
		** ONLY if this security should have a original face AND USER HAS NOT specified the original
		** face amount on the trade. If user has specified an original face amount then that amount is used.
		*/
		if (sOriginalFace[0] == 'Y' && !IsValueZero(zPv.pzPInfo[i].fOrigFace, 3) && IsValueZero(pzOriginal->fOrigFace, 3))
			pzTrans->fOrigFace = zPv.pzPInfo[i].fSharesRemoved * (zPv.pzPInfo[i].fOrigFace / zPv.pzPInfo[i].fShares);
    else
			pzTrans->fOrigFace = 0;
    ...
    /* 
		** If the security is set up to amortize and the portfolio is set up to amortize, calculate the 
		** amount of amortization for the sale and assign to the amort val column on the trans and reduce 
    ...
    */ 
    ...
    // Use rest of accumulated amount if there are zero shares left 
		...
		{
			/*
			** SB 7/20/01 For a transfer of security or for SL where user has not specified an original 
			...
			*/
 			...
 			if (*sPrimaryType == PTYPE_FUTURE)
			{
				...
				if (strcmp(zPv.pzPInfo[i].sDrCr, "DR") == 0) // short position
				{
					...
					{
						pzTrans->fPcplAmt = RoundDouble(pzTrans->fTotCost - pzTrans->fOrigCost, 2);
						strcpy_s(pzTrans->sDrCr, "DR");
					}
					else
					{
						pzTrans->fPcplAmt = RoundDouble(pzTrans->fOrigCost - pzTrans->fTotCost, 2);
    ...
    else
    {
      ...
      */
      ...
      pzTrans->fIncomeAmt = RoundDouble(fReduceValue * pzOriginal->fIncomeAmt, 2);
      ...
      pzRemaining->fCommGcr -= pzTrans->fCommGcr;
			...
			/*
			** If the security should have original face and user has specified the original face
			** on the transaction, use that number.
			*/
			if (sOriginalFace[0] == 'Y' && !IsValueZero(pzOriginal->fOrigFace, 3))
			{
				// If the calculated amount of original face is more than what the lot has, take as much as the lot has
				if (RoundDouble(fReduceValue * pzOriginal->fOrigFace, 3) > RoundDouble(zPv.pzPInfo[i].fOrigFace, 3))
					pzTrans->fOrigFace = RoundDouble(zPv.pzPInfo[i].fOrigFace, 3);
				else
					pzTrans->fOrigFace = RoundDouble(fReduceValue * pzOriginal->fOrigFace, 3);

				pzRemaining->fOrigFace -= pzTrans->fOrigFace;
			}
			// For futures TotCost has curr_libility and orig_cost has open libility. Recalculate
			// current liability for this lot and then figure out pcpl_amt by subtraction open_libility
			// from cur_libility. Also make sure dr_cr flag is correct by checking soign of pcpl_amt.
			if (*sPrimaryType == PTYPE_FUTURE)
			{
    ...
    */
    if ((strcmp(pzTrans->sTranType, "ML") == 0 || 
         strcmp(pzTrans->sTranType, "SL") == 0) && strcmp(sPayType, "D") == 0)
    {
			fDiscountIncome = RoundDouble(pzTrans->fPcplAmt - RoundDouble(pzTrans->fTotCost,2),2);
		...
		/*
		** If it a TIPS transaction, then add it to a table, to be posted after system
		** has calculated and posted phantom income and basis adjustment transactions,
		** else (not a TIPS transaction), post the transaction by calling UpdateHold.
		*/
		if (sPrimaryType[0] == PTYPE_BOND && sSecondaryType[0] == STYPE_TBILLS &&
			  sPayType[0] == PAYTYPE_INFPROTCTD)
		{
		...
		else
		{
			...
			zErr = CurrencySweepProcessor(*pzTrans, zPortmain, sCurrPrior, bDoTransaction, ADDVALUE, bInit);
			bInit = FALSE;
			bUpdateHoldCalled = TRUE;
	...
	*/
		  ...
		  sPayType[0] == PAYTYPE_INFPROTCTD)
	{
		for (i = 0; i < zTIPSTrades.iCount; i++)
		{
			...
			// if system is reversing & rebooking a transaction, don't generate PI/BA again
			...
			// of the position) between last income date and settlement date of this closing trade
			zErr = CalculatePhantomIncome(zTr.iID, zTr.sSecNo, zTr.sWi, zTr.lTaxlotNo, lLastIncDate, 
                                    zTr.lStlDate, zTr.lOpenTrdDate, zTr.fOrigCost, zTr.fUnits, 
			...
			if (IsValueZero(fPhantomIncome, 2))
				continue; // if no increase in value, no need to generatePI & BA transaction
			...
			zTr.fPcplAmt = zTr.fTotCost = zTr.fOrigCost = zTr.fOrigFace = zTr.fAccrInt = 0;
			zTr.fCommGcr = zTr.fNetComm = zTr.fMiscFee1 = zTr.fMiscFee2 = zTr.fSecFees = 0;
			...
			// currency sweep processor after PI or BA because none of these have any cash impact
			zErr = UpdateHold(zTr, zPortmain, zTrDesc, 0, 
				                NULL, sCurrPrior, bDoTransaction);
			if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
				return zErr;

			// Save the basis adjustment transactions in a table and post them after all the PIs are
			// posted so that all PIs can be combined together with a single xref_trans_no and all
			// BAs are combined together with another xref_trans_no.
			...
			// Adjust the cost on the (closing) transaction by the amount of previous BA which 
			// was just created for this taxlot.
	...
	if (bUpdateHoldCalled)
		zErr = CurrencySweepProcessor(*pzTrans, zPortmain, sCurrPrior, bDoTransaction, JUSTWRITEVALUE, bInit);
...
**/
ERRSTRUCT ProcessTechnicals(TRANS *pzTrans, TRANTYPE zTranType, TRADEINFO *pzOriginal, 
														TRADEINFO *pzRemaining, DTRANSDESC zDTransDesc[], int iNumDTItems,
                            char *sCurrPrior, long *plFirstTransNo, BOOL bDoTransaction)
{
  ERRSTRUCT zErr;
  PORTMAIN   zPortmain;
  double		fBaseAmt;
  ...
  */
  if (zTranType.lSecImpact == 1)
    ...
    return(PrintError("Invalid Security Impact", pzTrans->iID, lRecNo, sRecType, 47, 0, 0, "TALOC - PRCESSTECH1", FALSE));
  ...
  */
  ...
  zErr = CurrencySweepProcessor(*pzTrans,zPortmain,sCurrPrior, bDoTransaction,ADDANDWRITEVALUE,bInit);
...
ERRSTRUCT CurrencySweepProcessor(TRANS zTrans, PORTMAIN zPortmain, 
																 char *sCurrPrior, BOOL bDoTransaction, int iAction, BOOL bInit)
{
	ERRSTRUCT				zErr;
	...
	if (((strcmp(zTrans.sCurrId,zPortmain.sBaseCurrId) != 0 && zTrans.fPcplAmt != 0) ||
			 (strcmp(zTrans.sIncCurrId,zPortmain.sBaseCurrId) !=0 && zTrans.fIncomeAmt != 0)) &&
			strcmp(zTrans.sTranType,"TC") != 0 && strcmp(zTrans.sRevType,"TC") != 0 &&
			strcmp(zPortmain.sCurrHandler,"T") == 0)
	{
		if (strcmp(zTrans.sTranType, "RV") ==0)
		{
			...
			sprintf_s(sMsg, "Trantype: %s, DrCr: %s Not Found In Trantype Table", zTrans.sRevType, zTrans.sDrCr);
		}
		else
		{
			lpprSelectTrantype(&zTranType,zTrans.sTranType,zTrans.sDrCr,&zErr);
			sprintf_s(sMsg, "Trantype: %s, DrCr: %s Not Found In Trantype Table", zTrans.sTranType, zTrans.sDrCr);
		}
		if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
	    return(PrintError(sMsg, zTrans.iID, zTrans.lTransNo, "T", 46, 0, 0, "TALOC - CURRPROC1", FALSE));
		...
		if (bInit)
		{
			...
			if (zTranType.lCashImpact == -1) 
			{
				strcpy_s(zNewTrans.sSecNo,zPortmain.sBaseCurrId);//zTrans.sBaseCurrId);	
			...
			if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
			{
				if (zErr.iSqlError == SQLNOTFOUND)
				{
					sprintf_s(sMsg, "Security: %s, Wi: %s Not Found In Asset Table", zNewTrans.sSecNo, zNewTrans.sWi);
					zErr = PrintError(sMsg, zTrans.iID, zTrans.lTransNo, "T", 8, 0, 0, "TALOC - CURRPROC2", FALSE);
				}
		  ...
		  lpprSelectAsset(&zAssets, zNewTrans.sXSecNo, zNewTrans.sXWi, -1, &zErr);
			if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
			{
				if (zErr.iSqlError == SQLNOTFOUND)
				{
					sprintf_s(sMsg, "Security: %s, Wi: %s Not Found In Asset Table", zNewTrans.sXSecNo, zNewTrans.sXWi);
					zErr = PrintError(sMsg, zTrans.iID, zTrans.lTransNo, "T", 8, 0, 0, "TALOC - CURRPROC3", FALSE);
				}
...
//				zNewTrans.fTotCost += zTrans.fPcplAmt;
		...
		if ((zTranType.lCashImpact == 1 || zTranType.lCashImpact == -1) && (iAction == ADDANDWRITEVALUE))
		{
			zNewTrans.fUnits = zTrans.fPcplAmt;
			zNewTrans.fPcplAmt = zTrans.fPcplAmt;
		...
		if (iAction == ADDANDWRITEVALUE || iAction == JUSTWRITEVALUE)
		{
			if (zNewTrans.fPcplAmt != 0) 
			{
				...
				/* Pass the transaction to "updhold" for final processing */
				zErr = UpdateHold(zNewTrans, zPortmain, &zDTransDesc, iNumDTItems, 
					                NULL, sCurrPrior, bDoTransaction);
				if (zErr.iSqlError || zErr.iBusinessError)
					return(PrintError("Error Creating TC Trans", zNewTrans.iID, 0, "D", zErr.iBusinessError, 
														zErr.iSqlError, zErr.iIsamCode,  "TALOC CURRPROC4", FALSE));
			}
			...
			if (fTotIncomeAmt != 0)
			{
				...
				zErr = UpdateHold(zNewTrans, zPortmain, &zDTransDesc, iNumDTItems, 
					                NULL, sCurrPrior, bDoTransaction);
				if (zErr.iSqlError || zErr.iBusinessError)
					return(PrintError("Error Creating II TC Trans", zNewTrans.iID, zErr.iBusinessError,
														"D", 0, zErr.iSqlError, zErr.iIsamCode, "TALOC CURRPROC5", FALSE));
			}

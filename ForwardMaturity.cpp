...
ERRSTRUCT STDCALL WINAPI GenerateForwardMaturity(
    long lValDate, const char *sMode, int iID, const char *sSecNo,
    const char *sWi, const char *sSecXtend, const char *sAcctType) {
  long lForwardDate;
  ERRSTRUCT zErr;
  ...
  */
  if (sMode[0] == 'B') {
    zErr = ForwardMaturityUnloadAndSort(
        lForwardDate, lValDate - zSysSet.zSyssetng.iPaymentsStartDate);
  ...
  zErr = ForwardMaturityGeneration(
      zPmainTable, zPInfoTable, zSATable, zCTable, sMode, sSecNo, sWi,
      sSecXtend, sAcctType, lValDate - zSysSet.zSyssetng.iPaymentsStartDate,
      lValDate, lForwardDate);
...
ERRSTRUCT ForwardMaturityGeneration(PORTTABLE zPmainTable,
                                    PINFOTABLE zPInfoTable,
  ...
  for (i = 0; i < zPInfoTable.iNumPI; i++) {
    ...
    zErr = BuildForwardMatTable(
        sMode, sFileName, zPmainTable.pzPmain[m].iID, sSecNo, sWi, sSecXtend,
        sAcctType, lStartDate, lForwardDate,
        zPInfoTable.pzPRec[i].lStartPosition, &zFMTable);
    if (zErr.iSqlError != 0 || zErr.iBusinessError != 0) {
      lpfnPrintError("Forward Maturity Could Not Be Processed",
                     zPmainTable.pzPmain[m].iID, 0, "", zErr.iBusinessError,
                     zErr.iSqlError, zErr.iIsamCode,
                     " FORWARD MATURITY PROCESS1", FALSE);
    ...
    for (j = 0; j < zFMTable.iNumMat; j++) {
      // This module should mature only forwards, it can be restricted in query
      // that's getting all eligible records but this routine should check i too
      // instead of relying on the query
      ...
      if (sLastSecNo[0] == '\0' || /* First Time or same security as last */
        ...
        for (l = 0; l < zSTable.iNumSAcct; l++) {
          if (strcmp(zSTable.zSAcct[l].sAcctType,
                     zFMTable.pzForwardMat[j - 1].sAcctType) == 0) {
            bIncTypeFound = TRUE;
            break;
          }
        } // for l < NumSAcct
        ...
        zErr = CreateTrans(zFMTable.pzForwardMat[j - 1], fUnits, fOrigFace,
                           zCTable.zCurrency[k].fCurrExrate,
                           zPmainTable.pzPmain[m].sAcctMethod,
                           zSTable.zSAcct[l].sXrefAcctType, lValDate,
                           OpenLiability, CurrLiability);
        if (zErr.iSqlError != 0 || zErr.iBusinessError != 0) {
    ...
    if (j > 0) {
      ...
      if (strcmp(sLastSecNo, zFMTable.pzForwardMat[j].sSecNo) == 0 &&
        ...
        if (zErr.iSqlError == 0 && zErr.iBusinessError == 0) {
          ...
          for (l = 0; l < zSTable.iNumSAcct; l++) {
            if (strcmp(zSTable.zSAcct[l].sAcctType,
                       zFMTable.pzForwardMat[j].sAcctType) == 0) {
              bIncTypeFound = TRUE;
              break;
            }
          ...
          zErr = CreateTrans(zFMTable.pzForwardMat[j], fUnits, fOrigFace,
                             zCTable.zCurrency[k].fCurrExrate,
                             zPmainTable.pzPmain[m].sAcctMethod,
                             zSTable.zSAcct[l].sXrefAcctType, lValDate,
                             OpenLiability, CurrLiability);
...
**/
ERRSTRUCT BuildForwardMatTable(const char *sMode, const char *sFileName,
                               int iID, const char *sSecNo, const char *sWi,
                               const char *sSecXtend, const char *sAcctType,
                               long lStartDate, long lValDate,
                               long lStartPosition,
                               FORWARDMATSTRUCT *pzFMTable) {
  int iLastID;
  ...
  if (sMode[0] == 'B') {
    ...
    if (fp == NULL)
      zErr = lpfnPrintError("Error Opening File", iID, 0, "", 999, 0, 0,
                            "FUTURE MATURITY BUILDTABLE1", FALSE);
    ...
    if (fseek(fp, 0L, 0) < 0) // lStartPosition
      zErr = lpfnPrintError("Error Seeking File", iID, 0, "", 999, 0, 0,
                            "FUTURE MATURITY BUILDTABLE2", FALSE);
    ...
    */
    ...
    while (zErr.iSqlError == 0) {
      InitializeForwardMatStruct(&zTempMat);
      lpprForwardMaturityUnload(&zTempMat, lStartDate, lEndDate, sMode, iID,
                                sSecNo, sWi, sSecXtend, sAcctType, &zErr);
...
ERRSTRUCT ForwardMaturityUnloadAndSort(long lValDate, long lStartDate) {
  ...
  if (fp == NULL)
    zErr = lpfnPrintError("Error Opening File", 0, 0, "", 999, 0, 0,
                          "FORWARD MATURITY UNLOAD1", FALSE);

  lEndDate = GetPaymentsEndingDate(lValDate);
  /* Fetch all the records and write them to the file */
  while (zErr.iSqlError == 0 && zErr.iBusinessError == 0) {
    memset(&zTempMat, 0, sizeof(FMATSTRUCT));
    lpprForwardMaturityUnload(&zTempMat, lStartDate, lEndDate, "", 0, "", "",
                              "", "", &zErr);
    if (zErr.iSqlError == SQLNOTFOUND) {
      zErr.iSqlError = 0;
      break;
      ...
      zErr = lpfnPrintError("Error Fetching Unload Cursor", 0, 0, "", 0,
                            zErr.iSqlError, zErr.iIsamCode,
                            "FORWARD MATURITY FETCHUNLOAD", FALSE);
      break;
    }
    /* Get all the fields */
    sprintf_s(sUnlStr,
              "%d|%s|%f|%f|%f|%f|%s|%ld|%s|%f|%f|%d|%s|%d|%s|%f|%ld|%f|%s|\n",
              zTempMat.iID, zTempMat.sAcctType, zTempMat.fClosePrice,
              zTempMat.fCurExrate, zTempMat.fCurIncExrate,
              zTempMat.fCurliability, zTempMat.sCurrId, zTempMat.lExpDate,
              zTempMat.sIncCurrId, zTempMat.fOpenliability, zTempMat.fOrigFace,
              zTempMat.iSecID, zTempMat.sSecNo, zTempMat.iSecType,
              zTempMat.sSecXtend, zTempMat.fTrdUnit, zTempMat.lTransNo,
              zTempMat.fUnits, zTempMat.sWi);
    /* Write the string to the file) */
    if (fputs(sUnlStr, fp) == EOF)
      zErr = lpfnPrintError("Error Writing To The File", 0, 0, "", 999, 0, 0,
                            "FORWARD MATURITY UNLOAD2", FALSE);
  } /* while no error */
  fclose(fp);
...
**/
ERRSTRUCT CreateTrans(FMATSTRUCT zFMat, double fUnits, double fOrigFace,
                      double fBaseCurrExrate, char *sAcctMethod,
                      char *sIncAcctType, long lValDate, double OpenLiability,
                      double CurrLiability) {
  char sTemp[30];
  ...
  if (fBaseCurrExrate <= 0.0)
    return (lpfnPrintError("Invalid Exrate", 0, 0, "", 67, 0, 0,
                           "MATURITY CREATEATRANS1", FALSE));
  ...
  /*zTrans.fPcplAmt		=		zFMat.fCurliability -
     zFMat.fOpenliability; zTrans.fOrigCost	=
     zFMat.fOpenliability; zTrans.fTotCost		=
     zFMat.fCurliability;*/
  ...
  */
  ...
  if (j < 0)
    return (lpfnPrintError("Invalid Sec Type", zFMat.iID, 0, "", 18, 0, 0,
...
ERRSTRUCT AddRecordToForwardMatTable(FORWARDMATSTRUCT *pzMTab,
                                     FMATSTRUCT zMatStruct) {
  ERRSTRUCT zErr;
  ...
  if (pzMTab->iMatCreated == pzMTab->iNumMat) {
    ...
    pzMTab->pzForwardMat = (FMATSTRUCT *)realloc(
        pzMTab->pzForwardMat, pzMTab->iMatCreated * sizeof(FMATSTRUCT));
    if (pzMTab->pzForwardMat == NULL)
      return (lpfnPrintError("Insufficient Memory", 0, 0, "", 997, 0, 0,
                             "FARWARD MATURITY ADDMAT", FALSE));
  }

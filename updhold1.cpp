...
*                          routine is responsible to update LastTransNo(which is
*                          incremented by TranAlloc function) in portdir table.
*           sCurPrior      is a  flag telling us which program is calling
*                          this routine. Transaction Processor will call it with
*                          "C"(current) and roll will call it with "P"(prior).
...
*
* $Header:   J:/Performer/Data/archives/Master/C/UPDHOLD1.C-arc   1.64   07 Sep 2005 15:21:06   GriffinJ  $
*
...
**
History
// 2020-01-20 J#PER-10497 Fixed error for failing transaction when holdcash overflow occurs -mk.
																			...
																			char *sCurPrior, BOOL bDoTransaction)
{
  ...
  if (sCurPrior[0] != 'C' && sCurPrior[0] != 'P')
     return(PrintError("Invalid Current/Prior Indicator", zTR.iID, lRecNo, sRecType, 102, 0, 0, "UPDATE HOLD1", FALSE));
  ...
  */
  ...
  if (zErr.iSqlError)
    return(PrintError("Error Selecting TRANTYPE Record", zTR.iID,  lRecNo, sRecType, 0, 
											zErr.iSqlError, zErr.iIsamCode, "UPDH SELECT TRANTYPE", FALSE));

	// If updathold is responsible for doing a database transaction, start a new transaction
	...
	__try
	{
		...
		*/
		if (zTranType.sTranCode[0] == 'M')
			zErr = ProcessCash(zTR, zTranType.lCashImpact, bReversal, sCurPrior, zPR.bIncByLot);
		...
		/*  else if (zTranType.sTranCode[0] == 'O' || (zTranType.sTranCode[0] == 'C' && 
			(strcmp(zTR.sSecXtend, "TS") == 0 || strcmp(zTR.sSecXtend, "TL") == 0))) */
		{ 
			zErr = ProcessOpeningInHoldings(zTR, zPR, zTranType.lSecImpact, zTranType.lCashImpact, zTranType.iTradeDateInd, bReversal, sCurPrior);
			if (zErr.iBusinessError == 0 && zErr.iSqlError == 0)
				zErr = ProcessCash(zTR, zTranType.lCashImpact, bReversal, sCurPrior, zPR.bIncByLot);
		}
		else if (zTranType.sTranCode[0] == 'C')
		{ 
			zErr = ProcessClosingInHoldings(zTR, zPR.sBaseCurrId, zTranType.lSecImpact, bReversal, sCurPrior);
			if (zErr.iBusinessError == 0 && zErr.iSqlError == 0)
				zErr = ProcessCash(zTR, zTranType.lCashImpact, bReversal, sCurPrior, zPR.bIncByLot);
		}
		else if (zTranType.sTranCode[0] == 'S') /* Split Transaction */
			zErr = ProcessSplitInHoldings(zTR, zTranType.lSecImpact, zTranType.sAutoGen, bReversal, sCurPrior);
		else if (zTranType.sTranCode[0] == 'A') /* Adjustment Transaction */
		{
			zErr = ProcessAdjustmentInHoldings(&zTR, zPR, zTranType.lSecImpact, zTranType.lCashImpact, bReversal, sCurPrior);
			if (zErr.iBusinessError == 0 && zErr.iSqlError == 0)
				zErr = ProcessCash(zTR, zTranType.lCashImpact, bReversal, sCurPrior, zPR.bIncByLot);
		}
		else if (zTranType.sTranCode[0] == 'X') /* Transfer Transaction */
		{
			if (zTR.iID != zTR.sXID)
			{
				...
				return(PrintError("Inter Account Transfers Are Not Valid", zTR.iID, lRecNo, sRecType, 7, 0, 0, "UPDATE HOLD2", FALSE));
			}

			zErr = ProcessTransferInHoldings(zTR, zPR, zPR.sBaseCurrId, zTranType.lSecImpact, zTranType.lCashImpact, 
											 zTranType.iTradeDateInd, bReversal, sCurPrior);
			if (zErr.iBusinessError == 0 && zErr.iSqlError == 0 && zTranType.lCashImpact != 0)
				zErr = ProcessTransferInHoldcash(zTR, zPR, zTranType.lCashImpact, zTranType.iTradeDateInd, bReversal, sCurPrior);
		}   
		else
		{
			zErr = PrintError("Invalid TranCode", zTR.iID, lRecNo, sRecType, 103, 0, 0, "UPDATE HOLD3", FALSE);
			if (bDoTransaction) // in case of an error, rollback transaction
				lpfnRollbackTransaction();
			return zErr;
		} // invalid trancode
		...
		** use post date (which should be equal to Starsdat.TradeDate - this is important 
		** for trans import app when run for last business day of the month 
		** calendar eom falls on weekend, e.g. 9/29/06 file gets processed on 9/30/06 
		...
		*/
		if (zErr.iBusinessError == 0 && zErr.iSqlError == 0 && sCurPrior[0] == 'C')
		{
		...
		*/
		if (zErr.iBusinessError == 0 && zErr.iSqlError == 0 && sCurPrior[0] == 'C')
		{
			...
			{
				zErr = PrintError("Error updating PORTMAIN Record", zTR.iID, lRecNo, sRecType, 0, 
											zErr.iSqlError, zErr.iIsamCode, "UPDATE HOLD4", FALSE);
				if (bDoTransaction) // in case of an error, rollback transaction
					lpfnRollbackTransaction();
				return zErr;
			}
		...
		** portmain tables, the final step is to update the values on the portbal table
		*/
		...
		// depending on whether there was an error in any of the previous steps or not
	} //try
...
**/
ERRSTRUCT ProcessCash(TRANS zTR, long lCashImpact, BOOL bReversal, char *sCurPrior, BOOL bIncByLot)
{
  ERRSTRUCT zErr;
  long      lRecNo;
  char      sRecType[2];
  ...
  */
  if (lCashImpact == 0 || lCashImpact == 2) 
    ...
    return(PrintError("Invalid Cash Impact", zTR.iID, lRecNo, sRecType, 48, 0, 0, "UPDH PROCESS CASH1", FALSE));


	/*
	** SB 10/3/2012 - Opening part (cash impact = 1) for TC/DR should use sec exrate and all other transactions should use 
	**								base exrate. The decision regarding whether to use base exrate or sec exrate was being made later in 
	**								the function. In case of reversal of TC/DR, it was too late becuase that decision was being made
	**								after multiplying cash impact by -1. So, reversal of TC/DR used to end up using base exrate rather
	**								than sec exrate, which was messing up the exchange rate on the holdcash record. So, make and save
	**								the decision about which exchange rate to use before modifying cash impact in any way.
	*/
	if (!bReversal && strcmp(zTR.sTranType, "TC") == 0 && strcmp(zTR.sDrCr, "DR") == 0 && lCashImpact == 1)
		bUseSecXRate = TRUE;
	else if (bReversal && strcmp(zTR.sRevType, "TC") == 0 && strcmp(zTR.sDrCr, "DR") == 0 && lCashImpact == 1)
		bUseSecXRate = TRUE;
	else
		bUseSecXRate = FALSE;
...
//	lpprTimer(40);
  if (zErr.iSqlError)
    return(PrintError("Error Reading CURRENCY", zTR.iID, lRecNo, sRecType, 0, 
											zErr.iSqlError, zErr.iIsamCode, "UPDH PROCESS CASH2", FALSE));

	// SB - Changed on 10/19/98 so that secxtend gets copied with "RP" if it is not IN
  if (strcmp(zTR.sSecXtend, "IN") != 0)
    strcpy_s(zTR.sSecXtend, "RP"); 
  /* Process the principal portion of the trade */
	/* If TC DR trade use security exchange rate if it is the foreign currency (cashimpart = 1) */
	if (bUseSecXRate)
		zErr = ProcessMoneyInHoldings(zTR, lCashImpact, zTR.fPcplAmt, zTR.fSecBaseXrate, 
																	zTR.fSysXrate, sCurPrior, bReversal, bIncByLot);
	else
    zErr = ProcessMoneyInHoldings(zTR, lCashImpact, zTR.fPcplAmt, zTR.fBaseXrate, 
																	zTR.fSysXrate, sCurPrior, bReversal, bIncByLot);
  if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
    return zErr;
  ...
  */
  if (!IsValueZero(zTR.fIncomeAmt, 2))
  {
    ...
    {
 		  lpprCurrencySecno(zTR.sSecNo, zTR.sWi, &zTR.iSecID, zTR.sIncCurrId, &zErr);
	    if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
        return(PrintError("Error Reading CURRENCY", zTR.iID, lRecNo, sRecType,  0, 
													zErr.iSqlError, zErr.iIsamCode, "UPDH PROCESS CASH3", FALSE));
    } /* If CurrId != IncCurrId */
	
		strcpy_s(zTR.sCurrAcctType, zTR.sIncAcctType);
		strcpy_s(zTR.sSecXtend, "IN"); // force sec_xtend to be IN for income amout
		if (bUseSecXRate)
		  zErr = ProcessMoneyInHoldings(zTR, lCashImpact, zTR.fIncomeAmt, zTR.fSecBaseXrate, 
																		zTR.fIncSysXrate, sCurPrior, bReversal, bIncByLot);
	  else
		  zErr = ProcessMoneyInHoldings(zTR, lCashImpact, zTR.fIncomeAmt, zTR.fIncBaseXrate, 
																		zTR.fIncSysXrate, sCurPrior, bReversal, bIncByLot);
...
**/
ERRSTRUCT ProcessMoneyInHoldings(TRANS zTR, long lCashImpact, double fCashAdj, double fBaseXrate, double fSysXrate, 
																 char *sCurPrior, BOOL bReversal, BOOL bIncByLot)
{
  BOOL      bRecFound;
  ERRSTRUCT zErr;
  HOLDCASH  zHCR;
...
//	lpprTimer(41);
	lpprSelectHoldcash(&zHCR, zTR.iID, zTR.sSecNo, zTR.sWi, zTR.sSecXtend, zTR.sCurrAcctType, &zErr);
//	lpprTimer(42);
  ...
  CalculateNewCostXRate(zHCR.fTotCost, fCashAdj, zHCR.fBaseCostXrate, fBaseXrate, &zHCR.fBaseCostXrate);
  CalculateNewCostXRate(zHCR.fTotCost, fCashAdj, zHCR.fSysCostXrate, fSysXrate, &zHCR.fSysCostXrate);
  ...
  */
  ...
  CalculateCurrencyGainLoss(zHCR.fTotCost, zHCR.fBaseCostXrate, zHCR.fMktVal, zHCR.fMvBaseXrate, &zHCR.fCurrencyGl);

  /* if record already exist, update it, else add a new record */
  if (bRecFound)
  { 
		if (IsValueZero(zHCR.fUnits, 2))
			lpprDeleteHoldcash(zHCR.iID, zHCR.sSecNo, zHCR.sWi, zHCR.sSecXtend, zHCR.sAcctType, &zErr);
		else {

			lpprUpdateHoldcash(zHCR, &zErr);
			if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
				return(PrintError("Error Updating Cash CURRENCY. Please contact Client Support for an immediate fix.", zTR.iID, 0, "C", 138,
					zErr.iSqlError, zErr.iIsamCode, "UPDC PROCESS CASH5", FALSE));
  ...
  // so, we have to update divhist/accdiv properly as it is done for Income trans
  // Code below was borrowed from ProcessIncome and just tries to update divhist/accdiv 
  zErr = UpdateDivhistAccdiv(zTR, bIncByLot, bReversal, sCurPrior, "M");
...
** the divhist table.
**/
ERRSTRUCT ProcessIncome(TRANS zTR, BOOL bIncByLot, char *sCurPrior, BOOL bReversal)
{
  HOLDINGS  zHR;
  PAYREC    zPY;
  ...
  */
  ...
  if (zTR.lTaxlotNo != 0)
  {
	  ...
	  lpprSelectHoldings(&zHR, zTR.iID, zTR.sSecNo, zTR.sWi, zTR.sSecXtend, zTR.sAcctType, zTR.lTaxlotNo, &zErr);
	  if (zErr.iSqlError == SQLNOTFOUND)
		{
			zErr.iSqlError = 0;
	   	bRecFound = FALSE;
  ...
  */ 
  zErr = UpdateDivhistAccdiv(zTR, bIncByLot, bReversal, sCurPrior, "I");
  if (zErr.iSqlError != 0 && zErr.iSqlError != SQLNOTFOUND)
     return zErr;
...
**/
ERRSTRUCT ProcessOpeningInHoldings(TRANS zTR, PORTMAIN zPR, long lSecImpact, long lCashImpact,
																	 int iDateInd, BOOL bReversal, char *sCurPrior)
{
  BOOL      bRecFound;
  ERRSTRUCT zErr;
...
//	lpprTimer(43);
  zErr = GetSecurityCharacteristics(zTR.sSecNo, zTR.sWi, sPrimaryType, sSecondaryType, sPositionInd, sLotInd, 
...
//	lpprTimer(44);
  ...
  */
  ...
  {  
		InitializeHoldcashStruct(&zHCR);
//		lpprTimer(45);
	  lpprSelectHoldcash(&zHCR, zTR.iID, zTR.sSecNo, zTR.sWi, zTR.sSecXtend, zTR.sAcctType, &zErr);
		//lpprTimer(46);
	  if (zErr.iSqlError == SQLNOTFOUND)
		{
	    bRecFound = FALSE;
...
//		lpprTimer(45);
  ...
  */
  if (bReversal && bRecFound == FALSE && sLotExistsInd[0] == 'Y')
    return(PrintError("Incorrect Or Missing Taxlot Number", zTR.iID,
                      lRecNo, sRecType, 101, 0,0,"UPDH PROCESSOPEN1", FALSE));
  ...
  */
  if (strcmp(sLotInd,"S") == 0 && bRecFound)
  {
    ...
    */
    ...
    { 
      if (sPositionInd[0] == 'C')
				lpprDeleteHoldcash(zHR.iID, zHR.sSecNo, zHR.sWi, zHR.sSecXtend, zHR.sAcctType, &zErr);
      else
				lpprDeleteHoldings(zHR.iID, zHR.sSecNo, zHR.sWi, zHR.sSecXtend,zHR.sAcctType, zHR.lTransNo, &zErr);
    ...
    */
    if (sPositionInd[0] == 'C')
    {
      CopyFieldsFromHoldingsToHoldcash(&zHCR, zHR);
			...
			else // unlikely case of money market units were -ve before the transaction and this buy is making it zero
			{
				lpprDeleteHoldcash(zHR.iID, zHR.sSecNo, zHR.sWi, zHR.sSecXtend, zHR.sAcctType, &zErr);
				return zErr;
			}
  ...
  */
  ...
  {
    zErr = ProcessAverageInHoldings(&zTR, fTradingUnits, &fSysXrate, bReversal, sCostInd);
    if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
      return zErr;   

		zHR.fUnitCost = zTR.fUnitCost; // SB 10/19/99 Changed so that all the lots get new unit_cost
    zHR.fTotCost = zTR.fTotCost;
    zHR.fOrigCost = zTR.fOrigCost;
  ...
  */
  if (bReversal && bRecFound) 
	{
	  ...
	  if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
		...
		if (sCurPrior[0] == 'C')
		{
			// delete accruing (pending, i.e. not processed yet) divhist and accdiv also
			lpprDeleteAccruingDivhistOneLot(zTR.iID, zTR.sSecNo, zTR.sWi, zTR.sSecXtend,
																			zTR.sAcctType, zTR.lTaxlotNo, &zErr);
			if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
				return zErr;

			lpprDeleteAccruingAccdivOneLot(zTR.iID, zTR.sSecNo, zTR.sWi, zTR.sSecXtend,
																		 zTR.sAcctType, zTR.lTaxlotNo, &zErr);
			if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
				return zErr;
  ...
  else
  {
     ...
     */
		...
		// For a bonds, eligibility date for payments should be the settlement date, unless the 
		// settmenet date is less than the effective date (e.g. in case of FR or TS transactions),
		// in which case it should be effective date. For other type of security, eligibility date
		// should be effective date.
...
**/
ERRSTRUCT ProcessClosingInHoldings(TRANS zTR, char *sBaseCurr, long lSecImpact, BOOL  bReversal, char *sCurPrior)
{
  ...
  */
  ...
  if (bRecFound && sCurPrior[0] == 'C' && sLotInd[0] != 'S')
  {
    ...
    */
    if (bReversal)
    {
      lpprHolddelUpdate(zTR.iID, zTR.lTransNo, zTR.lRevTransNo, zTR.lEffDate, &zErr);     
      if (zErr.iSqlError)    // Send A Warning Message 
        PrintError("WARNING !!! Holddel Record Not Found-Potential Problms With Dividend Payments", 
				           zTR.iID, lRecNo, sRecType, 1001, 0, 0, "UPDH PROCESSCLOSE1", TRUE);
    } /* Reversal */
  ...
  */                         
  if (bReversal)
  {
		...
		{
		  lpprSelectHolddel(&zHD, zTR.lRevTransNo, zTR.lEffDate, zTR.iID, zTR.sSecNo, zTR.sWi, zTR.sSecXtend, zTR.sAcctType, zTR.lTaxlotNo, &zErr);
			if (zErr.iSqlError == 0)
			{
				/*
				** SB - 7/17/00 There is a unique situtaion in which the holddel record might
				** not be right. e.g. consider following trnsaction:
				**     1  PS  1500 units on 10/1/1999		resulting in 1500 units on holdings
				**     2  SL   600 units on 7/7/2000		resulting in 900 units on holdings
				**     3  SP   450 units on 6/15/2000		resulting in 1350 units on holdings
				**  As of SL   500 units on 7/2/2000		resulting in 850 units on holdings
				** When asof SL happens, trans # 2(SL of 600units) will be reversed and rebooked. When
				** reversing trans # 2, the Holddel will have 1500 units which is wrong, it should have
				** 1350 + 600 = 1950 units. In situation like that, the existing holddel should not be used.
				*/
				if (RoundDouble(zHR.fUnits + (zTR.fUnits * lSecImpact) - zHD.fUnits, 3) != 0)
					zErr.iSqlError = SQLNOTFOUND;
			} // if holddel found
    ...
    ** was before, so copy all the information from trans record and generate a warning.
    */
		if (zErr.iSqlError == SQLNOTFOUND)
		{
			bRecFound1 = FALSE;
			...
			{
				CopyFieldsFromTransToHoldings(&zHR, zTR);
				zHR.fUnits = zHR.fTotCost = zHR.fOrigCost = zHR.fOrigFace = zHR.fMktVal = 0;
			...
			if (sCurPrior[0] == 'C')
				PrintError("WARNING !!! Holddel Record Not Found-Potential Problms With Lot", 
		...
		/*
		** SB 6/22/00 - If it is a reversal, it is necessary to update the rev_trans_no on the original
		** holddel record. If the lot still exists (original closing transaction was a partial sell),
		** then the holddel will be updated above (inside the condition "bRecFound && sCurPrior[0] == 'C' && sLotInd[0] != 'S'"),
		** but if the lot was completely sold, holddel wil not get updated. So do that here.
		*/
  ...
  */
  if (bReversal == FALSE || (bReversal && bRecFound1 == FALSE))
  {
    ...
    */
    ...
    // SB - 1/10/1999 Recalculate the market value of the security after principal amount has been multipled by sec impact 
    ...
    if (sCostInd[0] == 'C')
    {
	  ...
	  if (zHR.fOrigFace + zTR.fOrigFace < 0)
		...
		// SB 4/22/2008 - For liability security, use total cost on transaction rather than calculating it based on open unit cost
		// since open unit cost for liability securities is zero.
		// SB 9/9/09 - For Liability, cost should be zero, liabilities don't have cost they have only liabilities, furthermore
		// use original cost from the transaction, not the total cost. Original cost on transaction has open liability
		// amount and it has current proceeds on total cost.
    else if (sCostInd[0] == 'L')
		{
			...
			return(PrintError("Invalid Cost Indicator", zTR.iID, lRecNo, 
                        sRecType, 100, 0, 0, "UPDH PROCESSCLOSE4", FALSE));
  ...
  */
  ...
  {  
    if (bRecFound == FALSE)
      return(PrintError("Invalid Or Missing Taxlot Number", zTR.iID, 
                        lRecNo, sRecType, 100, 0, 0, "UPDH PROCESSCLOSE5", FALSE));
  ...
  */
  ...
  zErr.iBusinessError = lpfnCalcGainLoss(zTR, "C", lSecImpact, sPrimaryType, sSecondaryType, sBaseCurr, 
																				 &fCGL, &fSGL, &fSTGL, &fMTGL, &fLTGL, &fTotGL);
...
**/
ERRSTRUCT ProcessSplitInHoldings(TRANS zTR, long lSecImpact, char *sAutoGen, BOOL bReversal, char *sCurPrior)
{
	BOOL      bRecFound, bRecFound1;
	ERRSTRUCT zErr;
	...
	*/                         
	if (bReversal)
	{
		...
		if (sCurPrior[0] != 'C')
			...
			lpprSelectHolddel(&zHD, zTR.lRevTransNo, zTR.lEffDate, zTR.iID, zTR.sSecNo, 
							  zTR.sWi, zTR.sSecXtend, zTR.sAcctType, zTR.lTaxlotNo, &zErr);
		if (zErr.iSqlError == SQLNOTFOUND)
		{
			bRecFound1= FALSE;
			zErr.iSqlError = 0;
	...
	if (lSecImpact != 1 && lSecImpact != -1)
		return(PrintError("Invalid Security Impact", zTR.iID, lRecNo, sRecType, 
			              47, 0, 0, "UPDH PROCESSSPLIT", FALSE));

	if (bReversal == FALSE || (bReversal && bRecFound1 == FALSE))
	{
		zErr = GetSecurityCharacteristics(zTR.sSecNo, zTR.sWi, sPrimaryType, sSecondaryType, sPositionInd, 
	...
	/* 
	** VI # 58465 If the units have been reduced to zero (can happen if transaction was created with wrong units), then delete 
	...
	*/
	...
	{
		if (bReversal == FALSE)
			lpprUpdateDivhistOneLot(zTR.sTranType, "T", zTR.lTransNo, zTR.iID,
									zTR.lTaxlotNo, zTR.lDivintNo, zTR.sTranType, &zErr);
		else
		{	
			lpprDeleteDivhistOneLot(zTR.iID, zTR.lTaxlotNo, zTR.lDivintNo, zTR.lRevTransNo, zTR.sRevType, &zErr);
			if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
				return zErr;
...
**/
ERRSTRUCT ProcessAdjustmentInHoldings(TRANS *pzTR, PORTMAIN zPR, long lSecImpact, long lCashImpact, BOOL bReversal, char *sCurPrior)
{
  ...
  /*
	** SB 10/17/2000 Untile now the TRANS record was being passed by value and this function was
	** temporarily changing some of the values (for convenience) which SHOULD NOT get reflected
	** in the calling functions. Now, in case of BA trades, we want to fill total cost and the 
	** original cost in the transaction, so TRANS is being passed as a pointer. So, make a copy 
	** of the transaction and make changes in the original and use the copy for temporary record.
	*/
	zTR = *pzTR;

  // The only adjustment transactions which affect holding File are SK, UC, AM, LD, SG ,PO, BA, RP 
  InitializeErrStruct(&zErr);
  ...
  // If the transaction is a reversal, use the reversal transaction type to identify the original trade
  if (bReversal)
  ...
  /* 
  ** Set/unset safekeeping, update cost, amortization, liquidating dividends set/unset gain/loss, pair offs, 
  ** basis adjustment, return of principal, set/unset restrictions, update date, update broker, 
	** purchase/sale of basis transactions are the only valid transactions for this function
  */
  ...
  // If the transction is an 'LD', 'BA', 'AM', 'RP', 'PZ', 'SZ' modify the pcpl amt by the security impact 
  if (strcmp(sTCode, "BA") == 0 || strcmp(sTCode, "LD") == 0 || strcmp(sTCode, "AM") == 0)
    zTR.fPcplAmt *= lSecImpact; 
	...
	{
    zTR.fPcplAmt   *= lSecImpact; 
		zTR.fTotCost = zTR.fOrigCost = zTR.fPcplAmt; // for purchase/sale of basis cost has to be same as principal amount
  ...
  if (bReversal)
  {
		zTR.fPcplAmt *= -1;
    zTR.fUnits *= -1;
		if (strcmp(sTCode, "RP") == 0 || strcmp(sTCode, "PZ") == 0 || strcmp(sTCode, "SZ") == 0)
		{
			zTR.fOrigCost *= -1;
			zTR.fTotCost *= -1;
	...
	// Update broker effects transaction only, it does not effect holdings/holdcash
	if (strcmp(sTCode, "UB") == 0)
	{
		...
		if (zErr.iSqlError == SQLNOTFOUND)
       return(PrintError("Invalid Transaction Number", zTR.iID, zTR.lOrigTransNo, "T", 50, 0, 0, "UPDH PROCESSADJUST1", FALSE));
      ...
      return(PrintError("Invalid Transaction Number. Transaction Has Been Reversed", zTR.iID, zTR.lOrigTransNo, "T", 50, 0, 0, "UPDH PROCESSADJUST2", FALSE));

		/*
		** In case of a regular(non-reversal) trade, before updating the broker code, store the old
		** value in broker code2 column (both on effected transaction and UB transaction). In case
		** of reversal use the value from broker code2 to restore the old value of broker code.
		*/
		if (bReversal) 
  ...
  ** primary currency, secondary currency, trading unit, postion file to update, etc
  */
  zErr = GetSecurityCharacteristics(zTR.sSecNo, zTR.sWi, sPrimaryType, sSecondaryType, sPositionInd, 
																		sLotInd, sCostInd, sLotExistsInd, sAvgInd, &fTradingUnits, sCurrId);
  if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
  ...
  */
  if (!bRecFound)
  {
    if (strcmp(sTCode, "LD") == 0 || 
       (bReversal && (strcmp(sTCode, "SR") == 0 || 
										  strcmp(sTCode, "UR") == 0 || 
											strcmp(sTCode, "UD") == 0)))
      return zErr;
    else
      return(PrintError("Invalid Or Missing Taxlot Number", zTR.iID,
                        lRecNo, sRecType, 51, 0, 0, "UPDH PROCESSADJUST3", FALSE));
  } /* Rec Not Found */
  ...
  if (sCurPrior[0] == 'C')
  {
    ...
    */
    if (bReversal)
    {
      ...
      if (zErr.iSqlError)
         PrintError("WARNING !!!! Holddel Record Not Found-Potential Problms With Dividend Payments", 
										zTR.iID, lRecNo, sRecType, 1001, 0, 0, "UPDH PROCESSADJUST4", TRUE);
    } // Reversal 
  } // Current 

  /* 
  ** If the transaction is a cancel of a adjustment transaction format the holdings record 
	** using the holddel record. First move the current market values, accrual and all relevant 
	** exrates to temp. variables, this is necessary to correctly reset the values on the lot.
  */                         
  if (bReversal)
  {
  ...
  if (bReversal == FALSE || (bReversal && bRecFound2 == FALSE))
  {
    ...
    {
			// SB 10/17/2000 - don't care if the old cost was zero or not, apply the adjustments anyway
			/*
			** SB 6/15/2002 - Until now the test was if BalToAdjust == "ORIG" which was
			** not being used (no calling program was populating it), now changed it b
			** if BalToAdjust == "CURR", fron end will now have to options with basis adjustment
			** transactions, adjust both current and origianl cost (only option until now) or
    ...
    {
			/* SB 9/18/98 Changed it to reflect the changes in N&B code after the original code was downloaded 
			zHR.fTotCost += zTR.fPcplAmt;
      zHR.fOrigCost += zTR.fPcplAmt;
      ...
      zHR.fCollateralUnits += zTR.fUnits;
		else if (strcmp(sTCode, "RP") == 0 || strcmp(sTCode, "PZ") == 0 || strcmp(sTCode, "SZ") == 0) // return of principal, purchase/sale of basis
		{
			zHR.fTotCost += zTR.fTotCost;
			zHR.fOrigCost += zTR.fOrigCost;
  ...
  {
		lpprSelectHedgxref(&zHG, zTR.iID, zTR.sSecNo, zTR.sWi, zTR.sSecXtend, zTR.sAcctType, zTR.lTaxlotNo, &zErr);
    if (zErr.iSqlError == SQLNOTFOUND)
		{
			bRecFound1 = FALSE;
      CopyFieldsFromTransToHedgeXref(&zHG, zTR);
		...
		if (bRecFound1)
		{
			if (zHG.fHedgeUnits == 0)
				lpprDeleteHedgxref(zHG.iID, zHG.sSecNo, zHG.sWi, zHG.sSecXtend, zHG.sAcctType, zHG.lTransNo, &zErr);
			else
  ...
  */
  ...
  if ((strcmp(sTCode, "AM") == 0 || strcmp(sTCode, "UC") == 0 ||
       strcmp(sTCode, "LD") == 0 || strcmp(sTCode, "BA") == 0 || 
			 strcmp(sTCode, "RP") == 0 || strcmp(sTCode, "PZ") == 0 || strcmp(sTCode, "SZ") == 0) &&
...
**/
ERRSTRUCT ProcessTransferInHoldings(TRANS zTR, PORTMAIN zPR, char *sBaseCurr, long lSecImpact, long lCashImpact,
                                    int iDateInd, BOOL bReversal, char *sCurPrior)
{
  ERRSTRUCT zErr;
...
/*  if (strcmp(zTR.sDrCr, "DR") == 0)
	...
	/*
	** SB - Changed the logic of secimpact on 12/31/1999 - We are using "DrCr" to determine 
	** whether it is "UP', "PL' or "RP", instead on whether it is short/long. So closing will
	** have security impact -1. Transfer of Short positions will be handled later on.
	*/
	lSecImpact = -1;
  zErr = ProcessClosingInHoldings(zTR, zPR.sBaseCurrId, lSecImpact, bReversal, sCurPrior);
  if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
    return zErr;
  ...
  */
  ...
  return (ProcessOpeningInHoldings(zTR, zPR, lSecImpact, lCashImpact, iDateInd, bReversal, sCurPrior));
																		...
																		int iDateInd, BOOL bReversal, char *sCurPrior)
{
  ...
  if (lCashImpact !=2)
    return(PrintError("Invalid Cash Impact", zTR.iID, lRecNo, sRecType,
                      48, 0, 0, "UPDH PROCESSXFERINHCASH", FALSE));
...
**/
ERRSTRUCT ProcessAverageInHoldings(TRANS *pzTR, double fTradingUnits, double *pfSysXrate, BOOL bReversal,
																	 char *sCostInd)
{
  char      sRecType[2];
  long      lRecNo;
  double    fTotalUnits, fTotalTotCost, fTotalOrigCost, fUnitOrigCost, fUnitTotCost, fTotalBaseTotCost;
	double		fTotalSysTotCost, fUnitLiability, fTotalOpenLiability, fTotalBaseOLiability, fTotalSysOLiability;
  ...
  else
  {
	...
	fTotalUnits = fTotalTotCost = fTotalOrigCost = fUnitOrigCost = fUnitTotCost = fUnitLiability = 0;
	fTotalBaseTotCost = fTotalSysTotCost = fTotalOpenLiability = fTotalBaseOLiability = fTotalSysOLiability = 0;
	fNewBaseXrate = fNewSysXrate = 1.0;

  lpprHsumSelect(&fTotalUnits, &fTotalTotCost, &fTotalOrigCost, &fTotalBaseTotCost, &fTotalSysTotCost, 
								 &fTotalOpenLiability, &fTotalBaseOLiability, &fTotalSysOLiability, 
								 pzTR->iID, pzTR->sSecNo, pzTR->sWi, pzTR->sSecXtend, pzTR->sAcctType, 0, &zErr);
  if ((zErr.iSqlError != SQLNOTFOUND && zErr.iSqlError != 0) || zErr.iBusinessError != 0)
    return(PrintError("Error Fetching Sum From HOLDINGS", pzTR->iID, lRecNo, sRecType, 0, 
											zErr.iSqlError, zErr.iIsamCode, "UPDH FETCH HSUM", FALSE));

  // Adjust the summarized values by the transaction values 
  fTotalUnits += pzTR->fUnits;

	// SB 1/17/08 - If this is a liability security then open liability needs to be averaged, 
	// if not then cost needs to be averaged
	if (sCostInd[0] == 'L')
	{
		...
		*/
		...
		if (!IsValueZero(fTotalUnits, 5) && !IsValueZero(fTradingUnits, 3))
			fUnitLiability	= RoundDouble((fTotalOpenLiability / fTotalUnits) / fTradingUnits, 10); 
		...
		{
			// Liability(and hence xrate) becomes zero in case of Reversal of a single lot. 
			// In this case the lot will be deleted any way, no need to give this warning
			if (!bReversal) 
				PrintError("WARNING !!! Defaulting Base Exrate To 1", pzTR->iID, lRecNo, sRecType, 68, 0, 0, "UPDH AVERAGE1A", TRUE);
			fNewBaseXrate = 1;
		}
		...
		{
			// Liability(and hence xrate) becomes zero in case of Reversal of a single lot. In this case the lot will be deleted any way, no need to give this warning		
			if (!bReversal) 
				PrintError("WARNING !!! Defaulting Sys Exrate To 1", pzTR->iID, lRecNo, sRecType,
					         68, 0, 0, "UPDH AVERAGE2A", TRUE);
			fNewSysXrate = 1;
		}
	...
	else
	{
		...
		/*
		** if orig cost is zero, add total cost - SB 7/10/02 Changed that logic, for BA transactions 
	...
	//  else
		...
		/* 
		** Calculate the total cost and original cost in terms of the base exchange rate and the system's exchange rate and 
	...
	//  else
	  ...
	  pzTR->fOrigCost = pzTR->fUnits * fUnitOrigCost * fTradingUnits;
		...
		if (IsValueZero(fNewBaseXrate, 12))
		{
			// Cost(and hence xrate) becomes zero in case of Reversal of a single lot. In this case the lot will be deleted any way, no need to give this warning
			if (!bReversal) 
				PrintError("WARNING !!! Defaulting Base Exrate To 1", pzTR->iID, lRecNo, sRecType, 68, 0, 0, "UPDH AVERAGE1B", TRUE);
			fNewBaseXrate = 1;
		}
  ...
  // update unit_cost, tot_cost and orig_cost in all the previous lots of the same security
  lpprHsumUpdate(fUnitTotCost, fTradingUnits, fUnitOrigCost, fUnitLiability, fNewBaseXrate, 
								 fNewSysXrate, pzTR->sTranType, pzTR->lTaxlotNo, pzTR->sTransSrce, 
								 pzTR->iID, pzTR->sSecNo, pzTR->sWi, pzTR->sSecXtend, pzTR->sAcctType, 0, &zErr);
	if (zErr.iSqlError)
    return(PrintError("Error Updating Records In HOLDINGS", pzTR->iID, lRecNo, sRecType, 0, 
											zErr.iSqlError, zErr.iIsamCode, "UPDH HSUM UPDATE", FALSE));
  ...
  if (zTR.lDivintNo != 0 && sCurPrior[0] == 'C')
  {
    if (bReversal) 
      ...
      strcpy_s(sType, zTR.sTranType); 
		...
		{
   	   if (strcmp(zTR.sTranType, "WH") == 0 || strcmp(zTR.sTranType, "AR") == 0 ||
           strcmp(zTR.sTranType, "RR") ==  0)  
         strcpy_s(sType, zTR.sTranType);
       else                                
         strcpy_s(sType, "AD");
		}
		...
		{
      if (bIncByLot)
				lpprUpdateDivhistOneLot(zTR.sTranType, "T", zTR.lTransNo, zTR.iID, zTR.lTaxlotNo, zTR.lDivintNo, sType, &zErr);
			else
				lpprUpdateDivhistAllLots(zTR.sTranType, "T", zTR.lTransNo, zTR.iID, zTR.lDivintNo, sType, &zErr);
    ...
    {
			if (bIncByLot)
				lpprDeleteDivhistOneLot(zTR.iID, zTR.lTaxlotNo, zTR.lDivintNo, zTR.lRevTransNo, sType, &zErr);
			else
				lpprDeleteDivhistAllLots(zTR.iID, zTR.lDivintNo, zTR.lRevTransNo, sType, &zErr);
			if (zErr.iSqlError != 0 && zErr.iSqlError != SQLNOTFOUND)
				return zErr;

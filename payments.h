...
// 2/11/2010 - VI#43627 Modified logic to create PIs for those accounts,
securities that apply  - mk
**/
...
typedef void(CALLBACK *LPPRDIVINTUNL)(DILIBSTRUCT *, long, long, const char *,
                                      ...
                                      long, ERRSTRUCT *);
typedef void(CALLBACK *LPPRMATURITYUNLOAD)(MATSTRUCT *, long, long,
...
typedef void(CALLBACK *LPFNAMUNLSTATEMENT)(const char *, const char *,
                                           const char *, const char *,
                                           const char *, const char *, long,
                                           AMORTSTRUCT *, ERRSTRUCT *);
typedef void(CALLBACK *LPFNSELECTBSCHDDATEPRICE)(long *, double *, const char *,
                                                 const char *, ERRSTRUCT *);
typedef void(CALLBACK *LPFNSELECTMINBSCHDDATEPRICE)(long *, double *,
                                                    const char *, const char *,
                                                    long, ERRSTRUCT *);
typedef void(CALLBACK *LPFN1PCHAR1LONG1PCHAR)(char *, long, char *,
                                              ERRSTRUCT *);

// valuationIO  function types
typedef void(CALLBACK *LPPRSELECTNEXTCALLDATEPRICE)(const char *, const char *,
                                                    long, double *, long *,
                                                    ERRSTRUCT *);

// amortize IO function types
typedef void(CALLBACK *LPPRAMORTUNLOAD)(const char *, int, const char *,
                                        const char *, const char *,
                                        const char *, long, AMORTSTRUCT *,
                                        ERRSTRUCT *);
typedef void(CALLBACK *LPPRUPDATEAMORTIZEHOLDINGS)(HOLDINGS, ERRSTRUCT *);
typedef void(CALLBACK *LPPRFORWARDMATURITYUNLOAD)(FMATSTRUCT *, long, long,
                                                  const char *, int,
                                                  const char *, const char *,
                                                  const char *, const char *,
                                                  ERRSTRUCT *);
...
typedef void(CALLBACK *LPPRPITIPSUNLOAD)(const char *, long, int, const char *,
                                         const char *, const char *,
                                         const char *, long, PITIPSSTRUCT *,
                                         ERRSTRUCT *);

// these are Delphi dll in DelphiCInterface
typedef double(CALLBACK *LPPRCALCULATEYIELD)(double, const char *, const char *,
                                             const char *, long, int);
typedef double(CALLBACK *LPPRCALCULATEPRICEGIVENYIELD)(double, const char *,
                                                       const char *,
                                                       const char *, long, int);
typedef void(CALLBACK *LPPRGETPORTFOLIORANGE)(int *, int *, int, int *,
                                              ERRSTRUCT *);
...
ERRSTRUCT BuildAmortTable(const char *sMode, const char *sFileName, int iID,
                          const char *sSecNo, const char *sWi,
                          const char *sSecXtend, const char *sAcctType,
                          long lTransNo, AMORTTABLE *pzDITable);
...
ERRSTRUCT CreateAccdivRecord(DILIBSTRUCT zDILS, long lValDate,
                             SUBACCTTABLE zSTable, double fBaseCurExrate,
...
ERRSTRUCT CalcSplitFactor(double fSplitRate, double *pzQuotient,
                          double *pzProduct);
...
ERRSTRUCT GetPortmainInfo(PORTTABLE zPortTable, int iID, long lTrdDate,
                          double *pzPortBaseXrate, BOOL *bIncByLot,
                          double *pzSwh, double *pzBwh);
ERRSTRUCT CreateTransFromAccdiv(TRANS *pzTR, ACCDIV zAccdiv, CURRTABLE zCTable,
                                double zPortBaseXrate, double zExRate,
                                double zIncExRate, char *sPrimaryType,
                                long lValDate);

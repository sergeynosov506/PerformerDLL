...
// ----------------------------------------------------------
static int MapDbError(const nanodbc::database_error& e, ERRSTRUCT* perr, const char* where)
{
    if (perr) InitializeErrStruct(perr);
    ...
    if (perr) {
        ...
        perr->iIsamCode = 0;
        if (where && *where) strncpy_s(perr->sRecType, where, _TRUNCATE);
...
HINSTANCE LoadLibrarySafe(LPCTSTR libFileName)
{
    ...
    if (!hInst)
    {
        ...
        PrintError(msg, 0, 0, (char*)"", 0, GetLastError(), 0, (char*)"LoadLibrarySafe", FALSE);
    }
    return hInst;
}
...
// ----------------------------------------------------------
// ---- helper: adapt legacy OLE DB conn string to ODBC Driver 18 ----
static std::string AdaptToODBC(std::string oleStr)
{
    ...
    if (p != std::string::npos) {
        ...
        oleStr.erase(p, (endProv == std::string::npos ? oleStr.size() : endProv + 1) - p);
    }
    ...
    auto pick = [&](const char* key) -> std::string {
        ...
        return oleStr.substr(pos, (end == std::string::npos ? oleStr.size() : end) - pos);
        };

    // Try common OLE DB keys
    std::string server = pick("Data Source=");
    if (server.empty()) server = pick("Server="); // some apps already pass ODBC-ish
    std::string db = pick("Initial Catalog=");
    if (db.empty())      db = pick("Database=");
    std::string uid = pick("User ID=");
    if (uid.empty())     uid = pick("UID=");
    std::string pwd = pick("Password=");
    if (pwd.empty())     pwd = pick("PWD=");
    ...
    if (!dsn.empty()) {
        odbc = "DSN=" + dsn + ";";
        if (!db.empty())  odbc += "Database=" + db + ";";
        if (!uid.empty()) odbc += "UID=" + uid + ";";
        if (!pwd.empty()) odbc += "PWD=" + pwd + ";";
    }
    else {
        odbc = "Driver={ODBC Driver 18 for SQL Server};";
        if (!server.empty()) odbc += "Server=" + server + ";";
        if (!db.empty())     odbc += "Database=" + db + ";";
        if (!uid.empty())    odbc += "UID=" + uid + ";";
        if (!pwd.empty())    odbc += "PWD=" + pwd + ";";
    }
    ...
    // If you use trusted certs in prod, switch to Encrypt=yes;TrustServerCertificate=no;
...
// If your nanodbc connection is defined in another translation unit, keep this extern.
// Otherwise, if it's here, remove extern and make it a real definition.
...
{
#ifdef DEBUG
    PrintError(const_cast<char*>("Entering"), 0, 0, const_cast<char*>(""),
        0, 0, 0, const_cast<char*>("InitializeOLEDBIO"), FALSE);
#endif
    ...
    if (!gInitialized)
    {
        // --- Build connection string via Login.DLL (legacy path), then adapt to ODBC ---
        ...
        try {
            if (sConnectStr[0]) {
                ...
                if (sODBCDSN[0]) {
                    ...
                    if (sSQLDBName[0]) odbcStr += std::string("Database=") + sSQLDBName + ";";
                    // UID/PWD may come via Login.DLL below; we keep the connection open now.
                    gConn = nanodbc::connection(odbcStr);
                }
                else {
                    pzErr->iSqlError = -1;
            ...
            pzErr->iSqlError = -1;
            PrintError(const_cast<char*>(e.what()), 0, 0, const_cast<char*>("E"),
                0, pzErr->iSqlError, 0, const_cast<char*>("INITOLEDBIO_CONNECT"), FALSE);
            return;
        }

        // --- Acquire User/Password (kept for app role / legacy modules that still expect it) ---
        int iUID = 0;
        // char sUser[128] = { 0 }; // Moved to global scope
        char sPassword[128] = { 0 };
        if (lpfnGetUserAndPassword(sAlias, sUser, sPassword, &iUID, FALSE) != 0) {
            pzErr->iSqlError = -1;
            PrintError(const_cast<char*>("Unable to call GetUserAndPassword from login.dll"),
                0, 0, const_cast<char*>(""), 0, -1, 0, const_cast<char*>("INITOLEDBIO3"), FALSE);
            return;
        }
    ...
    try
    {
        ...
        else
        {
            ...
            SelectHoldmap(lAsofDate,
                sHoldings, sHoldcash, sPortmain, sPortbal,

...
const char *SQL_MATURITY_UNLOAD_BASE =
    "SELECT h.id hid, h.sec_no, h.wi, h.sec_xtend, h.acct_type, h.trans_no, "
    "h.units, "
    "h.orig_face, h.sec_symbol, h.tot_cost, "
    "a.id aid, a.sec_type, a.trad_unit, a.curr_id, a.inc_curr_id, "
    "a.cur_exrate, a.cur_inc_exrate, "
    ...
    "CASE WHEN (hf.VariableRate IS NULL) OR (hf.VariableRate = 0) THEN 1 ELSE "
    "hf.VariableRate END VariableRate "
    "FROM Holdings h, assets a "
    "JOIN fixedinc f ON a.sec_no = f.sec_no AND a.when_issue = f.wi "
    "LEFT OUTER JOIN histpric hp1 ON a.sec_no = hp1.sec_no AND a.when_issue = "
    "hp1.wi AND f.maturity_date = hp1.price_date "
    "LEFT OUTER JOIN histpric hp2 ON a.curr_id = hp2.sec_no AND "
    "f.maturity_date = hp2.price_date "
    "LEFT OUTER JOIN histfinc hf ON a.sec_no = hf.sec_no AND a.when_issue = "
    "hf.wi AND f.maturity_date = hf.price_date "
...
const char *SQL_MATURITY_MODE_SECURITY =
...
static void FillMatStruct(nanodbc::result &result, MATSTRUCT *pzMS) {
  ...
  read_string(result, "inc_curr_id", pzMS->sIncCurrId,
              sizeof(pzMS->sIncCurrId));
...
static void FillSectypeStruct(nanodbc::result &result, SECTYPE *pzST) {
  ...
  read_string(result, "sec_type_desc", pzST->sSecTypeDesc,
              sizeof(pzST->sSecTypeDesc));
  read_string(result, "primary_type", pzST->sPrimaryType,
              sizeof(pzST->sPrimaryType));
  read_string(result, "secondary_type", pzST->sSecondaryType,
              sizeof(pzST->sSecondaryType));
  read_string(result, "third_type", pzST->sThirdType, sizeof(pzST->sThirdType));

  pzST->iStlDays = result.get<int>("stl_days", 0);
  read_string(result, "sec_fee_flag", pzST->sSecFeeFlag,
              sizeof(pzST->sSecFeeFlag));
  read_string(result, "pay_type", pzST->sPayType, sizeof(pzST->sPayType));

  pzST->iAccrualSched = result.get<int>("accrual_sched", 0);
  read_string(result, "commission_flag", pzST->sCommissionFlag,
              sizeof(pzST->sCommissionFlag));
  read_string(result, "position_ind", pzST->sPositionInd,
              sizeof(pzST->sPositionInd));
  ...
  read_string(result, "lot_exists_ind", pzST->sLotExistsInd,
              sizeof(pzST->sLotExistsInd));
...
DLLAPI void STDCALL MaturityUnload(MATSTRUCT *pzMS, long lStartDate,
                                   long lEndDate, char *sMode, int iID,
  ...
  try {
    ...
    bool needNewQuery =
        !(g_maturityState.lStartDate == lStartDate &&
          ...
          g_maturityState.sMode[0] == sMode[0] && g_maturityState.iID == iID &&
          (sMode[0] != 'S' ||
    ...
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"MaturityUnload", FALSE);
    g_maturityState.result.reset();
  } catch (...) {
    *pzErr = PrintError((char *)"Unexpected error in MaturityUnload", 0, 0,
                        (char *)"", 0, -1, 0, (char *)"MaturityUnload", FALSE);
    g_maturityState.result.reset();
  }
}
...
DLLAPI void STDCALL ForwardMaturityUnload(FMATSTRUCT *pzMS, long lStartDate,
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"ForwardMaturityUnload", FALSE);
    return;
  }
  ...
  try {
    ...
    bool needNewQuery =
        !(g_forwardState.lStartDate == lStartDate &&
          ...
          g_forwardState.sMode[0] == sMode[0] && g_forwardState.iID == iID &&
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"ForwardMaturityUnload", FALSE);
    g_forwardState.result.reset();
  } catch (...) {
    *pzErr = PrintError((char *)"Unexpected error in ForwardMaturityUnload", 0,
                        0, (char *)"", 0, -1, 0,
                        (char *)"ForwardMaturityUnload", FALSE);
    g_forwardState.result.reset();
  }
...
DLLAPI void STDCALL SelectAllSectypes(SECTYPE *pzST, ERRSTRUCT *pzErr) {
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"SelectAllSectypes", FALSE);
    return;
  }
  ...
  try {
    if (!g_sectypeState.result) {
      ...
      g_sectypeState.result =
          nanodbc::execute(gConn, NANODBC_TEXT(SQL_SELECT_ALL_SECTYPES));
    }
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectAllSectypes", FALSE);
    g_sectypeState.result.reset();
  } catch (...) {
    *pzErr =
        PrintError((char *)"Unexpected error in SelectAllSectypes", 0, 0,
                   (char *)"", 0, -1, 0, (char *)"SelectAllSectypes", FALSE);
    g_sectypeState.result.reset();
  }
}
...
#ifdef DEBUG
  PrintError("InitializeMaturityIO called (no-op with nanodbc RAII)", 0, 0, "",
             0, 0, 0, "InitializeMaturityIO", FALSE);
#endif
...
#ifdef DEBUG
  PrintError("FreeMaturityIO called (RAII cleanup)", 0, 0, "", 0, 0, 0,
             "FreeMaturityIO", FALSE);
#endif
}

...
* 2021-02-26 J# PER-11415 Adjustments for feesout based on perf.dll and reporting - mk.
...
//#define	EXTRADAILYAINFO		1 // number of daily asset info record (for an asset) created at a time
...
#define	EQUITYCASHSEGMENT		122	// Not part of equity or cash but cash flow goes to Equity Cash e.g. Preferred Stocks
...
typedef struct 
{
	...
	TAXINFO	zTInfo; // Tax adjustments - in case tax related RORs need to be calculated
	...
	double  fCNTWFFactor; //Fudge factor for net of consulting fee time weighted ror, type = 14
	...
	double	fNFTWWtdFlow;		// Calculated Wtd Flow for net of fee time weighted ror, type = 14
	double	fCNTWWtdFlow;		// Calculated Wtd Flow for net of consulting fee time weighted ror
	...
	double	fGFTEWtdFlow;		// Calculated Wtd Flow for gross of fee Tax Equivalent ror
...
{
	PERFKEY			zPK;
	char			sRecordType[2]; // W - Weighted, E - Equity + Equity CASH, F - FI + FI CASH	
	long			lParentRuleNo;
	...
	double			fBegStetaxAD;
	*/
	BOOL			bGotBeginMVFromPerformance; // Has it been read yet from performance table? 
	...
	long			lScratchDate;// date in daily db which can be over-written with current dsumdata/drtrnset records
	ALLRORS			zBeginIndex;        
	ALLRORS			zNewIndex;
	//BOOL			bWtdKey; //does perfrule associated with it has WtdRecInd = W
...
typedef struct 
{
	...
	int			iCount;
	PKEYSTRUCT	*pzPKey;
	//  char	sTaxCalc[1+NT]; // same as perfctrl.sTaxCalc SB 5/29/08 no longer needed, replaced by returnstocalculate in portiinfo table
...
// this structure keeps cumulative flows for the month for the segment (not the key)
// in order to save correct NetFlow number on Monthsum record 
// in cases of keys terminating and re-incepting multiple times within a month
// (like M.. T... IFT... IFT... I... M);
// also to keep calculated weighted flow for the month;
typedef struct 
{
	SUMMDATA	zMonthsum;
...
typedef ERRSTRUCT	(CALLBACK* LPFNCREATEPERFSCRIPT)(PTMPHDRDET, PARTASSET2, int, BOOL, PSCRHDRDET *);
typedef ERRSTRUCT	(CALLBACK* LPFNADDPERFRULE)(PERFRULETABLE *, PERFRULE, PTMPHDRDETTABLE);
typedef ERRSTRUCT	(CALLBACK* LPFNADDTEMPLATEHEADER)(PTMPHDRDETTABLE *, PTMPHDR, int *);
typedef ERRSTRUCT	(CALLBACK* LPFNADDTEMPLATEDETAIL)(PTMPHDRDETTABLE *, PTMPDET, int);
typedef ERRSTRUCT	(CALLBACK* LPFNADDASSET)(ASSETTABLE2 *, PARTASSET2, LEVELINFO, PARTSTYPETABLE, int *, long, long);
typedef ERRSTRUCT	(CALLBACK* LPFNADDSCRDETAIL)(PSCRDET zSDetail, PSCRHDRDET *pzScrHdrDet);
typedef ERRSTRUCT	(CALLBACK* LPFNFINDCREATESCRIPTHEADER)(int, long, ASSETTABLE2, PSCRHDRDETTABLE *, PSCRHDRDET *, 
															BOOL, long, int, int *);
typedef void		(CALLBACK* LPPRSELECTACTIVEPERFRETURNTYPE)(long *, long, long, long, ERRSTRUCT *);
...
static long alPrimesForChars[] = 
{
	...
	449, 457, 461, 509, 521, 541, 557, 569, 577, 593,
	601, 613, 617, 641, 653, 661, 673, 677, 701, 709,
	733, 757, 761, 769, 773, 797, 809, 821, 829, 853,
	857, 877, 881, 929, 937, 941, 953, 977, 997, 1009,
	1013, 1021, 1033, 1049, 1061, 1069, 1093, 1097, 1109, 1117,
	1129, 1153, 1181, 1193, 1201, 1213, 1217, 1229, 1237, 1249,
	1277, 1289, 1297, 1301, 1321, 1361, 1373, 1381, 1409, 1429,
	1433, 1453, 1481, 1489, 1493, 1549, 1553, 1597, 1601, 1609,
	1613, 1621, 1637, 1657, 1669, 1693, 1697, 1709, 1721, 1733,
...
static long alPrimesForLines[] = 
{
	...
	967, 971, 983, 991, 1019, 1031, 1039, 1051, 1063, 1087,
	1091, 1103, 1123, 1151, 1163, 1171, 1187, 1223, 1231, 1259,
	1279, 1283, 1291, 1303, 1307, 1319, 1327, 1367, 1399, 1423,
	1427, 1439, 1447, 1451, 1459, 1471, 1483, 1487, 1499, 1511,
	1523, 1531, 1543, 1559, 1567, 1571, 1579, 1583, 1607, 1619,
...
//	extern		LPFNACCDIVTABLE					lpfnFindDividendInAccdiv;
...
extern		LPPRPERFSCRIPTHEADER				lpprInsertPerfscriptHeader, lpprUpdatePerfscriptHeader;
...
ERRSTRUCT CalculatePerformance(PARTPMAIN *pzPdir, long lLastPerfDate, long lCurrentPerfDate, long lEarliestPerfDate,
							   PKEYTABLE *pzPKeyTable, char *sBaseCurrency, PERFCTRL zPCtrl, BOOL bOnlyCreateScripts, PERFRULETABLE *pzRuleTable);
ERRSTRUCT GetAllPerfkeys(int iID, PKEYTABLE *pzPKTable);
ERRSTRUCT GetAllReturnTypes(int iID, PKEYTABLE *pzPKTable);
ERRSTRUCT CalculateFlow(PKEYTABLE zPKTable, TRANSTABLE zTTable, PKEYASSETTABLE2 zPATable, ASSETTABLE2 zATable, PERFRULETABLE zRuleTable, 
						PERFASSETMERGETABLE zPAMTable, int iWhichFlow, long lLastPerfDate, long lCurrentPerfDate, 
						long lEarliestPerfDate, PARTPMAIN zPmain);
ERRSTRUCT GetValuesOnFixedPoints(ASSETTABLE2 *pzATable, HOLDINGTABLE *pzHTable, TRANSTABLE *pzTTable, PKEYASSETTABLE2 *pzPATable, 
								 PKEYTABLE *pzPTable, PERFRULETABLE zRuleTable, PARTPMAIN *pzPdir, ACCDIVTABLE zADTable, PERFASSETMERGETABLE zPAMTable);
...
ERRSTRUCT CopyAnExistingKey(PKEYTABLE *pzPKTable, int iKeyIndex,int iDateIndex, int iReturnsToCalculate);
ERRSTRUCT CopyTotalIfRequired(PKEYTABLE *pzPKTable, int iDIndex, int iReturnsToCalculate);
ERRSTRUCT CalculateWeightedValues(PKEYTABLE zPKTable, BOOL bSpecialCase, int iReturnsToCalculate);
void      NewNetCumWtd(double fPNet, double fPCum, BOOL bPPeriodEnd, long lPDaysSinceNond, double fNToday, 
					   double *pzNNet, double *pzNCum, double *pzNWtd, BOOL bAddToCum);
ERRSTRUCT CalculateReturnForAllKeys(PKEYTABLE zPTable, long lLastPerfDate, long lCurrentPerfDate, 
									PARTPMAIN zPdir, PERFCTRL zPCtrl, int bCalcSelected);
...
ERRSTRUCT SetTaxInformation(PKEYTABLE zPTable, int iKey, int iDate, RORSTRUCT *pzRor);
//ERRSTRUCT TaxRelatedReturns(PKEYTABLE zPTable, int iKey, int iDate, PORTTAX zPorttax, RORSTRUCT *pzRor);
void	  CreatePerformFromDInfo(PKEYTABLE zPTable, int iKey, int iDate, SUMMDATA *pzPerform);
void	  CreateMonthsumFromTwoSummdata(SUMMDATA zCurrentSum, SUMMDATA zPreviousSum, SUMMDATA *pzMSum, 
									    BOOL bCumOnly = FALSE);
ERRSTRUCT WeightedAveragePerformance(PKEYTABLE *pzPTable, PERFRULETABLE zRTable, PKEYASSETTABLE2 zPATable, 
									 ASSETTABLE2 zATable, long lLastPerfDate, long lCurrentPerfDate);
...
ERRSTRUCT GetWtdInfoAndDelKeys(PKEYTABLE *pzPTable, PERFRULETABLE zRTable, VALIDDPTABLE *pzVDPTable, PKEYASSETTABLE2 zPATable,
							   ASSETTABLE2 zATable, long lLastPerfDate, long lStartDate, long lEndDate);
ERRSTRUCT CalcWtdPerformance(VALIDDPTABLE zVDPTable, PKEYTABLE *pzPTable, long lLastPerfDate, long lStartDate,long lEndDate);

/* Prototype of functions defined in calcperf2.cpp */
DLLAPI ERRSTRUCT STDCALL TestAsset(PARTASSET2 zPAsset, PSCRDET *pzDetail, int iCount, BOOL bSecCurr, long lDate, 
...
ERRSTRUCT TestDoubleValue(char *sLowStr, char *sHighStr, char *sComparisonRule, double fTestValue, BOOL *bResult);
ERRSTRUCT TestIntegerValue(char *sLowStr, char *sHighStr, char *sComparisonRule, int iTestValue, BOOL *bResult);
...
ERRSTRUCT CreateDynamicPerfkeys(PKEYTABLE *pzPTable, PARTPMAIN zPmain, long lLastPerfDate, long lCurrentPerfDate, ASSETTABLE2 zATable, 
								PERFRULETABLE *pzRuleTable, PERFASSETMERGETABLE zPAM, long lInitPerfDate, BOOL bOnlyCreateScripts);
//ERRSTRUCT PerfkeyForWeightedParent(PKEYTABLE *pzPTable, PKEYASSETTABLE2 zPATable, ASSETTABLE2 zATable, PERFRULETABLE zRTable,
//																 long lParentRuleNo, long lKeyStartDate, long lKeyEndDate);
DLLAPI ERRSTRUCT STDCALL CreateVirtualPerfScript(PTMPHDRDET zTmpHdrDet, PARTASSET2 zPAsset, int iDateOffset, BOOL bSecCurr, PSCRHDRDET *pzScrHdrDet);
ERRSTRUCT GetSelectTypeValueForAnAsset(PARTASSET2 zPAsset, int iDateOffset, char *sSelectType, BOOL bSecCurr, char *sAlphaValue, 
									   double *pfFloatValue,  int *piIntegerValue);
...
DLLAPI ERRSTRUCT STDCALL CreateNewScript(PSCRHDRDET *pzScrHdrDet, PSCRHDRDETTABLE *pzSHDTable, 
									BOOL bSingleSecurity, char *sSecNo, char *sWi, char *SecDesc1);
ERRSTRUCT CreateNewPerfkey(PKEYTABLE *pzPTable, PERFRULE zPrule, long lScrhdrNo, int iSegmentTypeID, long lCurrentPerfDate);
long      FindParentPerfkeyNo(PKEYTABLE zPTable, long lParentRuleNo);
ERRSTRUCT FillHoldingAndAssetTables(PARTPMAIN *pzPdir, long lDate, ASSETTABLE2 *pzATable, HOLDINGTABLE *pzHTable, 
									long lLastPerfDate, long lCurrentPerfDate, char DoAccruals);
ERRSTRUCT FillTransAndAssetTables(int iID, int iVendorid, long lDate1, long lDate2, 
								  ASSETTABLE2 *pzATable, TRANSTABLE *pzTTable, BOOL bAdjustPerfDate);
ERRSTRUCT FillAccdivTable(int iID, long lLastPerfDate, long lCurrentPerfDate, ASSETTABLE2 *pzATable, ACCDIVTABLE *pzAccdivTable);
ERRSTRUCT GetAccruals(ACCDIVTABLE zADTable, ASSETTABLE2 *pzATable, TRANSTABLE *pzTTable, HOLDINGTABLE *pzHTable, int iVendorID,
					  long lPerfDate, long lLastPerfDate, long lCurrentPerfDate);
...
ERRSTRUCT FindAndAddCurrency(char *sCurrId, ASSETTABLE2 *pzATable, int iID, long lTransNo, int iVendorID, int *piIndex, long lLastPerfDate, long lCurrentPerfDate);
ERRSTRUCT CreatePKeyAssetTable(PKEYASSETTABLE2 *pzPATable, PKEYTABLE zPKTable, ASSETTABLE2 zATable, PERFASSETMERGETABLE zPAM, PERFRULETABLE zruleTable, 
							   long lDate, PARTPMAIN zPmain, long lLastPerfDate, long lCurrentPerfDate);
ERRSTRUCT GetHoldingValues(ASSETTABLE2 *pzATable, HOLDINGTABLE *pzHTable, TRANSTABLE *pzTTable, PKEYASSETTABLE2 *pzPATable, 
						   PKEYTABLE zPKTable, PERFRULETABLE zRuleTable, PARTPMAIN *pzPdir, ACCDIVTABLE zADTable, PERFASSETMERGETABLE zPAMTable,
						   long lDate, long lLastPerfDate, long lCurrPerfDAte, char cDoAccruals);
ERRSTRUCT GetLastAndBeginValues(int iID, long lLastPerfDate, PKEYTABLE *pzPKTable, int iReturnsToCalculate);
ERRSTRUCT GetScratchRecord(PKEYTABLE zPTable, int iPortfolioID, long lLastPerfDate, long lCurrentPerfDate);
//ERRSTRUCT GetStartIndexAndScratchRecord(PKEYTABLE zPTable, int iID, long lLastPerfDate, long lCurrentPerfDate);
...
ERRSTRUCT DeletePeriodPerformSet(PKEYTABLE zPTable, int iID, long lStartDate, long lEndDate, int bCalcSelected, long lInceptionDate);
...
ERRSTRUCT InsertRorRecord(RTRNSET zRorRec, char *sDailyMonthly2, char *sTableName2);
ERRSTRUCT UpdateRorRecord(RTRNSET zRorRec, char *sDailyMonthly3, char *sTableName3, long lPerformNo);
...
void      CalcPerfInitialize(ASSETTABLE2 *pzATable, HOLDINGTABLE *pzHTable, TRANSTABLE *pzTTable, PKEYASSETTABLE2 *pzPKATable, 
							 PERFRULETABLE *pzRuleTable, ACCDIVTABLE *pzADTable, PERFASSETMERGETABLE *zAMTable);
...
//	int FindDailyInfoOffset(long lDate, long lLastPerfDate, long lCurrentPerfDate);
...
DllExport int FindAssetInTable(ASSETTABLE2 zATable, char *sSecNo, char *sWi, BOOL bCheckLongShort, short iLongShort);
...
DllExport int FindScrHdrByHashkeyAndTmpNo(PSCRHDRDETTABLE zSHdrDetTable, long lHashkey, long lTmphdrNo, 
										  char *sSHAKey, PSCRHDRDET zSHdrDetail);
...
int		  FindPerfkeyByID(PKEYTABLE zPKTable, int iID, int iStartIndex, BOOL bSkipNewAndDeleted);
int       FindPerfkeyByRule(PKEYTABLE zPKTable, long lRuleNo, long lScrhdrNo, long lCurrFlagNo);
...
ERRSTRUCT AddMonthlyDataToTable(MONTHTABLE *pzMTable, SUMMDATA zMonthsum, RORSTRUCT zMonthlyRor);
ERRSTRUCT AddAccdivToTable(ACCDIVTABLE *pzAccdivTable, PARTACCDIV zPAccdiv);
DllExport ERRSTRUCT  AddAssetToTable(ASSETTABLE2 *pzAssetTable, PARTASSET2 zPAsset, LEVELINFO zLevels, 
									 PARTSTYPETABLE zPSTtable, int *piArrayIndex, long lLastPerfDate, long lCurrentPerfDate);
ERRSTRUCT AddPorttaxToTable(PORTTAXTABLE *pzPTaxTable, PORTTAX zPTax);
DllExport ERRSTRUCT  AddScriptHeaderToTable(PSCRHDR zPSHeader, PSCRHDRDETTABLE *pzSHDTable, int *piArrayIndex);
DllExport ERRSTRUCT  AddDetailToScrHdrDet(PSCRDET zSDetail, PSCRHDRDET *pzScrHdrDet);
DllExport ERRSTRUCT  AddTemplateHeaderToTable(PTMPHDRDETTABLE *pzTHdrDetTable, PTMPHDR zTHeader, int *piArrayIndex);
DllExport ERRSTRUCT  AddTemplateDetailToTable(PTMPHDRDETTABLE *pzTHdrDetTable, PTMPDET zTDetail, int iHeaderIndex);
...
DllExport ERRSTRUCT  AddPerfruleToTable(PERFRULETABLE *pzPRTable, PERFRULE zPRule, PTMPHDRDETTABLE zTHdrDetTable);
ERRSTRUCT AddAnItemToResultList(RESULTLIST *pzNewList, char *sStr);
ERRSTRUCT AddPerfAssetMergeToTable(PERFASSETMERGETABLE *pzAMTable, PERFASSETMERGE zPAMerge);
BOOL	  StrToBool(char *sStr);
char *	  BoolToStr(BOOL bBool);
ERRSTRUCT AddDailyInfoToAsset(PARTASSET2 *pzPAsset, LEVELINFO zDInfo, long lLastPerfDate, long lCurrentPerfDate);
ERRSTRUCT AddDailyInfoIfNeededOnPertfAssetMergeDates(ASSETTABLE2 *pzATable, PERFASSETMERGE zPAM, BOOL bAddToFromMergedAsset,
													 long lLastPerfDate, long lCurrentPerfDate);
BOOL	  AreTheseScriptsSame(PSCRHDRDET zSHD1, PSCRHDRDET zSHD2);
//void SetLevelsForADay(PARTASSET2 *pzPAsset, LEVELINFO zDInfo, long lLastPerfDate, long lCurrentPerfDate);
...
ERRSTRUCT AddValidDatePRuleToTable(VALIDDPTABLE *pzDPTable, VALIDDATEPRULE zVDPRecord);
ERRSTRUCT CreateDailyInfo(PKEYTABLE *pzPKTable, int iKeyIndex, long lLastPerfDate, long lCurrentPerfDate, int iReturnsToCalculate);
...
void	  LinkReturnsForTheMonth(ALLRORS zPreviousRor, ALLRORS zCurrentRor, ALLRORS *pzNewRor);
...
void	  TaxAdjustments(RORSTRUCT zRS, char *sTaxType, double *pfNetFlowAdj, double *pfWtdFlowAdj,
						 double *pfBegAccrAdj, double *pfEndAccrAdj);
...
double	  GFAndNFBaseROR(double fEMV, double fBMV, double fNFlow, double fWFlow, 
						 double fFudgeFactor, BOOL bInceptionRor, BOOL bTerminationRor);
double	  GFAndNFIncomeROR(double fEAccr, double fBAccr, double fInc, double fBegMV, double fFudgeFactor);	
int		  DWBaseRor(double fEndMV, double fFlow[], int iNumIterations, double fWeight[], double *pfROR);
ERRSTRUCT GetRorTypeIndex(char *sRorType, int *piIndex);
void      CalculateNewRorIndex(ALLRORS zBeginIndex, ALLRORS zRor, ALLRORS *pzNewIndex, int iWhichIndex);
...
double	  TWWeightedFlow(double fEndMV, double fBegMV, double fNFlow, double fFudge, 
					     double fRor, BOOL bInceptionRor);
...
ERRSTRUCT FillPerfAssetMergeTable(PERFASSETMERGETABLE *pzPAMTable, ASSETTABLE2 *pzATable, int iVendorID, long lLastPerfDate, long lCurrentPerfDate);
//int		  CurrentOrMergedAssetIndex(PERFASSETMERGETABLE zPAMTAble, int j);
int		  CurrentOrMergedAssetIndexForToday(PERFASSETMERGETABLE zPAMTable, int iAssetIndex, long lDate);
int		  FindAssetInperfAssetMergTable(PERFASSETMERGETABLE zPAMTable, int iAssetIndexToFind, BOOL bFindAssetMergedFrom);
...
DllExport void FreePerformanceLibrary();
...
int		  AddNewSegmentType(int iLevel1, int iLevel2, int iLevel3, char *sName, char *sAbbrev, char *sCode, 
							char *sSecNo, char *sWi, char *sSecDesc1, BOOL  bShouldExistInSegmap, BOOL	bSingleSecurity);

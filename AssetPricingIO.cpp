...
DLLAPI void STDCALL InsertPriclist(PRICLIST zPriclist, ERRSTRUCT *pzErr)
{
	...
	if (!gConn.connected())
	{
		*pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"D", 0, -1, 0, (char*)"InsertPriclist", FALSE);
		return;
	}
	...
	try
	{
		...
		nanodbc::prepare(stmt, NANODBC_TEXT(
			...
			"Yld_to_worst, Ytw_type, Yld_to_best, Ytb_type, Yld_to_earliest, Yte_type, "
		...
		nanodbc::timestamp tsPriceDate, tsDatePriceUpdated, tsDateExrateUpdated, tsCreateDate;
		...
		auto fCumulativeSplitFactor = RoundDouble(zPriclist.fCumulativeSplitFactor, 2);
		...
		int idx = 0;
		stmt.bind(idx++, zPriclist.sCusip);
		stmt.bind(idx++, zPriclist.sPriceExchange);
		stmt.bind(idx++, &tsPriceDate);
		stmt.bind(idx++, zPriclist.sPriceSource);
		...
		stmt.bind(idx++, zPriclist.sAnnDivFnote);

		stmt.bind(idx++, &zPriclist.iVolume);
		stmt.bind(idx++, zPriclist.sSpRating);
		stmt.bind(idx++, zPriclist.sMoodyRating);
		stmt.bind(idx++, zPriclist.sInternalRating);
		...
		stmt.bind(idx++, zPriclist.sYtwType);
		stmt.bind(idx++, &fYldToBest);
		stmt.bind(idx++, zPriclist.sYtbType);
		stmt.bind(idx++, &fYldToEarliest);
		stmt.bind(idx++, zPriclist.sYteType);
		...
		stmt.bind(idx++, zPriclist.sProcessed);
	...
	catch (const nanodbc::database_error& e)
	{
		...
		msg += e.what();
		*pzErr = PrintError((char*)msg.c_str(), 0, 0, (char*)"D", 0, -1, 0, (char*)"InsertPriclist", FALSE);
	}
	catch (...)
	{
		*pzErr = PrintError((char*)"Unexpected error in InsertPriclist", 0, 0, (char*)"D", 0, -1, 0, (char*)"InsertPriclist", FALSE);
	}
}
...
DLLAPI void STDCALL DeletePriclist(PRICLIST zPriclist, ERRSTRUCT *pzErr)
{
	...
	if (!gConn.connected())
	{
		*pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"D", 0, -1, 0, (char*)"DeletePriclist", FALSE);
		return;
	}
	...
	try
	{
		nanodbc::statement stmt(gConn);
		nanodbc::prepare(stmt, NANODBC_TEXT(
			"DELETE FROM priclist "
			"WHERE cusip = ? AND price_exchange = ? AND price_date = ? AND price_source = ?"));

		// Convert date from long (OLE Automation date) to nanodbc::timestamp
		nanodbc::timestamp tsPriceDate;
		long_to_timestamp(zPriclist.lPriceDate, tsPriceDate);

		stmt.bind(0, zPriclist.sCusip);
		stmt.bind(1, zPriclist.sPriceExchange);
		stmt.bind(2, &tsPriceDate);
		stmt.bind(3, zPriclist.sPriceSource);
	...
	catch (const nanodbc::database_error& e)
	{
		...
		msg += e.what();
		*pzErr = PrintError((char*)msg.c_str(), 0, 0, (char*)"D", 0, -1, 0, (char*)"DeletePriclist", FALSE);
	}
	catch (...)
	{
		*pzErr = PrintError((char*)"Unexpected error in DeletePriclist", 0, 0, (char*)"D", 0, -1, 0, (char*)"DeletePriclist", FALSE);
	}
}

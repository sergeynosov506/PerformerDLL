...
*              for re-use in dependent modules. Refactored for ODBC (2025): Removed OLE DB/ATL deps;
*              RAII focus, std::string for SQL, preserved AdjustSQL for dynamic tables.
* PUBLIC FUNCTION(S): Helpers for ERRSTRUCT, AdjustSQL, etc.
* NOTES: No COM/ATL; errors via exceptions or ERRSTRUCT.
* USAGE: Part of ODBCIO.DLL project (renamed from OLEDB.DLL).
* AUTHOR: Original: Valeriy Yegorov. (C) 2001 Effron Enterprises, Inc.; Refactored: 2025
**/

// History (updated):
// 2025-11-09 ODBC Migration: Remove CDataSource/CSession/CDBPropSet; std::string sSQL; nanodbc compat
// 2008-08-20 Updated for approles - yb
// 2007-12-04 Change to adjustsql() to replace all occurrences of %table%, not just first - yb.
// 09/04/2001 Overloaded AdjustSQL function - vay.
...
extern "C" {
//    typedef int (CALLBACK* LPFNGETUSERANDPASSWORD)(char*, char*, char*, int*, bool);
    //typedef void (CALLBACK* LPPRGETODBCDSNANDDBNAME)(char*, char*);
    typedef void (CALLBACK* LPPRGETCONNECTSTR)(char*, char*);
//    typedef void (CALLBACK* LPFNGETAPPROLENAMEANDPASSWORD)(char*, char*, char*, char*, char*);
//    typedef int (CALLBACK* LPFN1INT)(int);  // lpfnTimer
...
public:
    ...
    CQuery() : m_bPrepared(false), m_cRows(0) {
        m_sAdjSQL[0] = '\0';
    }
    ...
    void AdjustSQL(const std::string& sOldSQL, const std::string& sTblName, std::string& outSQL);
};
...
inline void CQuery::AdjustSQL(const std::string& sOldSQL, const std::string& sTblName, std::string& outSQL) {
    outSQL = sOldSQL;
    size_t pos = 0;
...
// Other overloads: Implement similarly (strtok_s -> string replace for all occurrences)
inline void CQuery::AdjustSQL(const char* sOldSQL, char* sDstTblName, char* sSrcTblName) {
    ...
    if (pos1 != std::string::npos) {
        ...
        if (end1 != std::string::npos) sql.replace(pos1, end1 - pos1 + 1, sDstTblName);
    }
    size_t pos2 = sql.find("%", pos1);  // After first
    if (pos2 != std::string::npos) {
        size_t end2 = sql.find("%", pos2 + 1);
        if (end2 != std::string::npos) sql.replace(pos2, end2 - pos2 + 1, sSrcTblName);
    }
    strncpy_s(m_sAdjSQL, sizeof(m_sAdjSQL), sql.c_str(), sql.length());
}
...
inline void CQuery::AdjustSQL(const wchar_t* sOldSQL, char* sDstTblName, char* sSrcTblName) {
    std::unique_ptr<char[]> temp(new char[MAXSQLSIZE]());
    size_t converted = 0;
    wcstombs_s(&converted, temp.get(), MAXSQLSIZE, sOldSQL, wcslen(sOldSQL));

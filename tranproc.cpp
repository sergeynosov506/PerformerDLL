 ...
 * FILENAME: TranProc.c
 *
 * DESCRIPTION: This routine will retrieve a group of transactions by portfolio
 * ID, security id and process tag.  It verifies and complete the data and
 ...
 *
 *
 * USAGE:  ERRSTRUCT WINAPI TranProc(int iID, char *sSecNo, char *sWi, long
 * lTagNo, char *sCurrPrior, BOOL bRebook, BOOL bDoTransaction) WHERE iID
 *					is the account identifier sSecNo
 *				is the security number of the assets that will
 * be processed sWi						is the when
 * issue of the assets that will be processed lTagNo
 *	identifies the block of transactions to be processed sCurrPrior
 *	identifies whether the program ios called for Current or Prior bRebook
 *			Flag to tell whether to Rebook trades that are eligible
 * for rebooking while doing a reversal bDoTransaction	Whether to do database
 * transaction (begin .. commit/rollback) or not
 *
 *
...
/**
** Function that calls InternalTranProc and upon return it calls the function to
* update all
** the entries in Dtrans which were successfully or unsuccessfully processed
** by InternalTranProc(all entries that match the given iID, SecNo, Wi and
* TagNo).
**/
ERRSTRUCT STDCALL WINAPI TranProc(int iID, char *sSecNo, char *sWi, long lTagNo,
  ...
  char sEmpty[] = "";
  __try {
    zErr = InternalTranProc(iID, sSecNo, sWi, lTagNo, sCurrPrior, bRebook,
                            sEmpty, FALSE);
  ...
  if (zErr.iSqlError == 0 && zErr.iBusinessError == 0) // Posted with no errors
    lpprUpdateDtrans(iID, sSecNo, sWi, lTagNo, (char *)"P", 0, 0, 0, lDate,
                     sTime, &zErr1);
  else // Error while posting
    lpprUpdateDtrans(iID, sSecNo, sWi, lTagNo, (char *)"E", zErr.iBusinessError,
                     zErr.iSqlError, zErr.iIsamCode, lDate, sTime, &zErr1);

  return zErr;
} // Tranproc

ERRSTRUCT STDCALL WINAPI TranProcInfo(int iID, char *sSecNo, char *sWi,
                                      long lTagNo, char *sFileName) {
  ERRSTRUCT zErr, zErr1;
  char sTime[9], sDate[12];
  long lDate;
  ...
  char sType[] = "I";
  zErr =
      InternalTranProc(iID, sSecNo, sWi, lTagNo, sType, TRUE, sFileName, FALSE);
  _strtime(sTime);
  _strdate(sDate);
  lpfnrstrdate(sDate, &lDate);

  if (zErr.iSqlError == 0 && zErr.iBusinessError == 0)
    lpprUpdateDtrans(iID, sSecNo, sWi, lTagNo, (char *)"I", 0, 0, 0, lDate,
                     sTime, &zErr1);
  else
    lpprUpdateDtrans(iID, sSecNo, sWi, lTagNo, (char *)"E", zErr.iBusinessError,
                     zErr.iSqlError, zErr.iIsamCode, lDate, sTime, &zErr1);
...
/**
** Main function which does all the processing but it is never called by other
* programs.
** Other programs call TranProc(or TranProcInfo) which calls this program and
* then updates
** the dtrans entry appropriately based on the error codes returned by this
* function.
**/
ERRSTRUCT InternalTranProc(int iID, char *sSecNo, char *sWi, long lTagNo,
                           char *sCurrPrior, BOOL bRebook, char *sFileName,
                           BOOL bDoTransaction) {
  TRANS zTrans;
  ASSETS zAssets;
  TRANTYPE zTranType;
  ...
  if (zErr.iSqlError == SQLNOTFOUND)
    return (PrintError("No Record In Dtrans Table", iID, 0, "", 106, 0, 0,
                       "TRANPROC1", FALSE));
  else if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
    return zErr;

  /*
   ** verify that account exist in portdir, security exist in assets and
   * sectypes exist for that
   ** security. Since this program is called for one account and security at a
   * time, we don't
  ...
  bAllFlag = FALSE;
  if (strcmp(zTrans.sSecNo, zAssets.sSecNo) != 0 ||
      strcmp(zTrans.sWi, zAssets.sWhenIssue) != 0)
    bAllFlag = TRUE;
  zErr = PrepareForTranAlloc(&zTranType, &zAssets, &zSecType, zTrans, bAllFlag);
  if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
    return zErr;

  // Retrieve the Process day, this value will be assigned to the post_date on
  ...
  /*
  ** Fetch all subsequent dtrans rows, check the branch account, security
  * number, account type
  ** and transaction type.  If all are valid, place the entry on the process
  * table
  */
  bRecFound = TRUE;
  while (bRecFound) {
    ...
    zErr = SetDefaultDatainTranproc(&zTrans, zPortmain, zAssets, zTranType,
                                    lPricingDate, lTradeDate,
                                    zSecType.sPrimaryType);
    if (zErr.iSqlError != 0 || zErr.iBusinessError != 0) {
      InitializePTable(&zProcessTable); // free memory
      return zErr;
    }
    ...
    /*
    ** If the effective date of the transaction is less than the current
    * processing date, set
    ** the as-of flag, unless the transaction is an opening with no trade date,
    * an income or a
    ** money transaction or a trade for a money market security or a trade for
    * single lot
    ** security (like cash or Money market)
    */
    if (zTrans.lEffDate < lTradeDate &&
        strcmp(zSecType.sPrimaryType, "M") != 0 &&
        strcmp(zSecType.sLotInd, "S") != 0) {
      if (zTranType.sTranCode[0] != 'I' && zTranType.sTranCode[0] != 'M' &&
          zTranType.sTranCode[0] != 'O' &&
          strcmp(zTrans.sTranType, "EX") != 0 &&
    ...
    /*
    ** If the tran code from the trantype table is set to "R" (signifies a
    * cancel trade), set
    ** the asof-flag to true.  Reversal transactions are always as-of by
    * definition.
    ** If the transaction is an auto generated income or a split transaction,
    * check the tax lot
  ...
  zErr = LockPortAndProcess(zProcessTable, zAssets, zPortmain, zTranType,
                            zSecType, lTradeDate, bAnyAsOfs, sCurrPrior,
                            bRebook, sFileName, bDoTransaction);
...
/**
** This is the controlling function of this routine, it locks the portfolio for
* processing,
** builds a cancel table of trades to be reversed if there were any as-of trades
* that trigger
** cancellations, merges the process table with the cancel table enteries that
* need to be
** rebooked and sends the trades to the TranAlloc routine for final processing
** before being sent to Updhold for updating
*/
ERRSTRUCT LockPortAndProcess(PTABLE zProTable, ASSETS zAssts, PORTMAIN zPmain,
                             TRANTYPE zTrType, SECTYPE zSType, long lCurrDate,
                             BOOL bAnyAsOfs, char *sCurrPrior, BOOL bRebook,
  ...
  /*
  ** If the AnyAsOfs flag is true, first fill the cancel table with all the
  * transactions falling
  ** in the required date range and then search the process table for the as-of
  * enteries and
  ...
  */
  ...
  if (bAnyAsOfs) {
    /*
    ** We have to find out the first trade out of the ProcessTable for which
    * AsOfFlag is TRUE,
    ** this will give us the earliest effdate. The cancel table will only
    * contain transactions
    ** whose effective date is greater than or equal to the effective date of
    * the 'as of' transaction
    */
    //    lFirstTransNo = 0;
    for (i = 0; i < zProTable.iCount; i++) {
      if (zProTable.pzTInfo[i].bAsOfFlag) {
        ...
        //{
        // if (lFirstTransNo == 0 || zProTable.pzTInfo[i].zTrans.lRevTransNo <
        // lFirstTransNo)
        //{
    ...
    if (pzTempTI != nullptr) {
      /*
      ** In case of an AsOf, the cancel table is filled up with all the
      * transaction belonging to
      ** an account with an effective date equal to or greater than the given
      * date, but in case
      ** of an Reversal, table is filled up with transactions belonging to the
      * given account with
      ** an effective date equal to or greater than the givan date AND having
      * transaction number
      ** equal to or greater than the number of the transaction we are trying to
      * reverse. So if it's
      ** a reversal, assign the reversal transaction number to the transno field
      * otherwise assign 1
      */
      if (pzTempTI->zTranType.sTranCode[0] == 'R') {
        ...
        // SB - Changed on 9/24/99 As per Leno's request - it should be strictly
        // greater than the effective date of the date and not greater than or
        // equal to
      ...
      zErr = FillOutCancelTable(&zCancelTable, pzTempTI->zTrans.iID, lEffDate,
                                lTransNo, lDtransNo, lBlockTransNo,
                                pzTempTI->zTrans.sSecNo, pzTempTI->zTrans.sWi);
      if (zErr.iSqlError != 0 || zErr.iBusinessError != 0) {
    ...
    /*
    ** Start processing the ProcessTable from the first AsOf entry and send any
    * eligble enteries
    ...
    */
    for (i = 0; i < zProTable.iCount; i++) {
      ...
      if (pzTempTI->bAsOfFlag) {
        zErr = ProcessAsOfTrade(
            zPmain.sAcctMethod, pzTempTI->zTrans, pzTempTI->zTranType.sTranCode,
            pzTempTI->sRevTranCode, pzTempTI->zTranType.sAutoGen,
            pzTempTI->sSellAcctType, zSType.sLotInd, sCurrPrior, bRebook,
            &zCancelTable);
        if (zErr.iBusinessError != 0 || zErr.iSqlError != 0) {
          InitializePTable(&zCancelTable); // free memory
          return zErr;
    ...
    */
    for (i = 0; i < zCancelTable.iCount; i++) {
      ...
      // If the trade is not marked to be cancel or the xtrans_no column is
      // zero, skip the trade
      if (pzTempTI->bReversable == FALSE)
        continue;
      else if (pzTempTI->zTrans.lXTransNo == 0 &&
               pzTempTI->zTranType.sTranCode[0] != 'X')
        continue;

      if (pzTempTI->zTrans.iID != pzTempTI->zTrans.iXID)
        continue;

      if (pzTempTI->zTranType.sTranCode[0] == 'X' &&
          pzTempTI->zTranType.lSecImpact == 0)
        continue;

      zErr = ProcessXTransNoTrade(zPmain.sAcctMethod, &zCancelTable, i,
                                  sCurrPrior, bRebook);
      if (zErr.iBusinessError != 0 || zErr.iSqlError != 0) {
        InitializePTable(&zCancelTable); // free memory
        return zErr;
  ...
  /*
  ** When user wants to reverse a trade in FrontEnd, it(frontend) first calls
  * TranProcInfo
  ** function which in turns call tranproc with CurrProc flag = "I". In this
  * situation
  ** tranproc doesn't actually have to do the reversal but it just creates the
  * list of
  ** transaction which it will reverse and write that information to a file(file
  * name is
  ** generated in fronend) and then returns.
  */
  if (strcmp(sCurrPrior, "I") == 0) {
    zErr =
        WriteReversalInformation(zCancelTable, sFileName, zPmain.sUniqueName);
    return zErr;
  }
  ...
  if (zCancelTable.iCount > 0) {
    zErr =
        ProcessCancelTable(zCancelTable, lCurrDate, sCurrPrior, bDoTransaction);
    if (zErr.iBusinessError != 0 || zErr.iSqlError != 0) {
      InitializePTable(&zCancelTable); // free memory
      return zErr;
    }
  ...
  // Merge the process table with the cancel table enteries that are eligible
  // for rebooking
  ...
  for (i = 0; i < zMergeTable.iCount; i++) {
    ...
    */
    ...
    case 'O':
      zErr = ProcessOpeninTranproc(&pzTempTI->zTrans, zAssts, zPmain, zTrType,
                                   lCurrDate);
      break;

    case 'C':
      zErr = ProcessCloseinTranproc(&pzTempTI->zTrans, zAssts, zPmain, zTrType,
                                    lCurrDate);
      break;
    ...
    default:
      zErr = PrintError("Invalid TranCode", pzTempTI->zTrans.iID,
                        pzTempTI->zTrans.lDtransNo, "D", 103, 0, 0,
                        "TPROC LOCKPORT2", FALSE);
    } // switch
    ...
    if (pzTempTI->zTranType.sTranCode[0] == 'M' ||
      ...
      /*
      ** If the entry is a rebooking, use the original trans_no to locate the
      * entry in the
      ** PAYTRAN table, otherwise use the dtrans_no to locate the entry in the
      * DPAYTRAN table
      */
      if (pzTempTI->bRebookFlag)
      ...
      zErr = GetDtransPayee(pzTempTI->zTrans.iID, lTransNo, &zPayTran,
                            pzTempTI->bRebookFlag);
    ...
    */
    if (pzTempTI->zTrans.sMiscDescInd[0] == 'Y') {
      ...
      ** If the entry is a rebooking, use the original trans_no to locate the
      * entry in the
      ** trans_desc table, otherwise use the dtrans_no to locate the entry in
      * the dtrans_desc table
      */
      if (pzTempTI->bRebookFlag)
        lTransNo = pzTempTI->zTrans.lOrigTransNo;
      else
      ...
      zErr = GetDtransDescript(pzTempTI->zTrans.iID, lTransNo, zDTranDescrip,
                               &iDTransDescCount, pzTempTI->bRebookFlag);
      if (zErr.iBusinessError != 0 || zErr.iSqlError != 0) {
        InitializePTable(&zCancelTable); // free memory
        InitializePTable(&zMergeTable);
        return zErr;
    ...
    /*
    ** Retrieve the assets, sectype and trantype rows for the transaction. If
    * the security
    ** number and the when issue columns on the assets match the security number
    * and when issue
    ** on the trans, there is no need to retrieve a new assets and sectype.  If
    * they do not
    ** match, set the bAllflag to true, and the function will retrieve all three
    * rows
    */
    bAllFlag = FALSE;
    ...
    zErr = PrepareForTranAlloc(&zTrType, &zAssts, &zSType, pzTempTI->zTrans,
                               bAllFlag);
    if (zErr.iBusinessError != 0 || zErr.iSqlError != 0) {
      InitializePTable(&zCancelTable); // free memory
      InitializePTable(&zMergeTable);
    ...
    */
    if (pzTempTI->bRebookFlag && zSType.sPrimaryType[0] == 'B' &&
        zSType.sPayType[0] == 'D' &&
    ...
    zErr = TranAlloc(&zTrans, zTrType, zSType, zAssts, zDTranDescrip,
                     iDTransDescCount, &zPayTran, sCurrPrior, bDoTransaction);
    if (zErr.iBusinessError != 0 || zErr.iSqlError != 0) {
      InitializePTable(&zCancelTable); // free memory
      InitializePTable(&zMergeTable);
      return zErr;
    ...
    /*
    ** If the transaction is a rebooking entry, update the original and 'RV'
    * transaction with
    ** the new transaction number assigned to the trade. If the transaction is a
    * close,
    ** transfer or an auto generated trade (receipt of dividend, stock split
    * etc), use the
    ** cross reference transaction number.
    */
    if (pzTempTI->bRebookFlag) {
      if (zTrType.sTranCode[0] == 'C' || zTrType.sTranCode[0] == 'X' ||
          zTrType.sTranCode[0] == 'S' ||
          (zTrType.sTranCode[0] == 'A' &&
           (strcmp(zTrans.sTranType, "LD") == 0 ||
            strcmp(zTrans.sTranType, "BA") == 0)))
      ...
      */
      lpprSelectRevNoAndCode(&lRevtrans, sTranCode, pzTempTI->zTrans.iID,
                             pzTempTI->zTrans.lOrigTransNo, &zErr);
      if (zErr.iSqlError) {
        InitializePTable(&zCancelTable); // free memory
        InitializePTable(&zMergeTable);
        return (PrintError("Error Reading TRANS", pzTempTI->zTrans.iID,
                           pzTempTI->zTrans.lDtransNo, "D", 0, zErr.iSqlError,
                           zErr.iIsamCode, "TPROC LOCKPORT3", FALSE));
      }
      ...
      if (zErr.iSqlError) {
        InitializePTable(&zCancelTable); // free memory
        InitializePTable(&zMergeTable);
        return (PrintError("Error Updating TRANS", pzTempTI->zTrans.iID,
                           pzTempTI->zTrans.lOrigTransNo, "T", 0,
                           zErr.iSqlError, zErr.iIsamCode, "TPROC LOCKPORT4",
                           FALSE));
      }
      ...
      if (zErr.iSqlError) {
        ...
        InitializePTable(&zMergeTable);
        return (PrintError("Error Updating TRANS", pzTempTI->zTrans.iID,
                           lRevtrans, "T", 0, zErr.iSqlError, zErr.iIsamCode,
                           "TPROC LOCKPORT5", FALSE));
      }
...
/**
** The purpose of this function is to identify subsequent transactions that need
* to be cancelled,
** mark the transaction on the cancel table to be cancelled and if necessary
* rebooked
**/
ERRSTRUCT ProcessAsOfTrade(char *sAcctMethod, TRANS zTrans, char *sOrgTranCode,
  ...
  for (i = 0; i < pzCancelTable->iCount; i++) {
    ...
    /*
     ** If the transaction is a cancel and the trans_no  on the cancel table is
     * less than the
     ** rev_trans_no on the cancel trade  - skip. If the effective date of the
     * transaction on the
     ** cancel table is less than or equal to the effective date of the 'as of'
     * trade, skip it,
     ** unless the current trade is a split.
     */
    if (sOrgTranCode[0] == 'R' &&
        pzTempTI->zTrans.lTransNo < zTrans.lRevTransNo)
      continue;
    else if (sOrgTranCode[0] != 'R' &&
             pzTempTI->zTrans.lEffDate <= zTrans.lEffDate &&
             strcmp(pzTempTI->zTranType.sTranCode, "S") != 0)
      continue;

    /*
    ** Check that the security no, when-issue and security extension match to
    * the transaction
    ** that triggered the 'as-of' process if they do not match, skip the
    * transaction
    */
    if (strcmp(pzTempTI->zTrans.sSecNo, zTrans.sSecNo) == 0 &&
        strcmp(pzTempTI->zTrans.sWi, zTrans.sWi) == 0) {
      /*
      ** if the sec_xtends do not match and the sec_xtend on the cancel table is
      * not a tech-long
      ** or tech-short, skip If the secxtend on the trans is 'sh' or 'up'  -
      * skip
      */
      if (strcmp(pzTempTI->zTrans.sSecXtend, zTrans.sSecXtend) != 0) {
        if ((strcmp(zTrans.sSecXtend, "UP") == 0 ||
             strcmp(zTrans.sSecXtend, "SH") == 0) ||
            (strcmp(pzTempTI->zTrans.sSecXtend, "TL") != 0 &&
             strcmp(pzTempTI->zTrans.sSecXtend, "TS") != 0))
          continue;
      }
      /*
    ...
    /*
    ** If the 'as of' transaction is a cancel and the entry on the cancel table
    * matches, there
    ** is no further work to be done, mark the transactions to be cancelled and
    * continue
    */
    if (sOrgTranCode[0] == 'R' &&
    ...
    /*
     ** If the 'as of' transaction is a cancel and the lot indicator is set for
     * a single lot
     ** security, and this is not the cancel record there is no subsequent
     * cancel work to be
    ...
    ** will not cancel any subsequent income payments as per Lawrence J. Cohn's
    * instructions
    */
    ...
    case 'O':
      zErr = ProcessAsOfOpen(sAcctMethod, zTrans, pzCancelTable, i,
                             sOrgTranCode, sRevTranCode, sCurrPrior, bRebook);
      break;

      // Closing Transactions
    case 'C':
      zErr = ProcessAsOfOthers(zTrans, pzCancelTable, i, sOrgTranCode,
                               sRevTranCode, sCurrPrior, bRebook);
      break;
    ...
    case 'A':
      zErr = ProcessAsOfAdjustment(zTrans, pzCancelTable, i, sOrgTranCode,
                                   sRevTranCode, sCurrPrior, bRebook);
      break;
    ...
    case 'S':
      ...
      // Money Transactions
    case 'M':
      zErr = ProcessAsOfMoney(zTrans, pzCancelTable, i, sOrgTranCode,
                              sRevTranCode);
      break;

      // Income Transactions
    case 'I':
      zErr = ProcessAsOfIncome(zTrans, pzCancelTable, i, sOrgTranCode,
                               sRevTranCode, sAutoGen, sCurrPrior, bRebook);
      break;
    ...
    default:
      return (PrintError("Invalid Transaction Code", zTrans.iID,
                         zTrans.lDtransNo, "D", 103, 0, 0,
                         "TPROC PROCESS ASOF3", FALSE));
    } // switch (sTranCode[0])
...
ERRSTRUCT ProcessAsOfOpen(char *sAcctMethod, TRANS zTrans,
                          PTABLE *pzCancelTable, int iCIndex,
  ...
  // Need to reverse and rebook (certain types of)trade if it is an average cost
  // portfolio
  ...
  */
  if (sOrgTranCode[0] == 'I' || sOrgTranCode[0] == 'M' ||
      (sOrgTranCode[0] == 'A' && (strcmp(zTrans.sTranType, "UC") != 0 &&
                                  strcmp(zTrans.sTranType, "LD") != 0 &&
                                  strcmp(zTrans.sTranType, "AM") != 0 &&
                                  strcmp(zTrans.sTranType, "PO") != 0 &&
                                  strcmp(zTrans.sTranType, "BA") != 0 &&
                                  strcmp(zTrans.sTranType, "PZ") != 0 &&
                                  strcmp(zTrans.sTranType, "SZ") != 0)))
    return zErr;
  ...
  */
  if (sOrgTranCode[0] != 'R' &&
      zTrans.lEffDate == pzCancelTable->pzTInfo[iCIndex].zTrans.lEffDate)
    return zErr;
  ...
  // SB 7/24/00 While doing reversal, now user will have an option not to rebook
  // the trades (to avoid multiple subsequent reverasal and rebooking if user is
  // doing multiple reversals).
  if (strcmp(sOrgTranCode, "R") == 0 && strcmp(sCurrPrior, "I") != 0 &&
      !bRebook)
...
/**
** For FUTURE use
** Function To Process AsOf Money transactions.  This function can only be
* effected by a cancel
...
**/
ERRSTRUCT ProcessAsOfIncome(TRANS zTrans, PTABLE *pzCancelTable, int iCIndex,
                            char *sOrgTranCode, char *sRevTranCode,
  ...
  */
  ...
  if (sOrgTranCode[0] == 'R') {
    ...
    */
    if (sRevTranCode[0] == 'O' || sRevTranCode[0] == 'S' ||
        sRevTranCode[0] == 'A') {
      pzCancelTable->pzTInfo[iCIndex].bReversable = TRUE;

      /*
      ** If the transaction is not an opening trade (split or adjustment), the
      * income
      ** transaction must be rebooked as the effect of cancelling a split or
      * adjustment changes
      ** the lot but unlike a cancel of an open, does not remove it.
      ** SB 7/24/00 - If user does not want to rebook the trade, don't rebook
      * it.
      */
      if (strcmp(sRevTranCode, "O") == 0 &&
          (bRebook || strcmp(sCurrPrior, "I") == 0))
        pzCancelTable->pzTInfo[iCIndex].bRebookFlag = TRUE;

    } // if open, split or adjustment

    /*
    ** If cancel of a closing transaction,  cancel and rebook all subsequent
    * income transactions
  ...
  // If the transaction is an as-of adjustment or a split and the tax lot id
  // match, process the trade
  else if (sOrgTranCode[0] == 'A' || sOrgTranCode[0] == 'S') {
    pzCancelTable->pzTInfo[iCIndex].bReversable = TRUE;
    pzCancelTable->pzTInfo[iCIndex].bRebookFlag = TRUE;
  } // Adjustment or Split
  // If the transaction is an as-of close, transfer or an open, all subsequent
  // income enteries must be cancelled.
  else {
    pzCancelTable->pzTInfo[iCIndex].bReversable = TRUE;
    pzCancelTable->pzTInfo[iCIndex].bRebookFlag = TRUE;
...
**/
ERRSTRUCT ProcessAsOfAdjustment(TRANS zTrans, PTABLE *pzCancelTable,
                                int iCIndex, char *sOrgTranCode,
                                char *sRevTranCode, char *sCurrPrior,
                                BOOL bRebook) {
  ERRSTRUCT zErr;
  ...
  */
  ...
  if (sOrgTranCode[0] == 'R') {
    ...
    if ((sRevTranCode[0] == 'O') &&
        ...
        (zTrans.lRevTransNo !=
         pzCancelTable->pzTInfo[iCIndex].zTrans.lTaxlotNo))
      return zErr;
    else
      // of an adjustment and the the transactions is a
      // 'SE', 'FY", 'AG' or 'AC', 'SK', 'SG', 'SR', 'UR', 'UG', 'UK', 'UD',
      // 'UB' skip
      if (sRevTranCode[0] == 'A') {
        if (strcmp(zTrans.sRevType, "SE") == 0 ||
            strcmp(zTrans.sRevType, "FY") == 0 ||
            strcmp(zTrans.sRevType, "AC") == 0 ||
            strcmp(zTrans.sRevType, "AG") == 0 ||
            strcmp(zTrans.sRevType, "SK") == 0 ||
            strcmp(zTrans.sRevType, "SG") == 0 ||
            strcmp(zTrans.sRevType, "SR") == 0 ||
            strcmp(zTrans.sRevType, "UR") == 0 ||
            strcmp(zTrans.sRevType, "UG") == 0 ||
            strcmp(zTrans.sRevType, "UK") == 0 ||
            strcmp(zTrans.sRevType, "UD") == 0 ||
            strcmp(zTrans.sRevType, "UB") == 0)
          return zErr;
        else
          return (ProcessAsOfOthers(zTrans, pzCancelTable, iCIndex,
                                    sOrgTranCode, sRevTranCode, sCurrPrior,
                                    bRebook));
      } // RevTranCode = "A"
      else
        return (ProcessAsOfOthers(zTrans, pzCancelTable, iCIndex, sOrgTranCode,
                                  sRevTranCode, sCurrPrior, bRebook));
  } // OrgTranCode = "R"
  else
    zErr = ProcessAsOfOthers(zTrans, pzCancelTable, iCIndex, sOrgTranCode,
                             sRevTranCode, sCurrPrior, bRebook);
...
**/
ERRSTRUCT ProcessAsOfOthers(TRANS zTrans, PTABLE *pzCancelTable, int iCIndex,
                            char *sOrgTranCode, char *sRevTranCode,
                            char *sCurrPrior, BOOL bRebook) {
  ERRSTRUCT zErr;
  TRANSINFO *pzTI;
  ...
  if (sOrgTranCode[0] == 'R') {
    ...
    // If the transaction is a cancel of an adjustment and the the transactions
    // is a 'SE', 'FY', 'AG', 'AC', 'SK', 'SG', 'SR', 'UR', 'UG', 'UK', 'UD',
    // 'UB', skip
    if (sRevTranCode[0] == 'A' && (strcmp(zTrans.sRevType, "SE") == 0 ||
                                   strcmp(zTrans.sRevType, "FY") == 0 ||
                                   strcmp(zTrans.sRevType, "AC") == 0 ||
                                   strcmp(zTrans.sRevType, "AG") == 0 ||
                                   strcmp(zTrans.sRevType, "SK") == 0 ||
    ...
    // If cancel of an open, UC, BA, AM, PO, RP, PZ, SZ then pick up
    // transactions against that lot
    if (sRevTranCode[0] == 'O' ||
        (sRevTranCode[0] == 'A' && (strcmp(zTrans.sRevType, "UC") == 0 ||
                                    strcmp(zTrans.sRevType, "BA") == 0 ||
                                    strcmp(zTrans.sRevType, "AM") == 0 ||
                                    strcmp(zTrans.sRevType, "PO") == 0 ||
                                    strcmp(zTrans.sRevType, "RP") == 0))) {
      strcpy_s(pzTI->zTrans.sTransSrce, "S");
      pzTI->bReversable = TRUE;

      // SB 8/23/99 - Don't rebook the splits if open transaction is being
      // cancelled SB 7/24/00 - If user does not want to rebook trade, don't
      // rebook vay 5/10/04 - Don't rebook UD for the taxlot where open
      // transaction is being cancelled
      if (strcmp(pzTI->zTranType.sTranCode, "S") == 0 ||
          (strcmp(sCurrPrior, "I") != 0 && !bRebook) ||
          (strcmp(pzTI->zTrans.sTranType, "UD") == 0 &&
           (pzTI->zTrans.lTaxlotNo == zTrans.lRevTransNo)))
    ...
    /*
    ** If cancel of a close and this is the trade, aggregate the trade and add
    * to table, if this
    ** is a subsequent trade, check the tax lot against the lot table and if a
    * match, aggregate and add
    */
    if (sRevTranCode[0] == 'C' || sRevTranCode[0] == 'S' ||
        (sRevTranCode[0] == 'X' && strcmp(zTrans.sRevType, "TC") != 0) ||
        (sRevTranCode[0] == 'A' && (strcmp(zTrans.sRevType, "LD") == 0 ||
                                    strcmp(zTrans.sRevType, "BA") == 0))) {
  ...
  // If the transaction is an as-of adjustment and the transaction is 'SE',
  // 'FY', 'AC', 'SK', 'AG', 'SG', 'UG', 'UK', 'SR', 'UR', 'UD', 'UB', skip it
  if (sOrgTranCode[0] == 'A' && (strcmp(zTrans.sTranType, "SE") == 0 ||
                                 strcmp(zTrans.sTranType, "FY") == 0 ||
                                 strcmp(zTrans.sTranType, "AC") == 0 ||
                                 strcmp(zTrans.sTranType, "AG") == 0 ||
                                 strcmp(zTrans.sTranType, "SK") == 0 ||
                                 strcmp(zTrans.sTranType, "SG") == 0 ||
                                 strcmp(zTrans.sTranType, "UG") == 0 ||
                                 strcmp(zTrans.sTranType, "UK") == 0 ||
                                 strcmp(zTrans.sTranType, "SR") == 0 ||
                                 strcmp(zTrans.sTranType, "UR") == 0 ||
                                 strcmp(zTrans.sTranType, "UD") == 0 ||
                                 strcmp(zTrans.sTranType, "UB") == 0))
    return zErr;
  ...
  */
  ...
  // If an as of split, 'UC', 'AM', 'BA', 'LD', 'PO', 'RP', 'PZ', 'SZ', pick up
  // transactions against that lot
  if (sOrgTranCode[0] == 'S' ||
      (sOrgTranCode[0] == 'A' && (strcmp(zTrans.sTranType, "UC") == 0 ||
                                  strcmp(zTrans.sTranType, "BA") == 0 ||
...
ERRSTRUCT ProcessXTransNoTrade(char *sAcctMethod, PTABLE *pzCancelTable,
  ...
  */
  if (pzTI->zTranType.sTranCode[0] != 'X') {
    ...
    while (bMatchFound == FALSE && i < pzCancelTable->iCount) {
      if (pzCancelTable->pzTInfo[i].zTrans.lTransNo == pzTI->zTrans.lXTransNo) {
        bMatchFound = TRUE;
        strcpy_s(pzCancelTable->pzTInfo[i].zTrans.sTransSrce,
                 pzTI->zTrans.sTransSrce);
      }
  ...
  if (bMatchFound == FALSE)
    return (PrintError("Invalid Cross Transaction Match", 0, 0, "", 994, 0, 0,
                       "TPROC XTRANSNO", FALSE));
  else
    pzTI = &(pzCancelTable->pzTInfo[i - 1]);

  // If the transaction is already marked as a cancel, there is no further work
  // to be done, return
  if (pzTI->zTranType.sTranCode[0] != 'X') {
    if (pzTI->bReversable)
      return zErr;
    ...
    /*
    ** If the transaction is not an exercise transaction, mark it to be
    * rebooked, exercise
    ** transaction are not rebooked due to the fact that they are automatically
    * generated
  ...
  // If the transaction is a transfer, the cancel transaction must look like a
  // cancel of an open
  if (sTranCode[0] == 'X') {
    ...
    if (zErr.iSqlError)
      return (PrintError("Error Selecting Sell Account Type", pzTI->zTrans.iID,
                         pzTI->zTrans.lTransNo, "T", 0, zErr.iSqlError,
                         zErr.iIsamCode, "TPROC XTRANS1", FALSE));
  } // If transfer
  ...
  */
  if (sTranCode[0] == 'C' || sTranCode[0] == 'X' ||
      pzCancelTable->pzTInfo[i].zTranType.sAutoGen[0] == 'Y')
  ...
  char sReversal[] = "R";
  zErr = ProcessAsOfTrade(sAcctMethod, zTrans, sReversal, sTranCode,
                          pzTI->zTranType.sAutoGen, sSellAcctType, sLotInd,
                          sCurrPrior, bRebook, pzCancelTable);
...
ERRSTRUCT AggregateTrades(PTABLE zPTable, int iPStartIndex, int *piPEndIndex,
  ...
  if (iPStartIndex < 0 || iPStartIndex > zPTable.iCount)
    return (PrintError("Programming Error", 0, 0, "", 999, 0, 0,
                       "TPROC AGGREGATETRADES1", FALSE));
  ...
  *piPEndIndex = iPStartIndex;
  ...
  if (pzTrans->lTransNo != pzTrans->lXrefTransNo)
    return (PrintError("Not A Master Transaction", pzTrans->iID,
                       pzTrans->lDtransNo, "D", 50, 0, 0,
                       "TPROC AGGREGATETRADES2", FALSE));

  /*
  ** Right now output transaction is the master transaction. Go through passed
  * table and keep
  ** adding transactions(to the master) for which XrefTransNo is same as
  * TransNo(or XrefTransNo)
  ** of master transaction. Since all the lots are in sequence, as soon as a
  * different XrefTransNo
...
ERRSTRUCT ProcessCancelTable(PTABLE zCanTable, long lCurrentDate,
  ...
  // cancel all the trade in reverse order (of the transno which in effect is
  // effdate + tradesort)
  for (i = zCanTable.iCount - 1; i >= 0; i--) {
    ...
    /*
     ** If a trade is broken into more than one lot, send only master
     * transactions to TranAlloc,
     ** which in turn will find other lots and process the whole trade
     */
    if (zCanTable.pzTInfo[i].zTrans.lXrefTransNo != 0 &&
        (zCanTable.pzTInfo[i].zTrans.lTransNo !=
         zCanTable.pzTInfo[i].zTrans.lXrefTransNo))
      continue;
    ...
    */
    ...
    zErr =
        PrepareForTranAlloc(&zTranType, &zAssets, &zSecType, zTrans, bAllFlag);
    if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
      return zErr;

    zErr = TranAlloc(&zTrans, zTranType, zSecType, zAssets, zDescTab,
                     iNumDTItems, NULL, sCurrPrior, bDoTransaction);
    if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
      return zErr;
    ...
    if (zErr.iSqlError)
      return (PrintError("Error Updating TRANS", zTrans.iID, lOrigTransNo, "T",
                         0, zErr.iSqlError, zErr.iIsamCode,
                         "TPROC PCANCEL TABLE", FALSE));
...
*/
ERRSTRUCT CreateMergeTable(PTABLE zProcessTab, PTABLE zCancelTab,
                           PTABLE *pzMergeTab) {
  int i, j, iEndIndex;
  ERRSTRUCT zErr;
  TRANSINFO zTInfo;
  ...
  while (i < zProcessTab.iCount || j < zCancelTab.iCount) {
    ...
    if (i < zProcessTab.iCount &&
        zProcessTab.pzTInfo[i].zTranType.sTranCode[0] == 'R') {
      i++;
      continue;
    }
    ...
    */
             ...
             (j < zCancelTab.iCount &&
              zProcessTab.pzTInfo[i].zTrans.lEffDate >
                  zCancelTab.pzTInfo[j].zTrans.lEffDate)) {
      InitializeTransInfo(&zTInfo);
      ...
      if (j < zCancelTab.iCount &&
          zCancelTab.pzTInfo[j].zTranType.iTradeSort <=
              zProcessTab.pzTInfo[i].zTranType.iTradeSort) {
        InitializeTransInfo(&zTInfo);
        zTInfo = zCancelTab.pzTInfo[j];
...
**/
ERRSTRUCT ProcessOpeninTranproc(TRANS *pzHTrans, ASSETS zAst, PORTMAIN zPmain,
...
/**
** This function makes sure that the transaction it's called with is a valid
* transaction.
**/
ERRSTRUCT VerifyTransaction(TRANS zTrns, TRANTYPE *pzTranType,
  ...
  if (zErr.iSqlError != 0 && zErr.iSqlError != SQLNOTFOUND)
    return (PrintError("Error Reading TRANTYPE", zTrns.iID, zTrns.lDtransNo,
                       "D", 0, zErr.iSqlError, zErr.iIsamCode,
                       "TPROC VERIFYTRANS8", FALSE));
  /*
  ...
  */
           ...
           (pzTranType->sTranCode[0] != 'I' &&
            pzTranType->sTranCode[0] != 'M' &&
            pzTranType->sTranCode[0] != 'O' &&
            pzTranType->sTranCode[0] != 'C' &&
            pzTranType->sTranCode[0] != 'X' &&
            pzTranType->sTranCode[0] != 'S' &&
            pzTranType->sTranCode[0] != 'A' && pzTranType->sTranCode[0] != 'R'))
    return (PrintError("Invalid TranType", zTrns.iID, zTrns.lDtransNo, "D", 46,
                       0, 0, "TPROC VERIFYTRANS9", FALSE));

  // if it's a reversal, make sure that original trade exist and it's not a
  // reversal
  if (pzTranType->sTranCode[0] == 'R') {
    lpprSelectRevNoAndCode(&lRevTransNo, psRevTranCode, zTrns.iID,
                           zTrns.lRevTransNo, &zErr);
    if (zErr.iSqlError == SQLNOTFOUND)
      ...
      return (PrintError("Error Reading TRANS", zTrns.iID, zTrns.lDtransNo, "D",
                         0, zErr.iSqlError, zErr.iIsamCode,
                         "TPROC VERIFYTRANS11", FALSE));
  ...
  if (zErr.iSqlError == SQLNOTFOUND)
    ...
    return (PrintError("Error Reading SUBACCT", zTrns.iID, zTrns.lDtransNo, "D",
                       0, zErr.iSqlError, zErr.iIsamCode, "TPROC VERIFYTRANS14",
                       FALSE));
...
ERRSTRUCT SetDefaultDatainTranproc(TRANS *pzTrans, PORTMAIN zPortmain,
  ...
  /*
  ** if transactions' currency is not same as portfolio's base currency, and
  * basexrate is 0,
  ** return with an error. Else(same currency id) and basexrate is zero, make it
  * one.
  ...
  */
  ...
  if (strcmp(pzTrans->sIncCurrId, zPortmain.sBaseCurrId) != 0) {
    if (pzTrans->fIncBaseXrate == 0)
      return (PrintError("Invalid Income BaseXrate", pzTrans->iID,
                         pzTrans->lDtransNo, "D", 121, 0, 0,
                         "TPROC SET DEFAULT2", FALSE));
  ...
  if (strcmp(pzTrans->sSecCurrId, zPortmain.sBaseCurrId) != 0) {
    if (IsValueZero(pzTrans->fSecBaseXrate, 12))
      return (PrintError("Invalid Security BaseXrate", pzTrans->iID,
                         pzTrans->lDtransNo, "D", 122, 0, 0,
                         "TPROC SET DEFAULT3", FALSE));
  ...
  if (strcmp(pzTrans->sAccrCurrId, zPortmain.sBaseCurrId) != 0) {
    if (IsValueZero(pzTrans->fAccrBaseXrate, 12))
      return (PrintError("Invalid Accrual BaseXrate", pzTrans->iID,
                         pzTrans->lDtransNo, "D", 123, 0, 0,
                         "TPROC SET DEFAULT4", FALSE));
  ...
  *piDTranDesCount = 0;
  ...
  while (zErr.iSqlError == 0 && zErr.iBusinessError == 0) {
    ...
    if (*piDTranDesCount == MAXDTRANDESC)
      return (PrintError("Versus Table Too Small", iID, lTransNo, "T", 113, 0,
                         0, "TPROC GETDTRANSDESC", FALSE));
...
ERRSTRUCT AddTransToPTable(PTABLE *pzPTable, TRANSINFO zTInfo) {
  ...
  if (pzPTable->iCount == pzPTable->iSize) {
    ...
    pzPTable->pzTInfo = (TRANSINFO *)realloc(
        pzPTable->pzTInfo, sizeof(TRANSINFO) * pzPTable->iSize);
    if (pzPTable->pzTInfo == NULL)
      return (PrintError("Insufficient Memory To Create Table", 0, 0, "", 997,
                         0, 0, "TPROC ADDTRANSTODPTABLE", FALSE));
  }
...
ERRSTRUCT FillOutCancelTable(PTABLE *pzCancelTable, int iID, long lEffDate,
  ...
  while (zErr.iBusinessError == 0 && zErr.iSqlError == 0) {
    ...
    lpprSelectTrans(&zTInfo.zTrans, &zTInfo.zTranType, iID, lEffDate, lTransNo,
                    sSecNo, sWI, &zErr);
    if (zErr.iSqlError == SQLNOTFOUND) {
    ...
    if (lTransNo == 0 ||
        (lTransNo != 0 && (zTInfo.zTrans.lTransNo != lTransNo &&
...
**/
ERRSTRUCT WriteReversalInformation(PTABLE zCancelTable, char *sFileName,
                                   char *sPortfolioName) {
  ERRSTRUCT Err;
  FILE *OutputFile;
  ...
  if (OutputFile == NULL) {
    ...
    return (
        PrintError(sMsg, 0, 0, "", 999, 0, 0, "TPROC WRITEREVERSAL", FALSE));
  }

  fprintf(OutputFile,
          "TransNo\tType\tTaxLotNo\t\tUnits\tTrade Date\tSettle "
          "Date\tEffective Date\tPrincipal  \t\tIncome\t\tRebookFlag \n");
  fprintf(OutputFile,
          "--------------------------------------------------------------------"
          "--------------------------------------------------------------------"
          "--------------------------------------------------------------------"
  ...
  for (i = 0; i < zCancelTable.iCount; i++) {
    ...
    fprintf(OutputFile,
            "%d\t%s\t%d\t%13.2lf\t%d/%d/%d \t%d/%d/%d \t%d/%d/%d \t%13.2lf "
            "\t%13.2lf\t%c\n",
            zCancelTable.pzTInfo[i].zTrans.lTransNo,
            zCancelTable.pzTInfo[i].zTrans.sTranType,
            zCancelTable.pzTInfo[i].zTrans.lTaxlotNo,
            zCancelTable.pzTInfo[i].zTrans.fUnits, Tmdy[0], Tmdy[1], Tmdy[2],
            Smdy[0], Smdy[1], Smdy[2], Emdy[0], Emdy[1], Emdy[2],
            zCancelTable.pzTInfo[i].zTrans.fPcplAmt,
            zCancelTable.pzTInfo[i].zTrans.fIncomeAmt,

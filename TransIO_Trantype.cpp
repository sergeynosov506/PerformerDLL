...
void AddTranType(TRANTYPE zTranType) {
  if (zTTable.iNumTType == NUMTRANTYPE)
    PrintError((char *)"Trantype Table is Too Small", 0, 0, (char *)"", 999, 0,
               0, (char *)"OLEDBIO ADDTRANTYPE", FALSE);
  else {
...
DLLAPI void STDCALL SelectTrantype(TRANTYPE *pzTrantype, char *sTranType,
                                   char *sDrCr, ERRSTRUCT *pzErr) {
  ...
  try {
    ...
    safe_bind_string(stmt, idx, sDrCr);
    ...
    if (result.next()) {
      ...
      read_string(result, "tran_type", zTranType.sTranType,
                  sizeof(zTranType.sTranType));
      read_string(result, "dr_cr", zTranType.sDrCr, sizeof(zTranType.sDrCr));
      read_string(result, "abbrv_desc", zTranType.sAbbrvDesc,
                  sizeof(zTranType.sAbbrvDesc));
      read_string(result, "full_desc", zTranType.sFullDesc,
                  sizeof(zTranType.sFullDesc));
      read_string(result, "tran_code", zTranType.sTranCode,
                  sizeof(zTranType.sTranCode));
      read_string(result, "rpt_code", zTranType.sRptCode,
                  sizeof(zTranType.sRptCode));

      zTranType.lCashImpact = result.get<long>("cash_impact");
      zTranType.lSecImpact = result.get<long>("sec_impact");
      read_string(result, "perf_impact", zTranType.sPerfImpact,
                  sizeof(zTranType.sPerfImpact));
      ...
      read_string(result, "trade_fees", zTranType.sTradeFees,
                  sizeof(zTranType.sTradeFees));
      read_string(result, "trade_reverse", zTranType.sTradeReverse,
                  sizeof(zTranType.sTradeReverse));
      read_string(result, "journal", zTranType.sJournal,
                  sizeof(zTranType.sJournal));
      read_string(result, "trade_screen", zTranType.sTradeScreen,
                  sizeof(zTranType.sTradeScreen));
      zTranType.iTradeSort = result.get<short>("trade_sort");
      read_string(result, "auto_gen", zTranType.sAutoGen,
                  sizeof(zTranType.sAutoGen));
    ...
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectTrantype", FALSE);
  } catch (...) {
    *pzErr = PrintError((char *)"Unexpected error in SelectTrantype", 0, 0,
                        (char *)"", 0, -1, 0, (char *)"SelectTrantype", FALSE);
  }
}

DLLAPI void STDCALL SelectTrancode(char *sTranCode, char *sTranType,
                                   char *sDrCr, ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"SelectTrancode", FALSE);
    return;
  }

  try {
    nanodbc::statement stmt(gConn);
    nanodbc::prepare(stmt, NANODBC_TEXT("select tran_code from TRANTYPE where "
                                        "tran_type = ? and dr_cr = ? "));
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectTrancode", FALSE);
  } catch (...) {
    *pzErr = PrintError((char *)"Unexpected error in SelectTrancode", 0, 0,
                        (char *)"", 0, -1, 0, (char *)"SelectTrancode", FALSE);
  }
}

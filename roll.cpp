...
*              function is capable of not only accepting a single portfolio, but
*              also the whole firm, a list of accounts, composites, managers,
*              securities and transactions. It sould be noted that if user has 
*              supplied a(list of) securities or transactions
*              then the positions(and cash balances) created by this function
*              does not represent the whole portfolio. 
* 
* PUBLIC FUNCTION(S): 
* 
* NOTES: 
...
* 2022-03-31 J#PER12156 Adjusted logic for earliest holdmap date to roll transactions back to start - mk.
* 2022-03-31 J#PER12156 Adjusted logic for earliest holdmap date to roll transactions back to inception - mk.
*H*/
...
*F*/
DLLAPI ERRSTRUCT STDCALL WINAPI Roll(int iID, char *sAlphaId1, char *sAlphaId2, long lNumericId1, 
																long lNumericId2, char *sAlphaFlag, char *sNumericFlag, 
																BOOL bInitDataSet, int iWhichDataSet, long lRollDate, int iResetPerfDate)
{
  ERRSTRUCT zErr;
  ...
  bPortmainSelected = bMultipleAccnts = FALSE;
	...
	// If not rolling for a month end date, don't reset the perf dates even if user has asked for it
  ...
  */
  ...
  else 
  {
    ...
    if (strcmp(sAlphaFlag, "S") == 0 || strcmp(sAlphaFlag, "FS") == 0)
      ...
      return(lpfnPrintError("Invalid Alpha Flag", 0, 0, "", 602, 0, 0, "ROLL3", FALSE));
  ...
  */ 
  if (iWhichDataSet != 1 && iWhichDataSet != 2 && iWhichDataSet != 3)
	...
	zErr = FindOutDestinationDataSet(lRollDate, iWhichDataSet, bPartialRoll, &lDestDate);
	if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
		return zErr;

	// Initialize tranproc with the new dates
  zErr = CallInitTranProc(lDestDate, "", "", "", "");
	if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
		return zErr;
  ...
  if (bMultipleAccnts)
  {
    zErr = FindOutSourceDataSet(zPR, "", "", lRollDate, &lSrceDate, &lTempLong, 
																&lTransStartDate, &lTransEndDate, bMultipleAccnts,FALSE, FALSE, &bInitDataSet, iWhichDataSet);
    if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
      return zErr;
  }
  ...
  else if (strcmp(sAlphaFlag, "FS") == 0)
  {
    ...
    */ 
    ...
    return(lpfnPrintError("Invalid Alpha Flag", 0, 0, "", 602, 0, 0, "ROLL5", FALSE));
  ...
  for (;;)
  {
    ...
    else if (strcmp(sAlphaFlag, "FS") == 0)
    {
      ...
      return(lpfnPrintError("Invalid Alpha Flag", 0, 0, "", 602, 0, 0, "ROLL6", FALSE));
	...
	__try
	{
		...
		/* 
		** Call the function which does the actual processing. If we are rolling for more than account, 
		** don't stop the processing, reset the error and continue, unless it is a firmwide roll for a 
		** securiy(write now reorg is doing that and it wants to stop if any error occurs in any of the account).
		*/
		zErr = RollAnAccount(&zPR, sSecNo, sWi, bPartialRoll, bInitDataSet, lRollDate, lSrceDate, lDestDate, 
												 lTransStartDate, lTransEndDate, bPortmainSelected, bMultipleAccnts, iResetPerfDate, 
												 iWhichDataSet, FALSE, FALSE);
		/*	  zErr = RollAnAccount(&zPR, sSecNo, sWi, bPartialRoll, sNumericFlag, lNumericId1, lNumericId2, 
												 bInitDataSet, lRollDate, lSrceDate, lDestDate, lTransStartDate,
												 lTransEndDate, bPortmainSelected, bMultipleAccnts,iResetPerfDate,FALSE);*/
	}
	...
	if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
	{
		...
		sprintf_s(sMsg, "Roll For Portfolio %s On %d/%d/%d Failed", zPR.sUniqueName, iMDY[0], iMDY[1], iMDY[2]);
		lpfnPrintError(sMsg, zPR.iID, 0, "", zErr.iBusinessError, zErr.iSqlError, zErr.iIsamCode, "ROLL7", TRUE);
	}
	else
	{
		...
		sprintf_s(sMsg, "Portfolio %s Successfully Rolled On %d/%d/%d", zPR.sUniqueName, iMDY[0], iMDY[1], iMDY[2]);
		lpfnPrintError(sMsg, zPR.iID, 0, "", zErr.iBusinessError, zErr.iSqlError,  zErr.iIsamCode, "ROLL8", TRUE);
	}
	if ((zErr.iSqlError != 0 || zErr.iBusinessError != 0) && 
		   bMultipleAccnts && strcmp(sAlphaFlag, "FS") != 0)
...
*F*/
DLLAPI ERRSTRUCT STDCALL WINAPI RollFromCurrent(int iID, char *sAlphaFlag, BOOL bInitDataSet, 
																					 int iWhichDataSet, long lRollDate, int iResetPerfDate)
{
  ERRSTRUCT zErr;
  PORTMAIN  zPR;
  ...
  if (!lpfnIsItAMonthEnd(lRollDate))
	...
	// Calling program MUST use AlphaFlag to indicate what the program is supposed to do,
  if (strcmp(sAlphaFlag, "F") != 0 && strcmp(sAlphaFlag, "B") != 0)
    return(lpfnPrintError("Invalid Alpha Flag", 0, 0, "", 602, 0, 0, "ROLLFROMCURRENT1", FALSE));
  ...
  if (strcmp(sAlphaFlag, "F") == 0)
	...
	/*
	** if not adhoc(1) or performance(2), destination is same as roll date. If the destination
	** does not exists in holdmap, "FindOutDestnationDataSet" function resets it(by reseting lDestDate)
  ...
  {
    zErr = FindOutSourceDataSet(zPR, "", "", lRollDate, &lSrceDate, &lTempLong, 
																&lTransStartDate, &lTransEndDate, bMultipleAccnts,TRUE, FALSE, &bInitDataSet, iWhichDataSet);
    if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
      return zErr;
  }
  else if (strcmp(sAlphaFlag, "B") == 0)
  {
    ...
    return(lpfnPrintError("Invalid Alpha Flag", 0, 0, "", 602, 0, 0, "ROLLFROMCURRENT2", FALSE));
  ...
  for (;;)
  {
    ...
    else if (strcmp(sAlphaFlag, "B") == 0)
    {
      ...
      return(lpfnPrintError("Invalid Alpha Flag", 0, 0, "", 602, 0, 0, "ROLLFROMCURRENT3", FALSE));
	...
	__try
	{
		...
		*/
		/*	  zErr = RollAnAccount(&zPR, sSecNo, sWi, FALSE, "", 0, 0, bInitDataSet, lRollDate, lSrceDate, lDestDate, 
												lTransStartDate, lTransEndDate, bPortmainSelected, bMultipleAccnts, iResetPerfDate, TRUE);*/
		zErr = RollAnAccount(&zPR, sSecNo, sWi, FALSE, bInitDataSet, lRollDate, lSrceDate, lDestDate, 
							 lTransStartDate, lTransEndDate, bPortmainSelected, bMultipleAccnts, iResetPerfDate, iWhichDataSet, TRUE, FALSE);
	}
	__except(lpfnAbortDBTransaction(bUseTrans)){}
	...
	if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
	{
		...
		sprintf_s(sMsg, "Roll For Portfolio %s On %d/%d/%d Failed", zPR.sUniqueName, iMDY[0], iMDY[1], iMDY[2]);
		lpfnPrintError(sMsg, zPR.iID, 0, "", zErr.iBusinessError, zErr.iSqlError,  zErr.iIsamCode, "ROLLFROMCURRENT4", TRUE);
	}
	else
	{
		// roll succeeded - commit changes
		...
		sprintf_s(sMsg, "Portfolio %s Successfully Rolled On %d/%d/%d", zPR.sUniqueName, iMDY[0], iMDY[1], iMDY[2]);
		lpfnPrintError(sMsg, zPR.iID, 0, "", zErr.iBusinessError, zErr.iSqlError, zErr.iIsamCode, "ROLLFROMCURRENT5", TRUE);
	}
	if ((zErr.iSqlError != 0 || zErr.iBusinessError != 0) && 
		   bMultipleAccnts && strcmp(sAlphaFlag, "FS") != 0)
		zErr.iSqlError = zErr.iBusinessError = 0;
...
DLLAPI ERRSTRUCT STDCALL WINAPI SettlementRoll(int iID, char *sAlphaFlag, long lRollDate, BOOL bDoTransaction)
{
  ERRSTRUCT zErr;
  ...
  bPortmainSelected = bMultipleAccnts = FALSE;
	...
	// Calling program MUST use AlphaFlag to indicate what the program is supposed to do,
  if (strcmp(sAlphaFlag, "F") != 0 && strcmp(sAlphaFlag, "B") != 0)
		return(lpfnPrintError("Invalid Alpha Flag", 0, 0, "", 602, 0, 0, "SETTLEMENT ROLL1", FALSE));
  ...
  */
  ...
  zErr = CallInitTranProc(lDestDate, "", "", "", "");
	...
	{
		lpprSelectAllHoldmap(sTemp, sTemp, sTemp, sTemp, sTemp, sTemp, sTemp, sDataType, &lTempLong, &zErr);
		if (zErr.iSqlError == SQLNOTFOUND)
		{
			zErr.iSqlError = 0;
			break;
			...
			return(lpfnPrintError("Error Reading Holdmap", 0, 0, "", 0,zErr.iSqlError,
						                 zErr.iIsamCode, "SETTLEMENT ROLL2", FALSE));

		if (strcmp(sDataType, "C") != 0)
			continue; // Even though we found an answer, let the query read all the entries so that the cursor can be proerply closed

		lSrceDate = lTempLong;
	} /* while no error */

	if (lSrceDate == -1)
		return(lpfnPrintError("No current Holdings Found", 0, 0, "", 0,zErr.iSqlError,
						               zErr.iIsamCode, "SETTLEMENT ROLL3", FALSE));
  if (strcmp(sAlphaFlag, "B") == 0)
    zPR.iID = iID;
  ...
  for (;;)
  {
    ...
    else if (strcmp(sAlphaFlag, "B") == 0)
    {
      ...
      return(lpfnPrintError("Invalid Alpha Flag", 0, 0, "", 602, 0, 0, "SETTLEMENT ROLL5", FALSE));
		...
		__try
		{
			...
			*/
			zErr = SettlementDateRollAnAccount(&zPR, lRollDate, lSrceDate, lDestDate, !bMultipleAccnts);
		}
...
/*F*
** This routine rolls an acount completely from inception, doesn't use any existing data as source
*F*/
DLLAPI ERRSTRUCT STDCALL WINAPI RollFromInception(int iID, int iWhichDataSet, long lRollDate, int iResetPerfDate)
{
  ERRSTRUCT zErr;
  PORTMAIN  zPR;
  BOOL      bUseTrans;
  ...
  zPR.iID = iID;
	...
	__try
	{
		...
		** Call the function which does the actual processing.
		*/
		zErr = RollAnAccount(&zPR, "", "", FALSE, TRUE, lRollDate, 0, lDestDate, 0, 0,
												 FALSE, FALSE, iResetPerfDate, iWhichDataSet, FALSE, TRUE);
	}
	__except(lpfnAbortDBTransaction(bUseTrans)){}
	...
	if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
	{
		...
		sprintf_s(sMsg, "Roll For Portfolio %s On %d/%d/%d Failed", zPR.sUniqueName, iMDY[0], iMDY[1], iMDY[2]);
		lpfnPrintError(sMsg, zPR.iID, 0, "", zErr.iBusinessError, zErr.iSqlError,  zErr.iIsamCode, "ROLLFROMINCEPTION1", TRUE);
	}
	else
	{
		// roll succeeded - commit changes
		...
		sprintf_s(sMsg, "Portfolio %s Successfully Rolled On %d/%d/%d", zPR.sUniqueName, iMDY[0], iMDY[1], iMDY[2]);
		lpfnPrintError(sMsg, zPR.iID, 0, "", zErr.iBusinessError, zErr.iSqlError, zErr.iIsamCode, "ROLLFROMINCEPTION2", TRUE);
	}
...
ERRSTRUCT RollAnAccount(PORTMAIN *pzPR, char *sSecNo, char *sWi, BOOL bPartialRoll, BOOL bInitDataSet, long lRollDate, 
												...
												BOOL bMultipleAccnts,BOOL iResetPerfDate, int iWhichDataSet, BOOL bRollFromCurrent, BOOL bRollFromInception)
{ 
  ...
  ** not exist on the roll date then no need to send an error message, else send an error.
  */
  if (bMultipleAccnts)
  {
  ...
  else
  {
    ...
    if (lRollDate < pzPR->lPurgeDate)
      return(lpfnPrintError("Portfolio Has Been Purged. No Transactions Going Back That Far", 
														pzPR->iID, 0, "", 604, 0, 0, "ROLL ROLLACCT3", FALSE));
	}/* Single Account */

	/*
	** There are two flags bRollFromCurrent and bRollFromInception, they both can't be true at the same time,
  ...
  */
  ...
  {
    if (lDestDate == lRollDate && bRollFromCurrent == FALSE && bRollFromInception == FALSE)
    {
      lSrceDate = lRollDate;
      zErr = GetLastTransNo(pzPR->iID, lSrceDate, &lSrceLastTransNo, TRUE);
      if (zErr.iSqlError == SQLNOTFOUND)
    ...
    {
      zErr = FindOutSourceDataSet(*pzPR, sSecNo, sWi, lRollDate, &lSrceDate, &lSrceLastTransNo, &lTransStartDate, 
								  &lTransEndDate, bMultipleAccnts, bRollFromCurrent, bRollFromInception, &bInitDataSet, iWhichDataSet);
      if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
        return zErr;
  ...
  */
  if ((!bMultipleAccnts && lSrceDate != lDestDate && (!bPartialRoll || bInitDataSet))
	  || bRollFromInception
	  )
  {
	  if (!(iWhichDataSet == 2 && !bInitDataSet))
		  zErr = AccntDestInit(pzPR->iID, sSecNo, sWi, lSrceDate, lDestDate, bInitPmainPbal, bRollFromInception);
	  if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
		  return zErr;
	...
	// process all asof transactions, as long as we are not rolling from inception forward
	lNewLastTransNo = lSrceLastTransNo;
  while (zErr.iSqlError == 0 && zErr.iBusinessError == 0 && !bRollFromInception)
  {
    lpprAsofTrans(pzPR->iID, lSrceDate, lSrceLastTransNo, sSecNo, sWi,bSpecificSecNo, &zTR, &zErr);
    if (zErr.iSqlError == SQLNOTFOUND)
    { 
      zErr.iSqlError = 0;
      break;
      ...
      return zErr;
		...
		__try
		{
			// SB 4/2/03 - It's wrong to change srcelasttransno, because the next time it is sent to 
			// lpprAsofTrans query, it's going to re open the query because one of the argument will 
			// now be different, so send another variable to CallUpdateHold function
			// zErr = CallUpdateHold(zTR, *pzPR, FALSE, &lSrceLastTransNo, iWhichDataSet, lRollDate, lDestDate, &bRollDateUpdated);
			zErr = CallUpdateHold(zTR, *pzPR, FALSE, &lNewLastTransNo , iWhichDataSet, lRollDate, lDestDate, &bRollDateUpdated);
			if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
				return zErr;
		}
	...
	// If RollFromInception is trye, make source date to be a day less than what it is (inception date), 
	// so the inception date transactions are not missed
  ...
  {
    if (bBackwardRoll)
      lpprBackwardTrans(pzPR->iID, lTransStartDate, lTransEndDate, sSecNo, sWi, bSpecificSecNo, &zTR, &zErr);
    else
      lpprForwardTrans(pzPR->iID, lTransStartDate, lTransEndDate, sSecNo, sWi, bSpecificSecNo, &zTR, &zErr);
    if (zErr.iSqlError == SQLNOTFOUND)
    { 
      zErr.iSqlError = 0;
      break;
      ...
      return zErr;
		...
		__try
		{
			...
			zErr = CallUpdateHold(zTR, *pzPR, bBackwardRoll, &lNewLastTransNo, iWhichDataSet, lRollDate, lDestDate, &bRollDateUpdated);
			if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
				return zErr;    
		}
		__except(zErr.iSqlError != 0 ? 1:0)
	...
	/*
	** update the roll date and lasttransno if it has not been updated yet (For DataSet 0,
  ** this gets updated after every single transaction - to avoid the problem of effect of 
...
{ 
  ...
  if (lRollDate < pzPR->lPurgeDate)
	...
	if (bNeedInitialization)
	{
		zErr = AccntDestInit(pzPR->iID, "", "", lSrceDate, lDestDate, TRUE, FALSE);
		if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
			return zErr;
	}
  ...
  while (zErr.iSqlError == 0 && zErr.iBusinessError == 0)
  {
    ...
    { 
      ...
      return zErr;
		...
		// Call update hold to reverse the transaction with settlement date is greater than roll date
    zErr = CallUpdateHold(zTR, *pzPR, TRUE, &lSrceLastTransNo, 3, lRollDate, lDestDate, &bRollDateUpdated);
    if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
      return zErr;    
...
*
ERRSTRUCT RollAnAccount(PORTMAIN *pzPR, char *sSecNo, char *sWi, BOOL bPartialRoll, char *sNumericFlag, 
												long lNumericId1, long lNumericId2, BOOL bInitDataSet, long lRollDate, 
												long lSrceDate, long lDestDate, long lTransStartDate, long lTransEndDate, 
												BOOL bPortmainAlreadySelected, BOOL bMultipleAccnts,BOOL iResetPerfDate,BOOL bRollFromCurrent)
{ 
  static int  iLastID = 0;
  ...
  * /
  ...
  else
  {
	...
	** (there is no source, nothing to copy) and process all the transactions.
	* /
  if (sNumericFlag[0] != '\0')
  {
		if (sNumericFlag != "T" && sNumericFlag != "TD" && sNumericFlag != "ED" &&sNumericFlag != "ND")
			return(lpfnPrintError("Invalid Numeric Flag", pzPR->iID, 0, "", 601, 0, 0, 
					                  "ROLL ROLLACCT4", FALSE));
    ...
    * /
    ...
    while (zErr.iSqlError == 0 && zErr.iBusinessError == 0)
    {
      if (strcmp(sNumericFlag, "T") == 0)
        lpfnTransNoTrans(pzPR->iID, lNumericId1, lNumericId2, sSecNo, sWi, bSpecificSecNo, &zTR, &zErr);
      else if (strcmp(sNumericFlag, "TD") == 0)
        lpfnTradeDateTrans(pzPR->iID, lNumericId1, lNumericId2, sSecNo,sWi, bSpecificSecNo, &zTR, &zErr);
      else if (strcmp(sNumericFlag, "ED") == 0)
        lpfnEffectDateTrans(pzPR->iID, lNumericId1, lNumericId2, sSecNo,sWi, bSpecificSecNo, &zTR, &zErr);
      else
        lpfnEntryDateTrans(pzPR->iID, lNumericId1, lNumericId2, sSecNo,sWi, bSpecificSecNo, &zTR, &zErr);
      if (zErr.iSqlError == SQLNOTFOUND)
      {
        zErr.iSqlError = 0;
        break;
  ...
  * /
  ...
  else // single account
  {
    ...
    {
      zErr = FindOutSourceDataSet(*pzPR, sSecNo, sWi, lRollDate, &lSrceDate, &lSrceLastTransNo, 
  ...
  * /
  if (!bMultipleAccnts && lSrceDate != lDestDate && (!bPartialRoll || bInitDataSet))
  {
    zErr = AccntDestInit(pzPR->iID, sSecNo, sWi, lSrceDate, lDestDate, bInitPmainPbal);
    if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
      return zErr;
  ...
  while (zErr.iSqlError == 0 && zErr.iBusinessError == 0)
  {
    lpfnAsofTrans(pzPR->iID, lSrceDate, lSrceLastTransNo, sSecNo, sWi,bSpecificSecNo, &zTR, &zErr);
    if (zErr.iSqlError == SQLNOTFOUND)
    { 
      zErr.iSqlError = 0;
      break;
      ...
      return zErr;
		...
		zErr = CallUpdateHold(zTR, *pzPR, FALSE, &lSrceLastTransNo, iWhichDataSet);
    if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
      return zErr;
  ...
  {
    if (bBackwardRoll)
      lpfnBackwardTrans(pzPR->iID, lTransStartDate, lTransEndDate, sSecNo, sWi, bSpecificSecNo, &zTR, &zErr);
    else
      lpfnForwardTrans(pzPR->iID, lTransStartDate, lTransEndDate, sSecNo, sWi, bSpecificSecNo, &zTR, &zErr);
    if (zErr.iSqlError == SQLNOTFOUND)
    { 
      zErr.iSqlError = 0;
      break;
    ...
    zErr = CallUpdateHold(zTR, *pzPR, bBackwardRoll, &lSrceLastTransNo, iWhichDataSet);
    if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
      return zErr;
	...
	/ *
	** update the roll date in the destination portmain if rolling to adhoc tables (For DataSet 0,
  ** this gets updated after every single transaction - to avoid the problem of effect of transaction
	** being applied to holdings without portmain reflecting it. For DataSet 2(performance) - no need
	** to update dates).
	* /
...
ERRSTRUCT CallUpdateHold(TRANS zTR, PORTMAIN zPR, BOOL bReversal, long *plLastTransNo, int iWhichDataSet, 
												 long lRollDate, long lDestinationDate, BOOL *pbRollDateUpdated)
{
  ERRSTRUCT		zErr;
  ...
  if (strcmp(zTR.sAcctMthd, "V") == 0)
  {
    while (!zErr.iSqlError)
    {
      ...
      if (strcmp(zTempTrnDesc.sCloseType, "VS") == 0)
        zDTranDesc[0] = zTempTrnDesc; // even though we found the record we were looking for, continue until SQLNOTFOUND, so that the query can be closed properly
    } // while no error
  } // if versus trade

	// Call update hold and make sure (6th flag - TRUE) update hold starts and commit/rollback 
	// the effect of that transaction depending on if there was no error/error.
  zErr = lpfnUpdateHold(zTR, zPR, zDTranDesc, 0, NULL, "P", FALSE);
  if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
    return zErr;

	// If rolling to an existing(monthend) table set, update lasttransno (if grearter than that in appropriate portmain)
...
ERRSTRUCT GetLastTransNo(int iID, long lDate, long *plLastTransNo, BOOL bIsNotFoundAnError)
{
  ...
  char			sTemp[STR80LEN], sPortmain[STR80LEN];
	...
	{
		zErr = ReadHoldingsMapForADate(lDate, sTemp, sTemp, sTemp, sPortmain, sTemp, sTemp, sTemp, FALSE);
		if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
			return zErr;
	}
  ...
  if (zErr.iSqlError || zErr.iBusinessError)
  {
    if (zErr.iSqlError == SQLNOTFOUND && !bIsNotFoundAnError)
      ...
      zErr = lpfnPrintError("Error Fetching Portmain cursor", 0, 0, "", 0, 
														zErr.iSqlError, zErr.iIsamCode, "ROLL GETLASTTRANS2", FALSE);
  }
...
/*
** Function to figure out destination data set. If the user has asked for ADHOC, Performance or
** settlement data set then that's the data set they get, else this function will check holdmap 
** table, if a record exists for the requested date (and data type is not 'C' (current)) that is 
** the destination data set else adhoc data set is the destination(if even that does not exist, 
** it's an error).
*/
ERRSTRUCT FindOutDestinationDataSet(long lRollDate, int iWhichDataSet, BOOL bPartialRoll, long *plDestDate)
{
    ...
    *plDestDate = lRollDate;
	...
	zErr = ReadHoldingsMapForADate(*plDestDate, sHoldings, sTemp, sTemp, sTemp, sTemp, sTemp, sTemp, FALSE);
  if (zErr.iSqlError == SQLNOTFOUND || _stricmp(sHoldings, "holdings") == 0)
  {
    ...
    */
    if (*plDestDate == ADHOC_DATE)
      return(lpfnPrintError("No Ad Hoc Data Set Exists", 0, 0, "", 603, 0, 0, "ROLL FINDOUTDESTINATION1", FALSE));
    else
    {
      *plDestDate = ADHOC_DATE;
      zErr = ReadHoldingsMapForADate(*plDestDate, sTemp, sTemp, sTemp, sTemp, sTemp, sTemp, sTemp, FALSE);
      if (zErr.iSqlError == SQLNOTFOUND)
        return(lpfnPrintError("No Ad Hoc Data Set Exists", 0, 0, "", 603, 0, 0, "ROLL FINDOUTDESTINATION2", FALSE));
      else if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
        return zErr;
								...
								BOOL bMultipleAccnts,BOOL bRollFromCurrent, BOOL bRollFromInception, BOOL *bInitDataSet, int iWhichDataSet)
{
  ...
  int				iRegForwardCount, iRegBackwardCount, iAsofForwardCount, iAsofBackwardCount;
  ...
  */
  ...
  while (zErr.iSqlError == 0 && zErr.iBusinessError == 0)
  {
    lpprSelectAllHoldmap(sTemp, sTemp, sTemp, sTemp, sTemp, sTemp, sTemp, sDataType, &lAsofDate, &zErr);
    if (zErr.iSqlError == SQLNOTFOUND)
		{
			zErr.iSqlError = 0;
      break;
      ...
      return(lpfnPrintError("Error Reading Holdmap", 0, 0, "", 0,zErr.iSqlError,
						                 zErr.iIsamCode, "ROLL FINDSOURCE1", FALSE));

		// Data in adhoc and performance tables are temporary, cann't use them as source
  ...
  */
	if (lStartDate == -1 && lEndDate == lTempMaxDate)
		return(lpfnPrintError("No Source Data to Be Considered As Base Point", 0, 0, "", 
													603, zErr.iSqlError, zErr.iIsamCode, "ROLL FINDSOURCE2", FALSE));
  ...
  */
  if (bMultipleAccnts)
  {
		if (!bSourceFound)
		{
			if (lStartDate == -1 || (lEndDate - lRollDate <= lRollDate - lStartDate))
			{
				*plSourceDate = lEndDate;
				bIsSourceCurrent = bIsEndCurrent;
			}
	  ...
	  zErr = GetTransactionDateRange(lRollDate, *plSourceDate, bIsSourceCurrent, plTransStartDate, plTransEndDate);
		return zErr;
	...
	if (lStartDate == -1)
	{
		...
		*bInitDataSet = FALSE;
		...
		else
		{
			...
			if (iWhichDataSet == 2)
				*plSourceDate = GetRollDate(zPR.iID, "pmperf", lEndDate);
			else
				*plSourceDate = lEndDate;
		}
		...
		if ((zErr.iSqlError != 0 && zErr.iSqlError != SQLNOTFOUND) || zErr.iBusinessError != 0)
			return zErr;

		zErr = GetTransactionDateRange(lRollDate, *plSourceDate, bIsEndCurrent, plTransStartDate, plTransEndDate);
		return zErr;
	}
	else
	{
		...
		{
			*plSourceDate = lEndDate;
		  zErr = GetTransactionDateRange(lRollDate, *plSourceDate, bIsStartCurrent, plTransStartDate, plTransEndDate);
			return zErr;
		}
		else if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
			return zErr;
  ...
  */
  ...
  if (lEndTransNo == -1)
  {
	...
	iRegForwardCount = iRegBackwardCount = iAsofForwardCount = iAsofBackwardCount = 0; 
  /* 
  ...
  ** between start date(excluded) and roll date(included).
	** First temporarily set the *plStartEffDate and *plEndEffDate to the appropriate
	** dates as if we are doing a forward roll. And then reset these variables as if we
	** are doing a backward roll.
  */
  zErr = GetTransactionDateRange(lRollDate, lStartDate, bIsStartCurrent, plTransStartDate, plTransEndDate);
	// RegTransCount "Forward"
	lpprRegTransCount(zPR.iID, *plTransStartDate, *plTransEndDate, sSecNo, sWi, bSpecificSecNo, &iRegForwardCount, &zErr);
	if (zErr.iSqlError || zErr.iBusinessError)
		return(lpfnPrintError("Error Reading Regular Forward Count From Trans", 
													0, 0, "", 0, zErr.iSqlError, zErr.iIsamCode, 
						              "ROLL FINDSOURCE3A", FALSE));

	// AsofTransCount "Forward"
  lpprAsofTransCount(zPR.iID, lStartDate, lStartTransNo, sSecNo, sWi, bSpecificSecNo, &iAsofForwardCount, &zErr);
	if (zErr.iSqlError || zErr.iBusinessError)
     return(lpfnPrintError("Error Reading Asof Forward Count From Trans", 
	                         0, 0, "", 0, zErr.iSqlError, zErr.iIsamCode, 
			                    "ROLL FINDSOURCE4A", FALSE));
    
  
	zErr = GetTransactionDateRange(lRollDate, lEndDate, bIsEndCurrent, plTransStartDate, plTransEndDate);
  // RegTransCount "Backward"
	lpprRegTransCount(zPR.iID, *plTransStartDate, *plTransEndDate, sSecNo, sWi, bSpecificSecNo, &iRegBackwardCount, &zErr);
  if (zErr.iSqlError || zErr.iBusinessError)
    return(lpfnPrintError("Error Reading Regular Backward Count From Trans", 0, 0, "", 0, zErr.iSqlError, 
													zErr.iIsamCode, "ROLL FINDSOURCE5A", FALSE));

  //AsofTransCount "Backward"
  lpprAsofTransCount(zPR.iID, *plTransStartDate, *plTransEndDate, sSecNo, sWi, bSpecificSecNo, &iAsofBackwardCount, &zErr);
  if (zErr.iSqlError || zErr.iBusinessError)
    return(lpfnPrintError("Error Reading Asof Backward Count From Trans", 0, 0, "", 0, zErr.iSqlError, 
													zErr.iIsamCode, "ROLL FINDSOURCE6A", FALSE));
  ...
  */
  if (iRegBackwardCount + iAsofBackwardCount < iRegForwardCount + iAsofForwardCount)
  {
    *plSourceDate = lEndDate;
...
/*
** This function is called from findsourcedataset function to figures out which transaction 
** set(based on eff_date range) to use based on roll date and source date. When this function 
** is called source date has already been identified and also it has been determined 
** whether the source is current or not. The asof_date for the "current"(data_type = "C") 
** holdings set is the pricing date (which is the last business date). Holdings on that date 
** will have trades with effective date later than asof_date, so if current is used as source, 
** we have to get trade_date from starsdate and use that date instead of source date.
*/
																 ...
																 long *plTransStartDate, long *plTransEndDate)
{
  ...
  lpprInitializeErrStruct(&zErr);
	...
	{
		if (bIsSourceCurrent) // this is very unlikely, rolling a portfolio to a date greater than current
...
ERRSTRUCT UpdateRollDate(int iID, long lRollDate, long lLastTransNo, long lDestDate)
{
  ...
  lpprInitializeErrStruct(&zErr);
	...
	{
		zErr = ReadHoldingsMapForADate(lDestDate, sTemp, sTemp, sTemp, sPortmain, sTemp, sTemp, sTemp, FALSE);
		if (zErr.iSqlError || zErr.iBusinessError)
			return zErr;
	}
...
** over it. If InitPortmainPbal is TRUE only then portdir and portbal are deleted.
...
ERRSTRUCT AccntDestInit(int iID, char *sSecNo, char *sWi, long lSrcDate, 
												long lDestDate, BOOL bInitPmainPbal, BOOL bRollFromInception)
{
  ERRSTRUCT		zErr;
  BOOL				bSrcExist, bSecSpecific, bSecInHoldings;
	char				sHoldings1[STR80LEN], sHoldcash1[STR80LEN], sPortbal1[STR80LEN], sPortmain1[STR80LEN], sHedgeXref1[STR80LEN], sPayrec1[STR80LEN], sHoldtot1[STR80LEN];
	char				sHoldings2[STR80LEN], sHoldcash2[STR80LEN], sPortbal2[STR80LEN], sPortmain2[STR80LEN], sHedgeXref2[STR80LEN], sPayrec2[STR80LEN], sHoldtot2[STR80LEN];
  ...
  /* 
	...
	** If initialization is done for a specific security, find out if the security 
	...
	if (bDestInit)
  {
    if (lLastDest != lDestDate || bLastSecSpecific != bSecSpecific)
  		lpfnUnprepareRollQueries("", UNPREP_ACCNTDESTINATIONQUERIES);
	...
	else
	{
		...
		zErr = ReadHoldingsMapForADate(lDestDate, sHoldings1, sHoldcash1, sPortbal1, 
                                   sPortmain1, sHedgeXref1, sPayrec1, sHoldtot1, TRUE);
		if (zErr.iSqlError == SQLNOTFOUND || _stricmp(sHoldings1, "holdings") == 0)
		{
			/*
			** If the passed date was the ADHOC date, return with an error, else read 
			** holdmap again for the ADHOC_DATE.
			*/
			if (lDestDate == ADHOC_DATE)
				return(lpfnPrintError("No Ad Hoc Data Set Exists", 0, 0, "", 603, 0, 0, "ROLL ACCNTDEL1", FALSE));
			else
			{
				...
				zErr = ReadHoldingsMapForADate(lDestDate, sHoldings1, sHoldcash1, sPortbal1, 
																		   sPortmain1, sHedgeXref1, sPayrec1, sHoldtot1, TRUE);
				if (zErr.iSqlError == SQLNOTFOUND)
					return(lpfnPrintError("No Ad Hoc Data Set Exists", 0, 0, "", 603, 0, 0, "ROLL ACCNTDEL2", FALSE));
				else if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
					return zErr;
			} /* if passed date was not the ad hoc date */
		} /* if record not found for the passed date */
  ...
  zErr = CallInitTranProc(lDestDate, "", "", "", "");
	...
	/* 
  ** Delete records from all the destination tables. If initializing for a specific
  ...
  */
  if (bSecSpecific)
  {
    ...
    */
    ...
    {
      /* Delete Holdings */
      lpprAccountDeleteHoldings(iID, sSecNo, sWi, sHoldings1, bSecSpecific, &zErr);
      if (zErr.iSqlError || zErr.iBusinessError)
        return zErr;
    }
    else
    {
      /* delete holdcash */
      lpprAccountDeleteHoldcash(iID, sSecNo, sWi, sHoldcash1, bSecSpecific, &zErr);
      if (zErr.iSqlError || zErr.iBusinessError)
        return zErr;
  ...
  if (zErr.iSqlError || zErr.iBusinessError)
	...
	/*
	** SB 11/14/2012 - Earlier this function was not called when source and destination dates were same but now, for
	** roll from inception, this function may be called even when source and destination are same (mainly to delete 
	** all data except portmain record). So, put this additional check to make sure portmain record doesn't get 
...
**/
	...
	** (destination/source date is different or whether it is for a specific security
	** or not is different), unprepare (previously prepared)source queries.
	*/
  if (bSrceInit)
  {
    if (lLastSrce != lSrcDate || lLastDest != lDestDate || bLastSpecificSecNo != bSecSpecific)
  		lpfnUnprepareRollQueries("", UNPREP_ACCNTSOURCEQUERIES);
  ...
  */
 	zErr = ReadHoldingsMapForADate(lSrcDate, sHoldings2, sHoldcash2, sPortbal2,
                                 sPortmain2, sHedgeXref2, sPayrec2, sHoldtot2, FALSE);
  if (zErr.iSqlError == SQLNOTFOUND)
  {
    bSrcExist = FALSE;
    zErr.iSqlError = 0;
  ...
  if (!bRollFromInception)
	{
		if (bSecSpecific)
		{
			if (bSecInHoldings)
			{
				...
				lpprAccountInsertHoldings(iID, sSecNo, sWi, sHoldings1, sHoldings2, bSecSpecific, &zErr);
				if (zErr.iSqlError || zErr.iBusinessError)
			...
			else
			{
				...
				lpprAccountInsertHoldcash(iID, sSecNo, sWi, sHoldcash1, sHoldcash2, bSecSpecific, &zErr);
				if (zErr.iSqlError || zErr.iBusinessError)
		...
		lpprAccountInsertHedgxref(iID, sSecNo, sWi, sHedgeXref1, sHedgeXref2, bSecSpecific, &zErr);
		if (zErr.iSqlError || zErr.iBusinessError)
		...
		lpprAccountInsertPayrec(iID, sSecNo, sWi, sPayrec1, sPayrec2, bSecSpecific, &zErr);
		if (zErr.iSqlError || zErr.iBusinessError)
			return(lpfnPrintError("Error Inserting Record Into Payrec",0, 0, "", 
														0, zErr.iSqlError, zErr.iIsamCode, 
...
**/
    /* Copy source portdir on to destination portdir */
		/*
		** SB 11/14/2012 - Added this additional check to make sure portmain record doesn't get inserted when it's 
  	...
  	if (lSrcDate != lDestDate)
		{
			...
			if (zErr.iSqlError || zErr.iBusinessError)
				return(lpfnPrintError("Error Inserting Record Into Portdir", 0, 0, "",
															0, zErr.iSqlError, zErr.iIsamCode, "ROLL ACCNTINITDEST12", FALSE));
		} 

		// if rolling from inception, set the last trans no in destination table to 0.
		if (bRollFromInception)
		{
			lpprUpdatePortmainLastTransNo(iID, 0, &zErr);
		  if (zErr.iSqlError || zErr.iBusinessError)
				return(lpfnPrintError("Error Inserting Record Into Portdir", 0, 0, "",
															0, zErr.iSqlError, zErr.iIsamCode, "ROLL ACCNTINITDEST13", FALSE));
...
/*
** Function to figure out if the passed security is in Holdings table or Holdcash table.
*/
ERRSTRUCT IsSecurityInHoldings(char *sSecNo, char *sWi, BOOL *pbInHoldings)
{
  ...
  if (zErr.iSqlError || zErr.iBusinessError)
  {
    sprintf_s(sMsg, "Error Reading Assets & SecTypes For Sec No - %s, Wi - %s", sSecNo, sWi);
...
ERRSTRUCT ReadHoldingsMapForADate(long lDate, char *sHoldings, char *sHoldcash, 
																	char *sPortbal, char *sPortmain, char *sHedgxref, 
																	char *sPayrec, char* sHoldtot, BOOL bSaveResult)
{
  ERRSTRUCT   zErr;
  static long lResultDate = -100;
  static char sLastHoldings[STR80LEN], sLastHoldcash[STR80LEN], sLastPortbal[STR80LEN], sLastPortmain[STR80LEN];
  static char sLastHedgxref[STR80LEN], sLastPayrec[STR80LEN], sLastHoldtot[STR80LEN];
  ...
  lpprSelectOneHoldmap(lDate, sHoldings, sHoldcash, sPortmain, sPortbal, sPayrec, sHedgxref, sHoldtot, &zErr);
  if (zErr.iSqlError == SQLNOTFOUND)
    return zErr;
  else if (zErr.iSqlError || zErr.iBusinessError)
    return(lpfnPrintError("Error Reading Holdmap File", 0, 0, "", 0, 
			                    zErr.iSqlError, zErr.iIsamCode, "ROLL READHMAP", FALSE));
...
*/
DLLAPI ERRSTRUCT STDCALL WINAPI InitRoll(char *sDBPath, char *sType, char *sMode, long lAsofDate, char *sErrFile)
{
	int				  iError;
	ERRSTRUCT   zErr;
//	HINSTANCE		hTEngineDll, hOledbIODll, hStarsUtilsDll;
...
//	static BOOL bInit = FALSE;
	...
	if (!bInit)
	{
		...
		lpprInitializeErrStruct = (LPPRERRSTRUCT)GetProcAddress(hTEngineDll, "InitializeErrStruct");
		...
		lpprInitializePortmainStruct = (LPPRPMAINPOINTER)GetProcAddress(hTEngineDll, "InitializePortmainStruct");
		...
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading InitTranProc Function", 0, 0, "", iError, 0, 0, "ROLL INIT1", FALSE));
		}

		lpfnInitTranProc = (LPFN1LONG3PCHAR1BOOL1PCHAR)GetProcAddress(hTEngineDll, "InitTranProc");
		if (!lpfnInitTranProc)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading InitTranProc Function", 0, 0, "", iError, 0, 0, "ROLL INIT2", FALSE));
		}
		...
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading UpdateHold Function", 0, 0, "", iError, 0, 0, "ROLL INIT3", FALSE));
		}
	  ...
	  // Load functions from StarsUtils.dll
		...
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading rmdyjul Function", 0, 0, "", iError, 0, 0, "ROLL INIT4", FALSE));
		}
		...
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading rjulmdy Function", 0, 0, "", iError, 0, 0, "ROLL INIT5", FALSE));
		}

		// Load functions from StarsIO.dll
		lpfnStartDBTransaction = (LPFNVOID)GetProcAddress(hOledbIODll, "StartDBTransaction");
		if (!lpfnStartDBTransaction)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading StartDBTransaction Function", 0, 0, "", iError, 0, 0, "ROLL INIT6", FALSE));
		}
	
		lpfnCommitDBTransaction = (LPFNVOID)GetProcAddress(hOledbIODll, "CommitDBTransaction");
		if (!lpfnCommitDBTransaction)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading CommitDBTransaction Function", 0, 0, "", iError, 0, 0, "ROLL INIT7", FALSE));
		}

		lpfnRollbackDBTransaction = (LPFNVOID)GetProcAddress(hOledbIODll, "RollbackDBTransaction");
		if (!lpfnRollbackDBTransaction)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading RollbackDBTransaction Function", 0, 0, "",  iError, 0, 0, "ROLL INIT8", FALSE));
		}
		...
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading GetTransCount Function", 0, 0, "", iError, 0, 0, "ROLL INIT8a", FALSE));
		}

		lpfnAbortDBTransaction = (LPFN1BOOL)GetProcAddress(hOledbIODll, "AbortDBTransaction");
		if (!lpfnAbortDBTransaction)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading AbortTransaction Function", 0, 0, "", iError, 0, 0, "ROLL INIT8b", FALSE));
		}

		lpfnUnprepareRollQueries = (LPFN1PCHAR1INT)GetProcAddress(hOledbIODll, "UnprepareRollQueries");
		if (!lpfnUnprepareRollQueries)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading UnprepareRollQueries Function", 0, 0, "", iError, 0, 0, "ROLL INIT9", FALSE));
		}

		lpprSelectPortmain = (LPPRPORTMAIN)GetProcAddress(hOledbIODll,"SelectPortmain");
		if (!	lpprSelectPortmain)
		{
		  iError = GetLastError();
			return(lpfnPrintError("Error Loading SelectPortmain Function", 0, 0, "", iError, 0, 0, "ROLL INIT10", FALSE));
		}

		lpprAccountInsertHoldings = (LPPR1LONG4PCHAR1BOOL)GetProcAddress(hOledbIODll, "AccountInsertHoldings");
	  if (!lpprAccountInsertHoldings)
		{
			iError = GetLastError();
	  	return(lpfnPrintError("Error Loading AccountInsertHold Function", 0, 0, "", iError, 0, 0, "ROLL INIT11", FALSE));
		}

		lpprAccountInsertHoldcash = (LPPR1LONG4PCHAR1BOOL)GetProcAddress(hOledbIODll, "AccountInsertHoldcash");
		if (!lpprAccountInsertHoldcash)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading AccountInsertHoldcash Function", 0, 0, "", iError, 0, 0, "ROLL INIT12", FALSE));
		}

		lpprAccountInsertHedgxref = (LPPR1LONG4PCHAR1BOOL)GetProcAddress(hOledbIODll, "AccountInsertHedgxref");
		if (!lpprAccountInsertHedgxref)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading AccountInsertHedgxref Function", 0, 0, "", iError, 0, 0, "ROLL INIT13", FALSE));
	  }

		lpprAccountInsertPayrec = (LPPR1LONG4PCHAR1BOOL)GetProcAddress(hOledbIODll, "AccountInsertPayrec");
	  if (!lpprAccountInsertPayrec)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading AccountInsertPayrec Function", 0, 0, "", iError, 0, 0, "ROLL INIT14", FALSE));
		}

		/*lpprAccountInsertPortbal = (LPPR1LONG2PCHAR)GetProcAddress(hOledbIODll, "AccountInsertPortbal");
		if(!lpprAccountInsertPortbal)
		{
			iError = GetLastError();
		...
		lpprAccountInsertPortmain = (LPPR1LONG2PCHAR)GetProcAddress(hOledbIODll, "AccountInsertPortmain");
		if(!lpprAccountInsertPortmain)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading AccountInsertPortmain Function", 0, 0, "", iError, 0, 0, "ROLL INIT15", FALSE));
		}

		lpprAccountDeleteHoldings = (LPPR1LONG3PCHAR1BOOL) GetProcAddress(hOledbIODll, "AccountDeleteHoldings");
		if(!lpprAccountDeleteHoldings)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading AccountDeleteHoldings Function", 0, 0, "", iError, 0, 0, "ROLL INIT16", FALSE));
		}

		lpprAccountDeleteHoldcash = (LPPR1LONG3PCHAR1BOOL) GetProcAddress(hOledbIODll, "AccountDeleteHoldcash");
		if(!lpprAccountDeleteHoldcash)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading AccountDeleteHoldcash Function", 0, 0, "", iError, 0, 0, "ROLL INIT17", FALSE));
		}

		lpprAccountDeleteHedgxref = (LPPR1LONG3PCHAR1BOOL) GetProcAddress(hOledbIODll, "AccountDeleteHedgxref");
		if(!lpprAccountDeleteHedgxref)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading AccountDeleteHedgxref Function", 0, 0, "", iError, 0, 0, "ROLL INIT18", FALSE));
		}

		lpprAccountDeletePayrec = (LPPR1LONG3PCHAR1BOOL) GetProcAddress(hOledbIODll, "AccountDeletePayrec");
		if(!lpprAccountDeletePayrec)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading AccountDeletePayrec Function", 0, 0, "", iError, 0, 0, "ROLL INIT19", FALSE));
		}
	
		lpprUpdatePortmainLastTransNo = (LPPR1INT1LONG)GetProcAddress(hOledbIODll, "UpdatePortmainLastTransNo");
    if (!lpprUpdatePortmainLastTransNo)
	  {
			iError = GetLastError();
			return(lpfnPrintError("Unable To Load UpdatePortmainLastTransNo function", 0, 0, "", iError, 0, 0, "ROLL INIT20", FALSE));
	  }

		/*lpprAccountDeletePortbal = (LPPR1LONG1PCHAR) GetProcAddress(hOledbIODll, "AccountDeletePortbal");
		if(!lpprAccountDeletePortbal)
		{
			iError = GetLastError();
	  ...
	  lpprAccountDeletePortmain = (LPPR1LONG1PCHAR) GetProcAddress(hOledbIODll, "AccountDeletePortmain");
		if(!lpprAccountDeletePortmain)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading AccountDeletePortmain Function", 0, 0, "", iError, 0, 0, "ROLL INIT20", FALSE));
		}

		lpprSelectAllHoldmap = (LPPR8PCHAR1PLONG)GetProcAddress(hOledbIODll, "ReadAllHoldmap");
		if(!lpprSelectAllHoldmap)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading FetchHMapCursor Function", 0, 0, "", iError, 0, 0, "ROLL INIT21", FALSE));
		}
		
		lpprAsofTransCount = (LPPR3LONG2PCHAR1BOOL1PINT) GetProcAddress(hOledbIODll, "SelectAsofTransCount");
		if(!lpprAsofTransCount)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading AsofTransCount", 0, 0, "", iError, 0, 0, "ROLL INIT22", FALSE));
		}
	
		lpprBackwardTrans = (LPPR3LONG2PCHAR1BOOL1PTRANS)GetProcAddress(hOledbIODll, "SelectBackwardTrans");
		if (!lpprBackwardTrans)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading BackwardTrans Function", 0, 0, "", iError, 0, 0, "ROLL INIT23", FALSE));
		} 
  
		lpprForwardTrans = (LPPR3LONG2PCHAR1BOOL1PTRANS)GetProcAddress(hOledbIODll, "SelectForwardTrans");
		if (!lpprForwardTrans)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading ForwardTrans Function", 0, 0, "", iError, 0, 0, "ROLL INIT24", FALSE));
		}  
		
		lpprRegTransCount = (LPPR3LONG2PCHAR1BOOL1PINT) GetProcAddress(hOledbIODll, "SelectRegularTransCount");
		if(!lpprRegTransCount)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading RegTransCount", 0, 0, "", iError, 0, 0, "ROLL INIT25", FALSE));
		}

		lpprDestUpd1Portmain = (LPPR3LONG1PCHAR) GetProcAddress(hOledbIODll, "UpdateRollDateAndLastTransNo");
		if(!lpprDestUpd1Portmain)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading DestUpd1Portmain", 0, 0, "", iError, 0, 0, "ROLL INIT26", FALSE));
		}

		lpprDestUpd2Portmain = (LPPR2LONG1PCHAR) GetProcAddress(hOledbIODll, "UpdateRollDate");
		if(!lpprDestUpd2Portmain)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading DestUpd2Portmain", 0, 0, "", iError, 0, 0, "ROLL INIT27", FALSE));
		}
	
		lpprSelectOneHoldmap = (LPPR1LONG7PCHAR)GetProcAddress(hOledbIODll, "SelectHoldmap");
		if(!lpprSelectOneHoldmap)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading SelectHoldmap", 0, 0, "", iError, 0, 0, "ROLL INIT28", FALSE));
		}

		lpprSelectStarsDate = (LPPR2PLONG)GetProcAddress(hOledbIODll, "SelectStarsDate");
		if(!lpprSelectStarsDate)
		{
	 		iError = GetLastError();
			return(lpfnPrintError("Error Loading SelectStarsDate", 0, 0, "", iError, 0, 0, "ROLL INIT29", FALSE));
		}

		lpprAsofTrans = (LPPR3LONG2PCHAR1BOOL1PTRANS) GetProcAddress(hOledbIODll, "SelectAsofTrans");
		if(!lpprAsofTrans)
		{
	 		iError = GetLastError();
			return(lpfnPrintError("Error Loading AsofTrans", 0, 0, "", iError, 0, 0, "ROLL INIT30", FALSE));
		}

		lpprSrcePortmain =  (LPPR1LONG1PCHAR1PLONG) GetProcAddress(hOledbIODll, "SelectLastTransNo");
		if(!lpprSrcePortmain)
		{
	 		iError = GetLastError();
			return(lpfnPrintError("Error Loading SrcePortmain", 0, 0, "", iError, 0, 0, "ROLL INIT31", FALSE));
		}

		lpprSelectRollDate = (LPPR1LONG1PCHAR1PLONG)GetProcAddress(hOledbIODll, "SelectRollDate");
		if (!lpprSelectRollDate)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading SrcePortmain", 0, 0, "", iError, 0, 0, "ROLL INIT31A", FALSE));
		}

		lpprSelectPositionIndicator = (LPPR3PCHAR)GetProcAddress(hOledbIODll, "SelectPositionIndicator");
		if(!lpprSelectPositionIndicator)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading SelectPositionIndicator", 0, 0, "", iError, 0, 0, "ROLL INIT32", FALSE));
		}

		lpfnIsItAMonthEnd = (LPFN1LONG)GetProcAddress(hStarsUtilsDll,"IsItAMonthEnd");   
		if(!lpfnIsItAMonthEnd)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading IsItAMonthEnd", 0, 0, "", iError, 0, 0, "ROLL INIT33", FALSE));
		}

		lpfnLastMonthEnd = (LP2FN1LONG)GetProcAddress(hStarsUtilsDll,"LastMonthEnd");   
		if(!lpfnLastMonthEnd)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading LastMonthEnd", 0, 0, "", iError, 0, 0, "ROLL INIT34", FALSE));
		}

		lpprUpdatePerfDate = (LPPR3LONG)GetProcAddress(hOledbIODll,"UpdatePerfDate");
		if(!lpprUpdatePerfDate)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading UpdatePerfDate", 0, 0, "", iError, 0, 0, "ROLL INIT35", FALSE));
		}
		
    lpprSelectTrndesc	= (LPPRDTRANSDESCSELECT)GetProcAddress(hOledbIODll, "SelectTransDesc");
		if(!lpprSelectTrndesc)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading SelectTrndesc", 0, 0, "", iError, 0, 0, "ROLL INIT36", FALSE));
		}

		lpprSelectTransBySettlementDate = (LPPRSELECTTRANS)GetProcAddress(hOledbIODll, "SelectTransBySettlementDate");
		if (!lpprSelectTransBySettlementDate)
		{
			iError = GetLastError();
			return(lpfnPrintError("Error Loading SelectTransBySettlementDate", 0, 0, "", iError, 0, 0, "ROLL INIT37", FALSE));
		}

    /*lpfnCompInsHedgxref = (LPFNCOMPINSHOLD)GetProcAddress(hOledbIODll, "CompInsHedgxref");
		if(!lpfnCompInsHedgxref)
		{
		 	iError = GetLastError();
	  	return(lpfnPrintError("Error Loading CompInsHedgxref Function", 0, 0, "", 
													iError, 0, 0, "ROLL INIT8", FALSE));
	 	}
		lpfnCompInsPortmain  = (LPFNCOMPINSHOLD)GetProcAddress(hOledbIODll, "CompInsPortmain");
		if(!lpfnCompInsPortmain)
		{
		 	iError = GetLastError();
	  	return(lpfnPrintError("Error Loading CompInsPortmain Function", 0, 0, "", 
													iError, 0, 0, "ROLL INIT8", FALSE));
		}
		lpfnCompInsPrec  = (LPFNCOMPINSHOLD)GetProcAddress(hOledbIODll, "CompInsPrec");
		if(!lpfnCompInsPayrec)
		{
		 	iError = GetLastError();
	  	return(lpfnPrintError("Error Loading CompInsPrec Function", 0, 0, "", 
													iError, 0, 0, "ROLL INIT8", FALSE));
		}
		lpfnCompInsPbal  = (LPFNCOMPINSHOLD)GetProcAddress(hOledbIODll, "CompInsPbal");
		if(!lpfnCompInsPortbal)
		{
		 	iError = GetLastError();
	  	return(lpfnPrintError("Error Loading CompInsPbal Function", 0, 0, "", 
			 										iError, 0, 0, "ROLL INIT8", FALSE));
		}
		lpfnCompInsHcash = (LPFNCOMPINSHOLD)GetProcAddress(hOledbIODll, "CompInsHcash");
		if(!lpfnCompInsHoldcash)
		{
		 	iError = GetLastError();
	  	return(lpfnPrintError("Error Loading CompInsHcash Function", 0, 0, "", 
													iError, 0, 0, "ROLL INIT8", FALSE));
		}
		lpfnCompInsHold  = (LPFNCOMPINSHOLD)GetProcAddress(hOledbIODll, "CompInsHold");
		if(!lpfnCompInsHoldings)
		{
		 	iError = GetLastError();
	  	return(lpfnPrintError("Error Loading CompInsHold Function", 0, 0, "", 
													iError, 0, 0, "ROLL INIT8", FALSE));
...
ERRSTRUCT CallInitTranProc(long lAsofDate, char *sDBAlias, char *sMode, char *sType, char *sErrFile)
{
	...
	zErr = lpfnInitTranProc(lAsofDate, sLastDBAlias, sLastMode, sLastType, FALSE, sLastErrFile);
	if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
	{
		bInit = FALSE;
		return zErr;
...
//		lpfnFirmAcct( &pzPR, sTemp, &zErr);
  ...
  if (zErr.iSqlError && zErr.iSqlError != SQLNOTFOUND)
    zErr = lpfnPrintError("Error Fetching Account", pzPR->iID, 0, "", 0, 
...
ERRSTRUCT FirmDestInit(char *sSecNo, char *sWi, long lSrcDate, long lDestDate)
{
  ...
  BOOL      bSrcExist, bSecSpecific, bSecInHoldings;
  int       iSecStatus;
  char			sHoldings[STR80LEN], sHoldcash[STR80LEN], sPortbal[STR80LEN], sPortmain[STR80LEN];
  char			sHedgeXref[STR80LEN], sPayrec[STR80LEN], sHoldtot[STR80LEN];
  char			sHoldings1[STR80LEN], sHoldcash1[STR80LEN], sPortbal1[STR80LEN], sPortmain1[STR80LEN];
  char			sHedgeXref1[STR80LEN], sPayrec1[STR80LEN], sHoldtot1[STR80LEN], sHoldings2[STR80LEN], sHoldcash2[STR80LEN];
  char			sPortbal2[STR80LEN], sPortmain2[STR80LEN], sHedgeXref2[STR80LEN], sPayrec2[STR80LEN], sHoldtot2[STR80LEN];
  ...
  else
  {
  ...
  else
	{
		...
		zErr = ReadHoldingsMapForADate(lDestDate, sHoldings, sHoldcash, sPortbal, 
																	 sPortmain, sHedgeXref, sPayrec, sHoldtot, TRUE);
		if (zErr.iSqlError == SQLNOTFOUND || strncmp(sHoldings, "holdings", 8) == 0)
		{
			/*
			...
			*/
			if (lDestDate == ADHOC_DATE || lDestDate == PERFORM_DATE)
				return(lpfnPrintError("No Adhoc/Reorg Data Set Exists", 0, 0, "", 603, 0, 0, 
															"ROLL FIRMDEL1", FALSE));
			else
			{
				lDestDate = ADHOC_DATE;
...
//      lpfnFirmDelHold(sSecNo, sWi, sHoldings, bSecSpecific, &zErr); 
  ...
  //    lpfnFirmDelHcash ( sSecNo, sWi, sHoldcash, bSecSpecific, &zErr);
    ...
    if (zErr.iSqlError || zErr.iBusinessError)
      return(lpfnPrintError("Error Deleting Record From Payrec", 0, 0, "", 0,
                        zErr.iSqlError, zErr.iIsamCode, "ROLL FIRMINITDEST4.0",
                        FALSE));
  }
  else
  {
    ...
    if (zErr.iSqlError || zErr.iBusinessError)
      return(lpfnPrintError("Error Deleting Record From Payrec", 0, 0, "", 0,
                        zErr.iSqlError, zErr.iIsamCode, "ROLL FIRMINITDEST4.1",
                          FALSE));
  }
  ...
  if (zErr.iSqlError || zErr.iBusinessError)
    return(lpfnPrintError("Error Deleting Record From Portbal", 0, 0, "",
                      0, zErr.iSqlError, zErr.iIsamCode, 
                      "ROLL FIRMINITDEST5", FALSE));
  /* delete portdir */
  //lpfnFirmDelPortmain(sPortmain, bSecSpecific, &zErr);
  if (zErr.iSqlError || zErr.iBusinessError)
    return(lpfnPrintError("Error Deleting Record From Portdir", 0, 0, "",
                      0, zErr.iSqlError, zErr.iIsamCode, 
                      "ROLL FIRMINITDEST6", FALSE));
  ...
  if (lSrcDate == lDestDate)
    ...
    zErr = ReadHoldingsMapForADate(lDestDate, sHoldings1, sHoldcash1, sPortbal1, 
                                 sPortmain1, sHedgeXref1, sPayrec1, sHoldtot1, FALSE);
		if (zErr.iSqlError == SQLNOTFOUND)
			return(lpfnPrintError("No Destination Data Set Exists", 0, 0, "", 603, 0, 0, 
                      "ROLL FIRMINSERT1", FALSE));
		else if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
      return zErr;
	}
...
//      lpfnFirmInsHold(sSecNo, sWi, iSecStatus, sHoldings1, sHoldings2, &zErr);
  ...
  //  lpfnFirmInsHedgxref(sSecNo, sWi, iSecStatus, sHedgeXref1, sHedgeXref2, &zErr);
    if (zErr.iSqlError || zErr.iBusinessError)
      return(lpfnPrintError("Error Inserting Record Into HedgeXref", 0, 0,
                        "", 0, zErr.iSqlError, zErr.iIsamCode, 
	...
	{	
		...
		if (zErr.iSqlError == SQLNOTFOUND || strncmp(sHoldings, "holdings", 8) == 0)
		{
			...
			*/
			...
			{
				lDestDate = ADHOC_DATE;
				zErr = ReadHoldingsMapForADate(lDestDate, sHoldings, sHoldcash, sPortbal, sPortmain, 
																			sHedgeXref, sPayrec, sHoldtot, TRUE);
				if (zErr.iSqlError == SQLNOTFOUND)
					return(lpfnPrintError("No Ad Hoc Data Set Exists", 0, 0, "", 603, 0, 0, 
                          "ROLL COMPDEL2", FALSE));
				else if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
		...
		if (zErr.iSqlError || zErr.iBusinessError)
			return(lpfnPrintError("Error Deleting Record From Payrec", 0, 0, "", 0,
                      zErr.iSqlError, zErr.iIsamCode, "ROLL COMPINITDEST4",
                        FALSE));
...
//  char sStatement[150];
  ...
  if (lDestDate == PERFORM_DATE)
	...
	{	
		...
		if (zErr.iSqlError || zErr.iBusinessError)
			return(lpfnPrintError("Error Deleting Record From Payrec", 0, 0, "", 0,
                      zErr.iSqlError, zErr.iIsamCode, "ROLL MGRINITDEST4",
                        FALSE));
...
ERRSTRUCT GetFSecBrAcct(PORTMAIN *pzPR)
{
  ...
  //EXEC SQL CLOSE FSEC_ACCT_CURSOR;
    ...
    zErr = lpfnPrintError("Error Fetching BrAcct From Cursor", 0, 0, "", 0,
...
********************************************************************************/
ERRSTRUCT CreateFSecBrAcctCursor(char *sSecNo, char *sWi, long lRollDate, long lSrceDate, long lDestDate)
{
  ERRSTRUCT   zErr;
  BOOL        bSecInHoldings;
  static BOOL bTableExists = FALSE;
    ...
    char sHoldings1[STR80LEN], sHoldcash1[STR80LEN], sHoldings2[STR80LEN], sHoldcash2[STR80LEN];
  ...
  */
  if (bSecInHoldings)
    sprintf_s(sStatement, "INSERT INTO FSEC_ACCT_TABLE SELECT br_acct FROM pmr:%s"
                        " WHERE sec_no = ? AND wi = ?", sHoldings2);
  else
    sprintf_s(sStatement, "INSERT INTO FSEC_ACCT_TABLE SELECT br_acct FROM pmr:%s"
                        " WHERE sec_no = ? AND wi = ? ", sHoldcash2);

  //EXEC SQL PREPARE FSEC_ACCT_ST1 FROM :sStatement;
  if (zErr.iSqlError || zErr.iBusinessError)
    return(lpfnPrintError("Error Preparing Statement To Read Holdings Table ", 0,
                      0, "", 0, zErr.iSqlError, zErr.iIsamCode,
                      "ROLL CREATEACCT4", FALSE));

  //EXEC SQL EXECUTE FSEC_ACCT_ST1 USING :sSecNo, :sWi;
  if (zErr.iSqlError || zErr.iBusinessError)
    return(lpfnPrintError("Error Executing Statement To Read Holdings Table ", 0,
                      0, "", 0, zErr.iSqlError, zErr.iIsamCode,
  ...
  */
  ...
  if (zErr.iSqlError || zErr.iBusinessError)
    return(lpfnPrintError("Error Preparing Statement To Read Trans Table", 0, 0,
                      "", 0, zErr.iSqlError, zErr.iIsamCode,
  ...
  if (zErr.iSqlError || zErr.iBusinessError)
    return(lpfnPrintError("Error Executing Statement To Read Trans Table", 0, 0,
                      "", 0, zErr.iSqlError, zErr.iIsamCode,
 ...
 // EXEC SQL OPEN FSEC_ACCT_CURSOR;
  if (zErr.iSqlError || zErr.iBusinessError)
    return(lpfnPrintError("Error Opening Temp Table To Read Branch Accounts", 0, 0,
                      "", 0, zErr.iSqlError, zErr.iIsamCode,

...
// SelectOneAccdiv
// ============================================================================
DLLAPI void SelectOneAccdiv(ACCDIV *pzAccdiv, int iID, long lDivintNo,
  ...
  try {
    ...
    if (result.next()) {
      ...
      pzAccdiv->lEntryDate =
          timestamp_to_long(result.get<nanodbc::timestamp>("entry_date"));

      read_string(result, "curr_id", pzAccdiv->sCurrId,
                  sizeof(pzAccdiv->sCurrId));
    ...
    *pzErr = PrintError((char *)e.what(), iID, 0, "", 0, -1, 0,
                        "SelectOneAccdiv", FALSE);
  }
}
...
DLLAPI void SelectPartCurrency(PARTCURR *pzCurrency, ERRSTRUCT *pzErr) {
  ...
  try {
    ...
    if (g_selectPartCurrencyState.result &&
        g_selectPartCurrencyState.result->next()) {
      g_selectPartCurrencyState.cRows++;
      read_string(*g_selectPartCurrencyState.result, "curr_id",
                  pzCurrency->sCurrId, sizeof(pzCurrency->sCurrId));
      read_string(*g_selectPartCurrencyState.result, "sec_no",
                  pzCurrency->sSecNo, sizeof(pzCurrency->sSecNo));
      read_string(*g_selectPartCurrencyState.result, "when_issue",
                  pzCurrency->sWi, sizeof(pzCurrency->sWi));
      pzCurrency->fCurrExrate =
          g_selectPartCurrencyState.result->get<double>("cur_exrate", 0.0);
    } else {
      pzErr->iSqlError = SQLNOTFOUND;
      g_selectPartCurrencyState.cRows = 0;
    ...
    *pzErr = PrintError((char *)e.what(), 0, 0, "", 0, -1, 0,
                        "SelectPartCurrency", FALSE);
    g_selectPartCurrencyState.result.reset();
  }
}
...
DLLAPI void SelectAllPartPortmain(PARTPMAIN *pzPortmain, ERRSTRUCT *pzErr) {
  ...
  try {
    ...
    if (g_selectAllPartPortmainState.result &&
        g_selectAllPartPortmainState.result->next()) {
      ...
      read_string(*g_selectAllPartPortmainState.result, "unique_name",
                  pzPortmain->sUniqueName, sizeof(pzPortmain->sUniqueName));
      pzPortmain->iFiscalMonth = g_selectAllPartPortmainState.result->get<int>(
          "FiscalYearEndMonth", 0);
      pzPortmain->iFiscalDay =
          g_selectAllPartPortmainState.result->get<int>("FiscalYearEndDay", 0);

      pzPortmain->lInceptionDate = timestamp_to_long(
          g_selectAllPartPortmainState.result->get<nanodbc::timestamp>(
              "InceptionDate"));
      read_string(*g_selectAllPartPortmainState.result, "AcctMethod",
                  pzPortmain->sAcctMethod, sizeof(pzPortmain->sAcctMethod));
      read_string(*g_selectAllPartPortmainState.result, "BaseCurrId",
                  pzPortmain->sBaseCurrId, sizeof(pzPortmain->sBaseCurrId));

      char sTemp[1 + NT];
      read_string(*g_selectAllPartPortmainState.result, "Income", sTemp,
                  sizeof(sTemp));
      pzPortmain->bIncome = Char2BOOL(sTemp);
      read_string(*g_selectAllPartPortmainState.result, "Actions", sTemp,
                  sizeof(sTemp));
      pzPortmain->bActions = Char2BOOL(sTemp);
      read_string(*g_selectAllPartPortmainState.result, "Mature", sTemp,
                  sizeof(sTemp));
      pzPortmain->bMature = Char2BOOL(sTemp);
      read_string(*g_selectAllPartPortmainState.result, "CAvail", sTemp,
                  sizeof(sTemp));
      pzPortmain->bCAvail = Char2BOOL(sTemp);
      read_string(*g_selectAllPartPortmainState.result, "FAvail", sTemp,
                  sizeof(sTemp));
      pzPortmain->bFAvail = Char2BOOL(sTemp);

      pzPortmain->lValDate = timestamp_to_long(
          g_selectAllPartPortmainState.result->get<nanodbc::timestamp>(
              "ValDate"));
      pzPortmain->lDeleteDate = timestamp_to_long(
          g_selectAllPartPortmainState.result->get<nanodbc::timestamp>(
              "DeleteDate"));
      read_string(*g_selectAllPartPortmainState.result, "IsInactive", sTemp,
                  sizeof(sTemp));
      pzPortmain->bIsInactive = Char2BOOL(sTemp);
      read_string(*g_selectAllPartPortmainState.result, "AmortMuni", sTemp,
                  sizeof(sTemp));
      pzPortmain->bAmortMuni = Char2BOOL(sTemp);
      read_string(*g_selectAllPartPortmainState.result, "AmortOther", sTemp,
                  sizeof(sTemp));
      pzPortmain->bAmortOther = Char2BOOL(sTemp);

      pzPortmain->lAmortStart = timestamp_to_long(
          g_selectAllPartPortmainState.result->get<nanodbc::timestamp>(
              "AmortStartDate"));
      read_string(*g_selectAllPartPortmainState.result, "IncByLot", sTemp,
                  sizeof(sTemp));
      pzPortmain->bIncByLot = Char2BOOL(sTemp);
      pzPortmain->lPurgeDate = timestamp_to_long(
          g_selectAllPartPortmainState.result->get<nanodbc::timestamp>(
              "PurgeDate"));
      pzPortmain->lRollDate = timestamp_to_long(
          g_selectAllPartPortmainState.result->get<nanodbc::timestamp>(
              "RollDate"));
      read_string(*g_selectAllPartPortmainState.result, "Tax", pzPortmain->sTax,
                  sizeof(pzPortmain->sTax));
      pzPortmain->iPortfolioType =
          g_selectAllPartPortmainState.result->get<int>("PortfolioType", 0);
      pzPortmain->lPricingEffectiveDate = timestamp_to_long(
          g_selectAllPartPortmainState.result->get<nanodbc::timestamp>(
              "PricingEffectiveDate"));

      pzPortmain->fMaxEqPct =
          g_selectAllPartPortmainState.result->get<double>("MaxEqPct", 0.0);
      pzPortmain->fMaxFiPct =
          g_selectAllPartPortmainState.result->get<double>("MaxFiPct", 0.0);
      pzPortmain->iVendorID =
          g_selectAllPartPortmainState.result->get<int>("DefaultVendorID", 0);
      pzPortmain->iReturnsToCalculate =
          g_selectAllPartPortmainState.result->get<int>("ReturnsToCalculate",
                                                        0);
      pzPortmain->iRorType =
          g_selectAllPartPortmainState.result->get<int>("DefaultReturnType", 0);

      // Column 30: case when mi.id is null then 'N' else 'Y' end
      // nanodbc access by index (0-based) or name. The query doesn't name this
      // column. Let's assume it's the 30th column (index 29). Wait, nanodbc get
      // by index is safer if name is missing. Indices: Id(0)...
      // DefaultReturnType(28), Case(29), AccretMuni(30), AccretOther(31)

      std::string sIsMarketIndex =
          g_selectAllPartPortmainState.result->get<std::string>(29, "N");
      pzPortmain->bIsMarketIndex = (sIsMarketIndex == "Y");

      read_string(*g_selectAllPartPortmainState.result, "AccretMuni", sTemp,
                  sizeof(sTemp));
      pzPortmain->bAccretMuni = Char2BOOL(sTemp);
      read_string(*g_selectAllPartPortmainState.result, "AccretOther", sTemp,
                  sizeof(sTemp));
      pzPortmain->bAccretOther = Char2BOOL(sTemp);
    } else {
      pzErr->iSqlError = SQLNOTFOUND;
      g_selectAllPartPortmainState.cRows = 0;
...
// ============================================================================
DLLAPI void SelectOnePartPortmain(PARTPMAIN *pzPortmain, int iID,
                                  ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);
  if (!gConn.connected())
    return;
  ...
  try {
    ...
    if (result.next()) {
      ...
      read_string(result, "unique_name", pzPortmain->sUniqueName,
                  sizeof(pzPortmain->sUniqueName));
      pzPortmain->iFiscalMonth = result.get<int>("FiscalYearEndMonth", 0);
      pzPortmain->iFiscalDay = result.get<int>("FiscalYearEndDay", 0);

      pzPortmain->lInceptionDate =
          timestamp_to_long(result.get<nanodbc::timestamp>("InceptionDate"));
      read_string(result, "AcctMethod", pzPortmain->sAcctMethod,
                  sizeof(pzPortmain->sAcctMethod));
      read_string(result, "BaseCurrId", pzPortmain->sBaseCurrId,
                  sizeof(pzPortmain->sBaseCurrId));

      char sTemp[1 + NT];
      read_string(result, "Income", sTemp, sizeof(sTemp));
      pzPortmain->bIncome = Char2BOOL(sTemp);
      read_string(result, "Actions", sTemp, sizeof(sTemp));
      pzPortmain->bActions = Char2BOOL(sTemp);
      read_string(result, "Mature", sTemp, sizeof(sTemp));
      pzPortmain->bMature = Char2BOOL(sTemp);
      read_string(result, "CAvail", sTemp, sizeof(sTemp));
      pzPortmain->bCAvail = Char2BOOL(sTemp);
      read_string(result, "FAvail", sTemp, sizeof(sTemp));
      pzPortmain->bFAvail = Char2BOOL(sTemp);

      pzPortmain->lValDate =
          timestamp_to_long(result.get<nanodbc::timestamp>("ValDate"));
      pzPortmain->lDeleteDate =
          timestamp_to_long(result.get<nanodbc::timestamp>("DeleteDate"));
      read_string(result, "IsInactive", sTemp, sizeof(sTemp));
      pzPortmain->bIsInactive = Char2BOOL(sTemp);
      read_string(result, "AmortMuni", sTemp, sizeof(sTemp));
      pzPortmain->bAmortMuni = Char2BOOL(sTemp);
      read_string(result, "AmortOther", sTemp, sizeof(sTemp));
      pzPortmain->bAmortOther = Char2BOOL(sTemp);

      pzPortmain->lAmortStart =
          timestamp_to_long(result.get<nanodbc::timestamp>("AmortStartDate"));
      read_string(result, "IncByLot", sTemp, sizeof(sTemp));
      pzPortmain->bIncByLot = Char2BOOL(sTemp);
      pzPortmain->lPurgeDate =
          timestamp_to_long(result.get<nanodbc::timestamp>("PurgeDate"));
      pzPortmain->lRollDate =
          timestamp_to_long(result.get<nanodbc::timestamp>("RollDate"));
      read_string(result, "Tax", pzPortmain->sTax, sizeof(pzPortmain->sTax));
      pzPortmain->iPortfolioType = result.get<int>("PortfolioType", 0);
      pzPortmain->lPricingEffectiveDate = timestamp_to_long(
          result.get<nanodbc::timestamp>("PricingEffectiveDate"));
      ...
      pzPortmain->iReturnsToCalculate =
          result.get<int>("ReturnsToCalculate", 0);
      pzPortmain->iRorType = result.get<int>("DefaultReturnType", 0);

      std::string sIsMarketIndex = result.get<std::string>(29, "N");
      pzPortmain->bIsMarketIndex = (sIsMarketIndex == "Y");

      read_string(result, "AccretMuni", sTemp, sizeof(sTemp));
      pzPortmain->bAccretMuni = Char2BOOL(sTemp);
      read_string(result, "AccretOther", sTemp, sizeof(sTemp));
      pzPortmain->bAccretOther = Char2BOOL(sTemp);
    } else {
      pzErr->iSqlError = SQLNOTFOUND;
    }
...
DLLAPI void SelectAllSubacct(char *sAcctType, char *sXrefAcctType,
                             ERRSTRUCT *pzErr) {
  InitializeErrStruct(pzErr);
  if (!gConn.connected())
    return;
  ...
  try {
    ...
    if (g_selectAllSubacctState.result &&
        g_selectAllSubacctState.result->next()) {
      g_selectAllSubacctState.cRows++;
      read_string(*g_selectAllSubacctState.result, "acct_type", sAcctType,
                  STR1LEN);
      read_string(*g_selectAllSubacctState.result, "xref_acct_type",
                  sXrefAcctType, STR1LEN);
    } else {
      pzErr->iSqlError = SQLNOTFOUND;
      g_selectAllSubacctState.cRows = 0;
...
int GetSelectPortfolioRangeID() { return g_selectPortfolioRangeState.iID; }
...
HRESULT MoveNextSelectPortfolioRange() {
  ...
  try {
    ...
    if (g_selectPortfolioRangeState.result &&
        g_selectPortfolioRangeState.result->next()) {
      g_selectPortfolioRangeState.cRows++;
      g_selectPortfolioRangeState.iID =
          g_selectPortfolioRangeState.result->get<int>("id", 0);
      return S_OK;
    } else {
      g_selectPortfolioRangeState.cRows = 0;

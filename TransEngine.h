...
typedef void			(CALLBACK* LPPRTRANSSELECT)(TRANS *, TRANTYPE *, int, long, long, char*, char*, ERRSTRUCT *);
...
extern LPPRSELECTHOLDINGS							lpprHoldcashForFifoAndAvgAcct, lpprHoldcashForLifoAcct, lpprHoldcashForHighAcct, 
...
extern LPPRSELECTHOLDINGS2						lpprHoldcashForMinimumGainAcct, lpprHoldcashForMaximumGainAcct,
																			lpprHoldingsForMinimumGainAcct, lpprHoldingsForMaximumGainAcct; 
extern LPPRSELECTHOLDINGS3						lpprMinimumGainHoldings, lpprMinimumLossHoldings, lpprMaximumLossHoldings; 
...
//extern LPPRSELECTPORTBAL						lpprSelectPortbal;
//extern LPPRUPDATEPORTBAL						lpprUpdatePortbal;
...
ERRSTRUCT ProcessMoneyInHoldings(TRANS zTR, long lCashImpact, double fCashAdj, double fBaseXrate, 
																 double fSysXrate, char *sCurPrior, BOOL bReversal, BOOL bIncByLot);
...
ERRSTRUCT ProcessSplitInHoldings(TRANS zTR, long lSecImpact, char *sAutoGen,
                                 BOOL bReversal, char *sCurPrior);
ERRSTRUCT ProcessAdjustmentInHoldings(TRANS *pzTR, PORTMAIN zPR, long lSecImpact, long lCashImpact, 
                                      BOOL bReversal, char *sCurPrior);
...
ERRSTRUCT ProcessTransferInHoldcash(TRANS zTR, PORTMAIN zPR, long lCashImpact, 
									int iDateInd, BOOL bReversal, char *sCurPrior);
ERRSTRUCT ProcessAverageInHoldings(TRANS *pzTR, double fTradingUnits, double *pzSysXrate, BOOL bReversal,
																	 char *sCostInd);
...
void      CopyFieldsFromHoldingsToHolddel(HOLDDEL *pzHD, HOLDINGS zHR);
void      CopyFieldsFromHolddelToHoldings(HOLDINGS *pzHR, HOLDDEL zHD);
void      CalculateNewCostXRate(double fHoldCost, double fTransCost, 
                                double fHoldCXR, double fTransCXR, double *pfNewXrate);
void      CalculateCurrencyGainLoss(double fTCost, double fBaseCXR, double fMV, 
                                    double fMVBaseCXR, double *pfCurrencyGL);
ERRSTRUCT GetSecurityCharacteristics(char *sSecNo, char *sWi, char *sPType, char *sSType, 
																			char *sPostnInd, char *sLotInd, char *sCostInd, 
																			char *sLotExistInd, char *sAvgInd, double *pfTrdUnit, char *sCurrId);
DLLAPI int STDCALL WINAPI SetErrorFileName(char *sErrFile);
DLLAPI ERRSTRUCT STDCALL WINAPI PrintError(char *sErrMsg, int, long lRecNo, char *sRecType,
								                      int iBusinessErr, int iSqlErr, int iIsamCode, 
																      char *sMsgIdentifier, BOOL bWarning);
...
DLLAPI ERRSTRUCT STDCALL WINAPI TranAlloc(TRANS *pzTrans, TRANTYPE zTranType, SECTYPE zSecType, ASSETS zAssets, 
																		 DTRANSDESC zDTranDesc[], int iNumDTItems, PAYTRAN *pzPayTran,
																		 char *sCurrPrior, BOOL bDoTransaction);
ERRSTRUCT ProcessOpenInTranAlloc(TRANS *pzTrans2, SECTYPE zSecType, ASSETS zAssets, 
																 TRANTYPE zTranType, DTRANSDESC zDTransDesc[], int iNumDTItems,
																 PAYTRAN *pzPayTran,
																 BOOL *bExerciseFlag, char *sCurrPrior, double *pzExerciseUnits, BOOL bDoTransaction);
...
ERRSTRUCT ProcessIncomeInTranAlloc(TRANS *pzTrans, DTRANSDESC zDTransDesc[], int iNumDTItems, 
																	 char *sCurrPrior, BOOL bDoTransaction);
...
ERRSTRUCT ProcessCancelInTranAlloc(TRANS *pzTrans3, DTRANSDESC zDTransDesc[], int iNumDTItems, 
																	 char *sCurrPrior, BOOL bDoTransaction);
...
ERRSTRUCT DecipherDescInfo(long *lOpenDate, double *pzSellUnits, double *pzUnitCost, char *sDescInfo, 
...
ERRSTRUCT ExerciseOptionInTranAlloc(TRANS *pzTrans, double fExerciseUnits, TRANTYPE zTranType,
                                    SECTYPE zSecType, ASSETS zAssets, 
																		DTRANSDESC zDTransDesc[], int iNumDTItems, 
																		char *sCurrPrior, BOOL bDoTransaction);
ERRSTRUCT CalcOptionPremium(TRANS zTrans, double *pzExerciseUnits, double *pzOptPrem);
...
ERRSTRUCT ProcessTechnicals(TRANS *pzTrans, TRANTYPE zTranType, TRADEINFO *pzOriginal, TRADEINFO *pzRemaining, 
                            DTRANSDESC zDTransDesc[], int iNumDTItems, char *sCurrPrior, long *plFirstTransNo, BOOL bDoTransaction);
ERRSTRUCT GetExistingPosition(TRANS zTrans, POSINFOTABLE *pzPv, double *pzTotalUnits, TRANTYPE zTranType, 
															SECTYPE zSectype, char *sOriginalFace, double fTradingUnit);
void      RemoveByAcctMethod(POSINFOTABLE zPv, double *pzRemainingSharesToRemove, double *pzTotalSharesRemoved,
							               double *pzTotalCostRemoved);
ERRSTRUCT CurrencySweepProcessor(TRANS zTrans,PORTMAIN zPortmain,char *sCurrPrior, BOOL bDoTransaction,int iAction,BOOL bInit);
...
ERRSTRUCT GetNewSecurityInfo(TRANS zTrans, ASSETS *pzAssets, SECTYPE *pzSecType, TRANTYPE *pzTranType);
...
ERRSTRUCT SelectHoldingsTransNo(int iID1, char *sSecNo, char *sWi, 
																char *sSecXtend, char *sAcctType, long *plTransNo);
...
void      CopyToTransFromTradeInfo(TRADEINFO zTI, TRANS *pzTrans, BOOL bCopyOrigFace);
DLLAPI void STDCALL WINAPI InitializeTransTable2(TRANSTABLE2 *pzTTable2);
DLLAPI ERRSTRUCT STDCALL WINAPI AddTransToTransTable2(TRANSTABLE2 *pzTTable2, TRANS zTrans);

/* Prototype of functions defined in TranProc.c file */
ERRSTRUCT InternalTranProc(int iID, char *sSecNo, char *sWi, long lTagNo, char *sCurrPrior, BOOL bReversal, char *sFileName, BOOL bDoTransaction);
ERRSTRUCT LockPortAndProcess(PTABLE zProTable, ASSETS zAssts, PORTMAIN zPDir, TRANTYPE zTrType, SECTYPE zSType, 
														 long lCurrDate, BOOL bAnyAsOfs, char *sCurrPrior, BOOL bReversal, char *sFileName, BOOL bDoTransaction);
ERRSTRUCT ProcessAsOfTrade(char *sAcctMethod, TRANS zTrans, char *sOrgTranCode, char *sRevTranCode, char *sAutoGen, 
													 char *sSellAcctType, char *sLotInd, char *sCurrPrior, BOOL bRebook, PTABLE *pzCancelTable);
ERRSTRUCT ProcessAsOfAdjustment(TRANS zTrans, PTABLE *pzCancelTable, int i, char *sOrgTranCode, char *sRevTranCode,
																char *sCurrPrior, BOOL bRebook);
ERRSTRUCT ProcessAsOfIncome(TRANS zTrans, PTABLE *pzCancelTable,  int i, char *sOrgTranCode, char *sRevTranCode, 
														char *sAutoGen, char *sCurrPrior, BOOL bRebook);
ERRSTRUCT ProcessAsOfMoney(TRANS zTrans, PTABLE *pzCancelTable,  int i, char *sOrgTranCode, char *sRevTranCode);
ERRSTRUCT ProcessAsOfOpen(char *sAcctMethod, TRANS zTrans, PTABLE *pzCancelTable, int i, 
													char *sOrgTranCode, char *sRevTranCode, char *sCurrPrior, BOOL bRebook);
ERRSTRUCT ProcessAsOfOthers(TRANS zTrans, PTABLE *pzCancelTable, int i, char *sOrgTranCode, char *sRevTranCode, char *sCurrPrior, BOOL bRebook);
ERRSTRUCT AddTradeToCancelTable(TRANS zTrans, PTABLE *pzCancelTable, char *sTranCode, int iTradeSort, BOOL bRebook);
ERRSTRUCT AggregateTrades(PTABLE zPTable, int iPStartIndex, int *piPEndIndex, TRANS *pzTrans);
ERRSTRUCT CreateMergeTable(PTABLE zProcecssTab, PTABLE zCancelTab, PTABLE *pzMergeTab); 
ERRSTRUCT ProcessCancelTable(PTABLE zCanTable, long lCurrentDate, char *sCurrPrior, BOOL bDoTransaction);
ERRSTRUCT ProcessOpeninTranproc(TRANS *pzHTrans, ASSETS zAst, PORTMAIN zPdir, TRANTYPE zTtype, long lcurrdate);
ERRSTRUCT ProcessCloseinTranproc(TRANS *pzHTrans, ASSETS zAst, PORTMAIN zPmain, TRANTYPE zTtype, long lcurrdate);
ERRSTRUCT ProcessTransferinTranproc(TRANS *pzHTrans, ASSETS zAst, PORTMAIN zPdir, TRANTYPE zTtype, SECTYPE zStype, 
																		DTRANSDESC zDtransDes[], int iDtransDesCount, long lcurrdate, char *sCurrPrior);
ERRSTRUCT VerifyTransaction(TRANS zTrns, TRANTYPE *pzTranType, char *psRevTranCode, char *psSellAcctType); 
...
ERRSTRUCT GetDtransDescript(int iID, long lTransNo, DTRANSDESC *pzDTranDes, int *piDTranDesCount, BOOL bRebookFlag);
ERRSTRUCT GetDtransPayee(int iID, long lTransNo, PAYTRAN *pzPayTran, BOOL bRebookFlag);
...
ERRSTRUCT WriteReversalInformation(PTABLE CancelTable, char* sFileName, char *sPortfolioName);
...
ERRSTRUCT CheckTradeToCancelTable(TRANS zTrans, PTABLE zCancelTable[], int *piCancelCount, 
																	char *sTranCode, char *sOrgTranCode, char *sRevTranCode,
																	char *sAutoGen, int iTradeSort, BOOL *bAddFlag);
ERRSTRUCT ProcessXTransNoTrade(char *sAcctMethod, PTABLE *pzCancelTable, int iCIndex, char *sCurrPrior, BOOL bRebook);
...
ERRSTRUCT ProcessIncomeInUpdbal(TRANS zTR, long lFiscal, char *sPSType, 
                                ...
                                long lCashImpact, long lSecImpact, long lPortbalImpact);
ERRSTRUCT ProcessMoneyInUpdbal(TRANS zTR, long lFiscal, long lCashImpact, 
                               long lSecImpact, long lPortbalImpact, char *sPerfImpact);
ERRSTRUCT ProcessOpeningInUpdbal(TRANS zTR, long lFiscal, char *sPSType, char *sSSType, 
																 char *sMktValInd, long lCashImpact, long lSecImpact,
                                 long lPortbalImpact, char *sPayType, char *sTax, BOOL bReversal);
ERRSTRUCT ProcessClosingInUpdbal(TRANS zTR, long lFiscal, char *sPSType, char *sSSType, char *sMktValInd, 
                                 long lCashImpact, long lSecImpact, long lPortbalImpact, char *sBaseCurr,
                                 char *sPayType, char *sTax, BOOL bReversal);
ERRSTRUCT ProcessAdjustmentInUpdbal(TRANS zTR, long lFiscal, char *sPSType, char *sSSType, char *sMktValInd, 
                                    long lCashImpact, long lSecImpact, long lPortbalImpact, BOOL bReversal);
ERRSTRUCT ProcessTransferInUpdbal(TRANS zTR, long lFiscal, char *sPSType, char *sSSType, char *sMktValInd,
                                  char *sPerfImpact, long lCashImpact, long lSecImpact, long lPortbalImpact,
                                  char *sBaseCurr, char *sPayType, char *sTaxStat, BOOL bReversal);
ERRSTRUCT CommonOpenCloseInUpdbal(TRANS zTR, long lFiscal, char *sPSType, char *sSSType, char *sMktValInd,
                                  long lCashImpact, long lSecImpact, long lPortbalImpact, char *sPayType, 
                                  char *sTax, PORTBAL *pzPBR, BOOL bReversal, char *sTranCode);

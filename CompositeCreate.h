...
/* Prototypes*/
typedef void (CALLBACK* LPPRSELECTALLMEMBERSOFACOMPOSITE)(int ,long ,int*,ERRSTRUCT*); 
typedef void (CALLBACK* LPPRSELECTPORTMAINTABLE)(PORTMAIN*,int,ERRSTRUCT*);
typedef void (CALLBACK* LPPRACCOUNTINSERTPORTMAIN)(long, char*,char*,ERRSTRUCT*);
typedef void (CALLBACK* LPPRGETSUMMARIZEDDATAFORCOMPOSITE)(SEGMAIN*, SUMMDATA*, UNITVALUE*, 
													double*, int, long, long, BOOL, char *, ERRSTRUCT*);
typedef void (CALLBACK* LPPRSUMMARIZETAXPERF)(int, long, long, char *, ERRSTRUCT*);
typedef void (CALLBACK* LPPRSUMMARIZEMONTHSUM)(int, long, long, long, char *, ERRSTRUCT*);
typedef void (CALLBACK* LPPRSUMMARIZEINCEPTIONSUMMDATA)(int, long, char *, ERRSTRUCT*);
typedef void (CALLBACK* LPPRSUBTRACTINCEPTIONSUMMDATA)(int, long, long, ERRSTRUCT*);

typedef void (CALLBACK* LPPR3LONG1INT)(long, long, long, int, ERRSTRUCT*);
typedef void (CALLBACK* LPPRCOPYSUMMARYDATA)(int, long, char*, char*, ERRSTRUCT*);
typedef void (CALLBACK* LPPRINSERTMAPCOMPMEMTRANSEX)(MAPCOMPMEMTRANSEX, ERRSTRUCT*);
typedef void (CALLBACK* LPPRDELETEMAPCOMPMEMTRANSEX)(long, int, ERRSTRUCT*);
typedef void (CALLBACK* LPPRBUILDMERGECOMPSEGMAP)(int, long, long, char*, ERRSTRUCT*);
typedef void (CALLBACK* LPPRBUILDMERGECOMPPORT)(int, long, char*, ERRSTRUCT*);
typedef void (CALLBACK* LPPRBUILDMERGESDATA)(int, long, long, char*, ERRSTRUCT*);
typedef void (CALLBACK* LPPRUPDATEUNITVALUEMONTHLYIPV)(char*, long, long, ERRSTRUCT*);
...
/* Exported Functions*/
DLLAPI void STDCALL WINAPI CreateComposite(int iOwnerID,long  lDate, int iPortfolioType, long iHoldingsFlag,ERRSTRUCT *zErr);
DLLAPI void STDCALL WINAPI InitializeDllRoutines(long lAsofDate, char *sDBAlias, char *sMode, char *sType, 
...
void CreateCompositeRecord_Internal(int iCompositeID, HOLDTABLE *pHoldingsTable, HOLDCASHTABLE *pHoldcashTable,
																		char *sHoldingsTable,char *sHoldCashTable,char *sTargetPortmain,
																		long lValDate, BOOL bInTrans, int iPortType, ERRSTRUCT* zErr);
...
void GetHoldingsHoldcashInTables(int iPortfolioID, HOLDTABLE *pHoldingsTable, HOLDCASHTABLE *pHoldcashTable, ERRSTRUCT *zErr);

...
DLLAPI BOOL STDCALL IsItManualForSecurity(char *sSecNo, char *sWi, long lPriceDate, ERRSTRUCT *pzErr)
{
    ...
    try
    {
        ...
        string sSQL = "SELECT price_source FROM histpric WHERE sec_no = ? AND wi = ? AND price_date = ? AND price_source IN ('M', 'W')";
        nanodbc::prepare(statement, sSQL);

        statement.bind(0, sSecNo);
        statement.bind(1, sWi);
        
        nanodbc::date dPriceDate = LongToDateStruct(lPriceDate);
        statement.bind(2, &dPriceDate);
    ...
    {
        *pzErr = PrintError("IsItManualForSecurity", 0, 0, sSecNo, 0, 0, 0, (char*)e.what(), FALSE);
    }
...
DLLAPI BOOL STDCALL IsManualClosingPrice(char *sSecNo, char *sWi, long lPriceDate, ERRSTRUCT *pzErr)
{
    ...
    {
        nanodbc::statement statement(gConn);
        string sSQL = "SELECT field_name FROM manprice WHERE sec_no = ? AND wi = ? AND price_date = ? AND field_name = 'close_price'";
        nanodbc::prepare(statement, sSQL);
    ...
    {
        *pzErr = PrintError("IsManualClosingPrice", 0, 0, sSecNo, 0, 0, 0, (char*)e.what(), FALSE);
    }

    return bRes;
}

ERRSTRUCT InternalGetSecurityPrice(char *sSecNo, char *sWi, long lPriceDate, PRICEINFO *pzPInfo, BOOL bFindLastPrice)
{
    ERRSTRUCT zErr;
    ...
    {
        nanodbc::statement statement(gConn);
        string sSQL = "SELECT hp1.date_price_updated, hp1.price_source, hp1.close_price, hp1.exrate, "
                      "hp1.inc_exrate, hp1.ann_div_cpn, "
                      "hp2.date_exrate_updated, hp2.exrate as currexrate, hp2.inc_exrate as currincexrate, "
                      ...
                      "FROM histpric hp1 "
                      "LEFT OUTER JOIN histfinc f ON hp1.sec_no = f.sec_no AND hp1.wi = f.wi AND hp1.price_date = f.price_date "
                      "LEFT OUTER JOIN histeqty e ON hp1.sec_no = e.sec_no AND hp1.wi = e.wi AND hp1.price_date = e.price_date "
                      "LEFT OUTER JOIN assets a ON hp1.sec_no = a.sec_no AND hp1.wi = a.when_issue "
                      "LEFT OUTER JOIN histpric hp2 ON a.curr_id = hp2.sec_no AND hp2.sec_no='N' AND hp2.price_date = hp1.price_date "
        ...
        if (result.next())
        {
            ...
            pzPInfo->lDatePriceUpdated = DateStructToLong(result.get<nanodbc::date>(0));
            ...
            pzPInfo->lDateExrateUpdated = DateStructToLong(result.get<nanodbc::date>(6));
            ...
            else
            {
                ...
                pzPInfo->fIncExrate = fIncExrate; // Note: Original code used fExrate here too, likely bug or intentional? "pzPInfo->fIncExrate = cmdGetSecurityPrice.m_zPInfo.fExrate;" - line 4093. I will follow original code logic but check if it was a typo.
                // Original code:
                // pzPInfo->fExrate = cmdGetSecurityPrice.m_zPInfo.fExrate;
                // pzPInfo->fIncExrate = cmdGetSecurityPrice.m_zPInfo.fExrate; 
                // Wait, line 4093 in view says: pzPInfo->fIncExrate = cmdGetSecurityPrice.m_zPInfo.fExrate;
                // This looks like a typo in original code, but I must preserve behavior unless I am sure.
            ...
            // Check Yield Type
            // Need access to zSyssetng.sYieldType. zSyssetng is global in TransIO.cpp?
            // I need to check if zSyssetng is available.
            // It is likely externed in TransIO.h or OLEDBIOCommon.h?
            // I will assume it is available or I need to declare it extern.
            // In TransIO.cpp line 4100: if (_stricmp(zSyssetng.sYieldType, "W") == 0)
            
            // extern SYSSETTINGS zSyssetng; // Removed: Redundant and incorrect type. Using global zSyssetng from OLEDBIOCommon.h
            ...
            if (pzPInfo->fClosePrice == 0)
                pzPInfo->bIsPriceValid = IsManualClosingPrice(pzPInfo->sSecNo, pzPInfo->sWi, pzPInfo->lPriceDate, &zErr);
        ...
        else
        {
            if (bFindLastPrice)
            {
                zErr.iBusinessError = LastBusinessDay(lPriceDate, (char*)"USA", (char*)"M", &lLastBusinessDate);
                if (zErr.iBusinessError != 0)
                    return zErr;

                zErr = InternalGetSecurityPrice(sSecNo, sWi, lLastBusinessDate, pzPInfo, FALSE);
            }
            else
            {
                ...
                {
                    PrintError("SecNo not found in Assets", 0, 0, sSecNo, 0, zErr.iSqlError, 0, "GETSECURITYPRICE", FALSE);
                    return zErr;
                }
    ...
    {
        zErr = PrintError("InternalGetSecurityPrice", 0, 0, "", 0, 0, 0, (char*)e.what(), FALSE);
    }

...
ERRSTRUCT STDCALL WINAPI CalculateNetFlowPerf(TRANS zTR, char *sPerfImpact,
  ...
  if (sPerfImpact[0] == ' ')
    return (lpfnPrintError((char *)"Invalid Performance Impact", zTR.iID,
                           zTR.lTransNo, (char *)"T", 49, 0, 0,
                           (char *)"CALC NFLOW1", FALSE));
  ...
  if (IsValueZero(zTR.fBaseXrate, 12))
    return (lpfnPrintError((char *)"Invalid BaseXrate", zTR.iID, zTR.lTransNo,
                           (char *)"T", 67, 0, 0, (char *)"CALC NFLOW2",
                           FALSE));

  if (IsValueZero(zTR.fIncBaseXrate, 12))
    return (lpfnPrintError((char *)"Invalid Income BaseXrate", zTR.iID,
                           zTR.lTransNo, (char *)"T", 121, 0, 0,
                           (char *)"CALC NFLOW3", FALSE));

  if (IsValueZero(zTR.fSecBaseXrate, 12))
    return (lpfnPrintError((char *)"Invalid Security BaseXrate", zTR.iID,
                           zTR.lTransNo, (char *)"T", 122, 0, 0,
                           (char *)"CALC NFLOW2", FALSE));

  if (IsValueZero(zTR.fAccrBaseXrate, 12))
    return (lpfnPrintError((char *)"Invalid Accrual BaseXrate", zTR.iID,
                           zTR.lTransNo, (char *)"T", 123, 0, 0,
                           (char *)"CALC NFLOW2", FALSE));
  ...
  if (strcmp(sPerfImpact, "C") == 0) {
    ...
    if (IsValueZero(zTR.fAccrInt, 6)) // don't want to double count the flow (as
                                      // incomesecflow and accrual flow)
  ...
  /* fees */
    ...
    // if this transaction is a credit transaction, reverse the effect of all
    // the flows
  ...
  /* sales */
    ...
    // if this transaction is a debit transaction, reverse the effect of all the
    // flows

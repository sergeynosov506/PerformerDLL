...
const char* SQL_UPDATE_AMORTIZE_HOLDINGS =
    ...
    "SET orig_yield = ?, cost_eff_mat_yld = ?, eff_mat_date = ?, eff_mat_price = ?, tot_cost = ? "
...
const char* SQL_AMORTIZE_UNLOAD_BASE =
    ...
    "trans_no, h.secid, h.units, trd_date, stl_date, eff_date, elig_date, orig_face, orig_yield, "
    ...
    "cur_exrate, cur_inc_exrate, first_cpn_date, last_cpn_date, amort_flag, accretion_flag "
    "FROM %HOLDINGS_TABLE% h, assets a, fixedinc f "
    "WHERE h.sec_no = a.sec_no AND h.wi = a.when_issue AND "
    "h.sec_no = f.sec_no AND h.wi = f.wi AND (amort_flag = 'Y' OR accretion_flag = 'Y')";

const char* SQL_AMORTIZE_MODE_ACCOUNT = " AND h.id = ?";
const char* SQL_AMORTIZE_MODE_SECURITY = " AND h.id = ? AND h.sec_no = ? AND h.wi = ? AND h.sec_xtend = ? AND h.acct_type = ? AND h.trans_no = ?";
...
const char* SQL_PITIPS_UNLOAD_BASE =
    ...
    "EXISTS (SELECT * FROM dbo.portmain p WHERE p.id=h.id AND p.deletedate IS NULL AND p.portfoliotype IN (3,4,5)) AND "
    "h.eff_date < ? AND f.maturity_date > ? AND DATEPART(dd, f.maturity_date) = DATEPART(dd, ?)";

const char* SQL_PITIPS_MODE_ACCOUNT = " AND h.id = ?";
const char* SQL_PITIPS_MODE_SECURITY = " AND h.id = ? AND h.sec_no = ? AND h.wi = ? AND h.sec_xtend = ? AND h.acct_type = ? AND h.trans_no = ?";
...
static AmortizeState g_amortizeState;
static PITIPSState g_pitipsState;
...
static void FillAmortStruct(nanodbc::result& result, AMORTSTRUCT* pzAM)
{
    ...
    read_string(result, "orig_trans_type", pzAM->sOrigTransType, sizeof(pzAM->sOrigTransType));
    ...
    read_string(result, "inc_curr_id", pzAM->sIncCurrId, sizeof(pzAM->sIncCurrId));
    ...
    read_string(result, "accretion_flag", pzAM->sAccretFlag, sizeof(pzAM->sAccretFlag));
}
...
static void FillPITIPSStruct(nanodbc::result& result, PITIPSSTRUCT* pzPI)
{
    ...
    read_string(result, "inc_curr_id", pzPI->sIncCurrId, sizeof(pzPI->sIncCurrId));
}
...
{
#ifdef DEBUG
    PrintError("Entering", 0, 0, sHoldings, 0, 0, 0, "UpdateAmortizeHoldings", FALSE);
#endif
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"", 0, -1, 0, (char*)"UpdateAmortizeHoldings", FALSE);
        return;
    }
    ...
    try
    {
        ...
        stmt.bind(paramIndex++, zHD.sSecNo);
        stmt.bind(paramIndex++, zHD.sWi);
        stmt.bind(paramIndex++, zHD.sSecXtend);
        stmt.bind(paramIndex++, zHD.sAcctType);
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), 0, 0, (char*)"", 0, -1, 0, (char*)"UpdateAmortizeHoldings", FALSE);
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in UpdateAmortizeHoldings", 0, 0, (char*)"", 0, -1, 0, (char*)"UpdateAmortizeHoldings", FALSE);
    }
}
...
DLLAPI void STDCALL AmortizeUnload(char *sMode, int iID, char *sSecNo, char *sWi, 
    char *sSecXtend, char *sAcctType, long lTransNo, AMORTSTRUCT *pzAM, ERRSTRUCT *pzErr)
{
#ifdef DEBUG
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"", 0, -1, 0, (char*)"AmortizeUnload", FALSE);
        return;
    }
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), 0, 0, (char*)"", 0, -1, 0, (char*)"AmortizeUnload", FALSE);
        g_amortizeState.result.reset();
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in AmortizeUnload", 0, 0, (char*)"", 0, -1, 0, (char*)"AmortizeUnload", FALSE);
        g_amortizeState.result.reset();
    }
}
...
DLLAPI void STDCALL PITIPSUnload(char *sMode, long lValDate, int iID, char *sSecNo, 
    char *sWi, char *sSecXtend, char *sAcctType, long lTransNo, PITIPSSTRUCT *pzPI, 
    ERRSTRUCT *pzErr)
{
#ifdef DEBUG
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"", 0, -1, 0, (char*)"PITIPSUnload", FALSE);
        return;
    }
    ...
    try
    {
        ...
        bool needNewQuery = !(
            ...
            (sMode[0] != 'S' || (
                strcmp(g_pitipsState.sSecNo, sSecNo) == 0 &&
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), 0, 0, (char*)"", 0, -1, 0, (char*)"PITIPSUnload", FALSE);
        g_pitipsState.result.reset();
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in PITIPSUnload", 0, 0, (char*)"", 0, -1, 0, (char*)"PITIPSUnload", FALSE);
        g_pitipsState.result.reset();
    }
}
...
#ifdef DEBUG
    PrintError("FreeAmortizeIO called (RAII cleanup)", 
        0, 0, "", 0, 0, 0, "FreeAmortizeIO", FALSE);
#endif
}

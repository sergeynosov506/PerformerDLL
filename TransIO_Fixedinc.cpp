...
DLLAPI void STDCALL SelectPartFixedinc(char *sSecNo, char *sWi, PARTFINC *pzPFinc, ERRSTRUCT *pzErr)
{
    ...
    if (!gConn.connected())
    {
        *pzErr = PrintError((char*)"Database not connected", 0, 0, (char*)"D", 0, -1, 0, (char*)"SelectPartFixedinc", FALSE);
        return;
    }
    ...
    try
    {
        ...
        nanodbc::prepare(stmt, NANODBC_TEXT(
            ...
            "dated_date, amort_flag, original_face, cur_yld, cur_ytm, zero_coupon, issue_date, "
        ...
        if (result.next())
        {
            ...
            read_string(result, "flat_code", pzPFinc->sFlatCode, sizeof(pzPFinc->sFlatCode));
            
            // 4. pay_type -> sPayType (char)
            read_string(result, "pay_type", pzPFinc->sPayType, sizeof(pzPFinc->sPayType));
            
            // 5. dated_date -> lDatedDate (datetime -> long)
            read_date(result, "dated_date", &pzPFinc->lDatedDate);
            
            // 6. amort_flag -> sAmortFlag (char)
            read_string(result, "amort_flag", pzPFinc->sAmortFlag, sizeof(pzPFinc->sAmortFlag));
            
            // 7. original_face -> sOriginalFace (char)
            read_string(result, "original_face", pzPFinc->sOriginalFace, sizeof(pzPFinc->sOriginalFace));
            ...
            read_string(result, "zero_coupon", pzPFinc->sZeroCoupon, sizeof(pzPFinc->sZeroCoupon));
            ...
            read_string(result, "accretion_flag", pzPFinc->sAccretFlag, sizeof(pzPFinc->sAccretFlag));
    ...
    catch (const nanodbc::database_error& e)
    {
        ...
        msg += e.what();
        *pzErr = PrintError((char*)msg.c_str(), 0, 0, (char*)"D", 0, -1, 0, (char*)"SelectPartFixedinc", FALSE);
    }
    catch (...)
    {
        *pzErr = PrintError((char*)"Unexpected error in SelectPartFixedinc", 0, 0, (char*)"D", 0, -1, 0, (char*)"SelectPartFixedinc", FALSE);
    }
}

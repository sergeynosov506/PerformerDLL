...
// - mk. 2008-02-14 Increased number of ratings    - mk 2007-11-08 Added Vendor
// id    - yb
...
typedef ERRSTRUCT(CALLBACK *LPPRTESTASSET)(PARTASSET2, PSCRDET *, int, BOOL,
                                           long, PARTPMAIN, BOOL *, long, long);

typedef void(CALLBACK *LPPRSELECTALLFORHOLDTOT)(int, long, HOLDINGSASSETS *,
                                                int, ERRSTRUCT *);
typedef void(CALLBACK *LPPRSELECTALLRATING)(RATING *, ERRSTRUCT *);
typedef void(CALLBACK *LPPRSELECTUNSUPERVISED)(int, char *, double *, double *,
                                               ERRSTRUCT *);
...
typedef void(CALLBACK *LPPRSELECTSEGMAINFORPORTFOLIO)(SEGMAIN *, int,
                                                      ERRSTRUCT *);
...
// LPPRERRSTRUCT
// lpprInitializeErrStruct; LPFNPSCRHDRDET
// lpfnCreateHashkeyForScript;
...
// LPFNFINDSCRIPT
// lpfnFindScrHdrByHashkeyAndTmpNo;
...
ERRSTRUCT	AddHoldingsAssetsToTable(HOLDINGSASSETSTABLE
*pHoldingsAssetsTable, HOLDINGSASSETS zHoldingsAssets); void
AddHoldingsToHoldtot(HOLDINGSASSETS zHoldingsAssets, HOLDTOT *pzHoldtot, long
lDate, MKTVALUES *pzSegmentTotals); ERRSTRUCT	AddScriptHeaderToTable(PSCRHDR
zPSHeader, int *piArrayIndex); ERRSTRUCT	CreateHoldtotRecord(PARTPMAIN
zPartPortMain, long lDate, HOLDINGSASSETSTABLE
*pHoldingsAssetsTable,SEGMAINTABLE *pzSegmainTable); ERRSTRUCT
LoadRatingTable(RATINGTABLE *pzRatingTable); double
GetNumericalRating(char * sRating, char sPrimaryType); ERRSTRUCT
GetHoldingsAndAssetTables(int iPortfolioID, long lDate, HOLDINGSASSETSTABLE
*pHoldingsAssetsTable, ASSETTABLE *pzATable); void
GetPAssetsFromHoldingsAssets(PARTASSET *pzPartAsset, HOLDINGSASSETS
zHoldingsAssets); void      InitializePerfScrHdr(PSCRHDR *pzHeader); void
InitializePerfScrDet(PSCRDET *pzDetail); void
InitializePartialAsset(PARTASSET *pzPAsset); void
InitializeHoldingsAssetsTable(HOLDINGSASSETSTABLE *pzHSTable); void
FillAdditionalHoldTotData(HOLDTOT *pzHoldtot, MKTVALUES zMV, MKTVALUES zTotMV);
ERRSTRUCT LoadSegmnentsTable(SEGMENTSTABLE *pzSegments);
void			InitializeSegmentsTable(SEGMENTSTABLE *pzSegmentsTable);
int				GetSegmentLevelId(const int iId);
int				GetPledgedSegmentId(const int
iIndustLevel1,const int iIndustLevel3); ERRSTRUCT LoadSegmapTable(SEGMAPTABLE
*pzSegmapTable); void			InitializeSegmapTable(SEGMAPTABLE
*pzSegmapTable); int				GetSegmapid(const int
iIndustLevel1,const int iIndustLevel2,const int iIndustLevel3); BOOL
IsAssetInternational(PARTPMAIN zPmain,char sCurrId[],const int IndustLevel1,int*
Level2); int				GetUnsupervisedSegmentId(void);
ERRSTRUCT	GetAllSegmentsFromSegmainForPortId(int iPortfolioID,SEGMAINTABLE
*pzSegmainTable); int				CheckIfSegmentExist(int
iSegTypeId,SEGMAINTABLE* pzSegmainTable); void
InitializeSegmainTable(SEGMAINTABLE *pzSegmainTable); ERRSTRUCT
AddSegmainToTable(SEGMAINTABLE *pzSegmainTable, SEGMAIN zSegmain); ERRSTRUCT
CreateSegmainsIfRequired(SEGMAINTABLE *pzSTable, PARTPMAIN zPmain, long
lCurrentDate, ASSETTABLE zATable, PERFRULETABLE zRuleTable); int
FindSegmentByType(SEGMAINTABLE zSTable, int iSegmentTypeID); void
CleanUpPortfolioTables(HOLDINGSASSETSTABLE *pzHATable, SEGMAINTABLE
*pzSTable, PERFRULETABLE *pzPTable, ASSETTABLE *pzATable); ERRSTRUCT
GetAllPerfrulesForAPortfolio(PERFRULETABLE *pzRuleTable, int iID, long lDate);
ERRSTRUCT GetSystemTables();
void			CopyHoldingsAssetsToAsset(HOLDINGSASSETS
zHoldingsAssets, PARTASSET *pzTempAsset); void
CleanUpSystemTables(); DLLAPI ERRSTRUCT STDCALL WINAPI InitHoldtot(long

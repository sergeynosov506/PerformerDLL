 ...
 *
 * DESCRIPTION: To create PI/BA trans for TIPS holdings of a portfolio for the
 * given date
 *
 * PUBLIC FUNCTION(S):
 *                      1. DLLAPI ERRSTRUCT STDCALL WINAPI CreatePITIPS(int
...
// memory,deprecated string function  - sergeyn 2/11/2010 - VI#43627 More
// changes  - mk 2/11/2010 - VI#43627 Modified logic to create PIs for those
// accounts, securities that apply  - mk 3/18/2004 - Fixed call to tranalloc -
// number of desc items is 0  - vay 3/18/2004 - Added DB transactions  - vay
...
** Valid Modes are B(batch), A(single account) and S (single security in an
* account).
*F*/
ERRSTRUCT STDCALL WINAPI GeneratePhantomIncome(long lValDate, char *sMode,
  ...
  if (strcmp(sSecNo, "") != 0 && strcmp(sWi, "") != 0) {
    ...
    if (zErr.iSqlError != 0 || zErr.iBusinessError != 0) {
      if (zErr.iSqlError == SQLNOTFOUND) {
        sprintf_s(sMsg, "Security: %s, Wi: %s Not Found In Asset Table", sSecNo,
                  sWi);
    ...
    if (strcmp(zAssets.sAutoDivint, "Y") != 0)
      return zErr;
  }
  ...
  zErr = PITIPSGeneration(zPmainTable, zPInfoTable, zSTTable, zCTable, lValDate,
                          sMode, sSecNo, sWi, sSecXtend, sAcctType, lTransNo);
...
ERRSTRUCT PITIPSGeneration(PORTTABLE zPMainTable, PINFOTABLE zPInfoTable,
                           ...
                           long lValDate, char *sMode, char *sSecNo, char *sWi,
  ...
  for (i = 0; i < zPMainTable.iNumPmain; i++) {
    ...
    zErr = BuildPITable(sMode, sFileName, lValDate, zPMainTable.pzPmain[i].iID,
                        sSecNo, sWi, sSecXtend, sAcctType, lTransNo, &zPITable);
    if (zErr.iSqlError != 0 || zErr.iBusinessError != 0) {
      lpfnPrintError("Phantom Income Could Not Be Processed",
                     zPMainTable.pzPmain[i].iID, 0, "", 999, 0, 0, "PITIPS2",
                     TRUE);
      lpprInitializeErrStruct(&zErr);
      continue;
    }
    ...
    for (j = 0; j < zPITable.iNumPI; j++) {
      ...
      */
      ...
      // if effective date on the lot is greater or equal to the current pricing
      // date, skip it
      ...
      if (strcmp(sSecNo, "") != 0 && strcmp(sWi, "") != 0) {
        ...
        lpprSelectAsset(&zAssets, zPITable.pzPI[j].sSecNo, zPITable.pzPI[j].sWi,
                        -1, &zErr);
        if (zErr.iSqlError != 0 || zErr.iBusinessError != 0) {
          if (zErr.iSqlError == SQLNOTFOUND) {
            sprintf_s(sErrMsg, "Security: %s, Wi: %s Not Found In Asset Table",
                      zPITable.pzPI[j].sSecNo, zPITable.pzPI[j].sWi);
            return (lpfnPrintError(sErrMsg, 0, 0, "T", 8, 0, 0, "PITIPSANF",
                                   FALSE));
          }
          return zErr;
        }
      ...
      */
      ...
      if (bDoTrans) {
        ...
        if (zErr.iBusinessError != 0)
          return (lpfnPrintError("Unable to start DB trans",
      ...
      __try {
        ...
        zErr = PITIPSCallTranAlloc(zPITable.pzPI[j],
      ...
      if (bDoTrans) {
        if (zErr.iSqlError == 0 && zErr.iBusinessError == 0)
          ...
          lpfnPrintError("Unable to post phantom income",
                         zPMainTable.pzPmain[i].iID, 0, "", zErr.iBusinessError,
                         zErr.iSqlError, 0, "PITIPS6", TRUE);
        }
      }
...
ERRSTRUCT PITIPSUnloadAndSort(long lValDate) {
  ...
  while (zErr.iSqlError == 0 && zErr.iBusinessError == 0) {
    ...
    sprintf_s(sUnlStr,
              "%s|%d|%s|%s|%s|%s|%s|%ld|%d|%f|%ld|%ld|%ld|%f|%f|"
              "%d|%f|%s|%s|%ld\n",
              "H\0", zTempPI.iID, zTempPI.sSecNo, zTempPI.sWi,
              zTempPI.sSecXtend, zTempPI.sSecSymbol, zTempPI.sAcctType,
              zTempPI.lTransNo, zTempPI.iSecID, zTempPI.fUnits,
              zTempPI.lTrdDate, zTempPI.lStlDate, zTempPI.lEffDate,
              zTempPI.fTotCost, zTempPI.fOrigCost, zTempPI.iSecType,
              zTempPI.fTradUnit, zTempPI.sCurrId, zTempPI.sIncCurrId,
...
*F*/
ERRSTRUCT BuildPITable(char *sMode, char *sFileName, long lValDate, int iID,
  ...
  if (sMode[0] == 'B') {
    ...
    */
    while ((zErr.iSqlError == 0 && zErr.iBusinessError == 0) &&
      ...
      if (sStr2[0] != 'H') {
        zErr = lpfnPrintError("Record Can Only Come From H(oldings)", iID, 0,
                              "", 995, 0, 0, "PITIPS BUILDTABLE", FALSE);
        break;
      }
      /*
    ...
    while (zErr.iSqlError == 0) {
      lpprPITIPSUnload(sMode, lValDate, iID, sSecNo, sWi, sSecXtend, sAcctType,
                       lTransNo, &zTempPI, &zErr);
...
ERRSTRUCT AddRecordToPITable(PITIPSTABLE *pzPITab, PITIPSSTRUCT zPI) {
  ...
  if (pzPITab->iPICreated == pzPITab->iNumPI) {
    ...
    pzPITab->pzPI = (PITIPSSTRUCT *)realloc(
        pzPITab->pzPI, pzPITab->iPICreated * sizeof(PITIPSSTRUCT));
...
ERRSTRUCT PITIPSCallTranAlloc(PITIPSSTRUCT zPI, char *sAcctMthd, SECTYPE zSType,
                              long lValDate) {
  ERRSTRUCT zErr;
  TRANS zTR;
  ASSETS zAssets;
  ...
  */
  ...
  zErr = lpfnCalculatePhantomIncome(
      zTR.iID, zTR.sSecNo, zTR.sWi, zTR.lTaxlotNo, lLastIncDate, zTR.lStlDate,
      zPI.lTrdDate, zPI.fOrigCost, zPI.fUnits, bFirstIncome, &fPhantomIncome);
  if (zErr.iSqlError != 0 || zErr.iBusinessError != 0)
    return zErr;
  ...
  if (IsValueZero(fPhantomIncome, 2))
    return zErr; // if no increase in value, no need to generatePI & BA
                 // transaction
  ...
  zErr = lpfnTranAlloc(&zTR, zTranType, zSType, zAssets, zDTr, 0, NULL, "C",
                       FALSE);
  if (zErr.iBusinessError != 0 || zErr.iSqlError != 0)
    return zErr;

...
DLLAPI void STDCALL SelectAllMembersOfAComposite(int iOwnerID, long lDate,
                                                 int *piID, ERRSTRUCT *pzErr) {
#ifdef DEBUG
  PrintError("Entering", 0, 0, "", 0, 0, 0, "SelectAllMembersOfAComposite",
             FALSE);
#endif
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"SelectAllMembersOfAComposite", FALSE);
    return;
  }
  ...
  try {
    ...
    bool needNewQuery =
        !(g_memberState.iOwnerID == iOwnerID && g_memberState.lDate == lDate &&
    ...
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectAllMembersOfAComposite", FALSE);
    g_memberState.result.reset();
  } catch (...) {
    *pzErr = PrintError(
        (char *)"Unexpected error in SelectAllMembersOfAComposite", 0, 0,
        (char *)"", 0, -1, 0, (char *)"SelectAllMembersOfAComposite", FALSE);
    g_memberState.result.reset();
  }
}
...
DLLAPI void STDCALL SelectSegmainForPortfolio(SEGMAIN *pzSegmain, int iID,
                                              ERRSTRUCT *pzErr) {
#ifdef DEBUG
  ...
  if (!gConn.connected()) {
    *pzErr = PrintError((char *)"Database not connected", 0, 0, (char *)"", 0,
                        -1, 0, (char *)"SelectSegmainForPortfolio", FALSE);
    return;
  }
  ...
  try {
    // Check if parameters changed or if this is a new query
    bool needNewQuery =
        !(g_segmainState.iOwnerID == iID && g_segmainState.cRows > 0);
    ...
    if (g_segmainState.result && g_segmainState.result->next()) {
      ...
      pzSegmain->iSegmentTypeID =
          g_segmainState.result->get<int>("segmenttype_id", 0);
      read_string(*g_segmainState.result, "segment_name",
                  pzSegmain->sSegmentName, sizeof(pzSegmain->sSegmentName));
      read_string(*g_segmainState.result, "segment_abbrev",
                  pzSegmain->sSegmentAbbrev, sizeof(pzSegmain->sSegmentAbbrev));
      read_string(*g_segmainState.result, "isinactive", pzSegmain->sIsInactive,
                  sizeof(pzSegmain->sIsInactive));
      pzSegmain->iSegLevel = g_segmainState.result->get<short>("seglevel", 0);
      read_string(*g_segmainState.result, "calculated", pzSegmain->sCalculated,
                  sizeof(pzSegmain->sCalculated));
      pzSegmain->iSequenceNo =
          g_segmainState.result->get<short>("sequence_no", 0);
    } else {
      // EOF reached
      pzErr->iSqlError = SQLNOTFOUND;
      g_segmainState.cRows = 0;
    ...
    *pzErr = PrintError((char *)msg.c_str(), iID, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectSegmainForPortfolio", FALSE);
    g_segmainState.result.reset();
  } catch (...) {
    *pzErr = PrintError((char *)"Unexpected error in SelectSegmainForPortfolio",
                        iID, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectSegmainForPortfolio", FALSE);
    g_segmainState.result.reset();
  }
...
static void FillTransStruct(nanodbc::result &result, TRANS *pzTR) {
  ...
  pzTR->iXID = result.get<int>("x_id", 0);
...
DLLAPI void STDCALL SelectTransFor(TRANS *pzTR, int iID, long lEffDate1,
                                   long lEffDate2, ERRSTRUCT *pzErr) {
#ifdef DEBUG
  ...
  try {
    // Check if parameters changed or if this is a new query
    bool needNewQuery =
        !(g_transState.iID == iID && g_transState.lEffDate1 == lEffDate1 &&
          g_transState.lEffDate2 == lEffDate2 && g_transState.cRows > 0);
    ...
    msg += e.what();
    *pzErr = PrintError((char *)msg.c_str(), 0, 0, (char *)"", 0, -1, 0,
                        (char *)"SelectTransFor", FALSE);
    g_transState.result.reset();
  } catch (...) {
    *pzErr = PrintError((char *)"Unexpected error in SelectTransFor", 0, 0,
                        (char *)"", 0, -1, 0, (char *)"SelectTransFor", FALSE);
    g_transState.result.reset();
  }
}
...
DLLAPI void STDCALL SelectHoldingsFor(int iID, HOLDINGS *pzHoldings,
                                      char *TableName, ERRSTRUCT *pzErr) {
  char sDLLHoldings[40];

  strcpy_s(sDLLHoldings,
           sHoldings); // Save Holdings table currently being used in DLL
  strcpy_s(sHoldings,
           TableName); // set Holdings_table_name internally used in the DLL
...
DLLAPI void STDCALL SelectHoldcashFor(int iID, HOLDCASH *pzHoldcash,
                                      char *TableName, ERRSTRUCT *pzErr) {
  char sDLLHoldcash[40];

  strcpy_s(sDLLHoldcash,
           sHoldcash); // Save Holdcash table currently being used in DLL
  strcpy_s(sHoldcash,
           TableName); // set Holdcash_table_name internally used in the DLL
  // call appropriate function from ValuationIO part
